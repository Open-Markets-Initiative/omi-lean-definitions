...
@[simp] theorem decode_encode (message : BasketRollBroadcast) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [OldBasketDataBcGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [NewBasketDataBcGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 BasketRootPartyGrpComp.encode BasketRootPartyGrpComp.decode BasketRootPartyGrpComp.decode_encode, Option.bind_some]
  dsimp only
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : TesInquireBoundaryPricesResponse) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : UpdateReferencePricesResponse) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
