...
/-!
# Eurex Exchange Enhanced Trading Interface v2.5
...
namespace Omi.EurexT7EtiFbeV25Server
...
def decode (bytes : List UInt8) : Option (DeleteAllOrderBroadcast × List UInt8) := do
  ...
  if fits_notAffectedOrdersGrpComp : notAffectedOrdersGrpComp_.length < 256 ^ 2 then
    pure ({ pad2, rbcHeaderMeComp, massActionReportId, securityId, marketSegmentId, targetPartyIdSessionId, targetPartyIdExecutingTrader, partyIdEnteringTrader, partyIdEnteringFirm, massActionReason, execInst, pad3, notAffectedOrdersGrpComp := ⟨notAffectedOrdersGrpComp_, fits_notAffectedOrdersGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : DeleteAllOrderBroadcast) : (encode message).length ≤ 1048658 := by
...
def decode (bytes : List UInt8) : Option (LogonResponse × List UInt8) := do
  ...
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ pad2, responseHeaderComp, throttleTimeInterval, throttleNoMsgs, throttleDisconnectLimit, heartBtInt, sessionInstanceId, tradSesMode, defaultCstmApplVerId, pad1 }, bytes)

@[simp] theorem encode_length (message : LogonResponse) : (encode message).length = 82 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (ModifyOrderNrResponse × List UInt8) := do
  ...
  let (productComplex, bytes) ← decodeUInt 1 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, leavesQty, cumQty, cxlQty, ordStatus, execType, execRestatementReason, productComplex, pad7 }, bytes)
...
def decode (bytes : List UInt8) : Option (ModifyOrderResponse × List UInt8) := do
  ...
  let (productComplex, bytes) ← decodeUInt 1 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsTimePriority, leavesQty, cumQty, cxlQty, ordStatus, execType, execRestatementReason, productComplex, pad7 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderNrResponse × List UInt8) := do
  ...
  let (productComplex, bytes) ← decodeUInt 1 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, securityId, execId, ordStatus, execType, execRestatementReason, productComplex, pad3 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderResponse × List UInt8) := do
  ...
  let (productComplex, bytes) ← decodeUInt 1 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, ordStatus, execType, execRestatementReason, productComplex, pad3 }, bytes)
...
def decode (bytes : List UInt8) : Option (OrderExecNotification × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    if fits_instrmntLegExecGrpComp : instrmntLegExecGrpComp_.length < 256 ^ 2 then
      pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, marketSegmentId, leavesQty, cumQty, cxlQty, execRestatementReason, productComplex, ordStatus, execType, triggered, pad7, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩, instrmntLegExecGrpComp := ⟨instrmntLegExecGrpComp_, fits_instrmntLegExecGrpComp⟩ }, bytes)
    else none
  else none
...
structure OrderExecReportBroadcast where
  ...
  account : Alpha 2
  partyIdPositionAccount : Alpha 20
  positionEffect : PositionEffect
  partyIdTakeUpTradingFirm : Alpha 5
  partyIdOrderOriginationFirm : Alpha 7
  partyIdBeneficiary : Alpha 9
...
def decode (bytes : List UInt8) : Option (OrderExecReportBroadcast × List UInt8) := do
  ...
  let (account, bytes) ← Alpha.decode 2 bytes
  let (partyIdPositionAccount, bytes) ← Alpha.decode 20 bytes
  let (positionEffect, bytes) ← PositionEffect.decode bytes
  let (partyIdTakeUpTradingFirm, bytes) ← Alpha.decode 5 bytes
  let (partyIdOrderOriginationFirm, bytes) ← Alpha.decode 7 bytes
  let (partyIdBeneficiary, bytes) ← Alpha.decode 9 bytes
  ...
  if fits_legOrdGrpComp : legOrdGrpComp_.length < 256 ^ 1 then
    if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
      if fits_instrmntLegExecGrpComp : instrmntLegExecGrpComp_.length < 256 ^ 2 then
        pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, price, stopPx, marketSegmentId, leavesQty, cumQty, cxlQty, orderQty, expireDate, partyIdExecutingUnit, partyIdSessionId, partyIdExecutingTrader, partyIdEnteringTrader, execRestatementReason, partyIdEnteringFirm, productComplex, ordStatus, execType, side, ordType, tradingCapacity, timeInForce, execInst, tradingSessionSubId, applSeqIndicator, account, partyIdPositionAccount, positionEffect, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, triggered, pad7, legOrdGrpComp := ⟨legOrdGrpComp_, fits_legOrdGrpComp⟩, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩, instrmntLegExecGrpComp := ⟨instrmntLegExecGrpComp_, fits_instrmntLegExecGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 2105578 := by
...
def decode (bytes : List UInt8) : Option (OrderExecResponse × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    if fits_instrmntLegExecGrpComp : instrmntLegExecGrpComp_.length < 256 ^ 2 then
      pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, marketSegmentId, leavesQty, cumQty, cxlQty, execRestatementReason, productComplex, ordStatus, execType, triggered, pad7, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩, instrmntLegExecGrpComp := ⟨instrmntLegExecGrpComp_, fits_instrmntLegExecGrpComp⟩ }, bytes)
    else none
  else none
...
structure TradeBroadcast where
  ...
  tradingCapacity : BitVec 8
  account : Alpha 2
  rootPartyIdPositionAccount : Alpha 20
  positionEffect : PositionEffect
  custOrderHandlingInst : Alpha 1
  freeText1 : Alpha 12
  freeText2 : Alpha 12
...
def decode (bytes : List UInt8) : Option (TradeBroadcast × List UInt8) := do
  ...
  let (tradingCapacity, bytes) ← decodeUInt 1 bytes
  let (account, bytes) ← Alpha.decode 2 bytes
  let (rootPartyIdPositionAccount, bytes) ← Alpha.decode 20 bytes
  let (positionEffect, bytes) ← PositionEffect.decode bytes
  let (custOrderHandlingInst, bytes) ← Alpha.decode 1 bytes
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (freeText2, bytes) ← Alpha.decode 12 bytes
...
end Omi.EurexT7EtiFbeV25Server
