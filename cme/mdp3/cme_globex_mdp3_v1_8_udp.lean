...
/-!
# CME Group Market Data Platform 3 v1.8
...
namespace Omi.CmeGlobexMdp3SbeV18Udp
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshSessionStatisticsGroup × List UInt8) := do
  ...
  pure ({ mdEntryPx, securityId, rptSeq, openCloseSettlFlag, mdUpdateAction, mdEntryTypeStatistics, mdEntrySizeOptional, padding1 }, bytes)
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshTradeSummaryGroup × List UInt8) := do
  ...
  let (padding2, bytes) ← Alpha.decode 2 bytes
  pure ({ mdEntryPx, mdEntrySize, securityId, rptSeq, numberOfOrders, aggressorSide, mdUpdateAction, mdTradeEntryId, padding2 }, bytes)
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshOrderBookGroup × List UInt8) := do
  ...
  pure ({ orderIdOptional, mdOrderPriority, mdEntryPxOptional, mdDisplayQtyOptional, securityId, mdUpdateAction, mdEntryTypeBook, padding6 }, bytes)
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefreshOrderBookGroup × List UInt8) := do
  ...
  let (mdEntryTypeBook, bytes) ← MdEntryTypeBook.decode bytes
  pure ({ orderId, mdOrderPriority, mdEntryPx, mdDisplayQty, mdEntryTypeBook }, bytes)
...
inductive Payload where
  ...
  | mdIncrementalRefreshOrderBook (message : MdIncrementalRefreshOrderBook) -- 43
  | snapshotFullRefreshOrderBook (message : SnapshotFullRefreshOrderBook) -- 44
...
def tag : Payload → BitVec 16
  ...
  | .mdIncrementalRefreshOrderBook _ => 43
  | .snapshotFullRefreshOrderBook _ => 44
  | .negotiate _ => 200
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (Payload × List UInt8) :=
  ...
  else if tag = 43 then (MdIncrementalRefreshOrderBook.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshOrderBook message, rest)
  else if tag = 44 then (SnapshotFullRefreshOrderBook.decode bytes).map fun (message, rest) => (.snapshotFullRefreshOrderBook message, rest)
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 2 < 65536 := by
  ...
  | mdInstrumentDefinitionSpread inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionSpread.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length, encodeMany_length_const LegacyLegsGroup.encode 18 LegacyLegsGroup.encode_length]
    omega
  ...
  | mdIncrementalRefreshTrade inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshTrade.encode, encodeMany_length_const IncrementalRefreshTradeGroup.encode 32 IncrementalRefreshTradeGroup.encode_length]
    omega
...
end Omi.CmeGlobexMdp3SbeV18Udp
