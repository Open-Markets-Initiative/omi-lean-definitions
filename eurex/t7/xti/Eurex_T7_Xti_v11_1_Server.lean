...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v11.1
...
namespace Omi.EurexT7XtiFbeV111Server
...
def decode (bytes : List UInt8) : Option (LogonResponse × List UInt8) := do
  ...
  if fits_publicKey : publicKey_.length < 256 ^ 2 then
    pure ({ pad2, responseHeaderComp, throttleTimeInterval, throttleNoMsgs, throttleDisconnectLimit, heartBtInt, sessionInstanceId, latestPublicKeySeqNo, marketId, tradSesMode, defaultCstmApplVerId, defaultCstmApplVerSubId, publicKey := ⟨publicKey_, fits_publicKey⟩, pad6 }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : LogonResponse) : (encode message).length ≤ 65635 := by
...
@[simp] theorem decode_encode (message : LogonResponse) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.publicKey.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (ModifyOrderNrResponse × List UInt8) := do
  ...
  if fits_orderEventGrpComp : orderEventGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, stopPx, leavesQty, cumQty, cxlQty, displayQty, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, transactionDelayIndicator, pad4, orderEventGrpComp := ⟨orderEventGrpComp_, fits_orderEventGrpComp⟩ }, bytes)
  else none
...
def decode (bytes : List UInt8) : Option (NewOrderNrResponse × List UInt8) := do
  ...
  let (orderEventGrpComp_, bytes) ← decodeMany OrderEventGrpComp.decode noOrderEvents.toNat bytes
  if fits_orderEventGrpComp : orderEventGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, securityId, execId, leavesQty, cxlQty, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, transactionDelayIndicator, pad4, orderEventGrpComp := ⟨orderEventGrpComp_, fits_orderEventGrpComp⟩ }, bytes)
  else none
...
def decode (bytes : List UInt8) : Option (NewOrderResponse × List UInt8) := do
  ...
  let (orderEventGrpComp_, bytes) ← decodeMany OrderEventGrpComp.decode noOrderEvents.toNat bytes
  if fits_orderEventGrpComp : orderEventGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, securityId, execId, leavesQty, cxlQty, trdRegTsEntryTime, trdRegTsTimePriority, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, transactionDelayIndicator, pad4, orderEventGrpComp := ⟨orderEventGrpComp_, fits_orderEventGrpComp⟩ }, bytes)
  else none
...
def decode (bytes : List UInt8) : Option (OrderExecNotification × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    if fits_orderEventGrpComp : orderEventGrpComp_.length < 256 ^ 1 then
      pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, leavesQty, cumQty, cxlQty, displayQty, marketSegmentId, orderIdSfx, execRestatementReason, side, ordStatus, execType, orderEventType, matchType, triggered, crossedIndicator, fixClOrdId, pad1, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩, orderEventGrpComp := ⟨orderEventGrpComp_, fits_orderEventGrpComp⟩ }, bytes)
    else none
  else none
...
def decode (bytes : List UInt8) : Option (OrderExecResponse × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    if fits_orderEventGrpComp : orderEventGrpComp_.length < 256 ^ 1 then
      pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, leavesQty, cumQty, cxlQty, displayQty, marketSegmentId, orderIdSfx, execRestatementReason, side, ordStatus, execType, matchType, triggered, crossedIndicator, transactionDelayIndicator, pad5, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩, orderEventGrpComp := ⟨orderEventGrpComp_, fits_orderEventGrpComp⟩ }, bytes)
    else none
  else none
...
end Omi.EurexT7XtiFbeV111Server
