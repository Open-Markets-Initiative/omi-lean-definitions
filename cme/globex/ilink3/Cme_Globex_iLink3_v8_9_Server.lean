...
namespace Omi.CmeGlobexIlink3SbeV89Server
...
def OrdStatus.codes : List UInt8 :=
  [0x30, 0x31, 0x32, 0x34, 0x35, 0x36, 0x38, 0x43, 0x45, 0x55]
...
def encode (message : NegotiationResponse) : List UInt8 :=
  encodeUIntLE 8 message.uuid
    ...
    ++ (encodeUInt 1 message.environmentIndicator
...
def decode (bytes : List UInt8) : Option (NegotiationResponse × List UInt8) := do
  ...
  let (environmentIndicator, bytes) ← decodeUInt 1 bytes
  let (credentials, bytes) ← Credentials.decode bytes
  pure ({ uuid, requestTimestamp, secretKeySecureIdExpiration, faultToleranceIndicator, splitMsg, previousSeqNo, previousUuid, environmentIndicator, credentials }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NegotiationResponse) : (encode message).length ≤ 65570 := by
...
def decode (bytes : List UInt8) : Option (NegotiationReject × List UInt8) := do
  ...
  let (splitMsg, bytes) ← decodeUInt 1 bytes
  let (environmentIndicator, bytes) ← decodeUInt 1 bytes
  pure ({ reason, uuid, requestTimestamp, errorCodes, faultToleranceIndicator, splitMsg, environmentIndicator }, bytes)

@[simp] theorem encode_length (message : NegotiationReject) : (encode message).length = 69 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (EstablishmentAck × List UInt8) := do
  ...
  let (splitMsg, bytes) ← decodeUInt 1 bytes
  let (environmentIndicator, bytes) ← decodeUInt 1 bytes
  pure ({ uuid, requestTimestamp, nextSeqNo, previousSeqNo, previousUuid, keepAliveInterval, secretKeySecureIdExpiration, faultToleranceIndicator, splitMsg, environmentIndicator }, bytes)

@[simp] theorem encode_length (message : EstablishmentAck) : (encode message).length = 39 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (EstablishmentReject × List UInt8) := do
  ...
  let (splitMsg, bytes) ← decodeUInt 1 bytes
  let (environmentIndicator, bytes) ← decodeUInt 1 bytes
  pure ({ reason, uuid, requestTimestamp, nextSeqNo, errorCodes, faultToleranceIndicator, splitMsg, environmentIndicator }, bytes)

@[simp] theorem encode_length (message : EstablishmentReject) : (encode message).length = 73 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, encodeUIntLE_length, encodeUInt_length]
...
theorem encode_length_le (message : PartyDetailsGroups) : (encode message).length ≤ 5613 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const PartyDetailsGroup.encode 22 PartyDetailsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : PartyDetailsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 PartyDetailsGroup.encode PartyDetailsGroup.decode PartyDetailsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.partyDetailsGroup.length_lt]
  rfl
...
theorem encode_length_le (message : TrdRegPublicationsGroups) : (encode message).length ≤ 513 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TrdRegPublicationsGroup.encode 2 TrdRegPublicationsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TrdRegPublicationsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 TrdRegPublicationsGroup.encode TrdRegPublicationsGroup.decode TrdRegPublicationsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.trdRegPublicationsGroup.length_lt]
  rfl
...
theorem encode_length_le (message : PartyDetailsDefinitionRequestAck) : (encode message).length ≤ 6285 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, SelfMatchPreventionInstruction.encode_length, CmtaGiveupCd.encode_length, CustOrderHandlingInst.encode_length, ListUpdateAction.encode_length]
  omega
...
@[simp] theorem decode_encode (message : PartyDetailsDefinitionRequestAck) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, SelfMatchPreventionInstruction.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, CmtaGiveupCd.decode_encode, some_bind]
  dsimp only
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, PartyDetailsGroups.decode_encode, some_bind]
  dsimp only
  rw [TrdRegPublicationsGroups.decode_encode, some_bind]
  rfl
...
/-- Execution Report New: 226 bytes -/
...
def encode (message : ExecutionReportNew) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (encodeUInt 1 message.possRetransFlag
    ++ (encodeUInt 1 message.splitMsg
    ++ (encodeUInt 1 message.crossTypeOptional
    ...
    ++ (encodeUInt 1 message.priorityIndicator))))))))))))))))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (ExecutionReportNew × List UInt8) := do
  ...
  let (possRetransFlag, bytes) ← decodeUInt 1 bytes
  let (splitMsg, bytes) ← decodeUInt 1 bytes
  let (crossTypeOptional, bytes) ← decodeUInt 1 bytes
...
@[simp] theorem decode_encode (message : ExecutionReportNew) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, OrdTypeOptional.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, ExecutionMode.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
...
/-- Execution Report Reject: 483 bytes -/
...
/-- Execution Report Elimination: 219 bytes -/
...
def decode (bytes : List UInt8) : Option (ExecutionReportElimination × List UInt8) := do
  ...
  let (reservationPrice, bytes) ← decodeUIntLE 8 bytes
  let (priorityIndicator, bytes) ← decodeUInt 1 bytes
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, orderId, price, stopPx, transactTime, sendingTimeEpoch, orderRequestId, crossIdOptional, hostCrossId, location, securityId, cumQty, orderQty, minQty, displayQty, expireDate, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, crossTypeOptional, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, discretionPrice, reservationPrice, priorityIndicator }, bytes)

@[simp] theorem encode_length (message : ExecutionReportElimination) : (encode message).length = 219 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length, ExecutionMode.encode_length]
...
/-- Calculated Ccy Last Qty: 9 bytes -/
structure CalculatedCcyLastQty where
  mantissa : BitVec 64
  exponent : BitVec 8
  deriving DecidableEq, Repr

namespace CalculatedCcyLastQty

def encode (message : CalculatedCcyLastQty) : List UInt8 :=
  encodeUIntLE 8 message.mantissa
    ++ (encodeUInt 1 message.exponent)

def decode (bytes : List UInt8) : Option (CalculatedCcyLastQty × List UInt8) := do
  let (mantissa, bytes) ← decodeUIntLE 8 bytes
  let (exponent, bytes) ← decodeUInt 1 bytes
  pure ({ mantissa, exponent }, bytes)

@[simp] theorem encode_length (message : CalculatedCcyLastQty) : (encode message).length = 9 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : CalculatedCcyLastQty) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : CalculatedCcyLastQty) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end CalculatedCcyLastQty

/-- Gross Trade Amt: 9 bytes -/
structure GrossTradeAmt where
  mantissa : BitVec 64
  exponent : BitVec 8
  deriving DecidableEq, Repr

namespace GrossTradeAmt

def encode (message : GrossTradeAmt) : List UInt8 :=
  encodeUIntLE 8 message.mantissa
    ++ (encodeUInt 1 message.exponent)

def decode (bytes : List UInt8) : Option (GrossTradeAmt × List UInt8) := do
  let (mantissa, bytes) ← decodeUIntLE 8 bytes
  let (exponent, bytes) ← decodeUInt 1 bytes
  pure ({ mantissa, exponent }, bytes)

@[simp] theorem encode_length (message : GrossTradeAmt) : (encode message).length = 9 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : GrossTradeAmt) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : GrossTradeAmt) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
theorem encode_length_le (message : FillsGroups) : (encode message).length ≤ 3828 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const FillsGroup.encode 15 FillsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : FillsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 FillsGroup.encode FillsGroup.decode FillsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.fillsGroup.length_lt]
  rfl

end FillsGroups

/-- Contra Gross Trade Amt: 9 bytes -/
structure ContraGrossTradeAmt where
  mantissa : BitVec 64
  exponent : BitVec 8
  deriving DecidableEq, Repr

namespace ContraGrossTradeAmt

def encode (message : ContraGrossTradeAmt) : List UInt8 :=
  encodeUIntLE 8 message.mantissa
    ++ (encodeUInt 1 message.exponent)

def decode (bytes : List UInt8) : Option (ContraGrossTradeAmt × List UInt8) := do
  let (mantissa, bytes) ← decodeUIntLE 8 bytes
  let (exponent, bytes) ← decodeUInt 1 bytes
  pure ({ mantissa, exponent }, bytes)

@[simp] theorem encode_length (message : ContraGrossTradeAmt) : (encode message).length = 9 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : ContraGrossTradeAmt) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ContraGrossTradeAmt) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ContraGrossTradeAmt

/-- Contra Calculated Ccy Last Qty: 9 bytes -/
structure ContraCalculatedCcyLastQty where
  mantissa : BitVec 64
  exponent : BitVec 8
  deriving DecidableEq, Repr

namespace ContraCalculatedCcyLastQty

def encode (message : ContraCalculatedCcyLastQty) : List UInt8 :=
  encodeUIntLE 8 message.mantissa
    ++ (encodeUInt 1 message.exponent)

def decode (bytes : List UInt8) : Option (ContraCalculatedCcyLastQty × List UInt8) := do
  let (mantissa, bytes) ← decodeUIntLE 8 bytes
  let (exponent, bytes) ← decodeUInt 1 bytes
  pure ({ mantissa, exponent }, bytes)

@[simp] theorem encode_length (message : ContraCalculatedCcyLastQty) : (encode message).length = 9 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : ContraCalculatedCcyLastQty) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ContraCalculatedCcyLastQty) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ContraCalculatedCcyLastQty

/-- Outright Order Events Group: 41 bytes -/
...
def encode (message : OutrightOrderEventsGroup) : List UInt8 :=
  encodeUIntLE 8 message.orderEventPx
    ...
    ++ (ContraGrossTradeAmt.encode message.contraGrossTradeAmt
    ++ (ContraCalculatedCcyLastQty.encode message.contraCalculatedCcyLastQty)))))))
...
def decode (bytes : List UInt8) : Option (OutrightOrderEventsGroup × List UInt8) := do
  ...
  let (contraGrossTradeAmt, bytes) ← ContraGrossTradeAmt.decode bytes
  let (contraCalculatedCcyLastQty, bytes) ← ContraCalculatedCcyLastQty.decode bytes
  pure ({ orderEventPx, orderEventText, orderEventExecId, orderEventQty, orderEventType, orderEventReason, contraGrossTradeAmt, contraCalculatedCcyLastQty }, bytes)

@[simp] theorem encode_length (message : OutrightOrderEventsGroup) : (encode message).length = 41 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, ContraGrossTradeAmt.encode_length, ContraCalculatedCcyLastQty.encode_length]
...
@[simp] theorem decode_encode (message : OutrightOrderEventsGroup) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, ContraGrossTradeAmt.decode_encode, some_bind]
  dsimp only
  rw [ContraCalculatedCcyLastQty.decode_encode, some_bind]
  rfl
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OutrightOrderEventsGroups) : (encode message).length ≤ 10458 := by
  have bound_outrightOrderEventsGroup := message.outrightOrderEventsGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const OutrightOrderEventsGroup.encode 41 OutrightOrderEventsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : OutrightOrderEventsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 OutrightOrderEventsGroup.encode OutrightOrderEventsGroup.decode OutrightOrderEventsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.outrightOrderEventsGroup.length_lt]
  rfl
...
structure ExecutionReportTradeOutright where
  ...
  benchmarkPrice : BitVec 64
  reservationPrice : BitVec 64
  priorityIndicator : BitVec 8
  displayLimitPrice : BitVec 64
...
def encode (message : ExecutionReportTradeOutright) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (CalculatedCcyLastQty.encode message.calculatedCcyLastQty
    ++ (GrossTradeAmt.encode message.grossTradeAmt
    ++ (encodeUIntLE 8 message.benchmarkPrice
    ++ (encodeUIntLE 8 message.reservationPrice
    ++ (encodeUInt 1 message.priorityIndicator
    ++ (encodeUIntLE 8 message.displayLimitPrice
...
def decode (bytes : List UInt8) : Option (ExecutionReportTradeOutright × List UInt8) := do
  ...
  let (leaves, bytes) ← decodeUIntLE 4 bytes
  ...
  let (calculatedCcyLastQty, bytes) ← CalculatedCcyLastQty.decode bytes
  let (grossTradeAmt, bytes) ← GrossTradeAmt.decode bytes
  let (benchmarkPrice, bytes) ← decodeUIntLE 8 bytes
  let (reservationPrice, bytes) ← decodeUIntLE 8 bytes
  let (priorityIndicator, bytes) ← decodeUInt 1 bytes
  let (displayLimitPrice, bytes) ← decodeUIntLE 8 bytes
  let (fillsGroups, bytes) ← FillsGroups.decode bytes
  let (outrightOrderEventsGroups, bytes) ← OutrightOrderEventsGroups.decode bytes
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, lastPx, orderId, price, stopPx, transactTime, sendingTimeEpoch, orderRequestId, secExecId, crossIdOptional, hostCrossId, location, securityId, orderQty, lastQty, cumQty, mdTradeEntryId, sideTradeId, tradeLinkId, leaves, tradeDate, expireDate, ordStatusTrd, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, aggressorIndicator, crossTypeOptional, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, ownership, discretionPrice, trdType, execRestatementReason, settlDate, maturityDate, calculatedCcyLastQty, grossTradeAmt, benchmarkPrice, reservationPrice, priorityIndicator, displayLimitPrice, fillsGroups, outrightOrderEventsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeOutright) : (encode message).length ≤ 14579 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, OrdTypeOptional.encode_length, ExecutionMode.encode_length, CalculatedCcyLastQty.encode_length, GrossTradeAmt.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeOutright) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, CalculatedCcyLastQty.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, GrossTradeAmt.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, FillsGroups.decode_encode, some_bind]
  dsimp only
  rw [OutrightOrderEventsGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : TradeLegsGroups) : (encode message).length ≤ 7398 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TradeLegsGroup.encode 29 TradeLegsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TradeLegsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 TradeLegsGroup.encode TradeLegsGroup.decode TradeLegsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.tradeLegsGroup.length_lt]
  rfl
...
theorem encode_length_le (message : SpreadOrderEventsGroups) : (encode message).length ≤ 5868 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SpreadOrderEventsGroup.encode 23 SpreadOrderEventsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SpreadOrderEventsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SpreadOrderEventsGroup.encode SpreadOrderEventsGroup.decode SpreadOrderEventsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.spreadOrderEventsGroup.length_lt]
  rfl
...
def decode (bytes : List UInt8) : Option (ExecutionReportTradeSpread × List UInt8) := do
  ...
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, lastPx, orderId, price, stopPx, transactTime, sendingTimeEpoch, orderRequestId, secExecId, crossIdOptional, hostCrossId, location, securityId, orderQty, lastQty, cumQty, mdTradeEntryId, sideTradeId, leaves, tradeDate, expireDate, ordStatusTrd, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, aggressorIndicator, crossTypeOptional, totalNumSecurities, execInst, executionMode, liquidityFlag, shortSaleType, fillsGroups, tradeLegsGroups, spreadOrderEventsGroups }, bytes)
...
theorem encode_length_le (message : ExecutionReportTradeSpread) : (encode message).length ≤ 17324 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, OrdTypeOptional.encode_length, ExecutionMode.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeSpread) (rest : List UInt8) :
  ...
  rw [List.append_assoc, FillsGroups.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, TradeLegsGroups.decode_encode, some_bind]
  dsimp only
  rw [SpreadOrderEventsGroups.decode_encode, some_bind]
  rfl
...
/-- Spread Leg Order Events Group: 41 bytes -/
...
@[simp] theorem encode_length (message : SpreadLegOrderEventsGroup) : (encode message).length = 41 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, ContraGrossTradeAmt.encode_length, ContraCalculatedCcyLastQty.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SpreadLegOrderEventsGroups) : (encode message).length ≤ 10458 := by
  have bound_spreadLegOrderEventsGroup := message.spreadLegOrderEventsGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SpreadLegOrderEventsGroup.encode 41 SpreadLegOrderEventsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SpreadLegOrderEventsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SpreadLegOrderEventsGroup.encode SpreadLegOrderEventsGroup.decode SpreadLegOrderEventsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.spreadLegOrderEventsGroup.length_lt]
  rfl
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeSpreadLeg) : (encode message).length ≤ 14505 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
  have bound_spreadLegOrderEventsGroups := SpreadLegOrderEventsGroups.encode_length_le message.spreadLegOrderEventsGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, Volatility.encode_length, OptionDelta.encode_length, TimeToExpiration.encode_length, RiskFreeRate.encode_length, encodeUInt_length, OrdTypeOptional.encode_length, CalculatedCcyLastQty.encode_length, GrossTradeAmt.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeSpreadLeg) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, Volatility.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, OptionDelta.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, TimeToExpiration.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, RiskFreeRate.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  ...
  rw [List.append_assoc, FillsGroups.decode_encode, some_bind]
  dsimp only
  rw [SpreadLegOrderEventsGroups.decode_encode, some_bind]
  rfl

end ExecutionReportTradeSpreadLeg

/-- Execution Report Modify: 226 bytes -/
...
def decode (bytes : List UInt8) : Option (ExecutionReportModify × List UInt8) := do
  ...
  let (discretionPrice, bytes) ← decodeUIntLE 8 bytes
  let (priorityIndicator, bytes) ← decodeUInt 1 bytes
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, orderId, price, stopPx, transactTime, sendingTimeEpoch, orderRequestId, crossIdOptional, hostCrossId, location, securityId, orderQty, cumQty, leaves, minQty, displayQty, expireDate, delayDuration, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, splitMsg, crossTypeOptional, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, delayToTime, discretionPrice, priorityIndicator }, bytes)

@[simp] theorem encode_length (message : ExecutionReportModify) : (encode message).length = 226 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length, ExecutionMode.encode_length]
...
/-- Execution Report Status: 513 bytes -/
...
def encode (message : ExecutionReportStatus) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (Alpha.encode message.cancelText))))))))))))))))))))))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (ExecutionReportStatus × List UInt8) := do
  ...
  let (cancelText, bytes) ← Alpha.decode 8 bytes
  pure ({ seqNum, uuid, text, execId, senderId, clordid, partyDetailsListReqId, orderId, priceOptional, stopPx, transactTime, sendingTimeEpoch, orderRequestId, ordStatusReqIdOptional, massStatusReqIdOptional, crossIdOptional, hostCrossId, location, securityId, orderQty, cumQty, leaves, minQty, displayQty, expireDate, ordStatus, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, lastRptRequested, crossTypeOptional, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, discretionPrice, reservationPrice, priorityIndicator, origOrderUser, cancelText }, bytes)

@[simp] theorem encode_length (message : ExecutionReportStatus) : (encode message).length = 513 := by
  unfold encode
...
/-- Execution Report Cancel: 247 bytes -/
...
theorem encode_length_le (message : PartyDetailsListReport) : (encode message).length ≤ 6219 := by
  ...
  have bound_trdRegPublicationsGroups := TrdRegPublicationsGroups.encode_length_le message.trdRegPublicationsGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, SelfMatchPreventionInstruction.encode_length, CmtaGiveupCd.encode_length, CustOrderHandlingInst.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionAck) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, DkReason.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
...
theorem encode_length_le (message : QuoteAckEntriesGroups) : (encode message).length ≤ 2808 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteAckEntriesGroup.encode 11 QuoteAckEntriesGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : QuoteAckEntriesGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 QuoteAckEntriesGroup.encode QuoteAckEntriesGroup.decode QuoteAckEntriesGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.quoteAckEntriesGroup.length_lt]
  rfl
...
def encode (message : MassQuoteAck) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (encodeUInt 1 message.manualOrderIndicator
    ++ (encodeUInt 1 message.noProcessedEntries
    ...
    ++ (encodeUInt 1 message.quoteEntryControl
...
def decode (bytes : List UInt8) : Option (MassQuoteAck × List UInt8) := do
  ...
  let (manualOrderIndicator, bytes) ← decodeUInt 1 bytes
  let (noProcessedEntries, bytes) ← decodeUInt 1 bytes
  ...
  let (quoteEntryControl, bytes) ← decodeUInt 1 bytes
  let (quoteAckEntriesGroups, bytes) ← QuoteAckEntriesGroups.decode bytes
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, requestTime, sendingTimeEpoch, quoteReqIdOptional, location, quoteId, quoteRejectReason, delayDuration, quoteAckStatus, manualOrderIndicator, noProcessedEntries, mmProtectionReset, splitMsg, liquidityFlag, shortSaleType, totNoQuoteEntriesOptional, possRetransFlag, delayToTime, quoteEntryOpen, quoteEntryControl, quoteAckEntriesGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MassQuoteAck) : (encode message).length ≤ 3160 := by
...
@[simp] theorem decode_encode (message : MassQuoteAck) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [QuoteAckEntriesGroups.decode_encode, some_bind]
  rfl
...
/-- Outright Trade Events Group: 45 bytes -/
...
def decode (bytes : List UInt8) : Option (OutrightTradeEventsGroup × List UInt8) := do
  ...
  let (contraGrossTradeAmt, bytes) ← ContraGrossTradeAmt.decode bytes
  let (contraCalculatedCcyLastQty, bytes) ← ContraCalculatedCcyLastQty.decode bytes
  pure ({ orderEventPx, orderEventText, orderEventExecId, orderEventQty, tradeAddendum, orderEventReason, originalOrderEventExecId, contraGrossTradeAmt, contraCalculatedCcyLastQty }, bytes)

@[simp] theorem encode_length (message : OutrightTradeEventsGroup) : (encode message).length = 45 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, ContraGrossTradeAmt.encode_length, ContraCalculatedCcyLastQty.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OutrightTradeEventsGroups) : (encode message).length ≤ 11478 := by
  have bound_outrightTradeEventsGroup := message.outrightTradeEventsGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const OutrightTradeEventsGroup.encode 45 OutrightTradeEventsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : OutrightTradeEventsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 OutrightTradeEventsGroup.encode OutrightTradeEventsGroup.decode OutrightTradeEventsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.outrightTradeEventsGroup.length_lt]
  rfl
...
def encode (message : ExecutionReportTradeAddendumOutright) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (encodeUIntLE 8 message.benchmarkPrice
    ++ (encodeUIntLE 4 message.mdTradeEntryId
    ++ (Alpha.encode message.execRefId
    ++ (FillsGroups.encode message.fillsGroups
...
def decode (bytes : List UInt8) : Option (ExecutionReportTradeAddendumOutright × List UInt8) := do
  ...
  let (benchmarkPrice, bytes) ← decodeUIntLE 8 bytes
  let (mdTradeEntryId, bytes) ← decodeUIntLE 4 bytes
  let (execRefId, bytes) ← Alpha.decode 40 bytes
  let (fillsGroups, bytes) ← FillsGroups.decode bytes
  let (outrightTradeEventsGroups, bytes) ← OutrightTradeEventsGroups.decode bytes
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, lastPx, orderId, transactTime, sendingTimeEpoch, secExecId, origSecondaryExecutionId, location, securityId, lastQty, sideTradeId, origSideTradeId, tradeDate, ordStatusTrdCxl, execType, side, manualOrderIndicator, possRetransFlag, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, discretionPrice, trdType, execRestatementReason, settlDate, maturityDate, calculatedCcyLastQty, grossTradeAmt, benchmarkPrice, mdTradeEntryId, execRefId, fillsGroups, outrightTradeEventsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeAddendumOutright) : (encode message).length ≤ 15572 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
  have bound_outrightTradeEventsGroups := OutrightTradeEventsGroups.encode_length_le message.outrightTradeEventsGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, OrdStatusTrdCxl.encode_length, ExecType.encode_length, encodeUInt_length, ExecutionMode.encode_length, CalculatedCcyLastQty.encode_length, GrossTradeAmt.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeAddendumOutright) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, OrdStatusTrdCxl.decode_encode, some_bind]
  dsimp only
  ...
  rw [List.append_assoc, FillsGroups.decode_encode, some_bind]
  dsimp only
  rw [OutrightTradeEventsGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : TradeAddendumLegsGroups) : (encode message).length ≤ 10458 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TradeAddendumLegsGroup.encode 41 TradeAddendumLegsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TradeAddendumLegsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 TradeAddendumLegsGroup.encode TradeAddendumLegsGroup.decode TradeAddendumLegsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.tradeAddendumLegsGroup.length_lt]
  rfl
...
theorem encode_length_le (message : SpreadTradeEventsGroups) : (encode message).length ≤ 6888 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SpreadTradeEventsGroup.encode 27 SpreadTradeEventsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SpreadTradeEventsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SpreadTradeEventsGroup.encode SpreadTradeEventsGroup.decode SpreadTradeEventsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.spreadTradeEventsGroup.length_lt]
  rfl
...
def decode (bytes : List UInt8) : Option (ExecutionReportTradeAddendumSpread × List UInt8) := do
  ...
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, lastPx, orderId, transactTime, sendingTimeEpoch, secExecId, origSecondaryExecutionId, location, securityId, mdTradeEntryId, lastQty, sideTradeId, origSideTradeId, tradeDate, ordStatusTrdCxl, execType, ordTypeOptional, side, manualOrderIndicator, possRetransFlag, totalNumSecurities, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, execRefId, fillsGroups, tradeAddendumLegsGroups, spreadTradeEventsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeAddendumSpread) : (encode message).length ≤ 21401 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, OrdStatusTrdCxl.encode_length, ExecType.encode_length, OrdTypeOptional.encode_length, encodeUInt_length, ExecutionMode.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeAddendumSpread) (rest : List UInt8) :
  ...
  rw [List.append_assoc, FillsGroups.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, TradeAddendumLegsGroups.decode_encode, some_bind]
  dsimp only
  rw [SpreadTradeEventsGroups.decode_encode, some_bind]
  rfl

end ExecutionReportTradeAddendumSpread

/-- Spread Leg Trade Events Group: 45 bytes -/
...
@[simp] theorem encode_length (message : SpreadLegTradeEventsGroup) : (encode message).length = 45 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, ContraGrossTradeAmt.encode_length, ContraCalculatedCcyLastQty.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SpreadLegTradeEventsGroups) : (encode message).length ≤ 11478 := by
  have bound_spreadLegTradeEventsGroup := message.spreadLegTradeEventsGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SpreadLegTradeEventsGroup.encode 45 SpreadLegTradeEventsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SpreadLegTradeEventsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SpreadLegTradeEventsGroup.encode SpreadLegTradeEventsGroup.decode SpreadLegTradeEventsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.spreadLegTradeEventsGroup.length_lt]
  rfl
...
def decode (bytes : List UInt8) : Option (ExecutionReportTradeAddendumSpreadLeg × List UInt8) := do
  ...
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, lastPx, orderId, transactTime, sendingTimeEpoch, secExecId, origSecondaryExecutionId, location, securityId, lastQty, sideTradeId, origSideTradeId, tradeDate, ordStatusTrdCxl, execType, manualOrderIndicator, possRetransFlag, side, settlDate, calculatedCcyLastQty, grossTradeAmt, execRefId, fillsGroups, spreadLegTradeEventsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeAddendumSpreadLeg) : (encode message).length ≤ 15542 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
  have bound_spreadLegTradeEventsGroups := SpreadLegTradeEventsGroups.encode_length_le message.spreadLegTradeEventsGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, OrdStatusTrdCxl.encode_length, ExecType.encode_length, encodeUInt_length, CalculatedCcyLastQty.encode_length, GrossTradeAmt.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeAddendumSpreadLeg) (rest : List UInt8) :
  ...
  rw [List.append_assoc, FillsGroups.decode_encode, some_bind]
  dsimp only
  rw [SpreadLegTradeEventsGroups.decode_encode, some_bind]
  rfl
...
@[simp] theorem decode_encode (message : ResponseLegsGroup) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, LegOptionDelta.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
...
theorem encode_length_le (message : ResponseLegsGroups) : (encode message).length ≤ 4848 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const ResponseLegsGroup.encode 19 ResponseLegsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ResponseLegsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 ResponseLegsGroup.encode ResponseLegsGroup.decode ResponseLegsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.responseLegsGroup.length_lt]
  rfl

end ResponseLegsGroups

/-- Broken Dates Response Group: 16 bytes -/
structure BrokenDatesResponseGroup where
...
namespace BrokenDatesResponseGroup

def encode (message : BrokenDatesResponseGroup) : List UInt8 :=
...
def decode (bytes : List UInt8) : Option (BrokenDatesResponseGroup × List UInt8) := do
...
@[simp] theorem encode_length (message : BrokenDatesResponseGroup) : (encode message).length = 16 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]

theorem encode_length_pos (message : BrokenDatesResponseGroup) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : BrokenDatesResponseGroup) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end BrokenDatesResponseGroup

/-- Broken Dates Response Groups -/
structure BrokenDatesResponseGroups where
  blockLength : BitVec 16
  brokenDatesResponseGroup : Bounded 1 BrokenDatesResponseGroup
  deriving DecidableEq, Repr

namespace BrokenDatesResponseGroups

def encode (message : BrokenDatesResponseGroups) : List UInt8 :=
  encodeUIntLE 2 message.blockLength
    ++ (encodeUInt 1 (BitVec.ofNat (8 * 1) message.brokenDatesResponseGroup.val.length)
    ++ (encodeMany BrokenDatesResponseGroup.encode message.brokenDatesResponseGroup.val))

def decode (bytes : List UInt8) : Option (BrokenDatesResponseGroups × List UInt8) := do
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
  let (numInGroup, bytes) ← decodeUInt 1 bytes
  let (brokenDatesResponseGroup_, bytes) ← decodeMany BrokenDatesResponseGroup.decode numInGroup.toNat bytes
  if fits_brokenDatesResponseGroup : brokenDatesResponseGroup_.length < 256 ^ 1 then
    pure ({ blockLength, brokenDatesResponseGroup := ⟨brokenDatesResponseGroup_, fits_brokenDatesResponseGroup⟩ }, bytes)
  else none

theorem encode_length_pos (message : BrokenDatesResponseGroups) : (encode message).length > 0 := by
  unfold encode
  simp only [encodeUIntLE_length, List.length_append, ← Nat.add_assoc]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : BrokenDatesResponseGroups) : (encode message).length ≤ 4083 := by
  have bound_brokenDatesResponseGroup := message.brokenDatesResponseGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const BrokenDatesResponseGroup.encode 16 BrokenDatesResponseGroup.encode_length]
  omega

@[simp] theorem decode_encode (message : BrokenDatesResponseGroups) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 BrokenDatesResponseGroup.encode BrokenDatesResponseGroup.decode BrokenDatesResponseGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.brokenDatesResponseGroup.length_lt]
  rfl

end BrokenDatesResponseGroups
...
structure SecurityDefinitionResponse where
  ...
  brokenDatesResponseGroups : BrokenDatesResponseGroups
  deriving DecidableEq, Repr
...
def encode (message : SecurityDefinitionResponse) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (BrokenDatesResponseGroups.encode message.brokenDatesResponseGroups))))))))))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (SecurityDefinitionResponse × List UInt8) := do
  ...
  let (brokenDatesResponseGroups, bytes) ← BrokenDatesResponseGroups.decode bytes
  pure ({ seqNum, uuid, text, financialInstrumentFullName, senderId, symbol, partyDetailsListReqId, securityReqId, securityResponseId, sendingTimeEpoch, securityGroup, securityType, location, securityIdOptional, currency, maturityMonthYear, delayDuration, startDate, endDate, maxNoOfSubstitutions, sourceRepoId, terminationType, securityResponseType, expirationCycle, manualOrderIndicator, splitMsg, autoQuoteRequest, possRetransFlag, brokenDateTermType, responseLegsGroups, brokenDatesResponseGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SecurityDefinitionResponse) : (encode message).length ≤ 9361 := by
  have bound_responseLegsGroups := ResponseLegsGroups.encode_length_le message.responseLegsGroups
  have bound_brokenDatesResponseGroups := BrokenDatesResponseGroups.encode_length_le message.brokenDatesResponseGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, MaturityMonthYear.encode_length, encodeUInt_length]
  omega
...
@[simp] theorem decode_encode (message : SecurityDefinitionResponse) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, ResponseLegsGroups.decode_encode, some_bind]
  dsimp only
  rw [BrokenDatesResponseGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : AffectedOrdersGroups) : (encode message).length ≤ 8163 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const AffectedOrdersGroup.encode 32 AffectedOrdersGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : AffectedOrdersGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 AffectedOrdersGroup.encode AffectedOrdersGroup.decode AffectedOrdersGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.affectedOrdersGroup.length_lt]
  rfl
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderMassActionReport) : (encode message).length ≤ 8293 := by
  have bound_affectedOrdersGroups := AffectedOrdersGroups.encode_length_le message.affectedOrdersGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, MassActionOrdTyp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : OrderMassActionReport) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, MassActionOrdTyp.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [AffectedOrdersGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : QuoteCancelAckEntriesGroups) : (encode message).length ≤ 2298 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteCancelAckEntriesGroup.encode 9 QuoteCancelAckEntriesGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : QuoteCancelAckEntriesGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 QuoteCancelAckEntriesGroup.encode QuoteCancelAckEntriesGroup.decode QuoteCancelAckEntriesGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.quoteCancelAckEntriesGroup.length_lt]
  rfl
...
theorem encode_length_le (message : QuoteCancelAckSetsGroups) : (encode message).length ≤ 1023 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteCancelAckSetsGroup.encode 4 QuoteCancelAckSetsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : QuoteCancelAckSetsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 QuoteCancelAckSetsGroup.encode QuoteCancelAckSetsGroup.decode QuoteCancelAckSetsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.quoteCancelAckSetsGroup.length_lt]
  rfl
...
structure QuoteCancelAck where
  ...
  noProcessedEntries32 : BitVec 32
  mmProtectionReset : BitVec 8
...
def encode (message : QuoteCancelAck) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (encodeUIntLE 4 message.noProcessedEntries32
    ++ (encodeUInt 1 message.mmProtectionReset
    ...
    ++ (Alpha.encode message.cancelText
    ++ (encodeUInt 1 message.quoteEntryOpen
    ++ (encodeUInt 1 message.cxlLinkedSessions
    ++ (encodeUInt 1 message.quoteEntryControl
...
def decode (bytes : List UInt8) : Option (QuoteCancelAck × List UInt8) := do
  ...
  let (noProcessedEntries32, bytes) ← decodeUIntLE 4 bytes
  let (mmProtectionReset, bytes) ← decodeUInt 1 bytes
  ...
  let (cancelText, bytes) ← Alpha.decode 8 bytes
  let (quoteEntryOpen, bytes) ← decodeUInt 1 bytes
  let (cxlLinkedSessions, bytes) ← decodeUInt 1 bytes
  let (quoteEntryControl, bytes) ← decodeUInt 1 bytes
  ...
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, requestTime, sendingTimeEpoch, cancelledSymbol, location, quoteId, quoteRejectReason, delayDuration, manualOrderIndicator, quoteCxlStatus, noProcessedEntries32, mmProtectionReset, unsolicitedCancelType, splitMsg, totNoQuoteEntriesOptional, liquidityFlag, possRetransFlag, delayToTime, origOrderUser, cancelText, quoteEntryOpen, cxlLinkedSessions, quoteEntryControl, quoteCancelAckEntriesGroups, quoteCancelAckSetsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteCancelAck) : (encode message).length ≤ 3691 := by
...
@[simp] theorem decode_encode (message : QuoteCancelAck) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, QuoteCancelAckEntriesGroups.decode_encode, some_bind]
  dsimp only
  rw [QuoteCancelAckSetsGroups.decode_encode, some_bind]
  rfl

end QuoteCancelAck

/-- Execution Report Pending Cancel: 219 bytes -/
structure ExecutionReportPendingCancel where
  seqNum : BitVec 32
  uuid : BitVec 64
  execId : Alpha 40
  senderId : Alpha 20
...
namespace ExecutionReportPendingCancel

def encode (message : ExecutionReportPendingCancel) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ++ (encodeUIntLE 8 message.uuid
    ++ (Alpha.encode message.execId
    ++ (Alpha.encode message.senderId
    ...
    ++ (encodeUIntLE 4 message.orderQty
    ++ (encodeUIntLE 4 message.cumQty
    ++ (encodeUIntLE 4 message.leavesQtyOptional
    ++ (encodeUIntLE 4 message.minQty
    ++ (encodeUIntLE 4 message.displayQty
    ++ (encodeUIntLE 2 message.expireDate
    ++ (OrdTypeOptional.encode message.ordTypeOptional
...
-- a long run of fields nests deeper than the elaborator's default limit
set_option maxRecDepth 4096 in
def decode (bytes : List UInt8) : Option (ExecutionReportPendingCancel × List UInt8) := do
  let (seqNum, bytes) ← decodeUIntLE 4 bytes
  let (uuid, bytes) ← decodeUIntLE 8 bytes
  let (execId, bytes) ← Alpha.decode 40 bytes
  let (senderId, bytes) ← Alpha.decode 20 bytes
  ...
  let (orderQty, bytes) ← decodeUIntLE 4 bytes
  let (cumQty, bytes) ← decodeUIntLE 4 bytes
  let (leavesQtyOptional, bytes) ← decodeUIntLE 4 bytes
  let (minQty, bytes) ← decodeUIntLE 4 bytes
  let (displayQty, bytes) ← decodeUIntLE 4 bytes
  let (expireDate, bytes) ← decodeUIntLE 2 bytes
  let (ordTypeOptional, bytes) ← OrdTypeOptional.decode bytes
  ...
  let (origOrderUser, bytes) ← Alpha.decode 8 bytes
  let (cancelText, bytes) ← Alpha.decode 8 bytes
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, orderId, price, transactTime, sendingTimeEpoch, orderRequestId, location, securityId, orderQty, cumQty, leavesQtyOptional, minQty, displayQty, expireDate, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, splitMsg, liquidityFlag, delayToTime, discretionPrice, reservationPrice, priorityIndicator, origOrderUser, cancelText }, bytes)

@[simp] theorem encode_length (message : ExecutionReportPendingCancel) : (encode message).length = 219 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length]

theorem encode_length_pos (message : ExecutionReportPendingCancel) : (encode message).length > 0 := by
  rw [encode_length]
  decide

set_option maxRecDepth 4096 in
@[simp] theorem decode_encode (message : ExecutionReportPendingCancel) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ExecutionReportPendingCancel

/-- Execution Report Pending Replace: 196 bytes -/
structure ExecutionReportPendingReplace where
  seqNum : BitVec 32
  uuid : BitVec 64
  execId : Alpha 40
  senderId : Alpha 20
...
namespace ExecutionReportPendingReplace

def encode (message : ExecutionReportPendingReplace) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ++ (encodeUIntLE 8 message.uuid
    ++ (Alpha.encode message.execId
    ++ (Alpha.encode message.senderId
...
-- a long run of fields nests deeper than the elaborator's default limit
set_option maxRecDepth 4096 in
def decode (bytes : List UInt8) : Option (ExecutionReportPendingReplace × List UInt8) := do
  let (seqNum, bytes) ← decodeUIntLE 4 bytes
  let (uuid, bytes) ← decodeUIntLE 8 bytes
  let (execId, bytes) ← Alpha.decode 40 bytes
  let (senderId, bytes) ← Alpha.decode 20 bytes
  ...
  let (discretionPrice, bytes) ← decodeUIntLE 8 bytes
  let (priorityIndicator, bytes) ← decodeUInt 1 bytes
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, orderId, price, transactTime, sendingTimeEpoch, orderRequestId, location, securityId, orderQty, cumQty, leaves, minQty, displayQty, expireDate, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, splitMsg, liquidityFlag, shortSaleType, delayToTime, discretionPrice, priorityIndicator }, bytes)

@[simp] theorem encode_length (message : ExecutionReportPendingReplace) : (encode message).length = 196 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length]

theorem encode_length_pos (message : ExecutionReportPendingReplace) : (encode message).length > 0 := by
  rw [encode_length]
  decide

set_option maxRecDepth 4096 in
@[simp] theorem decode_encode (message : ExecutionReportPendingReplace) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ExecutionReportPendingReplace

/-- Mass Quote Response Entries Group: 20 bytes -/
structure MassQuoteResponseEntriesGroup where
  orderIdOptional : BitVec 64
  quoteEntryId : BitVec 32
  quoteSetId : BitVec 16
  securityId : BitVec 32
  sideSide : BitVec 8
  quoteEntryRejectReasonOptional : BitVec 8
  deriving DecidableEq, Repr

namespace MassQuoteResponseEntriesGroup

def encode (message : MassQuoteResponseEntriesGroup) : List UInt8 :=
  encodeUIntLE 8 message.orderIdOptional
    ++ (encodeUIntLE 4 message.quoteEntryId
    ++ (encodeUIntLE 2 message.quoteSetId
    ++ (encodeUIntLE 4 message.securityId
    ++ (encodeUInt 1 message.sideSide
    ++ (encodeUInt 1 message.quoteEntryRejectReasonOptional)))))

def decode (bytes : List UInt8) : Option (MassQuoteResponseEntriesGroup × List UInt8) := do
  let (orderIdOptional, bytes) ← decodeUIntLE 8 bytes
  let (quoteEntryId, bytes) ← decodeUIntLE 4 bytes
  let (quoteSetId, bytes) ← decodeUIntLE 2 bytes
  let (securityId, bytes) ← decodeUIntLE 4 bytes
  let (sideSide, bytes) ← decodeUInt 1 bytes
  let (quoteEntryRejectReasonOptional, bytes) ← decodeUInt 1 bytes
  pure ({ orderIdOptional, quoteEntryId, quoteSetId, securityId, sideSide, quoteEntryRejectReasonOptional }, bytes)

@[simp] theorem encode_length (message : MassQuoteResponseEntriesGroup) : (encode message).length = 20 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : MassQuoteResponseEntriesGroup) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : MassQuoteResponseEntriesGroup) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end MassQuoteResponseEntriesGroup

/-- Mass Quote Response Entries Groups -/
structure MassQuoteResponseEntriesGroups where
  blockLength : BitVec 16
  massQuoteResponseEntriesGroup : Bounded 1 MassQuoteResponseEntriesGroup
  deriving DecidableEq, Repr

namespace MassQuoteResponseEntriesGroups

def encode (message : MassQuoteResponseEntriesGroups) : List UInt8 :=
  encodeUIntLE 2 message.blockLength
    ++ (encodeUInt 1 (BitVec.ofNat (8 * 1) message.massQuoteResponseEntriesGroup.val.length)
    ++ (encodeMany MassQuoteResponseEntriesGroup.encode message.massQuoteResponseEntriesGroup.val))

def decode (bytes : List UInt8) : Option (MassQuoteResponseEntriesGroups × List UInt8) := do
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
  let (numInGroup, bytes) ← decodeUInt 1 bytes
  let (massQuoteResponseEntriesGroup_, bytes) ← decodeMany MassQuoteResponseEntriesGroup.decode numInGroup.toNat bytes
  if fits_massQuoteResponseEntriesGroup : massQuoteResponseEntriesGroup_.length < 256 ^ 1 then
    pure ({ blockLength, massQuoteResponseEntriesGroup := ⟨massQuoteResponseEntriesGroup_, fits_massQuoteResponseEntriesGroup⟩ }, bytes)
  else none

theorem encode_length_pos (message : MassQuoteResponseEntriesGroups) : (encode message).length > 0 := by
  unfold encode
  simp only [encodeUIntLE_length, List.length_append, ← Nat.add_assoc]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MassQuoteResponseEntriesGroups) : (encode message).length ≤ 5103 := by
  have bound_massQuoteResponseEntriesGroup := message.massQuoteResponseEntriesGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const MassQuoteResponseEntriesGroup.encode 20 MassQuoteResponseEntriesGroup.encode_length]
  omega

@[simp] theorem decode_encode (message : MassQuoteResponseEntriesGroups) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 MassQuoteResponseEntriesGroup.encode MassQuoteResponseEntriesGroup.decode MassQuoteResponseEntriesGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.massQuoteResponseEntriesGroup.length_lt]
  rfl

end MassQuoteResponseEntriesGroups
...
structure MassQuoteResponse where
  ...
  quoteEntryOpen : BitVec 8
  quoteEntryControl : BitVec 8
  massQuoteResponseEntriesGroups : MassQuoteResponseEntriesGroups
  deriving DecidableEq, Repr
...
def encode (message : MassQuoteResponse) : List UInt8 :=
  encodeUIntLE 4 message.seqNum
    ...
    ++ (encodeUInt 1 message.quoteEntryOpen
    ++ (encodeUInt 1 message.quoteEntryControl
    ++ (MassQuoteResponseEntriesGroups.encode message.massQuoteResponseEntriesGroups)))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (MassQuoteResponse × List UInt8) := do
  ...
  let (quoteEntryOpen, bytes) ← decodeUInt 1 bytes
  let (quoteEntryControl, bytes) ← decodeUInt 1 bytes
  let (massQuoteResponseEntriesGroups, bytes) ← MassQuoteResponseEntriesGroups.decode bytes
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, transactTime, sendingTimeEpoch, quoteReqIdOptional, location, quoteId, quoteRejectReason, quoteAckStatus, manualOrderIndicator, noProcessedEntries, mmProtectionReset, splitMsg, liquidityFlag, shortSaleType, totNoQuoteEntriesOptional, possRetransFlag, delayToTime, quoteEntryOpen, quoteEntryControl, massQuoteResponseEntriesGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MassQuoteResponse) : (encode message).length ≤ 5453 := by
  have bound_massQuoteResponseEntriesGroups := MassQuoteResponseEntriesGroups.encode_length_le message.massQuoteResponseEntriesGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length]
  omega
...
@[simp] theorem decode_encode (message : MassQuoteResponse) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [MassQuoteResponseEntriesGroups.decode_encode, some_bind]
  rfl
...
inductive ServerPayload where
  ...
  | executionReportPendingCancel (message : ExecutionReportPendingCancel) -- 564
  | executionReportPendingReplace (message : ExecutionReportPendingReplace) -- 565
  | massQuoteResponse (message : MassQuoteResponse) -- 571
  deriving DecidableEq, Repr
...
def tag : ServerPayload → BitVec 16
  ...
  | .executionReportPendingCancel _ => 564
  | .executionReportPendingReplace _ => 565
  | .massQuoteResponse _ => 571
...
def encode : ServerPayload → List UInt8
  ...
  | .executionReportPendingCancel message => ExecutionReportPendingCancel.encode message
  | .executionReportPendingReplace message => ExecutionReportPendingReplace.encode message
  | .massQuoteResponse message => MassQuoteResponse.encode message

/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 65570 := by
  cases message with
  ...
  | quoteCancelAck inner =>
    ...
    simp only [encode]
    omega
  | executionReportPendingCancel inner =>
    simp only [encode, ExecutionReportPendingCancel.encode_length]
    omega
  | executionReportPendingReplace inner =>
    simp only [encode, ExecutionReportPendingReplace.encode_length]
    omega
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (ServerPayload × List UInt8) :=
  ...
  else if tag = 564 then (ExecutionReportPendingCancel.decode bytes).map fun (message, rest) => (.executionReportPendingCancel message, rest)
  else if tag = 565 then (ExecutionReportPendingReplace.decode bytes).map fun (message, rest) => (.executionReportPendingReplace message, rest)
  else if tag = 571 then (MassQuoteResponse.decode bytes).map fun (message, rest) => (.massQuoteResponse message, rest)
  else none
...
theorem decode_encode (message : ServerPacket) : decode (encode message) = some message := by
  unfold decode encode
  rw [decodeAll_encodeMany ServerSimpleOpenFrame.encode ServerSimpleOpenFrame.decode ServerSimpleOpenFrame.decode_encode ServerSimpleOpenFrame.encode_length_pos message.serverSimpleOpenFrame _ (encodeMany_length_ge ServerSimpleOpenFrame.encode ServerSimpleOpenFrame.encode_length_pos message.serverSimpleOpenFrame), some_bind]
  rfl

end ServerPacket

end Omi.CmeGlobexIlink3SbeV89Server
