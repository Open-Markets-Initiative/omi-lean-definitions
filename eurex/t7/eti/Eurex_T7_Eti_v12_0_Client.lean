...
namespace Omi.EurexT7EtiFbeV120Client
...
/-- Approve Tes Trade Request: 258 bytes -/
...
@[simp] theorem encode_length (message : ApproveTesTradeRequest) : (encode message).length = 258 := by
  unfold encode
...
/-- Cross Request Side Grp Comp: 176 bytes -/
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterClipRequest) : (encode message).length ≤ 47002 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const CrossRequestSideGrpComp.encode 176 CrossRequestSideGrpComp.encode_length, encodeMany_length_const SideCrossLegGrpComp.encode 8 SideCrossLegGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterTesTradeRequest) : (encode message).length ≤ 36970 := by
...
def encode (message : ModifyOrderRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ...
    ++ encodeUInt 1 message.execInst
    ++ encodeUInt 1 message.timeInForce
    ++ Alpha.encode message.checkSumCorrection
    ++ Alpha.encode message.pad3
    ++ encodeMany LegOrdGrpComp.encode message.legOrdGrpComp.val
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyOrderRequest) : (encode message).length ≤ 2330 := by
  have bound_legOrdGrpComp := message.legOrdGrpComp.length_lt
  unfold encode
...
/-- Modify Order Short Request: 130 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyTesTradeRequest × List UInt8) := do
  ...
  if fits_sideAllocGrpComp : sideAllocGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, lastPx, transBkdTime, compressionId, relatedClosePrice, relatedPx, marketSegmentId, packageId, tesExecId, relatedMarketSegmentId, trdType, tradeReportType, tradePublishIndicator, swapClearer, tradeReportText, tradeReportId, pad1, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyTesTradeRequest) : (encode message).length ≤ 14402 := by
  have bound_sideAllocGrpComp := message.sideAllocGrpComp.length_lt
  have bound_trdInstrmntLegGrpComp := message.trdInstrmntLegGrpComp.length_lt
  unfold encode
...
def decode (bytes : List UInt8) : Option (NewOrderRequest × List UInt8) := do
  ...
  let (legOrdGrpComp_, bytes) ← decodeMany LegOrdGrpComp.decode noLegOnbooks.toNat bytes
  if fits_legOrdGrpComp : legOrdGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, stopPx, partyIdClientId, executingTrader, matchInstCrossId, expireDate, tradingSessionSubId, applSeqIndicator, ordType, priceValidityCheckType, valueCheckTypeValue, orderOrigination, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, account, partyIdPositionAccount, positionEffect, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, fixClOrdId, partyEndClientIdentification, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, orderAttributeRiskReduction, pad4, partyIdInvestmentDecisionMaker, orderAttributeLiquidityProvision, tradingCapacity, productComplex, marketSegmentId, securityId, orderQty, price, side, execInst, timeInForce, checkSumCorrection, pad3, legOrdGrpComp := ⟨legOrdGrpComp_, fits_legOrdGrpComp⟩ }, bytes)
  else none
...
/-- New Order Short Request: 122 bytes -/
...
@[simp] theorem encode_length (message : NewOrderShortRequest) : (encode message).length = 122 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (NewOrderSingleRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, price, orderQty, stopPx, clOrdId, securityId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, expireDate, marketSegmentId, matchInstCrossId, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, applSeqIndicator, productComplex, side, ordType, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, orderAttributeRiskReduction, timeInForce, execInst, tradingSessionSubId, tradingCapacity, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, account, partyIdPositionAccount, positionEffect, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, fixClOrdId, partyEndClientIdentification, pad2v2 }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UploadTesTradeRequest) : (encode message).length ≤ 73690 := by
...
end Omi.EurexT7EtiFbeV120Client
