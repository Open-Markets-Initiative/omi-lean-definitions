...
/-!
# CME Group Market Data Platform 3 v1.10
...
namespace Omi.CmeGlobexMdp3SbeV110Udp
...
/-- Repo Underlyings Group: 108 bytes -/
...
def decode (bytes : List UInt8) : Option (RepoUnderlyingsGroup × List UInt8) := do
  ...
  pure ({ underlyingSymbol, underlyingSecurityIdOptional, underlyingSecurityAltId, underlyingSecurityAltIdSource, underlyingFinancialInstrumentFullName, underlyingSecurityType, underlyingCountryOfIssue, underlyingIssuer, underlyingMaxLifeTime, underlyingMinDaysToMaturity }, bytes)

@[simp] theorem encode_length (message : RepoUnderlyingsGroup) : (encode message).length = 108 := by
  simp [encode]
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 2 < 65536 := by
  ...
  | mdInstrumentDefinitionRepo inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionRepo.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length, encodeMany_length_const RepoUnderlyingsGroup.encode 108 RepoUnderlyingsGroup.encode_length, encodeMany_length_const RepoRelatedInstrumentsGroup.encode 32 RepoRelatedInstrumentsGroup.encode_length]
    omega
...
end Omi.CmeGlobexMdp3SbeV110Udp
