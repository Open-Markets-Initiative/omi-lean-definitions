...
/-!
# TMX Group Quantum Feed Level 1 v2.8
...
namespace Omi.TmxTsxQuantumfeedlevel1XmtV28
...
def decode (bytes : List UInt8) : Option (TradeReportMessage × List UInt8) := do
  ...
  pure ({ symbol, price, volume, buyBrokerNumber, sellBrokerNumber, bypass, tradeTimeStamp, settlementTerms, crossType, lastSalePrice, openingTrade, listingMarket, productType, tradeNumber, tradeDate, isMidOnly, isDark, isConditional, tradingSystemTimeStamp }, bytes)

@[simp] theorem encode_length (message : TradeReportMessage) : (encode message).length = 65 := by
  unfold encode
...
@[simp] theorem decode_encode (message : TradeReportMessage) (rest : List UInt8) :
  ...
  dsimp only
  rw [List.append_assoc, IsConditional.decode_encode, some_bind]
  dsimp only
  rw [decodeUIntLE_encodeUIntLE, some_bind]
  rfl
...
/-- Trade Cancelled Message: 58 bytes -/
...
def decode (bytes : List UInt8) : Option (TradeCancelledMessage × List UInt8) := do
  ...
  let (tradeDate, bytes) ← decodeUIntLE 4 bytes
  let (tradingSystemTimeStamp, bytes) ← decodeUIntLE 8 bytes
  pure ({ symbol, price, volume, buyBrokerNumber, sellBrokerNumber, tradeTimeStamp, lastSalePrice, listingMarket, productType, tradeNumber, tradeDate, tradingSystemTimeStamp }, bytes)

@[simp] theorem encode_length (message : TradeCancelledMessage) : (encode message).length = 58 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (QuoteMessage × List UInt8) := do
  ...
  let (tradingSystemTimeStamp, bytes) ← decodeUIntLE 8 bytes
  pure ({ symbol, bidPrice, bidSize, askPrice, askSize, tradingSystemTimeStamp }, bytes)

@[simp] theorem encode_length (message : QuoteMessage) : (encode message).length = 44 := by
  unfold encode
...
end Omi.TmxTsxQuantumfeedlevel1XmtV28
