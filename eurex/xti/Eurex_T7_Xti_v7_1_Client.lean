...
namespace Omi.EurexT7XtiFbeV71Client
...
/-- New Order Single Request: 218 bytes -/
...
def decode (bytes : List UInt8) : Option (RfqRequest × List UInt8) := do
  ...
  let (side, bytes) ← decodeUInt 1 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, orderQty, marketSegmentId, rfqPublishIndicator, rfqRequesterDisclosureInstruction, side, pad1 }, bytes)
...
end Omi.EurexT7XtiFbeV71Client
