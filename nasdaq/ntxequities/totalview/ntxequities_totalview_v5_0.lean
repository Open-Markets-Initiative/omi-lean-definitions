...
inductive MessageType where
  ...
  | mwcbStatusMessage -- Mwcb Status Message
  ...
  | addOrderMpidAttributionMessage -- Add Order Mpid Attribution Message
...
def ofByte? (byte : UInt8) : Option MessageType :=
  ...
  else if byte = 0x57 then some .mwcbStatusMessage
  ...
  else if byte = 0x46 then some .addOrderMpidAttributionMessage
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 0 < 65536 := by
  ...
  | mwcbStatusMessage inner =>
    simp [encodeBody, h, Payload.encode, MwcbStatusMessage.encode]
  ...
  | addOrderMpidAttributionMessage inner =>
    simp [encodeBody, h, Payload.encode, AddOrderMpidAttributionMessage.encode]
