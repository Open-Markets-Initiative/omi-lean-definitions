...
def decode (bytes : List UInt8) : Option (AddComplexInstrument × List UInt8) := do
  ...
  if fits_instrmtLegGrpComp : instrmtLegGrpComp_.length < 256 ^ 1 then
    pure ({ securityId, transactTime, securityDesc, securitySubType, productComplex, impliedMarketIndicator, lastFragment, pad1, legRatioMultiplier, pad3, instrmtLegGrpComp := ⟨instrmtLegGrpComp_, fits_instrmtLegGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : AddComplexInstrument) : (encode message).length ≤ 8232 := by
  have bound_instrmtLegGrpComp := message.instrmtLegGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, encodeMany_length_const InstrmtLegGrpComp.encode 32 InstrmtLegGrpComp.encode_length]
  omega
