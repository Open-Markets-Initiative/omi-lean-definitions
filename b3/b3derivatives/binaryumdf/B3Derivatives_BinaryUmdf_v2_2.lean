...
/-!
# Brasil, Bolsa, Balcão Binary Unified Market Data Feed v2.2
...
namespace Omi.B3B3derivativesBinaryumdfSbeV22
...
def encode (message : SecurityDefinitionMessage) : List UInt8 :=
  encodeUIntLE 8 message.securityId
    ...
    ++ (encodeUInt 1 message.optPayoutType
...
def decode (bytes : List UInt8) : Option (SecurityDefinitionMessage × List UInt8) := do
  ...
  let (optPayoutType, bytes) ← decodeUInt 1 bytes
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SecurityDefinitionMessage) : (encode message).length ≤ 17837 := by
...
/-- Snapshot Full Refresh Header 30 Message: 34 bytes -/
...
def encode (message : SnapshotFullRefreshHeader30Message) : List UInt8 :=
  encodeUIntLE 8 message.securityId
    ...
    ++ (encodeUIntLE 2 message.lastSequenceVersion))))))))
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefreshHeader30Message × List UInt8) := do
  ...
  let (lastSequenceVersion, bytes) ← decodeUIntLE 2 bytes
  pure ({ securityId, lastMsgSeqNumProcessed, totNumReports, totNumBids, totNumOffers, totNumStats, offset26Padding2, lastRptSeq, lastSequenceVersion }, bytes)

@[simp] theorem encode_length (message : SnapshotFullRefreshHeader30Message) : (encode message).length = 34 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length]
...
/-- Order Mb O 50 Message: 72 bytes -/
...
def decode (bytes : List UInt8) : Option (OrderMbO50Message × List UInt8) := do
  ...
  let (mdCorporateOffsetPriceOptional, bytes) ← decodeUIntLE 8 bytes
  let (mdEntrySizeQuantity, bytes) ← decodeUIntLE 8 bytes
  let (offset28Padding4, bytes) ← Alpha.decode 4 bytes
  ...
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (mdEntryPrevSize, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, matchEventIndicator, mdUpdateAction, mdEntryType, offset11Padding1, mdCorporateOffsetPriceOptional, mdEntrySizeQuantity, offset28Padding4, enteringFirm, mdInsertTimestamp, secondaryOrderId, rptSeq, transactTime, mdEntryPrevSize }, bytes)

@[simp] theorem encode_length (message : OrderMbO50Message) : (encode message).length = 72 := by
  unfold encode
...
/-- Delete Order Mb O 51 Message: 52 bytes -/
structure DeleteOrderMbO51Message where
  securityId : BitVec 64
  matchEventIndicator : BitVec 8
  offset9Padding1 : Alpha 1
  mdEntryType : MdEntryType
  offset11Padding5 : Alpha 5
  mdEntrySizeQuantity : BitVec 64
  secondaryOrderId : BitVec 64
  transactTime : BitVec 64
  rptSeq : BitVec 32
...
def encode (message : DeleteOrderMbO51Message) : List UInt8 :=
  encodeUIntLE 8 message.securityId
    ...
    ++ (Alpha.encode message.offset9Padding1
    ++ (MdEntryType.encode message.mdEntryType
    ++ (Alpha.encode message.offset11Padding5
    ++ (encodeUIntLE 8 message.mdEntrySizeQuantity
    ++ (encodeUIntLE 8 message.secondaryOrderId
    ++ (encodeUIntLE 8 message.transactTime
    ++ (encodeUIntLE 4 message.rptSeq
...
def decode (bytes : List UInt8) : Option (DeleteOrderMbO51Message × List UInt8) := do
  ...
  let (offset9Padding1, bytes) ← Alpha.decode 1 bytes
  let (mdEntryType, bytes) ← MdEntryType.decode bytes
  let (offset11Padding5, bytes) ← Alpha.decode 5 bytes
  let (mdEntrySizeQuantity, bytes) ← decodeUIntLE 8 bytes
  let (secondaryOrderId, bytes) ← decodeUIntLE 8 bytes
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  let (mdCorporateOffsetPriceOptional, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, matchEventIndicator, offset9Padding1, mdEntryType, offset11Padding5, mdEntrySizeQuantity, secondaryOrderId, transactTime, rptSeq, mdCorporateOffsetPriceOptional }, bytes)

@[simp] theorem encode_length (message : DeleteOrderMbO51Message) : (encode message).length = 52 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (MassDeleteOrdersMbO52Message × List UInt8) := do
  ...
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  pure ({ securityId, matchEventIndicator, mdUpdateAction, mdEntryType, offset11Padding5, transactTime, rptSeq }, bytes)
...
def decode (bytes : List UInt8) : Option (Trade53Message × List UInt8) := do
  ...
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  pure ({ securityId, matchEventIndicator, tradingSessionId, tradeCondition, mdFuturePrice, mdEntrySizeQuantity, tradeId, mdEntryBuyer, mdEntrySeller, tradeDate, trdSubType, offset43Padding1, transactTime, rptSeq }, bytes)
...
def decode (bytes : List UInt8) : Option (ForwardTrade54Message × List UInt8) := do
  ...
  let (trdSubType, bytes) ← decodeUInt 1 bytes
  let (padding3, bytes) ← Alpha.decode 3 bytes
  pure ({ securityId, matchEventIndicator, tradingSessionId, tradeCondition, mdFuturePrice, mdEntrySizeQuantity, tradeId, mdEntryBuyer, mdEntrySeller, tradeDate, transactTime, rptSeq, sellerDays, mdEntryInterestRate, trdSubType, padding3 }, bytes)
...
def decode (bytes : List UInt8) : Option (ExecutionSummary55Message × List UInt8) := do
  ...
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, offset8Padding2, aggressorSide, offset11Padding1, lastPx, fillQty, tradedHiddenQty, cxlQty, aggressorTime, rptSeq, transactTime }, bytes)
...
def decode (bytes : List UInt8) : Option (TradeBust57Message × List UInt8) := do
  ...
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  pure ({ securityId, matchEventIndicator, tradingSessionId, offset10Padding2, mdFuturePrice, mdEntrySizeQuantity, tradeId, tradeDate, offset34Padding2, transactTime, rptSeq }, bytes)
...
def encode (message : SnapshotFullRefreshOrdersMbO71MessageNoMDEntriesGroup) : List UInt8 :=
  encodeUIntLE 8 message.mdCorporateOffsetPriceOptional
    ++ (encodeUIntLE 8 message.mdEntrySizeQuantity
    ++ (Alpha.encode message.offset16Padding4
    ++ (encodeUIntLE 4 message.enteringFirm
    ++ (encodeUIntLE 8 message.mdInsertTimestamp
    ++ (encodeUIntLE 8 message.secondaryOrderId
    ++ (MdEntryType.encode message.mdEntryType
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefreshOrdersMbO71MessageNoMDEntriesGroup × List UInt8) := do
  let (mdCorporateOffsetPriceOptional, bytes) ← decodeUIntLE 8 bytes
  let (mdEntrySizeQuantity, bytes) ← decodeUIntLE 8 bytes
  let (offset16Padding4, bytes) ← Alpha.decode 4 bytes
  let (enteringFirm, bytes) ← decodeUIntLE 4 bytes
  let (mdInsertTimestamp, bytes) ← decodeUIntLE 8 bytes
  let (secondaryOrderId, bytes) ← decodeUIntLE 8 bytes
  let (mdEntryType, bytes) ← MdEntryType.decode bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  pure ({ mdCorporateOffsetPriceOptional, mdEntrySizeQuantity, offset16Padding4, enteringFirm, mdInsertTimestamp, secondaryOrderId, mdEntryType, matchEventIndicator }, bytes)
...
end Omi.B3B3derivativesBinaryumdfSbeV22
