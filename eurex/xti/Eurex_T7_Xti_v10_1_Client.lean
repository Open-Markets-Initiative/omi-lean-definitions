...
namespace Omi.EurexT7XtiFbeV101Client
...
def decode (bytes : List UInt8) : Option (XetraEnLightEnterQuoteRequest × List UInt8) := do
  ...
  let (freeText4, bytes) ← Alpha.decode 16 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, bidPx, offerPx, bidSize, offerSize, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, validUntilTime, marketSegmentId, negotiationId, valueCheckTypeQuantity, valueCheckTypeValue, tradingCapacity, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, partyExecutingFirm, partyExecutingTrader, freeText1, freeText2, freeText4, pad7 }, bytes)
...
def decode (bytes : List UInt8) : Option (XetraEnLightOpenNegotiationRequest × List UInt8) := do
  ...
  if fits_xetraEnLightTargetPartiesComp : xetraEnLightTargetPartiesComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, securityId, bidPx, offerPx, orderQty, validUntilTime, autoExecExpiryTime, autoExecLimitPrice, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, settlDate, autoExecReferencePriceOffset, autoExecMinNoOfQuotes, numberOfRespDisclosureInstruction, side, valueCheckTypeValue, valueCheckTypeQuantity, respondentType, bidPxIsLocked, offerPxIsLocked, sideIsLocked, tradingCapacity, autoExecType, sideDisclosureInstruction, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, orderOrigination, quoteCondition, partyExecutingFirm, partyExecutingTrader, freeText5, quoteReqId, freeText1, freeText2, freeText4, pad4, xetraEnLightTargetPartiesComp := ⟨xetraEnLightTargetPartiesComp_, fits_xetraEnLightTargetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : XetraEnLightOpenNegotiationRequest) : (encode message).length ≤ 4418 := by
...
def decode (bytes : List UInt8) : Option (XetraEnLightUpdateNegotiationRequest × List UInt8) := do
  ...
  let (xetraEnLightTargetPartiesComp_, bytes) ← decodeMany XetraEnLightTargetPartiesComp.decode noTargetPartyIDs.toNat bytes
  if fits_xetraEnLightTargetPartiesComp : xetraEnLightTargetPartiesComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, bidPx, offerPx, orderQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, autoExecExpiryTime, autoExecLimitPrice, autoExecReferencePriceOffset, marketSegmentId, negotiationId, settlDate, autoExecMinNoOfQuotes, numberOfRespDisclosureInstruction, side, valueCheckTypeValue, valueCheckTypeQuantity, respondentType, tradingCapacity, autoExecType, sideDisclosureInstruction, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, orderOrigination, quoteCondition, partyExecutingFirm, partyExecutingTrader, freeText1, freeText2, freeText4, freeText5, pad7, xetraEnLightTargetPartiesComp := ⟨xetraEnLightTargetPartiesComp_, fits_xetraEnLightTargetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : XetraEnLightUpdateNegotiationRequest) : (encode message).length ≤ 4386 := by
  have bound_xetraEnLightTargetPartiesComp := message.xetraEnLightTargetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, QuoteCondition.encode_length, encodeMany_length_const XetraEnLightTargetPartiesComp.encode 16 XetraEnLightTargetPartiesComp.encode_length]
  omega
...
end Omi.EurexT7XtiFbeV101Client
