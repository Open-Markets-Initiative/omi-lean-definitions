...
namespace Omi.EurexT7EtiFbeV111Client
...
def decode (bytes : List UInt8) : Option (AddComplexInstrumentRequest × List UInt8) := do
  ...
  if fits_instrmtLegGrpComp : instrmtLegGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, securitySubType, productComplex, multilegModel, complianceText, pad1, instrmtLegGrpComp := ⟨instrmtLegGrpComp_, fits_instrmtLegGrpComp⟩ }, bytes)
  else none
...
/-- Approve Tes Trade Request: 242 bytes -/
structure ApproveTesTradeRequest where
  ...
  custOrderHandlingInst : CustOrderHandlingInst
  complianceText : Alpha 20
  partyEndClientIdentification : Alpha 5
  pad3 : Alpha 3
  deriving DecidableEq, Repr
...
def decode (bytes : List UInt8) : Option (ApproveTesTradeRequest × List UInt8) := do
  ...
  let (custOrderHandlingInst, bytes) ← CustOrderHandlingInst.decode bytes
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  let (partyEndClientIdentification, bytes) ← Alpha.decode 5 bytes
...
@[simp] theorem encode_length (message : ApproveTesTradeRequest) : (encode message).length = 242 := by
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderComplexRequest) : (encode message).length ≤ 2282 := by
  have bound_legOrdGrpComp := message.legOrdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, CustOrderHandlingInst.encode_length, encodeMany_length_const LegOrdGrpComp.encode 8 LegOrdGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsHitQuoteRequest) : (encode message).length ≤ 6490 := by
...
end Omi.EurexT7EtiFbeV111Client
