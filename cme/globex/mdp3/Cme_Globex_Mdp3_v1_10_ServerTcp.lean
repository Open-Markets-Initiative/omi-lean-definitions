...
theorem encode_length_le (message : RepoUnderlyingsGroups) : (encode message).length ≤ 27543 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const RepoUnderlyingsGroup.encode 108 RepoUnderlyingsGroup.encode_length]
  omega
...
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 41583 := by
  cases message with
  | channelReset inner =>
