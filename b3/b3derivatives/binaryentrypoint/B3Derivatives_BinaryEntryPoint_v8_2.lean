...
/-!
# Brasil, Bolsa, Balcão Binary Entry Point v8.2
...
namespace Omi.B3B3derivativesBinaryentrypointSbeV82
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderSingleMessage) : (encode message).length ≤ 643 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderCancelReplaceRequestMessage) : (encode message).length ≤ 660 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderCrossMessage) : (encode message).length ≤ 5189 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportNewMessage) : (encode message).length ≤ 684 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportModifyMessage) : (encode message).length ≤ 698 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, OrdStatus.encode_length, encodeUIntLE_length, encodeUInt_length, MultiLegReportingType.encode_length, Ordtype.encode_length, TimeInForce.encode_length, Alpha.encode_length, InvestorId.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeMessage) : (encode message).length ≤ 682 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportRejectMessage) : (encode message).length ≤ 930 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportForwardMessage) : (encode message).length ≤ 667 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteRequestMessage) : (encode message).length ≤ 1897 := by
...
def decode (bytes : List UInt8) : Option (QuoteStatusReportMessage × List UInt8) := do
  ...
  let (memo, bytes) ← Memo.decode bytes
  let (text, bytes) ← Text.decode bytes
  pure ({ bidirectionalBusinessHeader, quoteRejectReason, securityId, quoteReqId, quoteId, tradeIdOptional, contraBroker, transactTime, quoteStatus, quoteStatusResponseTo, account, sideOptional, settlTypeOptional, price8Optional, orderQty, senderLocation, enteringTrader, executingTrader, fixedRatePercentage8Optional, executeUnderlyingTrade, daysToSettlementOptional, deskId, memo, text }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteStatusReportMessage) : (encode message).length ≤ 886 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  have bound_text := Text.encode_length_le message.text
  unfold encode
...
def decode (bytes : List UInt8) : Option (QuoteMessage × List UInt8) := do
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ bidirectionalBusinessHeader, securityId, quoteReqId, quoteId, transactTime, price8Optional, orderQty, side, settlType, account, senderLocation, enteringTrader, executingTrader, fixedRatePercentage8, executeUnderlyingTrade, daysToSettlement, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteMessage) : (encode message).length ≤ 616 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteRequestRejectMessage) : (encode message).length ≤ 2156 := by
  have bound_sidesGroups := SidesGroups.encode_length_le message.sidesGroups
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  have bound_text := Text.encode_length_le message.text
...
end Omi.B3B3derivativesBinaryentrypointSbeV82
