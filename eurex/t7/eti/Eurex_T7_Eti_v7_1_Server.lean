...
theorem encode_length_le (message : MassQuoteResponse) : (encode message).length ≤ 8234 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, NrResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteEntryAckGrpComp.encode 32 QuoteEntryAckGrpComp.encode_length]
  omega
...
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 12594 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const OrderBookItemGrpComp.encode 48 OrderBookItemGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TesApproveBroadcast) (rest : List UInt8) :
  ...
  dsimp only
  simp only [message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, ↓reduceDIte]
  rfl

/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : TesApproveBroadcast) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
@[simp] theorem decode_encode (message : TesBroadcast) (rest : List UInt8) :
  ...
  rw [decodeMany_bounded 1 SrqsRelatedTradeIdGrpComp.encode SrqsRelatedTradeIdGrpComp.decode SrqsRelatedTradeIdGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.sideAllocGrpBcComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, ↓reduceDIte]
  rfl

/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : TesBroadcast) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
