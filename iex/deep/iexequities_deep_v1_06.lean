...
inductive MessageType where
  ...
  | priceLevelBuyUpdateMessage -- Price Level Buy Update Message
  | priceLevelSellUpdateMessage -- Price Level Sell Update Message
...
def toByte : MessageType → UInt8
  ...
  | .priceLevelBuyUpdateMessage => 0x38
  | .priceLevelSellUpdateMessage => 0x35
...
def ofByte? (byte : UInt8) : Option MessageType :=
  ...
  else if byte = 0x38 then some .priceLevelBuyUpdateMessage
  else if byte = 0x35 then some .priceLevelSellUpdateMessage
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 0 < 65536 := by
  ...
  | priceLevelBuyUpdateMessage inner =>
    simp [encodeBody, h, MessageData.encode, PriceLevelBuyUpdateMessage.encode]
  | priceLevelSellUpdateMessage inner =>
    simp [encodeBody, h, MessageData.encode, PriceLevelSellUpdateMessage.encode]
