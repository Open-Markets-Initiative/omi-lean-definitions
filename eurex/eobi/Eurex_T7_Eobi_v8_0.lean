...
/-!
# Eurex Exchange Enhanced Order Book Interface v8.0
...
namespace Omi.EurexT7EobiFbeV80
...
def decode (bytes : List UInt8) : Option (ExecutionSummary × List UInt8) := do
  ...
  pure ({ securityId, aggressorTime, requestTime, execId, lastQty, aggressorSide, tradeCondition, pad6, lastPx, restingHiddenQty, restingCxlQty }, bytes)
...
def decode (bytes : List UInt8) : Option (MdInstrumentEntryGrpComp × List UInt8) := do
  ...
  let (tradeCondition, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ mdEntryPx, mdEntrySize, mdEntryType, tradeCondition, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (TradeReport × List UInt8) := do
  ...
  let (algorithmicTradeIndicator, bytes) ← decodeUInt 1 bytes
  let (tradeCondition, bytes) ← decodeUInt 1 bytes
  pure ({ securityId, transactTime, lastQty, lastPx, trdMatchId, matchType, matchSubType, algorithmicTradeIndicator, tradeCondition }, bytes)
