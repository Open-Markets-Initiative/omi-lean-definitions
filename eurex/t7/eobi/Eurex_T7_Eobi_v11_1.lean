...
/-!
# Eurex Exchange Enhanced Order Book Interface v11.1
...
namespace Omi.EurexT7EobiFbeV111
...
/-- Add Complex Instrument: 712 bytes -/
...
def decode (bytes : List UInt8) : Option (AddComplexInstrument × List UInt8) := do
  ...
  if fits_instrmtLegGrpComp : instrmtLegGrpComp_.length = 20 then
    pure ({ securityId, transactTime, securityDesc, securitySubType, productComplex, impliedMarketIndicator, lastFragment, pad1, legRatioMultiplier, noLegs, pad3, instrmtLegGrpComp := ⟨instrmtLegGrpComp_, fits_instrmtLegGrpComp⟩ }, bytes)
  else none

@[simp] theorem encode_length (message : AddComplexInstrument) : (encode message).length = 712 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, encodeMany_length_const InstrmtLegGrpComp.encode 32 InstrmtLegGrpComp.encode_length, message.instrmtLegGrpComp.length_eq]
...
/-- Add Flexible Instrument: 80 bytes -/
...
def decode (bytes : List UInt8) : Option (AddFlexibleInstrument × List UInt8) := do
  ...
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ securityId, transactTime, securityDesc, securityType, putOrCall, exerciseStyle, settlMethod, maturityDate, strikePrice, optAttribute, pad4 }, bytes)

@[simp] theorem encode_length (message : AddFlexibleInstrument) : (encode message).length = 80 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (ExecutionSummary × List UInt8) := do
  ...
  pure ({ securityId, aggressorTime, requestTime, execId, lastQty, aggressorSide, pad1, tradeCondition, tradingHhiIndicator, pad3, lastPx, restingHiddenQty, restingCxlQty }, bytes)
...
@[simp] theorem encode_length (message : InstrumentSummary) : (encode message).length = 536 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (OrderDetailsComp × List UInt8) := do
  ...
  let (pad5, bytes) ← Alpha.decode 5 bytes
  let (price, bytes) ← decodeUIntLE 8 bytes
  pure ({ trdRegTsTimePriority, displayQty, side, ordType, hhiIndicator, pad5, price }, bytes)
...
@[simp] theorem encode_length (message : OrderModify) : (encode message).length = 80 := by
  unfold encode
...
@[simp] theorem decode_encode (message : OrderModify) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, OrderDetailsComp.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [Alpha.decode_encode, some_bind]
...
def decode (bytes : List UInt8) : Option (TesTradeReport × List UInt8) := do
  ...
  let (pad6, bytes) ← Alpha.decode 6 bytes
  let (nonDisclosedTradeVolume, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, transactTime, lastQty, lastPx, trdMatchId, trdType, tradeCondition, multiLegReportingType, multiLegPriceModel, pad6, nonDisclosedTradeVolume }, bytes)

@[simp] theorem encode_length (message : TesTradeReport) : (encode message).length = 56 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length]
...
theorem encode_length_le (message : Payload) : (encode message).length ≤ 792 := by
  ...
  | tesTradeReport inner =>
    simp only [encode, TesTradeReport.encode_length]
    omega
  | topOfBook inner =>
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 2 < 256 ^ 2 := by
  ...
  | snapshotOrder inner =>
    ...
    omega
  | tesTradeReport inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, encodeUIntLE_length, TesTradeReport.encode_length]
    omega
  | topOfBook inner =>
