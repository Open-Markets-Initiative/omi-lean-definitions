...
namespace Omi.EurexT7EtiFbeV80Client
...
def decode (bytes : List UInt8) : Option (ApproveBasketTradeRequest × List UInt8) := do
  ...
  if fits_basketSideAllocExtGrpComp : basketSideAllocExtGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, basketProfileId, rootPartySubIdType, trdType, tradeReportType, basketTradeReportText, tradeReportId, basketSideTradeReportId, pad2v2, basketSideAllocExtGrpComp := ⟨basketSideAllocExtGrpComp_, fits_basketSideAllocExtGrpComp⟩ }, bytes)
  else none
...
/-- Delete Basket Trade Request: 66 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteBasketTradeRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, basketProfileId, trdType, tradeReportType, tradeReportId, pad5 }, bytes)

@[simp] theorem encode_length (message : DeleteBasketTradeRequest) : (encode message).length = 66 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (DeleteOrderComplexRequest × List UInt8) := do
  ...
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, securityId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, targetPartyIdSessionId, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, fixClOrdId, pad1 }, bytes)
...
def decode (bytes : List UInt8) : Option (DeleteOrderSingleRequest × List UInt8) := do
  ...
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  let (pad5, bytes) ← Alpha.decode 5 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, simpleSecurityId, targetPartyIdSessionId, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, fixClOrdId, pad5 }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterBasketTradeRequest) : (encode message).length ≤ 38842 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
  have bound_instrmtMatchSideGrpComp := message.instrmtMatchSideGrpComp.length_lt
  have bound_basketSideAllocGrpComp := message.basketSideAllocGrpComp.length_lt
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterTesTradeRequest) : (encode message).length ≤ 36954 := by
...
def decode (bytes : List UInt8) : Option (ModifyBasketTradeRequest × List UInt8) := do
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_instrmtMatchSideGrpComp : instrmtMatchSideGrpComp_.length < 256 ^ 1 then
      if fits_basketSideAllocGrpComp : basketSideAllocGrpComp_.length < 256 ^ 1 then
        pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, maturityMonthYear, basketProfileId, trdType, tradeReportType, basketTradeReportText, tradeReportId, pad2v2, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, instrmtMatchSideGrpComp := ⟨instrmtMatchSideGrpComp_, fits_instrmtMatchSideGrpComp⟩, basketSideAllocGrpComp := ⟨basketSideAllocGrpComp_, fits_basketSideAllocGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- Modify Order Single Request: 266 bytes -/
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyTesTradeRequest) : (encode message).length ≤ 14386 := by
  have bound_sideAllocGrpComp := message.sideAllocGrpComp.length_lt
  have bound_trdInstrmntLegGrpComp := message.trdInstrmntLegGrpComp.length_lt
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SideAllocGrpComp.encode 24 SideAllocGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 24 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
/-- Srqs Enter Quote Request: 362 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsEnterQuoteRequest × List UInt8) := do
  ...
  let (partyIdTakeUpTradingFirm, bytes) ← Alpha.decode 5 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, bidPx, offerPx, underlyingPx, underlyingDeltaPercentage, bidSize, offerSize, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, negotiationId, valueCheckTypeQuantity, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, tradingCapacity, partyExecutingFirm, partyExecutingTrader, freeText1, freeText2, freeText3, freeText5, positionEffect, account, partyIdBeneficiary, custOrderHandlingInst, partyIdOrderOriginationFirm, partyIdPositionAccount, partyIdLocationId, complianceText, partyIdTakeUpTradingFirm, pad1 }, bytes)

@[simp] theorem encode_length (message : SrqsEnterQuoteRequest) : (encode message).length = 362 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length]
...
/-- Srqs Hit Quote Request: 378 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsHitQuoteRequest × List UInt8) := do
  ...
  let (partyIdTakeUpTradingFirm, bytes) ← Alpha.decode 5 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, quoteId, orderQty, validUntilTime, underlyingQty, underlyingPriceStipValue, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, negotiationId, side, valueCheckTypeQuantity, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, tradingCapacity, tradePublishIndicator, partyExecutingFirm, partyExecutingTrader, firmTradeId, freeText1, freeText2, freeText3, freeText5, positionEffect, account, partyIdBeneficiary, custOrderHandlingInst, partyIdOrderOriginationFirm, partyIdPositionAccount, partyIdLocationId, complianceText, partyIdTakeUpTradingFirm, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsHitQuoteRequest) : (encode message).length = 378 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length]
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequest × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, securityId, bidPx, offerPx, orderQty, quoteRefPrice, underlyingDeltaPercentage, validUntilTime, marketSegmentId, securitySubType, quoteType, quoteSubType, quoteQualifier, numberOfRespDisclosureInstruction, side, quoteRefPriceSource, tradeUnderlying, negotiateUnderlying, productComplex, partyExecutingFirm, partyExecutingTrader, freeText5, quoteReqId, pad2v2, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequest) : (encode message).length ≤ 12498 := by
    ...
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
              pure ({ networkMsgId, pad2, requestHeaderComp, securityId, lastPx, transBkdTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad1, trdType, productComplex, tradeReportType, tradePublishIndicator, skipValidations, trdRptStatus, tradePlatform, hedgeType, partyIdSettlementLocation, valueCheckTypeMinLotSize, tradeReportId, tradeReportText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad3, sideAllocExtGrpComp := ⟨sideAllocExtGrpComp_, fits_sideAllocExtGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
            else none
          else none
        else none
      else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UploadTesTradeRequest) : (encode message).length ≤ 75714 := by
  ...
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SideAllocExtGrpComp.encode 176 SideAllocExtGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 24 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
end Omi.EurexT7EtiFbeV80Client
