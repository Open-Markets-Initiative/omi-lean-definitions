...
/-!
# Miami International Holdings Express Orders v2.6
...
namespace Omi.MiaxPearlequitiesExpressordersMeoV26
...
inductive OrderStatus where
  ...
  | invalidForCurrentSymbolTradingStatus -- Invalid For Current Symbol Trading Status
...
def toByte : OrderStatus → UInt8
  ...
  | .invalidForCurrentSymbolTradingStatus => 0x70
...
def listed (byte : UInt8) : OrderStatus :=
  ...
  else if byte = 0x70 then .invalidForCurrentSymbolTradingStatus
...
theorem ofByte_toByte (value : OrderStatus) : ofByte value.toByte = value := by
  ...
  | invalidForCurrentSymbolTradingStatus => decide
...
def ModifyStatus.codes : List UInt8 :=
  [0x20, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x5A, 0x61, 0x62, 0x63, 0x64, 0x74, 0x79, 0x7A, 0x30, 0x31, 0x32, 0x33, 0x34, 0x37, 0x38, 0x39, 0x2A]
...
inductive NbboIndicator where
  | orderSetNewNbbo -- Order Set New Nbbo
  | firstOrderToSetMbboEqualToNbbo -- First Order To Set Mbbo Equal To Nbbo
  | notApplicable -- Not Applicable
...
def toByte : NbboIndicator → UInt8
  | .orderSetNewNbbo => 0x59
  | .firstOrderToSetMbboEqualToNbbo => 0x46
  | .notApplicable => 0x4E
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : NbboIndicator :=
  if byte = 0x59 then .orderSetNewNbbo
  else if byte = 0x46 then .firstOrderToSetMbboEqualToNbbo
  else .notApplicable
...
theorem ofByte_toByte (value : NbboIndicator) : ofByte value.toByte = value := by
  cases value with
  | orderSetNewNbbo => decide
  | firstOrderToSetMbboEqualToNbbo => decide
  | notApplicable => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
def encode (message : NewOrderNotification) : List UInt8 :=
  encodeUIntLE 8 message.matchingEngineTime
    ...
    ++ (Alpha.encode message.reserved18)))))))))))))))))))))))))
...
def encode (message : ModifyOrderNotification) : List UInt8 :=
  encodeUIntLE 8 message.matchingEngineTime
    ...
    ++ (Alpha.encode message.locateAccount
    ++ (Alpha.encode message.reserved26)))))))))))))))
...
def decode (bytes : List UInt8) : Option (ModifyOrderNotification × List UInt8) := do
  ...
  let (locateAccount, bytes) ← Alpha.decode 4 bytes
  let (reserved26, bytes) ← Alpha.decode 26 bytes
  pure ({ matchingEngineTime, mpid, clientOrderId, originalClientOrderId, symbolId, orderId, price, size, leavesQty, modifyOrderInstructions, minQty, maxFloorQty, pendingModifyStatus, pendingRejectReason, locateAccount, reserved26 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderNotification) : (encode message).length = 121 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, PendingModifyStatus.encode_length, PendingRejectReason.encode_length]
...
/-- Order Price Update Notification: 38 bytes -/
...
def decode (bytes : List UInt8) : Option (OrderPriceUpdateNotification × List UInt8) := do
  ...
  pure ({ matchingEngineTime, symbolId, orderId, workingPrice, nbboIndicator, reserved9 }, bytes)

@[simp] theorem encode_length (message : OrderPriceUpdateNotification) : (encode message).length = 38 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, NbboIndicator.encode_length, Alpha.encode_length]
...
inductive SequencedMessage where
  ...
  | orderPriceUpdateNotification (message : OrderPriceUpdateNotification) -- "PU" 0x5055
...
def tag : SequencedMessage → BitVec 16
  ...
  | .orderPriceUpdateNotification _ => 20565
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (SequencedMessage × List UInt8) :=
  ...
  else if tag = 20565 then (OrderPriceUpdateNotification.decode bytes).map fun (message, rest) => (.orderPriceUpdateNotification message, rest)
...
def decode (bytes : List UInt8) : Option (ModifyOrderRequestMessage × List UInt8) := do
  ...
  pure ({ reserved8, mpid, clientOrderId, originalClientOrderId, symbolId, price, size, modifyOrderInstructions, minQty, maxFloorQty, locateAccount, reserved28 }, bytes)
...
end Omi.MiaxPearlequitiesExpressordersMeoV26
