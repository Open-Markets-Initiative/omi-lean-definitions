...
/-!
# National Association of Securities Dealers Automated Quotations (Nasdaq) Spread Top Of Market v2.1
...
namespace Omi.NasdaqPhlxoptionsSpreadtopofmarketItchV21MoldUdp64
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 0 < 256 ^ 2 := by
  ...
  | strategyTradingActionMessage inner =>
    ...
    omega
  | strategyBestBidAndAskUpdateMessage inner =>
    simp only [UdpPayload.encode, List.length_append, encodeUInt_length, StrategyBestBidAndAskUpdateMessage.encode_length]
    omega
  | strategyBestBidUpdateMessage inner =>
    simp only [UdpPayload.encode, List.length_append, encodeUInt_length, StrategyBestBidUpdateMessage.encode_length]
    omega
  | strategyBestAskUpdateMessage inner =>
    simp only [UdpPayload.encode, List.length_append, encodeUInt_length, StrategyBestAskUpdateMessage.encode_length]
    omega
...
end Omi.NasdaqPhlxoptionsSpreadtopofmarketItchV21MoldUdp64
