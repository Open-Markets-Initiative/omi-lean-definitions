...
Note: Nbbo Appendage Indicator chooses the National Bbo Appendage Shortform or National Bbo Appendage Longform attached, or none: it is written from the choice, and a value it does not list is not decoded.

Note: Bolo Appendage Indicator chooses the Bolo Appendage Short Form or Bolo Appendage Long Form or Bolo Appendage Mpid Form attached, or none: it is written from the choice, and a value it does not list is not decoded.
...
namespace Omi.NasdaqUqdfOutputUtpV30C
...
inductive SpecialCondition where
  | onesidedNationalBboAtMarketClose -- Onesided National Bbo At Market Close
...
structure NationalBboAppendageShortform where
...
namespace NationalBboAppendageShortform

def encode (message : NationalBboAppendageShortform) : List UInt8 :=
...
def decode (bytes : List UInt8) : Option (NationalBboAppendageShortform × List UInt8) := do
...
@[simp] theorem encode_length (message : NationalBboAppendageShortform) : (encode message).length = 11 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, encodeUInt_length]

theorem encode_length_pos (message : NationalBboAppendageShortform) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : NationalBboAppendageShortform) (rest : List UInt8) :
...
end NationalBboAppendageShortform

/-- National Bbo Appendage Longform: 27 bytes -/
structure NationalBboAppendageLongform where
  nbboQuoteCondition : Alpha 1
  nationalBestBidMarketCenter : Alpha 1
  nationalBestBidPriceLong : BitVec 64
  nationalBestBidSizeLong : BitVec 32
  nationalBestAskMarketCenter : Alpha 1
  nationalBestAskPriceLong : BitVec 64
  nationalBestAskSizeLong : BitVec 32
  deriving DecidableEq, Repr
...
def encode (message : NationalBboAppendageLongform) : List UInt8 :=
  Alpha.encode message.nbboQuoteCondition
    ++ (Alpha.encode message.nationalBestBidMarketCenter
    ++ (encodeUInt 8 message.nationalBestBidPriceLong
    ++ (encodeUInt 4 message.nationalBestBidSizeLong
    ++ (Alpha.encode message.nationalBestAskMarketCenter
    ++ (encodeUInt 8 message.nationalBestAskPriceLong
    ++ (encodeUInt 4 message.nationalBestAskSizeLong))))))

def decode (bytes : List UInt8) : Option (NationalBboAppendageLongform × List UInt8) := do
  let (nbboQuoteCondition, bytes) ← Alpha.decode 1 bytes
  let (nationalBestBidMarketCenter, bytes) ← Alpha.decode 1 bytes
  let (nationalBestBidPriceLong, bytes) ← decodeUInt 8 bytes
  let (nationalBestBidSizeLong, bytes) ← decodeUInt 4 bytes
  let (nationalBestAskMarketCenter, bytes) ← Alpha.decode 1 bytes
  let (nationalBestAskPriceLong, bytes) ← decodeUInt 8 bytes
  let (nationalBestAskSizeLong, bytes) ← decodeUInt 4 bytes
  pure ({ nbboQuoteCondition, nationalBestBidMarketCenter, nationalBestBidPriceLong, nationalBestBidSizeLong, nationalBestAskMarketCenter, nationalBestAskPriceLong, nationalBestAskSizeLong }, bytes)
...
def tag : NbboAppendageChoice → BitVec 8
  | .nationalBboAppendageShortform _ => 50
...
def encode : NbboAppendageChoice → List UInt8
  | .nationalBboAppendageShortform message => NationalBboAppendageShortform.encode message
...
theorem encode_length_le (message : NbboAppendageChoice) : (encode message).length ≤ 27 := by
  ...
  | nationalBboAppendageShortform inner =>
    simp only [encode, NationalBboAppendageShortform.encode_length]
    omega
...
def decode (tag : BitVec 8) (bytes : List UInt8) : Option (NbboAppendageChoice × List UInt8) :=
  if tag = 50 then (NationalBboAppendageShortform.decode bytes).map fun (message, rest) => (.nationalBboAppendageShortform message, rest)
  else if tag = 51 then (NationalBboAppendageLongform.decode bytes).map fun (message, rest) => (.nationalBboAppendageLongform message, rest)
...
structure UtpQuoteShortformMessage where
...
namespace UtpQuoteShortformMessage

def encode (message : UtpQuoteShortformMessage) : List UInt8 :=
...
def decode (bytes : List UInt8) : Option (UtpQuoteShortformMessage × List UInt8) := do
  ...
  pure ({ marketCenterOriginator, subMarketCenterId, sipTimestamp, timestamp1, participantToken, symbolShort, bidPriceShort, bidSizeShort, askPriceShort, askSizeShort, quoteCondition, sipGeneratedUpdateFlag, luldBboIndicator, retailInterestIndicator, luldNationalBboIndicator, nbboAppendageChoice }, bytes)

theorem encode_length_pos (message : UtpQuoteShortformMessage) : (encode message).length > 0 := by
  unfold encode
  simp only [MarketCenterOriginator.encode_length, List.length_append, ← Nat.add_assoc]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UtpQuoteShortformMessage) : (encode message).length ≤ 72 := by
  unfold encode
...
@[simp] theorem decode_encode (message : UtpQuoteShortformMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
...
end UtpQuoteShortformMessage

/-- Utp Quote Longform Message -/
structure UtpQuoteLongformMessage where
  marketCenterOriginator : MarketCenterOriginator
  subMarketCenterId : SubMarketCenterId
  sipTimestamp : BitVec 64
  timestamp1 : BitVec 64
...
namespace UtpQuoteLongformMessage

def encode (message : UtpQuoteLongformMessage) : List UInt8 :=
  MarketCenterOriginator.encode message.marketCenterOriginator
    ++ (SubMarketCenterId.encode message.subMarketCenterId
    ++ (encodeUInt 8 message.sipTimestamp
    ++ (encodeUInt 8 message.timestamp1
    ...
    ++ (encodeUInt 8 message.askPriceLong
...
def decode (bytes : List UInt8) : Option (UtpQuoteLongformMessage × List UInt8) := do
  let (marketCenterOriginator, bytes) ← MarketCenterOriginator.decode bytes
  let (subMarketCenterId, bytes) ← SubMarketCenterId.decode bytes
  let (sipTimestamp, bytes) ← decodeUInt 8 bytes
  let (timestamp1, bytes) ← decodeUInt 8 bytes
  ...
  let (askPriceLong, bytes) ← decodeUInt 8 bytes
  ...
  let (nbboAppendageChoice, bytes) ← NbboAppendageChoice.decode nbboAppendageIndicator bytes
  pure ({ marketCenterOriginator, subMarketCenterId, sipTimestamp, timestamp1, participantToken, timestamp2, symbolLong, bidPriceLong, bidSizeLong, askPriceLong, askSizeLong, quoteCondition, sipGeneratedUpdateFlag, luldBboIndicator, retailInterestIndicator, luldNationalBboIndicator, finraAdfMpidAppendageIndicator, nbboAppendageChoice }, bytes)

theorem encode_length_pos (message : UtpQuoteLongformMessage) : (encode message).length > 0 := by
  unfold encode
  simp only [MarketCenterOriginator.encode_length, List.length_append, ← Nat.add_assoc]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UtpQuoteLongformMessage) : (encode message).length ≤ 103 := by
  unfold encode
  cases message.nbboAppendageChoice with
  | nationalBboAppendageShortform inner =>
    simp only [NbboAppendageChoice.encode, List.length_append, ← Nat.add_assoc, MarketCenterOriginator.encode_length, SubMarketCenterId.encode_length, encodeUInt_length, Alpha.encode_length, QuoteCondition.encode_length, SipGeneratedUpdateFlag.encode_length, LuldBboIndicator.encode_length, RetailInterestIndicator.encode_length, LuldNationalBboIndicator.encode_length, FinraAdfMpidAppendageIndicator.encode_length, NationalBboAppendageShortform.encode_length]
    omega
  | nationalBboAppendageLongform inner =>
    simp only [NbboAppendageChoice.encode, List.length_append, ← Nat.add_assoc, MarketCenterOriginator.encode_length, SubMarketCenterId.encode_length, encodeUInt_length, Alpha.encode_length, QuoteCondition.encode_length, SipGeneratedUpdateFlag.encode_length, LuldBboIndicator.encode_length, RetailInterestIndicator.encode_length, LuldNationalBboIndicator.encode_length, FinraAdfMpidAppendageIndicator.encode_length, NationalBboAppendageLongform.encode_length]
    omega
  | noNationalBboChange inner =>
    simp only [NbboAppendageChoice.encode, List.length_append, ← Nat.add_assoc, MarketCenterOriginator.encode_length, SubMarketCenterId.encode_length, encodeUInt_length, Alpha.encode_length, QuoteCondition.encode_length, SipGeneratedUpdateFlag.encode_length, LuldBboIndicator.encode_length, RetailInterestIndicator.encode_length, LuldNationalBboIndicator.encode_length, FinraAdfMpidAppendageIndicator.encode_length, NbboAppendageAbsent.encode_length]
    omega
  | noNationalBboCanBeCalculated inner =>
    simp only [NbboAppendageChoice.encode, List.length_append, ← Nat.add_assoc, MarketCenterOriginator.encode_length, SubMarketCenterId.encode_length, encodeUInt_length, Alpha.encode_length, QuoteCondition.encode_length, SipGeneratedUpdateFlag.encode_length, LuldBboIndicator.encode_length, RetailInterestIndicator.encode_length, LuldNationalBboIndicator.encode_length, FinraAdfMpidAppendageIndicator.encode_length, NbboAppendageAbsent.encode_length]
    omega
...
@[simp] theorem decode_encode (message : UtpQuoteLongformMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, MarketCenterOriginator.decode_encode, some_bind]
  dsimp only
...
def decode (bytes : List UInt8) : Option (FinraAdfMarketParticipantQuotationMessage × List UInt8) := do
  ...
  pure ({ marketCenterOriginator, subMarketCenterId, sipTimestamp, timestamp1, participantToken, timestamp2, symbolLong, bidPriceLong, bidSizeLong, askPriceLong, askSizeLong, quoteCondition, finraMarketParticipant }, bytes)

@[simp] theorem encode_length (message : FinraAdfMarketParticipantQuotationMessage) : (encode message).length = 74 := by
  unfold encode
  simp only [List.length_append, MarketCenterOriginator.encode_length, SubMarketCenterId.encode_length, encodeUInt_length, Alpha.encode_length, QuoteCondition.encode_length]
...
structure BoloAppendageShortForm where
  boloBestBidMarketCenter : Alpha 1
  boloBidPriceShort : BitVec 16
  boloBidSize : BitVec 16
  boloBestAskMarketCenter : Alpha 1
  boloAskPriceShort : BitVec 16
  boloAskSize : BitVec 16
  deriving DecidableEq, Repr

namespace BoloAppendageShortForm

def encode (message : BoloAppendageShortForm) : List UInt8 :=
  Alpha.encode message.boloBestBidMarketCenter
    ++ (encodeUInt 2 message.boloBidPriceShort
    ++ (encodeUInt 2 message.boloBidSize
    ++ (Alpha.encode message.boloBestAskMarketCenter
    ++ (encodeUInt 2 message.boloAskPriceShort
    ++ (encodeUInt 2 message.boloAskSize)))))

def decode (bytes : List UInt8) : Option (BoloAppendageShortForm × List UInt8) := do
  let (boloBestBidMarketCenter, bytes) ← Alpha.decode 1 bytes
  let (boloBidPriceShort, bytes) ← decodeUInt 2 bytes
  let (boloBidSize, bytes) ← decodeUInt 2 bytes
  let (boloBestAskMarketCenter, bytes) ← Alpha.decode 1 bytes
  let (boloAskPriceShort, bytes) ← decodeUInt 2 bytes
  let (boloAskSize, bytes) ← decodeUInt 2 bytes
  pure ({ boloBestBidMarketCenter, boloBidPriceShort, boloBidSize, boloBestAskMarketCenter, boloAskPriceShort, boloAskSize }, bytes)

@[simp] theorem encode_length (message : BoloAppendageShortForm) : (encode message).length = 10 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, encodeUInt_length]

theorem encode_length_pos (message : BoloAppendageShortForm) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : BoloAppendageShortForm) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
...
end BoloAppendageShortForm

/-- Bolo Appendage Long Form: 22 bytes -/
structure BoloAppendageLongForm where
  boloBestBidMarketCenter : Alpha 1
  boloBidPriceLong : BitVec 64
  boloBidSize : BitVec 16
  boloBestAskMarketCenter : Alpha 1
  boloAskPriceLong : BitVec 64
  boloAskSize : BitVec 16
  deriving DecidableEq, Repr

namespace BoloAppendageLongForm

def encode (message : BoloAppendageLongForm) : List UInt8 :=
  Alpha.encode message.boloBestBidMarketCenter
    ++ (encodeUInt 8 message.boloBidPriceLong
    ++ (encodeUInt 2 message.boloBidSize
    ++ (Alpha.encode message.boloBestAskMarketCenter
    ++ (encodeUInt 8 message.boloAskPriceLong
    ++ (encodeUInt 2 message.boloAskSize)))))

def decode (bytes : List UInt8) : Option (BoloAppendageLongForm × List UInt8) := do
  let (boloBestBidMarketCenter, bytes) ← Alpha.decode 1 bytes
  let (boloBidPriceLong, bytes) ← decodeUInt 8 bytes
  let (boloBidSize, bytes) ← decodeUInt 2 bytes
  let (boloBestAskMarketCenter, bytes) ← Alpha.decode 1 bytes
  let (boloAskPriceLong, bytes) ← decodeUInt 8 bytes
  let (boloAskSize, bytes) ← decodeUInt 2 bytes
  pure ({ boloBestBidMarketCenter, boloBidPriceLong, boloBidSize, boloBestAskMarketCenter, boloAskPriceLong, boloAskSize }, bytes)

@[simp] theorem encode_length (message : BoloAppendageLongForm) : (encode message).length = 22 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, encodeUInt_length]

theorem encode_length_pos (message : BoloAppendageLongForm) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : BoloAppendageLongForm) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
...
def decode (bytes : List UInt8) : Option (BoloAppendageMpidForm × List UInt8) := do
  ...
  pure ({ boloBestBidMarketCenter, boloBidPriceLong, boloBidSize, boloBestAskMarketCenter, boloAskPriceLong, boloAskSize, boloBestBidMarketParticipantIdentifier, boloBestAskMarketParticipantIdentifier }, bytes)
...
def tag : BoloAppendageChoice → BitVec 8
  | .boloAppendageShortForm _ => 50
  | .boloAppendageLongForm _ => 51
  | .boloAppendageMpidForm _ => 53
  | .noBoloChange _ => 48
  | .noBoloCanBeCalculated _ => 49

def encode : BoloAppendageChoice → List UInt8
  | .boloAppendageShortForm message => BoloAppendageShortForm.encode message
  | .boloAppendageLongForm message => BoloAppendageLongForm.encode message
...
theorem encode_length_le (message : BoloAppendageChoice) : (encode message).length ≤ 30 := by
  ...
  | boloAppendageShortForm inner =>
    simp only [encode, BoloAppendageShortForm.encode_length]
    omega
  | boloAppendageLongForm inner =>
    simp only [encode, BoloAppendageLongForm.encode_length]
    omega
...
def decode (tag : BitVec 8) (bytes : List UInt8) : Option (BoloAppendageChoice × List UInt8) :=
  if tag = 50 then (BoloAppendageShortForm.decode bytes).map fun (message, rest) => (.boloAppendageShortForm message, rest)
  else if tag = 51 then (BoloAppendageLongForm.decode bytes).map fun (message, rest) => (.boloAppendageLongForm message, rest)
  else if tag = 53 then (BoloAppendageMpidForm.decode bytes).map fun (message, rest) => (.boloAppendageMpidForm message, rest)
...
structure CombinedQuoteMessageShortFormMessage where
  ...
  participantToken : BitVec 64
  symbolShort : Alpha 5
  protectedBidPriceShort : BitVec 16
  protectedBidSizeShort : BitVec 16
  protectedAskPriceShort : BitVec 16
  protectedAskSizeShort : BitVec 16
  quoteCondition : QuoteCondition
  sipGeneratedUpdateFlag : SipGeneratedUpdateFlag
  luldBboIndicator : LuldBboIndicator
  retailInterestIndicator : RetailInterestIndicator
...
def encode (message : CombinedQuoteMessageShortFormMessage) : List UInt8 :=
  MarketCenterOriginator.encode message.marketCenterOriginator
    ++ (SubMarketCenterId.encode message.subMarketCenterId
    ++ (encodeUInt 8 message.sipTimestamp
    ++ (encodeUInt 8 message.timestamp1
    ++ (encodeUInt 8 message.participantToken
    ++ (Alpha.encode message.symbolShort
    ++ (encodeUInt 2 message.protectedBidPriceShort
    ++ (encodeUInt 2 message.protectedBidSizeShort
    ++ (encodeUInt 2 message.protectedAskPriceShort
    ++ (encodeUInt 2 message.protectedAskSizeShort
    ++ (QuoteCondition.encode message.quoteCondition
    ++ (SipGeneratedUpdateFlag.encode message.sipGeneratedUpdateFlag
    ++ (LuldBboIndicator.encode message.luldBboIndicator
    ++ (RetailInterestIndicator.encode message.retailInterestIndicator
...
def decode (bytes : List UInt8) : Option (CombinedQuoteMessageShortFormMessage × List UInt8) := do
  ...
  let (participantToken, bytes) ← decodeUInt 8 bytes
  let (symbolShort, bytes) ← Alpha.decode 5 bytes
  let (protectedBidPriceShort, bytes) ← decodeUInt 2 bytes
  let (protectedBidSizeShort, bytes) ← decodeUInt 2 bytes
  let (protectedAskPriceShort, bytes) ← decodeUInt 2 bytes
  let (protectedAskSizeShort, bytes) ← decodeUInt 2 bytes
  let (quoteCondition, bytes) ← QuoteCondition.decode bytes
  let (sipGeneratedUpdateFlag, bytes) ← SipGeneratedUpdateFlag.decode bytes
  let (luldBboIndicator, bytes) ← LuldBboIndicator.decode bytes
  let (retailInterestIndicator, bytes) ← RetailInterestIndicator.decode bytes
...
structure CombinedQuoteMessageLongFormMessage where
  marketCenterOriginator : MarketCenterOriginator
  subMarketCenterId : SubMarketCenterId
  sipTimestamp : BitVec 64
  timestamp1 : BitVec 64
  ...
  symbolLong : Alpha 11
  protectedBidPriceLong : BitVec 64
  protectedBidSizeLong : BitVec 32
  protectedAskPriceLong : BitVec 64
  protectedAskSizeLong : BitVec 32
  quoteCondition : QuoteCondition
  sipGeneratedUpdateFlag : SipGeneratedUpdateFlag
  luldBboIndicator : LuldBboIndicator
  retailInterestIndicator : RetailInterestIndicator
...
def encode (message : CombinedQuoteMessageLongFormMessage) : List UInt8 :=
  MarketCenterOriginator.encode message.marketCenterOriginator
    ++ (SubMarketCenterId.encode message.subMarketCenterId
    ++ (encodeUInt 8 message.sipTimestamp
    ++ (encodeUInt 8 message.timestamp1
    ...
    ++ (Alpha.encode message.symbolLong
    ++ (encodeUInt 8 message.protectedBidPriceLong
    ++ (encodeUInt 4 message.protectedBidSizeLong
    ++ (encodeUInt 8 message.protectedAskPriceLong
    ++ (encodeUInt 4 message.protectedAskSizeLong
    ++ (QuoteCondition.encode message.quoteCondition
    ++ (SipGeneratedUpdateFlag.encode message.sipGeneratedUpdateFlag
    ++ (LuldBboIndicator.encode message.luldBboIndicator
    ++ (RetailInterestIndicator.encode message.retailInterestIndicator
...
def decode (bytes : List UInt8) : Option (CombinedQuoteMessageLongFormMessage × List UInt8) := do
  ...
  let (symbolLong, bytes) ← Alpha.decode 11 bytes
  let (protectedBidPriceLong, bytes) ← decodeUInt 8 bytes
  let (protectedBidSizeLong, bytes) ← decodeUInt 4 bytes
  let (protectedAskPriceLong, bytes) ← decodeUInt 8 bytes
  let (protectedAskSizeLong, bytes) ← decodeUInt 4 bytes
  let (quoteCondition, bytes) ← QuoteCondition.decode bytes
  let (sipGeneratedUpdateFlag, bytes) ← SipGeneratedUpdateFlag.decode bytes
  let (luldBboIndicator, bytes) ← LuldBboIndicator.decode bytes
  let (retailInterestIndicator, bytes) ← RetailInterestIndicator.decode bytes
...
structure OddLotQuoteMessageShortFormMessage where
  marketCenterOriginator : MarketCenterOriginator
  subMarketCenterId : SubMarketCenterId
  sipTimestamp : BitVec 64
  timestamp1 : BitVec 64
...
def encode (message : OddLotQuoteMessageShortFormMessage) : List UInt8 :=
  MarketCenterOriginator.encode message.marketCenterOriginator
    ++ (SubMarketCenterId.encode message.subMarketCenterId
    ++ (encodeUInt 8 message.sipTimestamp
    ++ (encodeUInt 8 message.timestamp1
...
structure OddLotQuoteMessageLongFormMessage where
  marketCenterOriginator : MarketCenterOriginator
  subMarketCenterId : SubMarketCenterId
  sipTimestamp : BitVec 64
  timestamp1 : BitVec 64
...
def encode (message : OddLotQuoteMessageLongFormMessage) : List UInt8 :=
  MarketCenterOriginator.encode message.marketCenterOriginator
    ++ (SubMarketCenterId.encode message.subMarketCenterId
    ++ (encodeUInt 8 message.sipTimestamp
    ++ (encodeUInt 8 message.timestamp1
...
def tag : QuoteMessagePayload → BitVec 8
  | .utpQuoteShortformMessage _ => 69
  | .utpQuoteLongformMessage _ => 70
  | .finraAdfMarketParticipantQuotationMessage _ => 77
  | .combinedQuoteMessageShortFormMessage _ => 67
  | .combinedQuoteMessageLongFormMessage _ => 68
  | .oddLotQuoteMessageShortFormMessage _ => 65
  | .oddLotQuoteMessageLongFormMessage _ => 66

def encode : QuoteMessagePayload → List UInt8
  | .utpQuoteShortformMessage message => UtpQuoteShortformMessage.encode message
  | .utpQuoteLongformMessage message => UtpQuoteLongformMessage.encode message
  | .finraAdfMarketParticipantQuotationMessage message => FinraAdfMarketParticipantQuotationMessage.encode message
  | .combinedQuoteMessageShortFormMessage message => CombinedQuoteMessageShortFormMessage.encode message
  | .combinedQuoteMessageLongFormMessage message => CombinedQuoteMessageLongFormMessage.encode message
  | .oddLotQuoteMessageShortFormMessage message => OddLotQuoteMessageShortFormMessage.encode message
...
theorem encode_length_le (message : QuoteMessagePayload) : (encode message).length ≤ 145 := by
  ...
  | utpQuoteShortformMessage inner =>
    ...
    simp only [encode]
    omega
  | utpQuoteLongformMessage inner =>
    have bound_inner := UtpQuoteLongformMessage.encode_length_le inner
    simp only [encode]
    omega
  | finraAdfMarketParticipantQuotationMessage inner =>
    simp only [encode, FinraAdfMarketParticipantQuotationMessage.encode_length]
    omega
  | combinedQuoteMessageShortFormMessage inner =>
    have bound_inner := CombinedQuoteMessageShortFormMessage.encode_length_le inner
    simp only [encode]
    omega
  | combinedQuoteMessageLongFormMessage inner =>
    have bound_inner := CombinedQuoteMessageLongFormMessage.encode_length_le inner
    simp only [encode]
    omega
  | oddLotQuoteMessageShortFormMessage inner =>
    have bound_inner := OddLotQuoteMessageShortFormMessage.encode_length_le inner
    simp only [encode]
    omega
  | oddLotQuoteMessageLongFormMessage inner =>
    have bound_inner := OddLotQuoteMessageLongFormMessage.encode_length_le inner
    simp only [encode]
    omega

def decode (tag : BitVec 8) (bytes : List UInt8) : Option (QuoteMessagePayload × List UInt8) :=
  if tag = 69 then (UtpQuoteShortformMessage.decode bytes).map fun (message, rest) => (.utpQuoteShortformMessage message, rest)
  else if tag = 70 then (UtpQuoteLongformMessage.decode bytes).map fun (message, rest) => (.utpQuoteLongformMessage message, rest)
  ...
  else if tag = 67 then (CombinedQuoteMessageShortFormMessage.decode bytes).map fun (message, rest) => (.combinedQuoteMessageShortFormMessage message, rest)
  else if tag = 68 then (CombinedQuoteMessageLongFormMessage.decode bytes).map fun (message, rest) => (.combinedQuoteMessageLongFormMessage message, rest)
  else if tag = 65 then (OddLotQuoteMessageShortFormMessage.decode bytes).map fun (message, rest) => (.oddLotQuoteMessageShortFormMessage message, rest)
  else if tag = 66 then (OddLotQuoteMessageLongFormMessage.decode bytes).map fun (message, rest) => (.oddLotQuoteMessageLongFormMessage message, rest)
  else none
...
def decode (bytes : List UInt8) : Option (SessionCloseRecapMessage × List UInt8) := do
  ...
  if fits_marketCenterCloseRecap : marketCenterCloseRecap_.length < 256 ^ 2 then
    pure ({ marketCenterOriginator, subMarketCenterId, sipTimestamp, timestamp1, participantToken, symbolLong, nationalBestBidMarketCenter, nationalBestBidPriceLong, nationalBestBidSize, nationalBestAskMarketCenter, nationalBestAskPriceLong, nationalBestAskSize, specialCondition, marketCenterCloseRecap := ⟨marketCenterCloseRecap_, fits_marketCenterCloseRecap⟩ }, bytes)
  else none
...
theorem encode_length_le (message : SessionCloseRecapMessage) : (encode message).length ≤ 2162729 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, MarketCenterOriginator.encode_length, SubMarketCenterId.encode_length, encodeUInt_length, Alpha.encode_length, SpecialCondition.encode_length, encodeMany_length_const MarketCenterCloseRecap.encode 33 MarketCenterCloseRecap.encode_length]
  omega
...
/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : CategoryPayload) : (encode message).length ≤ 2162730 := by
  cases message with
  | quoteMessage inner =>
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : Message) : (encode message).length ≤ 2162734 := by
  unfold encode
