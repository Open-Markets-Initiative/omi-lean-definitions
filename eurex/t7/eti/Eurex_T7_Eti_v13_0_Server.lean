...
/-!
# Eurex Exchange Enhanced Trading Interface v13.0
...
namespace Omi.EurexT7EtiFbeV130Server
...
def decode (bytes : List UInt8) : Option (OldBasketDataBcGrpComp × List UInt8) := do
  ...
  let (basketTradeReportText, bytes) ← Alpha.decode 20 bytes
  let (pad2, bytes) ← Alpha.decode 2 bytes
  pure ({ transactTime, basketTrdMatchId, origBasketTrdMatchId, tradeReportId, maturityMonthYear, optionalEarlyTerminationIndicator, basketTradeReportType, basketTradeReportText, pad2 }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : BasketRollBroadcast) : (encode message).length ≤ 4245490 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const BasketRootPartyGrpComp.encode 40 BasketRootPartyGrpComp.encode_length, OldBasketDataBcGrpComp.encode_length, NewBasketDataBcGrpComp.encode_length, encodeMany_length_const OldBasketInstrmtMatchSideGrpComp.encode 80 OldBasketInstrmtMatchSideGrpComp.encode_length, encodeMany_length_const NewBasketInstrmtMatchSideGrpComp.encode 80 NewBasketInstrmtMatchSideGrpComp.encode_length, encodeMany_length_const OldBasketSideAllocGrpComp.encode 32 OldBasketSideAllocGrpComp.encode_length, encodeMany_length_const NewBasketSideAllocGrpComp.encode 32 NewBasketSideAllocGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 94528 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 108672 := by
...
end Omi.EurexT7EtiFbeV130Server
