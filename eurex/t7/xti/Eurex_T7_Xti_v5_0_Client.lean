...
namespace Omi.EurexT7XtiFbeV50Client
...
def decode (bytes : List UInt8) : Option (CrossRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, complianceId, marketSegmentId, orderQty }, bytes)
...
/-- Modify Order Single Request: 162 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, securityId, price, stopPx, pegOffsetValueAbs, pegOffsetValuePct, complianceId, orderQty, displayQty, displayLowQty, displayHighQty, expireDate, marketSegmentId, matchInstCrossId, targetPartyIdSessionId, applSeqIndicator, side, ordType, priceValidityCheckType, timeInForce, execInst, tradingSessionSubId, stopPxIndicator, tradingCapacity, ownershipIndicator, freeText1, freeText4, pad2v2 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderSingleRequest) : (encode message).length = 162 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleShortRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, securityId, price, complianceId, orderQty, matchInstCrossId, enrichmentRuleId, side, priceValidityCheckType, timeInForce, execInst, tradingCapacity, pad1 }, bytes)
...
/-- New Order Single Request: 138 bytes -/
...
def decode (bytes : List UInt8) : Option (NewOrderSingleRequest × List UInt8) := do
  ...
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (freeText4, bytes) ← Alpha.decode 16 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, price, stopPx, pegOffsetValueAbs, pegOffsetValuePct, clOrdId, securityId, complianceId, orderQty, displayQty, displayLowQty, displayHighQty, expireDate, marketSegmentId, matchInstCrossId, applSeqIndicator, side, ordType, priceValidityCheckType, timeInForce, execInst, tradingSessionSubId, tradingCapacity, freeText1, freeText4 }, bytes)

@[simp] theorem encode_length (message : NewOrderSingleRequest) : (encode message).length = 138 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (NewOrderSingleShortRequest × List UInt8) := do
  ...
  let (tradingCapacity, bytes) ← decodeUInt 1 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, price, clOrdId, complianceId, orderQty, matchInstCrossId, enrichmentRuleId, side, priceValidityCheckType, timeInForce, execInst, tradingCapacity, pad1 }, bytes)
...
def decode (bytes : List UInt8) : Option (QuoteActivationRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, complianceId, marketSegmentId, targetPartyIdSessionId, massActionType, pad7 }, bytes)
...
def decode (bytes : List UInt8) : Option (RfqRequest × List UInt8) := do
  ...
  let (side, bytes) ← decodeUInt 1 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, complianceId, marketSegmentId, orderQty, side, pad7 }, bytes)
...
theorem encode_length_le (message : ClientPayload) : (encode message).length ≤ 8210 := by
  cases message with
  | crossRequest inner =>
...
end Omi.EurexT7XtiFbeV50Client
