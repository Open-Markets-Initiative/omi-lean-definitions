...
/-!
# Miami International Holdings Top Of Market v1.1.a
...
namespace Omi.MiaxPearlequitiesTopofmarketMachV11A
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : SystemTimeMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : SymbolUpdateMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : SystemStateMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : SecurityTradingStatusNotificationMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
def decode (bytes : List UInt8) : Option (CompactTopOfMarketBestBidAndOfferMessage × List UInt8) := do
  ...
  pure ({ nanoseconds, symbolId, bidPrice, bidSize, offerPrice, offerSize }, bytes)
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : CompactTopOfMarketBestBidAndOfferMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []

end CompactTopOfMarketBestBidAndOfferMessage

/-- Wide Top Of Market Best Bid And Offer Message: 16 bytes -/
...
@[simp] theorem encode_length (message : WideTopOfMarketBestBidAndOfferMessage) : (encode message).length = 16 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : WideTopOfMarketBestBidAndOfferMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
def decode (bytes : List UInt8) : Option LastSaleMessage := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (symbolId, bytes) ← decodeUIntLE 4 bytes
  ...
  let flags_ := bytes
  if fits_flags : flags_.length ≤ 65481 then
    pure { nanoseconds, symbolId, tradeId, correctionNumber, price, size, flags := ⟨flags_, fits_flags⟩ }
  else none
...
theorem encode_length_le (message : LastSaleMessage) : (encode message).length ≤ 65510 := by
  have bound_flags := message.flags.length_le
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length]
  omega

theorem decode_encode (message : LastSaleMessage) : decode (encode message) = some message := by
  unfold decode encode
  rw [decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [decodeUIntLE_encodeUIntLE, some_bind]
  ...
  rw [decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [dite_eq_left message.flags.length_le]
  rfl
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : TradeCancelMessage) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
theorem encode_length_le (message : Data) : (encode message).length ≤ 65510 := by
  cases message with
  ...
  | lastSaleMessage inner =>
    have bound_inner := LastSaleMessage.encode_length_le inner
    simp only [encode]
    omega
...
def decode (tag : BitVec 8) (bytes : List UInt8) : Option Data :=
  if tag = 49 then (SystemTimeMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.systemTimeMessage message) else none
  else if tag = 1 then (SymbolUpdateMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.symbolUpdateMessage message) else none
  else if tag = 83 then (SystemStateMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.systemStateMessage message) else none
  else if tag = 4 then (SecurityTradingStatusNotificationMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.securityTradingStatusNotificationMessage message) else none
  else if tag = 2 then (CompactTopOfMarketBestBidAndOfferMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.compactTopOfMarketBestBidAndOfferMessage message) else none
  else if tag = 3 then (WideTopOfMarketBestBidAndOfferMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.wideTopOfMarketBestBidAndOfferMessage message) else none
  else if tag = 10 then (LastSaleMessage.decode bytes).map fun message => .lastSaleMessage message
  else if tag = 11 then (TradeCancelMessage.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.tradeCancelMessage message) else none
  else none

theorem decode_encode (message : Data) :
    decode (tag message) (encode message) = some message := by
  cases message with
  | systemTimeMessage message => simp [decode, encode, tag, SystemTimeMessage.decode_encode_nil]
  | symbolUpdateMessage message => simp [decode, encode, tag, SymbolUpdateMessage.decode_encode_nil]
  | systemStateMessage message => simp [decode, encode, tag, SystemStateMessage.decode_encode_nil]
  | securityTradingStatusNotificationMessage message => simp [decode, encode, tag, SecurityTradingStatusNotificationMessage.decode_encode_nil]
  | compactTopOfMarketBestBidAndOfferMessage message => simp [decode, encode, tag, CompactTopOfMarketBestBidAndOfferMessage.decode_encode_nil]
  | wideTopOfMarketBestBidAndOfferMessage message => simp [decode, encode, tag, WideTopOfMarketBestBidAndOfferMessage.decode_encode_nil]
  | lastSaleMessage message => simp [decode, encode, tag, LastSaleMessage.decode_encode]
  | tradeCancelMessage message => simp [decode, encode, tag, TradeCancelMessage.decode_encode_nil]
...
def decode (bytes : List UInt8) : Option ApplicationMessage := do
  ...
  let data ← Data.decode messageType bytes
  pure { data }
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ApplicationMessage) : (encode message).length ≤ 65511 := by
  unfold encode
...
/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : Payload) : (encode message).length ≤ 65511 := by
  cases message with
  | heartbeat inner =>
...
def decode (tag : BitVec 8) (bytes : List UInt8) : Option Payload :=
  if tag = 0 then (Heartbeat.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.heartbeat message) else none
  ...
  else if tag = 3 then (ApplicationMessage.decode bytes).map fun message => .applicationMessage message
  else none
...
def decodeBody (sequenceNumber : BitVec 64) (bytes : List UInt8) : Option MachMessage := do
  ...
  let payload ← Payload.decode machPacketType bytes
...
/-- Size rule: Mach Packet Length counts the bytes after it plus 10, so it is written from the body and checked on decode; Sequence Number is read ahead of it -/
...
end Omi.MiaxPearlequitiesTopofmarketMachV11A
