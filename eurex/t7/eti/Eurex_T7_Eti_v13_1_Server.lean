...
/-!
# Eurex Exchange Enhanced Trading Interface v13.1
...
namespace Omi.EurexT7EtiFbeV131Server
...
/-- Delete Order Broadcast: 138 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteOrderBroadcast × List UInt8) := do
  ...
  let (side, bytes) ← decodeUInt 1 bytes
  let (pad5, bytes) ← Alpha.decode 5 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, cumQty, cxlQty, marketSegmentId, partyIdEnteringTrader, partyIdSessionId, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, productComplex, side, pad5 }, bytes)

@[simp] theorem encode_length (message : DeleteOrderBroadcast) : (encode message).length = 138 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecNotification) : (encode message).length ≤ 2111546 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  have bound_instrmntLegExecGrpComp := message.instrmntLegExecGrpComp.length_lt
  have bound_orderEventGrpComp := message.orderEventGrpComp.length_lt
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 2113770 := by
...
end Omi.EurexT7EtiFbeV131Server
