...
namespace Omi.IexIexequitiesDeepIextpV106
...
def ofByte? (byte : UInt8) : Option SystemEvent :=
  if byte = 0x53 then some .startOfSystemHours
...
def ofByte? (byte : UInt8) : Option Detail :=
  ...
  else if byte = 0x4E then some .detailNotAvailable
  else none
...
end Omi.IexIexequitiesDeepIextpV106
