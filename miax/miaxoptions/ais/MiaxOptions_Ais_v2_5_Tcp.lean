...
/-!
# Miami International Holdings Administrative Information Subscriber v2.5
...
namespace Omi.MiaxMiaxoptionsAisMachV25Tcp
...
def decode (bytes : List UInt8) : Option (ComplexLiquiditySeekingEventNotificationMessage × List UInt8) := do
  ...
  let (attributableId, bytes) ← Alpha.decode 4 bytes
  let (reserved8, bytes) ← Alpha.decode 8 bytes
  pure ({ nanoseconds, strategyId, eventType, eventId, side, priceLong, matchedQuantity, imbalanceQuantitySigned, attributableId, reserved8 }, bytes)
...
end Omi.MiaxMiaxoptionsAisMachV25Tcp
