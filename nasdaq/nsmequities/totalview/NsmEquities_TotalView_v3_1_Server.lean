...
inductive TradingState where
  | haltedOrPausedAllMarkets -- Halted Or Paused All Markets
  | haltedOrPausedNasdaqOnly -- Halted Or Paused Nasdaq Only
  | quotationOnlyCrossSro -- Quotation Only Cross Sro
  | quotationOnlyNasdaqOnly -- Quotation Only Nasdaq Only
  | trading -- Trading
...
def toByte : TradingState → UInt8
  | .haltedOrPausedAllMarkets => 0x48
  | .haltedOrPausedNasdaqOnly => 0x56
  | .quotationOnlyCrossSro => 0x51
  | .quotationOnlyNasdaqOnly => 0x52
  | .trading => 0x54
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : TradingState :=
  if byte = 0x48 then .haltedOrPausedAllMarkets
  else if byte = 0x56 then .haltedOrPausedNasdaqOnly
  else if byte = 0x51 then .quotationOnlyCrossSro
  else if byte = 0x52 then .quotationOnlyNasdaqOnly
  else .trading
...
theorem ofByte_toByte (value : TradingState) : ofByte value.toByte = value := by
  cases value with
  | haltedOrPausedAllMarkets => decide
  | haltedOrPausedNasdaqOnly => decide
  | quotationOnlyCrossSro => decide
  | quotationOnlyNasdaqOnly => decide
  | trading => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
def decode (bytes : List UInt8) : Option (StockDirectoryMessage × List UInt8) := do
  ...
  pure ({ stockAlpha6, marketCategory, financialStatusIndicator, roundLotSize, roundLotsOnly := roundLotsOnly_ }, bytes)
...
def decode (bytes : List UInt8) : Option (MarketParticipantPositionMessage × List UInt8) := do
  ...
  pure ({ mpid, stockAlphanumeric6, primaryMarketMaker, marketMakerMode, marketParticipantState }, bytes)

@[simp] theorem encode_length (message : MarketParticipantPositionMessage) : (encode message).length = 13 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (AddOrderMessage × List UInt8) := do
  ...
  pure ({ orderReferenceNumber, side, sharesNumeric6, stockAlpha6, price }, bytes)
...
def decode (bytes : List UInt8) : Option (AddOrderWithMpidMessage × List UInt8) := do
  ...
  pure ({ orderReferenceNumber, side, sharesNumeric6, stockAlpha6, price, attribution }, bytes)

@[simp] theorem encode_length (message : AddOrderWithMpidMessage) : (encode message).length = 39 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, Side.encode_length]
...
def decode (bytes : List UInt8) : Option (TradeMessage × List UInt8) := do
  ...
  let (price, bytes) ← Alpha.decode 10 bytes
  let (matchNumber, bytes) ← Alpha.decode 12 bytes
  pure ({ orderReferenceNumber, side, sharesNumeric6, stockAlpha6, price, matchNumber }, bytes)
...
def decode (bytes : List UInt8) : Option (CrossTradeMessage × List UInt8) := do
  ...
  pure ({ sharesNumeric9, stockAlpha6, crossPrice, matchNumber, crossType }, bytes)

@[simp] theorem encode_length (message : CrossTradeMessage) : (encode message).length = 38 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (NetOrderImbalanceIndicatorMessage × List UInt8) := do
  ...
  pure ({ pairedShares, imbalanceShares, imbalanceDirection, stockAlpha6, farPrice, nearPrice, currentReferencePrice, crossType, priceVariationIndicator }, bytes)

@[simp] theorem encode_length (message : NetOrderImbalanceIndicatorMessage) : (encode message).length = 57 := by
  unfold encode
...
theorem encode_length_le (message : SequencedMessage) : (encode message).length ≤ 57 := by
  cases message with
...
theorem encode_length_le (message : SequencedDataPacket) : (encode message).length ≤ 58 := by
  unfold encode
...
/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 58 := by
  cases message with
  | debugPacket inner =>
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ServerPacket) : (encode message).length ≤ 60 := by
  unfold encode
