...
theorem encode_length_le (message : ApproveBasketTradeRequest) : (encode message).length ≤ 44986 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const BasketSideAllocExtGrpComp.encode 176 BasketSideAllocExtGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ApproveBasketTradeRequest) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 BasketSideAllocExtGrpComp.encode BasketSideAllocExtGrpComp.decode BasketSideAllocExtGrpComp.decode_encode, some_bind]
  dsimp only
...
@[simp] theorem decode_encode (message : ModifyTesTradeRequest) (rest : List UInt8) :
  ...
  rw [decodeMany_bounded 1 SrqsRelatedTradeIdGrpComp.encode SrqsRelatedTradeIdGrpComp.decode SrqsRelatedTradeIdGrpComp.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.sideAllocGrpComp.length_lt, dite_eq_left message.trdInstrmntLegGrpComp.length_lt, dite_eq_left message.srqsRelatedTradeIdGrpComp.length_lt]
  rfl
...
theorem encode_length_le (message : SrqsOpenNegotiationRequest) : (encode message).length ≤ 12490 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuotReqLegsGrpComp.encode 24 QuotReqLegsGrpComp.encode_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
...
theorem encode_length_le (message : SrqsUpdateNegotiationRequest) : (encode message).length ≤ 6338 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
