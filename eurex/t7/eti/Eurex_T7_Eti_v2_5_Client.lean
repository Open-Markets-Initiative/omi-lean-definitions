...
namespace Omi.EurexT7EtiFbeV25Client
...
/-- Delete All Order Request: 50 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteAllOrderRequest × List UInt8) := do
  ...
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, complianceId, marketSegmentId, targetPartyIdSessionId, targetPartyIdExecutingTrader, pad4 }, bytes)

@[simp] theorem encode_length (message : DeleteAllOrderRequest) : (encode message).length = 50 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length]
...
def decode (bytes : List UInt8) : Option (MassQuoteRequest × List UInt8) := do
  ...
  if fits_quoteEntryGrpComp : quoteEntryGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, quoteId, complianceId, marketSegmentId, enrichmentRuleId, priceValidityCheckType, quoteSizeType, pad7, quoteEntryGrpComp := ⟨quoteEntryGrpComp_, fits_quoteEntryGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyOrderComplexRequest) : (encode message).length ≤ 2234 := by
...
/-- Modify Order Single Request: 202 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleRequest × List UInt8) := do
  ...
  let (freeText3, bytes) ← Alpha.decode 12 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, price, stopPx, complianceId, orderQty, expireDate, marketSegmentId, simpleSecurityId, targetPartyIdSessionId, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, applSeqIndicator, side, ordType, priceValidityCheckType, timeInForce, execInst, tradingSessionSubId, tradingCapacity, account, partyIdPositionAccount, positionEffect, ownershipIndicator, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, pad4 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderSingleRequest) : (encode message).length = 202 := by
  unfold encode
...
/-- Modify Order Single Short Request: 66 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleShortRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, price, complianceId, orderQty, simpleSecurityId, enrichmentRuleId, side, priceValidityCheckType, timeInForce, execInst, tradingCapacity, pad1 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderSingleShortRequest) : (encode message).length = 66 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderComplexRequest) : (encode message).length ≤ 2218 := by
  have bound_legOrdGrpComp := message.legOrdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const LegOrdGrpComp.encode 8 LegOrdGrpComp.encode_length]
  omega
...
/-- New Order Single Request: 178 bytes -/
...
/-- New Order Single Short Request: 58 bytes -/
...
def decode (bytes : List UInt8) : Option (NewOrderSingleShortRequest × List UInt8) := do
  ...
  let (tradingCapacity, bytes) ← decodeUInt 1 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, price, clOrdId, complianceId, orderQty, simpleSecurityId, enrichmentRuleId, side, priceValidityCheckType, timeInForce, execInst, tradingCapacity, pad1 }, bytes)

@[simp] theorem encode_length (message : NewOrderSingleShortRequest) : (encode message).length = 58 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
theorem encode_length_le (message : ClientPayload) : (encode message).length ≤ 8218 := by
  cases message with
...
end Omi.EurexT7EtiFbeV25Client
