...
namespace Omi.EurexT7XtiFbeV100Server
...
theorem encode_length_le (message : XetraEnLightNegotiationRequesterNotification) : (encode message).length ≤ 4370 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const XetraEnLightTargetPartiesComp.encode 16 XetraEnLightTargetPartiesComp.encode_length]
  omega
...
end Omi.EurexT7XtiFbeV100Server
