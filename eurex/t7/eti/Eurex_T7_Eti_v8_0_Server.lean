...
/-!
# Eurex Exchange Enhanced Trading Interface v8.0
...
namespace Omi.EurexT7EtiFbeV80Server
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 12698 := by
...
/-- Srqs Deal Notification: 410 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsDealNotification × List UInt8) := do
  ...
  let (partyIdTakeUpTradingFirm, bytes) ← Alpha.decode 5 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, underlyingPriceStipValue, underlyingPx, lastPx, lastQty, negotiationId, tradeId, requestingPartySubIdType, trdRptStatus, tradeRequestResult, messageEventSource, tradingCapacity, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, targetPartyEnteringTrader, firmTradeId, firmNegotiationId, freeText1, freeText2, freeText3, freeText5, positionEffect, account, partyIdBeneficiary, custOrderHandlingInst, partyIdOrderOriginationFirm, partyIdPositionAccount, partyIdLocationId, complianceText, partyIdTakeUpTradingFirm, pad1 }, bytes)

@[simp] theorem encode_length (message : SrqsDealNotification) : (encode message).length = 410 := by
  unfold encode
...
def encode (message : SrqsOpenNegotiationNotification) : List UInt8 :=
  Alpha.encode message.pad2
    ...
    ++ (encodeUInt 1 message.quoteType
    ++ (encodeUInt 1 message.quoteSubType
    ++ (encodeUInt 1 message.quoteQualifier
    ++ (encodeUInt 1 message.quoteStatus
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationNotification × List UInt8) := do
  ...
  let (quoteType, bytes) ← decodeUInt 1 bytes
  let (quoteSubType, bytes) ← decodeUInt 1 bytes
  let (quoteQualifier, bytes) ← decodeUInt 1 bytes
  let (quoteStatus, bytes) ← decodeUInt 1 bytes
  let (noLegs, bytes) ← decodeUInt 1 bytes
  let (side, bytes) ← decodeUInt 1 bytes
  let (quoteRefPriceSource, bytes) ← decodeUInt 1 bytes
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, negotiationStartTime, securityId, bidPx, offerPx, leavesQty, lastQty, lastPx, quoteRefPrice, underlyingDeltaPercentage, expireTime, negotiationId, marketSegmentId, securitySubType, numberOfRespondents, quoteType, quoteSubType, quoteQualifier, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, productComplex, negotiateUnderlying, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, freeText5, pad2v2, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationNotification) : (encode message).length ≤ 6450 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 12554 := by
  have bound_quotReqLegsGrpComp := message.quotReqLegsGrpComp.length_lt
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
...
def decode (bytes : List UInt8) : Option (SrqsQuoteNotification × List UInt8) := do
  ...
  let (partyIdTakeUpTradingFirm, bytes) ← Alpha.decode 5 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteId, secondaryQuoteId, bidPx, bidSize, offerPx, offerSize, underlyingPx, underlyingDeltaPercentage, negotiationId, quotingStatus, tradingCapacity, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, quoteReqId, freeText1, freeText2, freeText3, freeText5, positionEffect, account, partyIdBeneficiary, custOrderHandlingInst, partyIdOrderOriginationFirm, partyIdPositionAccount, partyIdLocationId, complianceText, partyIdTakeUpTradingFirm, pad6 }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 31002 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 39034 := by
...
end Omi.EurexT7EtiFbeV80Server
