...
/-!
# Brasil, Bolsa, Balcão Binary Entry Point v8.1
...
namespace Omi.B3B3derivativesBinaryentrypointSbeV81
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeMessage) : (encode message).length ≤ 676 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
end Omi.B3B3derivativesBinaryentrypointSbeV81
