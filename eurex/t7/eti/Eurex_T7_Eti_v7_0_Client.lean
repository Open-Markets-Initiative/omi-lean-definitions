...
namespace Omi.EurexT7EtiFbeV70Client
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterTesTradeRequest) : (encode message).length ≤ 32874 := by
...
def decode (bytes : List UInt8) : Option (ModifyTesTradeRequest × List UInt8) := do
  ...
  if fits_sideAllocGrpComp : sideAllocGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, lastPx, transBkdTime, marketSegmentId, packageId, tesExecId, relatedMarketSegmentId, negotiationId, srqsRelatedTradeId, trdType, tradeReportType, tradePublishIndicator, tradeReportText, tradeReportId, pad2v2, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyTesTradeRequest) : (encode message).length ≤ 10306 := by
  have bound_sideAllocGrpComp := message.sideAllocGrpComp.length_lt
  have bound_trdInstrmntLegGrpComp := message.trdInstrmntLegGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SideAllocGrpComp.encode 24 SideAllocGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length]
  omega
...
/-- Srqs Enter Quote Request: 98 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsEnterQuoteRequest × List UInt8) := do
  ...
  let (partyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, bidPx, offerPx, underlyingPx, underlyingDeltaPercentage, bidSize, offerSize, marketSegmentId, negotiationId, valueCheckTypeQuantity, partyExecutingFirm, partyExecutingTrader, freeText1 }, bytes)

@[simp] theorem encode_length (message : SrqsEnterQuoteRequest) : (encode message).length = 98 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- Srqs Hit Quote Request: 114 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsHitQuoteRequest × List UInt8) := do
  ...
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, quoteId, orderQty, validUntilTime, underlyingQty, underlyingPriceStipValue, marketSegmentId, negotiationId, side, valueCheckTypeQuantity, partyExecutingFirm, partyExecutingTrader, firmTradeId, freeText1, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsHitQuoteRequest) : (encode message).length = 114 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequest) : (encode message).length ≤ 12362 := by
...
/-- Srqs Update Deal Status Request: 74 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsUpdateDealStatusRequest × List UInt8) := do
  ...
  let (partyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, underlyingPriceStipValue, underlyingPx, marketSegmentId, negotiationId, tradeId, pad4, lastQty, tradeReportType, trdRptStatus, partyExecutingFirm, partyExecutingTrader, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsUpdateDealStatusRequest) : (encode message).length = 74 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsUpdateNegotiationRequest) : (encode message).length ≤ 6202 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  ...
  if fits_sideAllocExtGrpComp : sideAllocExtGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ networkMsgId, pad2, requestHeaderComp, securityId, lastPx, transBkdTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, srqsRelatedTradeId, trdType, productComplex, tradeReportType, tradePublishIndicator, skipValidations, trdRptStatus, tradePlatform, hedgeType, partyIdSettlementLocation, tradeReportId, tradeReportText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad2v2, sideAllocExtGrpComp := ⟨sideAllocExtGrpComp_, fits_sideAllocExtGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UploadTesTradeRequest) : (encode message).length ≤ 71634 := by
...
end Omi.EurexT7EtiFbeV70Client
