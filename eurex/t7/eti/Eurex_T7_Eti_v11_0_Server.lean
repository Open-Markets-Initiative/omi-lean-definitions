...
/-!
# Eurex Exchange Enhanced Trading Interface v11.0
...
namespace Omi.EurexT7EtiFbeV110Server
...
def decode (bytes : List UInt8) : Option (RiskLimitsRptGrpComp × List UInt8) := do
  ...
  let (pad2, bytes) ← Alpha.decode 2 bytes
  pure ({ riskLimitQty, riskLimitOpenQty, riskLimitNetPositionQty, nettingCoefficient, quoteWeightingCoefficient, riskLimitType, riskLimitRequestingPartyRole, riskLimitViolationIndicator, riskLimitGroup, pad2 }, bytes)
...
theorem encode_length_le (message : PreTradeRiskLimitResponse) : (encode message).length ≤ 12314 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, NrResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const RiskLimitsRptGrpComp.encode 48 RiskLimitsRptGrpComp.encode_length]
  omega
...
end Omi.EurexT7EtiFbeV110Server
