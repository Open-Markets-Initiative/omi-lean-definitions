...
namespace Omi.NasdaqNsmequitiesOrdersOuchV42Client
...
inductive CustomerType where
  ...
  | notARetailDesignated -- Not A Retail Designated
  | default -- Default
...
def toByte : CustomerType → UInt8
  | .retailDesignated => 0x52
  | .notARetailDesignated => 0x4E
  | .default => 0x20
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : CustomerType :=
  if byte = 0x52 then .retailDesignated
  else if byte = 0x4E then .notARetailDesignated
  else .default
...
def decode (bytes : List UInt8) : Option (EnterOrderMessage × List UInt8) := do
  ...
  pure ({ orderToken, buySellIndicator, shares, stock, price, timeInForce, firm, display, capacity, intermarketSweepEligibility, minimumQuantity, crossType, customerType }, bytes)

@[simp] theorem encode_length (message : EnterOrderMessage) : (encode message).length = 48 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, BuySellIndicator.encode_length, encodeUInt_length, Display.encode_length, Capacity.encode_length, IntermarketSweepEligibility.encode_length, CrossType.encode_length, CustomerType.encode_length]
...
structure ReplaceOrderMessage where
  ...
  replacementOrderTokenToken14 : Alpha 14
  shares : BitVec 32
  price : BitVec 32
  timeInForce : BitVec 32
  display : Display
...
def encode (message : ReplaceOrderMessage) : List UInt8 :=
  Alpha.encode message.existingOrderToken
    ++ (Alpha.encode message.replacementOrderTokenToken14
    ++ (encodeUInt 4 message.shares
    ++ (encodeUInt 4 message.price
    ++ (encodeUInt 4 message.timeInForce
    ++ (Display.encode message.display
...
def decode (bytes : List UInt8) : Option (ReplaceOrderMessage × List UInt8) := do
  ...
  let (replacementOrderTokenToken14, bytes) ← Alpha.decode 14 bytes
  let (shares, bytes) ← decodeUInt 4 bytes
  let (price, bytes) ← decodeUInt 4 bytes
  let (timeInForce, bytes) ← decodeUInt 4 bytes
  let (display, bytes) ← Display.decode bytes
  let (intermarketSweepEligibility, bytes) ← IntermarketSweepEligibility.decode bytes
  let (minimumQuantity, bytes) ← decodeUInt 4 bytes
  pure ({ existingOrderToken, replacementOrderTokenToken14, shares, price, timeInForce, display, intermarketSweepEligibility, minimumQuantity }, bytes)
...
theorem encode_length_le (message : UnsequencedMessage) : (encode message).length ≤ 48 := by
  cases message with
...
theorem encode_length_le (message : UnsequencedDataPacket) : (encode message).length ≤ 49 := by
  unfold encode
...
/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : ClientPayload) : (encode message).length ≤ 49 := by
  cases message with
  | debugPacket inner =>
...
end Omi.NasdaqNsmequitiesOrdersOuchV42Client
