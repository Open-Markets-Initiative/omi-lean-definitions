...
Note: Amend Basket Trade Request is not framed: its length Body Len is not the integer that leads it.

Note: Approve Basket Trade Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Delete Basket Trade Request is not framed: its length Body Len is not the integer that leads it.

Note: Delete Clip Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Enter Basket Trade Request is not framed: its length Body Len is not the integer that leads it.

Note: Enter Clip Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Modify Basket Trade Request is not framed: its length Body Len is not the integer that leads it.

Note: Modify Order Complex Request is not framed: its length Body Len is not the integer that leads it.

Note: Modify Order Complex Short Request is not framed: its length Body Len is not the integer that leads it.
...
Note: New Order Complex Short Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Srqs Enter Quote Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Hit Quote Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Open Negotiation Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Quoting Status Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Update Deal Status Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Update Negotiation Request is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7EtiFbeV71Client
...
def decode (bytes : List UInt8) : Option (AddComplexInstrumentRequest × List UInt8) := do
  ...
  if fits_instrmtLegGrpComp : instrmtLegGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, securitySubType, productComplex, complianceText, pad2v2, instrmtLegGrpComp := ⟨instrmtLegGrpComp_, fits_instrmtLegGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : AddComplexInstrumentRequest) : (encode message).length ≤ 8210 := by
...
/-- Add Flexible Instrument Request: 58 bytes -/
...
def decode (bytes : List UInt8) : Option (AddFlexibleInstrumentRequest × List UInt8) := do
  ...
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, strikePrice, marketSegmentId, maturityDate, settlMethod, optAttribute, putOrCall, exerciseStyle, complianceText }, bytes)

@[simp] theorem encode_length (message : AddFlexibleInstrumentRequest) : (encode message).length = 58 := by
  unfold encode
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_instrmtMatchSideGrpComp : instrmtMatchSideGrpComp_.length < 256 ^ 1 then
      if fits_basketSideAllocGrpComp : basketSideAllocGrpComp_.length < 256 ^ 1 then
        pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, maturityMonthYear, trdType, tradeReportType, basketTradeReportType, basketTradeReportText, tradeReportId, pad5, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, instrmtMatchSideGrpComp := ⟨instrmtMatchSideGrpComp_, fits_instrmtMatchSideGrpComp⟩, basketSideAllocGrpComp := ⟨basketSideAllocGrpComp_, fits_basketSideAllocGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : AmendBasketTradeRequest) : (encode message).length ≤ 38850 := by
...
/-- Basket Side Alloc Ext Grp Comp: 176 bytes -/
...
def decode (bytes : List UInt8) : Option (BasketSideAllocExtGrpComp × List UInt8) := do
  ...
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  pure ({ allocQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, packageId, sideMarketSegmentId, allocId, side, positionEffect, tradingCapacity, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, orderAttributeRiskReduction, partyExecutingFirm, partyExecutingTrader, account, freeText1, freeText2, freeText3, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdPositionAccount, partyIdLocationId, custOrderHandlingInst, complianceText }, bytes)

@[simp] theorem encode_length (message : BasketSideAllocExtGrpComp) : (encode message).length = 176 := by
  unfold encode
...
structure ApproveBasketTradeRequest where
  ...
  basketSideTradeReportId : Alpha 20
  pad6 : Alpha 6
  basketSideAllocExtGrpComp : Bounded 1 BasketSideAllocExtGrpComp
  deriving DecidableEq, Repr
...
def encode (message : ApproveBasketTradeRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ...
    ++ encodeUIntLE 2 message.trdType
    ++ encodeUInt 1 message.tradeReportType
    ++ encodeUInt 1 (BitVec.ofNat (8 * 1) message.basketSideAllocExtGrpComp.val.length)
    ++ Alpha.encode message.basketTradeReportText
    ++ Alpha.encode message.tradeReportId
    ++ Alpha.encode message.basketSideTradeReportId
    ++ Alpha.encode message.pad6
...
def decode (bytes : List UInt8) : Option (ApproveBasketTradeRequest × List UInt8) := do
  ...
  if fits_basketSideAllocExtGrpComp : basketSideAllocExtGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, rootPartySubIdType, trdType, tradeReportType, basketTradeReportText, tradeReportId, basketSideTradeReportId, pad6, basketSideAllocExtGrpComp := ⟨basketSideAllocExtGrpComp_, fits_basketSideAllocExtGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ApproveBasketTradeRequest) : (encode message).length ≤ 44986 := by
  have bound_basketSideAllocExtGrpComp := message.basketSideAllocExtGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const BasketSideAllocExtGrpComp.encode 176 BasketSideAllocExtGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ApproveBasketTradeRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 BasketSideAllocExtGrpComp.encode BasketSideAllocExtGrpComp.decode BasketSideAllocExtGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.basketSideAllocExtGrpComp.length_lt, ↓reduceDIte]
  rfl

end ApproveBasketTradeRequest

/-- Approve Tes Trade Request: 226 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteClipRequest × List UInt8) := do
  ...
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, securityId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, pad2v2, crossRequestId, pad4 }, bytes)
...
/-- Delete Order Complex Request: 98 bytes -/
...
@[simp] theorem encode_length (message : DeleteOrderComplexRequest) : (encode message).length = 98 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (DeleteOrderSingleRequest × List UInt8) := do
  ...
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, simpleSecurityId, targetPartyIdSessionId, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, fixClOrdId, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (EnterBasketTradeRequest × List UInt8) := do
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_instrmtMatchSideGrpComp : instrmtMatchSideGrpComp_.length < 256 ^ 1 then
      if fits_basketSideAllocGrpComp : basketSideAllocGrpComp_.length < 256 ^ 1 then
        pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, maturityMonthYear, trdType, tradeReportType, basketTradeReportType, basketTradeReportText, tradeReportId, pad1, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, instrmtMatchSideGrpComp := ⟨instrmtMatchSideGrpComp_, fits_instrmtMatchSideGrpComp⟩, basketSideAllocGrpComp := ⟨basketSideAllocGrpComp_, fits_basketSideAllocGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterBasketTradeRequest) : (encode message).length ≤ 38834 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
  have bound_instrmtMatchSideGrpComp := message.instrmtMatchSideGrpComp.length_lt
  have bound_basketSideAllocGrpComp := message.basketSideAllocGrpComp.length_lt
  unfold encode
...
/-- Cross Request Side Grp Comp: 160 bytes -/
...
def decode (bytes : List UInt8) : Option (CrossRequestSideGrpComp × List UInt8) := do
  ...
  let (partyIdLocationId, bytes) ← Alpha.decode 2 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, maximumPrice, inputSource, side, tradingCapacity, executingTraderQualifier, orderAttributeLiquidityProvision, partyIdInvestmentDecisionMakerQualifier, positionEffect, custOrderHandlingInst, account, partyIdPositionAccount, freeText1, freeText2, freeText3, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdTakeUpTradingFirm, sideComplianceText, partyIdLocationId, pad7 }, bytes)

@[simp] theorem encode_length (message : CrossRequestSideGrpComp) : (encode message).length = 160 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, Alpha.encode_length]
...
def decode (bytes : List UInt8) : Option (EnterClipRequest × List UInt8) := do
  ...
  if fits_crossRequestSideGrpComp : crossRequestSideGrpComp_.length < 256 ^ 1 then
    if fits_sideCrossLegGrpComp : sideCrossLegGrpComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, securityId, price, orderQty, marketSegmentId, crossId, crossRequestId, sideDisclosureInstruction, priceDisclosureInstruction, orderQtyDisclosureInstruction, rootPartyContraFirm, rootPartyContraTrader, pad4, crossRequestSideGrpComp := ⟨crossRequestSideGrpComp_, fits_crossRequestSideGrpComp⟩, sideCrossLegGrpComp := ⟨sideCrossLegGrpComp_, fits_sideCrossLegGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterClipRequest) : (encode message).length ≤ 42914 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const CrossRequestSideGrpComp.encode 160 CrossRequestSideGrpComp.encode_length, encodeMany_length_const SideCrossLegGrpComp.encode 8 SideCrossLegGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : EnterClipRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 CrossRequestSideGrpComp.encode CrossRequestSideGrpComp.decode CrossRequestSideGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 SideCrossLegGrpComp.encode SideCrossLegGrpComp.decode SideCrossLegGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.crossRequestSideGrpComp.length_lt, message.sideCrossLegGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (SideAllocGrpComp × List UInt8) := do
  ...
  let (partyExecutingFirm, bytes) ← Alpha.decode 5 bytes
  let (partyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  pure ({ allocQty, individualAllocId, side, partyExecutingFirm, partyExecutingTrader }, bytes)
        ...
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
              pure ({ networkMsgId, pad2, requestHeaderComp, securityId, lastPx, transBkdTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad1, trdType, productComplex, tradeReportType, tradePublishIndicator, partyIdSettlementLocation, hedgeType, tradeReportText, tradeReportId, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad7, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
            else none
          else none
        else none
      else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterTesTradeRequest) : (encode message).length ≤ 34914 := by
...
@[simp] theorem decode_encode (message : EnterTesTradeRequest) (rest : List UInt8) :
  ...
  dsimp only
  simp only [message.sideAllocGrpComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (ModifyBasketTradeRequest × List UInt8) := do
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_instrmtMatchSideGrpComp : instrmtMatchSideGrpComp_.length < 256 ^ 1 then
      if fits_basketSideAllocGrpComp : basketSideAllocGrpComp_.length < 256 ^ 1 then
        pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, maturityMonthYear, trdType, tradeReportType, basketTradeReportText, tradeReportId, pad6, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, instrmtMatchSideGrpComp := ⟨instrmtMatchSideGrpComp_, fits_instrmtMatchSideGrpComp⟩, basketSideAllocGrpComp := ⟨basketSideAllocGrpComp_, fits_basketSideAllocGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyBasketTradeRequest) : (encode message).length ≤ 38850 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
  have bound_instrmtMatchSideGrpComp := message.instrmtMatchSideGrpComp.length_lt
  have bound_basketSideAllocGrpComp := message.basketSideAllocGrpComp.length_lt
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyOrderComplexRequest) : (encode message).length ≤ 2298 := by
...
/-- Modify Order Complex Short Request: 106 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderComplexShortRequest × List UInt8) := do
  ...
  let (executingTraderQualifier, bytes) ← decodeUInt 1 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, securityId, price, orderQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, matchInstCrossId, enrichmentRuleId, applSeqIndicator, productComplex, side, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, execInst, timeInForce, tradingCapacity, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, pad3 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderComplexShortRequest) : (encode message).length = 106 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- Modify Order Single Request: 258 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleRequest × List UInt8) := do
  ...
  let (freeText3, bytes) ← Alpha.decode 12 bytes
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, price, orderQty, stopPx, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, expireDate, marketSegmentId, simpleSecurityId, matchInstCrossId, targetPartyIdSessionId, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, applSeqIndicator, side, ordType, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, timeInForce, execInst, tradingSessionSubId, tradingCapacity, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, account, partyIdPositionAccount, positionEffect, ownershipIndicator, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, fixClOrdId }, bytes)

@[simp] theorem encode_length (message : ModifyOrderSingleRequest) : (encode message).length = 258 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length]
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleShortRequest × List UInt8) := do
  ...
  let (executingTraderQualifier, bytes) ← decodeUInt 1 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, price, orderQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, simpleSecurityId, matchInstCrossId, enrichmentRuleId, side, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, timeInForce, applSeqIndicator, execInst, tradingCapacity, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, pad4 }, bytes)
...
def decode (bytes : List UInt8) : Option (ModifyTesTradeRequest × List UInt8) := do
  ...
  if fits_sideAllocGrpComp : sideAllocGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
        pure ({ networkMsgId, pad2, requestHeaderComp, lastPx, transBkdTime, marketSegmentId, packageId, tesExecId, relatedMarketSegmentId, negotiationId, pad1, trdType, tradeReportType, tradePublishIndicator, tradeReportText, tradeReportId, pad4, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyTesTradeRequest) : (encode message).length ≤ 12346 := by
  have bound_sideAllocGrpComp := message.sideAllocGrpComp.length_lt
  have bound_trdInstrmntLegGrpComp := message.trdInstrmntLegGrpComp.length_lt
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SideAllocGrpComp.encode 24 SideAllocGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ModifyTesTradeRequest) (rest : List UInt8) :
  ...
  rw [decodeMany_bounded 1 SrqsRelatedTradeIdGrpComp.encode SrqsRelatedTradeIdGrpComp.decode SrqsRelatedTradeIdGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.sideAllocGrpComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (NewOrderComplexRequest × List UInt8) := do
  ...
  let (legOrdGrpComp_, bytes) ← decodeMany LegOrdGrpComp.decode noLegs.toNat bytes
  if fits_legOrdGrpComp : legOrdGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, securityId, price, orderQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, expireDate, matchInstCrossId, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, applSeqIndicator, productComplex, side, ordType, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, orderAttributeRiskReduction, execInst, timeInForce, tradingCapacity, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, partyIdLocationId, complianceText, custOrderHandlingInst, partyIdPositionAccount, freeText1, freeText2, freeText3, fixClOrdId, pad2v2, legOrdGrpComp := ⟨legOrdGrpComp_, fits_legOrdGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderComplexRequest) : (encode message).length ≤ 2274 := by
  have bound_legOrdGrpComp := message.legOrdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const LegOrdGrpComp.encode 8 LegOrdGrpComp.encode_length]
  omega
...
/-- New Order Complex Short Request: 98 bytes -/
...
@[simp] theorem encode_length (message : NewOrderComplexShortRequest) : (encode message).length = 98 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (NewOrderSingleRequest × List UInt8) := do
  ...
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, price, orderQty, stopPx, clOrdId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, expireDate, marketSegmentId, simpleSecurityId, matchInstCrossId, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, applSeqIndicator, side, ordType, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, orderAttributeRiskReduction, timeInForce, execInst, tradingSessionSubId, tradingCapacity, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, account, partyIdPositionAccount, positionEffect, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, fixClOrdId, pad4 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderSingleShortRequest × List UInt8) := do
  ...
  let (executingTraderQualifier, bytes) ← decodeUInt 1 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, price, orderQty, clOrdId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, simpleSecurityId, matchInstCrossId, enrichmentRuleId, side, applSeqIndicator, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, timeInForce, execInst, tradingCapacity, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, pad4 }, bytes)
...
def decode (bytes : List UInt8) : Option (PreTradeRiskLimitsDefinitionRequest × List UInt8) := do
  ...
  if fits_riskLimitQtyGrpComp : riskLimitQtyGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, riskLimitPlatform, partyDetailStatus, riskLimitGroup, partyDetailExecutingUnit, pad1, riskLimitQtyGrpComp := ⟨riskLimitQtyGrpComp_, fits_riskLimitQtyGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : PreTradeRiskLimitsDefinitionRequest) : (encode message).length ≤ 4114 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const RiskLimitQtyGrpComp.encode 16 RiskLimitQtyGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : PreTradeRiskLimitsDefinitionRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 RiskLimitQtyGrpComp.encode RiskLimitQtyGrpComp.decode RiskLimitQtyGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.riskLimitQtyGrpComp.length_lt, ↓reduceDIte]
  rfl
...
/-- Srqs Enter Quote Request: 234 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsEnterQuoteRequest × List UInt8) := do
  ...
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, bidPx, offerPx, underlyingPx, underlyingDeltaPercentage, bidSize, offerSize, marketSegmentId, negotiationId, valueCheckTypeQuantity, partyExecutingFirm, partyExecutingTrader, freeText1, freeText5, pad4 }, bytes)

@[simp] theorem encode_length (message : SrqsEnterQuoteRequest) : (encode message).length = 234 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- Srqs Hit Quote Request: 250 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsHitQuoteRequest × List UInt8) := do
  ...
  let (freeText5, bytes) ← Alpha.decode 132 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, quoteId, orderQty, validUntilTime, underlyingQty, underlyingPriceStipValue, marketSegmentId, negotiationId, side, valueCheckTypeQuantity, partyExecutingFirm, partyExecutingTrader, firmTradeId, freeText1, freeText5, pad7 }, bytes)

@[simp] theorem encode_length (message : SrqsHitQuoteRequest) : (encode message).length = 250 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequest × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, securityId, bidPx, offerPx, orderQty, quoteRefPrice, underlyingDeltaPercentage, marketSegmentId, securitySubType, quoteType, quoteSubType, numberOfRespDisclosureInstruction, side, quoteRefPriceSource, tradeUnderlying, negotiateUnderlying, productComplex, partyExecutingFirm, partyExecutingTrader, freeText5, quoteReqId, pad3, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequest) : (encode message).length ≤ 12490 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuotReqLegsGrpComp.encode 24 QuotReqLegsGrpComp.encode_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (SrqsUpdateNegotiationRequest × List UInt8) := do
  ...
  let (targetPartiesComp_, bytes) ← decodeMany TargetPartiesComp.decode noTargetPartyIDs.toNat bytes
  if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, orderQty, marketSegmentId, negotiationId, numberOfRespDisclosureInstruction, side, quoteCancelType, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, freeText5, pad4, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsUpdateNegotiationRequest) : (encode message).length ≤ 6338 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
    ...
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
              pure ({ networkMsgId, pad2, requestHeaderComp, securityId, lastPx, transBkdTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad1, trdType, productComplex, tradeReportType, tradePublishIndicator, skipValidations, trdRptStatus, tradePlatform, hedgeType, partyIdSettlementLocation, tradeReportId, tradeReportText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad4, sideAllocExtGrpComp := ⟨sideAllocExtGrpComp_, fits_sideAllocExtGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
            else none
          else none
        else none
      else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UploadTesTradeRequest) : (encode message).length ≤ 73674 := by
  ...
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SideAllocExtGrpComp.encode 176 SideAllocExtGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
theorem encodeBody_length_lt (message : ClientMessage) : (encodeBody message).length + 4 < 256 ^ 4 := by
  ...
  | srqsHitQuoteRequest inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUIntLE_length, SrqsHitQuoteRequest.encode_length]
    omega
...
end Omi.EurexT7EtiFbeV71Client
