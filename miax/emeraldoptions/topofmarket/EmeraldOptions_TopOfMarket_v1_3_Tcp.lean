...
/-!
# Miami International Holdings Top of Market v1.3
...
namespace Omi.MiaxEmeraldoptionsTopofmarketMachV13Tcp
...
/-- Ebbo Condition: one byte code -/
def EbboCondition.codes : List UInt8 :=
  [0x41, 0x42, 0x43, 0x52, 0x54]

inductive EbboCondition where
  | regular -- Regular
  | publicCustomerInterest -- Public Customer Interest
  | notFirm -- Not Firm
  | reserved -- Reserved
  | tradingHalt -- Trading Halt
  | unlisted (byte : { byte : UInt8 // byte ∉ EbboCondition.codes }) -- any other code, kept as it is
  deriving DecidableEq, Repr

namespace EbboCondition

def toByte : EbboCondition → UInt8
  | .regular => 0x41
...
/-- The constructor of a listed code -/
def listed (byte : UInt8) : EbboCondition :=
...
def ofByte (byte : UInt8) : EbboCondition :=
  if known : byte ∈ codes then listed byte else .unlisted ⟨byte, known⟩

theorem ofByte_toByte (value : EbboCondition) : ofByte value.toByte = value := by
  cases value with
  | regular => decide
...
def encode (value : EbboCondition) : List UInt8 :=
  [value.toByte]

def decode : List UInt8 → Option (EbboCondition × List UInt8)
  | byte :: rest => some (ofByte byte, rest)
  | [] => none

@[simp] theorem encode_length (value : EbboCondition) : (encode value).length = 1 :=
  rfl

@[simp] theorem decode_encode (value : EbboCondition) (rest : List UInt8) :
    decode (encode value ++ rest) = some (value, rest) := by
  simp [decode, encode, ofByte_toByte]
...
def TradeCondition.codes : List UInt8 :=
  [0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4A, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50, 0x51, 0x52, 0x53, 0x54, 0x58, 0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6A, 0x6B, 0x6C, 0x6D, 0x6E, 0x6F, 0x70, 0x71, 0x72, 0x73, 0x74]
...
def listed (byte : UInt8) : TradeCondition :=
  if byte = 0x41 then .cancelOfTradePreviouslyReportedOtherThanAsTheLastOrOpening
...
structure SimpleTopOfMarketBestBidOrOfferCompactBidMessage where
  nanoseconds : BitVec 32
  productId : BitVec 32
  ebboPriceShort : BitVec 16
  ebboSizeShort : BitVec 16
  ebboPriorityCustomerSizeShort : BitVec 16
  ebboCondition : EbboCondition
  deriving DecidableEq, Repr
...
def encode (message : SimpleTopOfMarketBestBidOrOfferCompactBidMessage) : List UInt8 :=
  encodeUIntLE 4 message.nanoseconds
    ++ (encodeUIntLE 4 message.productId
    ++ (encodeUIntLE 2 message.ebboPriceShort
    ++ (encodeUIntLE 2 message.ebboSizeShort
    ++ (encodeUIntLE 2 message.ebboPriorityCustomerSizeShort
    ++ (EbboCondition.encode message.ebboCondition)))))

def decode (bytes : List UInt8) : Option (SimpleTopOfMarketBestBidOrOfferCompactBidMessage × List UInt8) := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriceShort, bytes) ← decodeUIntLE 2 bytes
  let (ebboSizeShort, bytes) ← decodeUIntLE 2 bytes
  let (ebboPriorityCustomerSizeShort, bytes) ← decodeUIntLE 2 bytes
  let (ebboCondition, bytes) ← EbboCondition.decode bytes
  pure ({ nanoseconds, productId, ebboPriceShort, ebboSizeShort, ebboPriorityCustomerSizeShort, ebboCondition }, bytes)

@[simp] theorem encode_length (message : SimpleTopOfMarketBestBidOrOfferCompactBidMessage) : (encode message).length = 15 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, EbboCondition.encode_length]
...
@[simp] theorem decode_encode (message : SimpleTopOfMarketBestBidOrOfferCompactBidMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [EbboCondition.decode_encode, some_bind]
  rfl

end SimpleTopOfMarketBestBidOrOfferCompactBidMessage

/-- Simple Top Of Market Best Bid Or Offer Compact Bid With Priority Customer Message: 15 bytes -/
structure SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage where
  nanoseconds : BitVec 32
  productId : BitVec 32
  ebboPriceShort : BitVec 16
  ebboSizeShort : BitVec 16
...
namespace SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage

def encode (message : SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage) : List UInt8 :=
  encodeUIntLE 4 message.nanoseconds
    ++ (encodeUIntLE 4 message.productId
    ++ (encodeUIntLE 2 message.ebboPriceShort
    ++ (encodeUIntLE 2 message.ebboSizeShort
...
def decode (bytes : List UInt8) : Option (SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage × List UInt8) := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriceShort, bytes) ← decodeUIntLE 2 bytes
  let (ebboSizeShort, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem encode_length (message : SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage) : (encode message).length = 15 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, EbboCondition.encode_length]

theorem encode_length_pos (message : SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage
...
/-- Simple Top Of Market Best Bid Or Offer Compact Offer With Priority Customer Message: 15 bytes -/
structure SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage where
  nanoseconds : BitVec 32
  productId : BitVec 32
  ebboPriceShort : BitVec 16
  ebboSizeShort : BitVec 16
...
namespace SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage

def encode (message : SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage) : List UInt8 :=
  encodeUIntLE 4 message.nanoseconds
    ++ (encodeUIntLE 4 message.productId
    ++ (encodeUIntLE 2 message.ebboPriceShort
    ++ (encodeUIntLE 2 message.ebboSizeShort
...
def decode (bytes : List UInt8) : Option (SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage × List UInt8) := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriceShort, bytes) ← decodeUIntLE 2 bytes
  let (ebboSizeShort, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem encode_length (message : SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage) : (encode message).length = 15 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, EbboCondition.encode_length]

theorem encode_length_pos (message : SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage
...
structure SimpleTopOfMarketBestBidOrOfferWideBidMessage where
  nanoseconds : BitVec 32
  productId : BitVec 32
  ebboPriceLong : BitVec 32
  ebboSizeLong : BitVec 32
  ebboPriorityCustomerSizeLong : BitVec 32
  ebboCondition : EbboCondition
  deriving DecidableEq, Repr
...
def encode (message : SimpleTopOfMarketBestBidOrOfferWideBidMessage) : List UInt8 :=
  encodeUIntLE 4 message.nanoseconds
    ++ (encodeUIntLE 4 message.productId
    ++ (encodeUIntLE 4 message.ebboPriceLong
    ++ (encodeUIntLE 4 message.ebboSizeLong
    ++ (encodeUIntLE 4 message.ebboPriorityCustomerSizeLong
    ++ (EbboCondition.encode message.ebboCondition)))))

def decode (bytes : List UInt8) : Option (SimpleTopOfMarketBestBidOrOfferWideBidMessage × List UInt8) := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriceLong, bytes) ← decodeUIntLE 4 bytes
  let (ebboSizeLong, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriorityCustomerSizeLong, bytes) ← decodeUIntLE 4 bytes
  let (ebboCondition, bytes) ← EbboCondition.decode bytes
  pure ({ nanoseconds, productId, ebboPriceLong, ebboSizeLong, ebboPriorityCustomerSizeLong, ebboCondition }, bytes)
...
/-- Simple Top Of Market Best Bid Or Offer Wide Bid With Priority Customer Message: 21 bytes -/
structure SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage where
  nanoseconds : BitVec 32
  productId : BitVec 32
  ebboPriceLong : BitVec 32
  ebboSizeLong : BitVec 32
...
namespace SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage

def encode (message : SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage) : List UInt8 :=
  encodeUIntLE 4 message.nanoseconds
    ++ (encodeUIntLE 4 message.productId
    ++ (encodeUIntLE 4 message.ebboPriceLong
    ++ (encodeUIntLE 4 message.ebboSizeLong
...
def decode (bytes : List UInt8) : Option (SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage × List UInt8) := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriceLong, bytes) ← decodeUIntLE 4 bytes
  let (ebboSizeLong, bytes) ← decodeUIntLE 4 bytes
...
@[simp] theorem encode_length (message : SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage) : (encode message).length = 21 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, EbboCondition.encode_length]

theorem encode_length_pos (message : SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage
...
/-- Simple Top Of Market Best Bid Or Offer Wide Offer With Priority Customer Message: 21 bytes -/
structure SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage where
  nanoseconds : BitVec 32
  productId : BitVec 32
  ebboPriceLong : BitVec 32
  ebboSizeLong : BitVec 32
...
namespace SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage

def encode (message : SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage) : List UInt8 :=
  encodeUIntLE 4 message.nanoseconds
    ++ (encodeUIntLE 4 message.productId
    ++ (encodeUIntLE 4 message.ebboPriceLong
    ++ (encodeUIntLE 4 message.ebboSizeLong
...
def decode (bytes : List UInt8) : Option (SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage × List UInt8) := do
  let (nanoseconds, bytes) ← decodeUIntLE 4 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  let (ebboPriceLong, bytes) ← decodeUIntLE 4 bytes
  let (ebboSizeLong, bytes) ← decodeUIntLE 4 bytes
...
@[simp] theorem encode_length (message : SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage) : (encode message).length = 21 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, EbboCondition.encode_length]

theorem encode_length_pos (message : SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage
...
inductive Data where
  ...
  | simpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage (message : SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage) -- "h" 0x68
  | simpleTopOfMarketBestBidOrOfferCompactOfferMessage (message : SimpleTopOfMarketBestBidOrOfferCompactOfferMessage) -- "O" 0x4F
  | simpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage (message : SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage) -- "i" 0x69
  | simpleTopOfMarketBestBidOrOfferWideBidMessage (message : SimpleTopOfMarketBestBidOrOfferWideBidMessage) -- "W" 0x57
  | simpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage (message : SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage) -- "j" 0x6A
  | simpleTopOfMarketBestBidOrOfferWideOfferMessage (message : SimpleTopOfMarketBestBidOrOfferWideOfferMessage) -- "A" 0x41
  | simpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage (message : SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage) -- "k" 0x6B
...
def tag : Data → BitVec 8
  ...
  | .simpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage _ => 104
  | .simpleTopOfMarketBestBidOrOfferCompactOfferMessage _ => 79
  | .simpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage _ => 105
  | .simpleTopOfMarketBestBidOrOfferWideBidMessage _ => 87
  | .simpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage _ => 106
  | .simpleTopOfMarketBestBidOrOfferWideOfferMessage _ => 65
  | .simpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage _ => 107
...
def encode : Data → List UInt8
  ...
  | .simpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage message => SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage.encode message
  ...
  | .simpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage message => SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage.encode message
  | .simpleTopOfMarketBestBidOrOfferWideBidMessage message => SimpleTopOfMarketBestBidOrOfferWideBidMessage.encode message
  | .simpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage message => SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage.encode message
  | .simpleTopOfMarketBestBidOrOfferWideOfferMessage message => SimpleTopOfMarketBestBidOrOfferWideOfferMessage.encode message
  | .simpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage message => SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage.encode message
...
theorem encode_length_le (message : Data) : (encode message).length ≤ 72 := by
  ...
  | simpleTopOfMarketBestBidOrOfferCompactBidMessage inner =>
    ...
    omega
  | simpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage inner =>
    simp only [encode, SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferCompactOfferMessage inner =>
    ...
    omega
  | simpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage inner =>
    simp only [encode, SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferWideBidMessage inner =>
    ...
    omega
  | simpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage inner =>
    simp only [encode, SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferWideOfferMessage inner =>
    ...
    omega
  | simpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage inner =>
    simp only [encode, SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage.encode_length]
    omega
...
def decode (tag : BitVec 8) (bytes : List UInt8) : Option (Data × List UInt8) :=
  ...
  else if tag = 104 then (SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage.decode bytes).map fun (message, rest) => (.simpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage message, rest)
  ...
  else if tag = 105 then (SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage.decode bytes).map fun (message, rest) => (.simpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage message, rest)
  else if tag = 87 then (SimpleTopOfMarketBestBidOrOfferWideBidMessage.decode bytes).map fun (message, rest) => (.simpleTopOfMarketBestBidOrOfferWideBidMessage message, rest)
  else if tag = 106 then (SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage.decode bytes).map fun (message, rest) => (.simpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage message, rest)
  else if tag = 65 then (SimpleTopOfMarketBestBidOrOfferWideOfferMessage.decode bytes).map fun (message, rest) => (.simpleTopOfMarketBestBidOrOfferWideOfferMessage message, rest)
  else if tag = 107 then (SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage.decode bytes).map fun (message, rest) => (.simpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage message, rest)
...
theorem encode_length_le (message : ApplicationMessage) : (encode message).length ≤ 73 := by
  ...
  | simpleTopOfMarketBestBidOrOfferCompactBidMessage inner =>
    ...
    omega
  | simpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage inner =>
    simp only [Data.encode, List.length_append, encodeUInt_length, SimpleTopOfMarketBestBidOrOfferCompactBidWithPriorityCustomerMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferCompactOfferMessage inner =>
    ...
    omega
  | simpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage inner =>
    simp only [Data.encode, List.length_append, encodeUInt_length, SimpleTopOfMarketBestBidOrOfferCompactOfferWithPriorityCustomerMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferWideBidMessage inner =>
    simp only [Data.encode, List.length_append, encodeUInt_length, SimpleTopOfMarketBestBidOrOfferWideBidMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage inner =>
    simp only [Data.encode, List.length_append, encodeUInt_length, SimpleTopOfMarketBestBidOrOfferWideBidWithPriorityCustomerMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferWideOfferMessage inner =>
    simp only [Data.encode, List.length_append, encodeUInt_length, SimpleTopOfMarketBestBidOrOfferWideOfferMessage.encode_length]
    omega
  | simpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage inner =>
    simp only [Data.encode, List.length_append, encodeUInt_length, SimpleTopOfMarketBestBidOrOfferWideOfferWithPriorityCustomerMessage.encode_length]
    omega
  | simpleDoubleSidedTopOfMarketBestBidOrOfferCompactMessage inner =>
...
end Omi.MiaxEmeraldoptionsTopofmarketMachV13Tcp
