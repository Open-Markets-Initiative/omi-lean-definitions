...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v12.0
...
namespace Omi.EurexT7XtiFbeV120Server
...
/-- Extended Deletion Report: 330 bytes -/
...
def decode (bytes : List UInt8) : Option (ExtendedDeletionReport × List UInt8) := do
  ...
  let (pad2v2, bytes) ← Alpha.decode 2 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, price, leavesQty, cumQty, cxlQty, orderQty, displayQty, displayLowQty, displayHighQty, stopPx, pegOffsetValueAbs, pegOffsetValuePct, quoteId, marketSegmentId, orderIdSfx, expireDate, matchInstCrossId, partyIdExecutingUnit, partyIdSessionId, partyIdExecutingTrader, partyIdEnteringTrader, execRestatementReason, ordStatus, execType, side, ordType, tradingCapacity, timeInForce, execInst, tradingSessionSubId, applSeqIndicator, freeText1, freeText2, freeText4, partyEnteringFirm, partyEnteringTrader, partyExecutingFirm, partyExecutingTrader, fixClOrdId, triggered, pad2v2 }, bytes)

@[simp] theorem encode_length (message : ExtendedDeletionReport) : (encode message).length = 330 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (OrderExecReportBroadcast × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    if fits_orderEventGrpComp : orderEventGrpComp_.length < 256 ^ 1 then
      pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, price, leavesQty, cumQty, cxlQty, orderQty, displayQty, displayLowQty, displayHighQty, stopPx, pegOffsetValueAbs, pegOffsetValuePct, quoteId, marketSegmentId, orderIdSfx, expireDate, matchInstCrossId, partyIdExecutingUnit, partyIdSessionId, partyIdExecutingTrader, partyIdEnteringTrader, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, orderEventType, matchType, side, ordType, tradingCapacity, timeInForce, execInst, tradingSessionSubId, applSeqIndicator, partyEnteringFirm, partyEnteringTrader, partyExecutingFirm, partyExecutingTrader, freeText1, freeText2, freeText4, fixClOrdId, triggered, crossedIndicator, tradeAtCloseOptIn, bboSetting, pad2v2, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩, orderEventGrpComp := ⟨orderEventGrpComp_, fits_orderEventGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 14626 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  have bound_orderEventGrpComp := message.orderEventGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 32 FillsGrpComp.encode_length, encodeMany_length_const OrderEventGrpComp.encode 24 OrderEventGrpComp.encode_length]
...
end Omi.EurexT7XtiFbeV120Server
