...
namespace Omi.CmeGlobexMdp3SbeV19Udp
...
inductive MdEntryType where
  ...
  | thresholdLimitsAndPriceBandVariation -- Threshold Limits And Price Band Variation
  deriving DecidableEq, Repr
...
structure MdInstrumentDefinitionFutureLegacy where
  matchEventIndicator : BitVec 8
...
namespace MdInstrumentDefinitionFutureLegacy

def encode (message : MdInstrumentDefinitionFutureLegacy) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionFutureLegacy × List UInt8) := do
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
...
end MdInstrumentDefinitionFutureLegacy
...
structure MdInstrumentDefinitionSpreadLegacy where
  matchEventIndicator : BitVec 8
  totNumReportsOptional : BitVec 32
  securityUpdateAction : SecurityUpdateAction
  lastUpdateTime : BitVec 64
...
namespace MdInstrumentDefinitionSpreadLegacy

def encode (message : MdInstrumentDefinitionSpreadLegacy) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
    ++ encodeUIntLE 4 message.totNumReportsOptional
    ++ SecurityUpdateAction.encode message.securityUpdateAction
    ++ encodeUIntLE 8 message.lastUpdateTime
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionSpreadLegacy × List UInt8) := do
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (totNumReportsOptional, bytes) ← decodeUIntLE 4 bytes
  let (securityUpdateAction, bytes) ← SecurityUpdateAction.decode bytes
  let (lastUpdateTime, bytes) ← decodeUIntLE 8 bytes
...
@[simp] theorem decode_encode (message : MdInstrumentDefinitionSpreadLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  have fits_eventsGroup : message.eventsGroup.val.length < 256 := by simpa using message.eventsGroup.length_lt
  have fits_feedTypesGroup : message.feedTypesGroup.val.length < 256 := by simpa using message.feedTypesGroup.length_lt
  have fits_instAttribGroup : message.instAttribGroup.val.length < 256 := by simpa using message.instAttribGroup.length_lt
...
end MdInstrumentDefinitionSpreadLegacy
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshBookGroup × List UInt8) := do
  ...
  pure ({ mdEntryPxOptionalEx, mdEntrySizeShortOptional, securityId, rptSeq, numberOfOrdersOptional, mdPriceLevel, mdUpdateAction, mdEntryTypeBook, padding5 }, bytes)
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshBookOrderIdGroup × List UInt8) := do
  ...
  pure ({ orderId, mdOrderPriority, mdDisplayQtyOptional, referenceId, orderUpdateAction, padding2 }, bytes)
...
structure MdIncrementalRefreshBookLegacy where
  transactTime : BitVec 64
  matchEventIndicator : BitVec 8
  padding2 : Alpha 2
  blockLength : BitVec 16
...
namespace MdIncrementalRefreshBookLegacy

def encode (message : MdIncrementalRefreshBookLegacy) : List UInt8 :=
  encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 1 message.matchEventIndicator
    ++ Alpha.encode message.padding2
    ++ encodeUIntLE 2 message.blockLength
...
def decode (bytes : List UInt8) : Option (MdIncrementalRefreshBookLegacy × List UInt8) := do
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (padding2, bytes) ← Alpha.decode 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshBookLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end MdIncrementalRefreshBookLegacy
...
structure MdIncrementalRefreshDailyStatisticsLegacy where
  transactTime : BitVec 64
  matchEventIndicator : BitVec 8
  padding2 : Alpha 2
  blockLength : BitVec 16
...
namespace MdIncrementalRefreshDailyStatisticsLegacy

def encode (message : MdIncrementalRefreshDailyStatisticsLegacy) : List UInt8 :=
  encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 1 message.matchEventIndicator
    ++ Alpha.encode message.padding2
    ++ encodeUIntLE 2 message.blockLength
...
def decode (bytes : List UInt8) : Option (MdIncrementalRefreshDailyStatisticsLegacy × List UInt8) := do
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (padding2, bytes) ← Alpha.decode 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshDailyStatisticsLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end MdIncrementalRefreshDailyStatisticsLegacy

/-- Incremental Refresh Limits Banding Group: 32 bytes -/
structure IncrementalRefreshLimitsBandingGroup where
  highLimitPriceEx : BitVec 64
  lowLimitPriceEx : BitVec 64
  maxPriceVariationEx : BitVec 64
  securityId : BitVec 32
  rptSeq : BitVec 32
  deriving DecidableEq, Repr
...
def encode (message : IncrementalRefreshLimitsBandingGroup) : List UInt8 :=
  encodeUIntLE 8 message.highLimitPriceEx
    ++ encodeUIntLE 8 message.lowLimitPriceEx
    ++ encodeUIntLE 8 message.maxPriceVariationEx
    ++ encodeUIntLE 4 message.securityId
    ++ encodeUIntLE 4 message.rptSeq

def decode (bytes : List UInt8) : Option (IncrementalRefreshLimitsBandingGroup × List UInt8) := do
  let (highLimitPriceEx, bytes) ← decodeUIntLE 8 bytes
  let (lowLimitPriceEx, bytes) ← decodeUIntLE 8 bytes
  let (maxPriceVariationEx, bytes) ← decodeUIntLE 8 bytes
  let (securityId, bytes) ← decodeUIntLE 4 bytes
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  pure ({ highLimitPriceEx, lowLimitPriceEx, maxPriceVariationEx, securityId, rptSeq }, bytes)
...
structure MdIncrementalRefreshLimitsBandingLegacy where
  transactTime : BitVec 64
  matchEventIndicator : BitVec 8
  padding2 : Alpha 2
  blockLength : BitVec 16
...
namespace MdIncrementalRefreshLimitsBandingLegacy

def encode (message : MdIncrementalRefreshLimitsBandingLegacy) : List UInt8 :=
  encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 1 message.matchEventIndicator
    ++ Alpha.encode message.padding2
    ++ encodeUIntLE 2 message.blockLength
...
def decode (bytes : List UInt8) : Option (MdIncrementalRefreshLimitsBandingLegacy × List UInt8) := do
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (padding2, bytes) ← Alpha.decode 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshLimitsBandingLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end MdIncrementalRefreshLimitsBandingLegacy
...
structure MdIncrementalRefreshSessionStatisticsLegacy where
  transactTime : BitVec 64
  matchEventIndicator : BitVec 8
  padding2 : Alpha 2
  blockLength : BitVec 16
...
namespace MdIncrementalRefreshSessionStatisticsLegacy

def encode (message : MdIncrementalRefreshSessionStatisticsLegacy) : List UInt8 :=
  encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 1 message.matchEventIndicator
    ++ Alpha.encode message.padding2
    ++ encodeUIntLE 2 message.blockLength
...
def decode (bytes : List UInt8) : Option (MdIncrementalRefreshSessionStatisticsLegacy × List UInt8) := do
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (padding2, bytes) ← Alpha.decode 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshSessionStatisticsLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end MdIncrementalRefreshSessionStatisticsLegacy
...
structure SnapshotFullRefreshLegacy where
...
namespace SnapshotFullRefreshLegacy

def encode (message : SnapshotFullRefreshLegacy) : List UInt8 :=
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefreshLegacy × List UInt8) := do
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
structure MdInstrumentDefinitionOptionLegacy where
  matchEventIndicator : BitVec 8
  totNumReportsOptional : BitVec 32
  securityUpdateAction : SecurityUpdateAction
  lastUpdateTime : BitVec 64
...
namespace MdInstrumentDefinitionOptionLegacy

def encode (message : MdInstrumentDefinitionOptionLegacy) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
    ++ encodeUIntLE 4 message.totNumReportsOptional
    ++ SecurityUpdateAction.encode message.securityUpdateAction
    ++ encodeUIntLE 8 message.lastUpdateTime
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionOptionLegacy × List UInt8) := do
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (totNumReportsOptional, bytes) ← decodeUIntLE 4 bytes
  let (securityUpdateAction, bytes) ← SecurityUpdateAction.decode bytes
  let (lastUpdateTime, bytes) ← decodeUIntLE 8 bytes
...
@[simp] theorem decode_encode (message : MdInstrumentDefinitionOptionLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  have fits_eventsGroup : message.eventsGroup.val.length < 256 := by simpa using message.eventsGroup.length_lt
  have fits_feedTypesGroup : message.feedTypesGroup.val.length < 256 := by simpa using message.feedTypesGroup.length_lt
  have fits_instAttribGroup : message.instAttribGroup.val.length < 256 := by simpa using message.instAttribGroup.length_lt
...
end MdInstrumentDefinitionOptionLegacy
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshTradeSummaryGroup × List UInt8) := do
  ...
  let (padding2, bytes) ← Alpha.decode 2 bytes
  pure ({ mdEntryPxEx, mdEntrySizeShort, securityId, rptSeq, numberOfOrders, aggressorSide, mdUpdateAction, mdTradeEntryId, padding2 }, bytes)
...
structure MdIncrementalRefreshTradeSummaryLegacy where
  transactTime : BitVec 64
  matchEventIndicator : BitVec 8
  padding2 : Alpha 2
  blockLength : BitVec 16
...
namespace MdIncrementalRefreshTradeSummaryLegacy

def encode (message : MdIncrementalRefreshTradeSummaryLegacy) : List UInt8 :=
  encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 1 message.matchEventIndicator
    ++ Alpha.encode message.padding2
    ++ encodeUIntLE 2 message.blockLength
...
def decode (bytes : List UInt8) : Option (MdIncrementalRefreshTradeSummaryLegacy × List UInt8) := do
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (padding2, bytes) ← Alpha.decode 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshTradeSummaryLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end MdIncrementalRefreshTradeSummaryLegacy
...
def decode (bytes : List UInt8) : Option (IncrementalRefreshOrderBookGroup × List UInt8) := do
  ...
  pure ({ orderIdOptional, mdOrderPriority, mdEntryPxOptionalEx, mdDisplayQtyOptional, securityId, mdUpdateAction, mdEntryTypeBook, padding6 }, bytes)
...
structure MdIncrementalRefreshOrderBookLegacy where
  transactTime : BitVec 64
  matchEventIndicator : BitVec 8
  padding2 : Alpha 2
  blockLength : BitVec 16
...
namespace MdIncrementalRefreshOrderBookLegacy

def encode (message : MdIncrementalRefreshOrderBookLegacy) : List UInt8 :=
  encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 1 message.matchEventIndicator
    ++ Alpha.encode message.padding2
    ++ encodeUIntLE 2 message.blockLength
...
def decode (bytes : List UInt8) : Option (MdIncrementalRefreshOrderBookLegacy × List UInt8) := do
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  let (padding2, bytes) ← Alpha.decode 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshOrderBookLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end MdIncrementalRefreshOrderBookLegacy
...
structure SnapshotFullRefreshOrderBookLegacy where
  lastMsgSeqNumProcessed : BitVec 32
  totNumReports : BitVec 32
  securityId : BitVec 32
  noChunks : BitVec 32
...
namespace SnapshotFullRefreshOrderBookLegacy

def encode (message : SnapshotFullRefreshOrderBookLegacy) : List UInt8 :=
  encodeUIntLE 4 message.lastMsgSeqNumProcessed
    ++ encodeUIntLE 4 message.totNumReports
    ++ encodeUIntLE 4 message.securityId
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefreshOrderBookLegacy × List UInt8) := do
  let (lastMsgSeqNumProcessed, bytes) ← decodeUIntLE 4 bytes
  let (totNumReports, bytes) ← decodeUIntLE 4 bytes
  let (securityId, bytes) ← decodeUIntLE 4 bytes
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshOrderBookLegacy) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
...
end SnapshotFullRefreshOrderBookLegacy
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefresh × List UInt8) := do
  ...
  let (snapshotFullRefreshGroup_, bytes) ← decodeMany SnapshotFullRefreshGroup.decode numInGroup.toNat bytes
  if fits_snapshotFullRefreshGroup : snapshotFullRefreshGroup_.length < 256 then
    pure ({ lastMsgSeqNumProcessed, totNumReports, securityId, rptSeq, transactTime, lastUpdateTime, tradeDate, mdSecurityTradingStatus, highLimitPriceEx, lowLimitPriceEx, maxPriceVariationEx, blockLength, snapshotFullRefreshGroup := ⟨snapshotFullRefreshGroup_, fits_snapshotFullRefreshGroup⟩ }, bytes)
  else none
...
structure MdInstrumentDefinitionFuture where
  ...
  displayFactorEx : BitVec 64
  mainFraction : BitVec 8
  subFraction : BitVec 8
  priceDisplayFormat : BitVec 8
  unitOfMeasure : Alpha 30
  unitOfMeasureQtyEx : BitVec 64
  tradingReferencePriceEx : BitVec 64
  settlPriceType : BitVec 8
  openInterestQty : BitVec 32
  clearedVolume : BitVec 32
  highLimitPriceEx : BitVec 64
  ...
  contractMultiplierUnit : BitVec 8
  flowScheduleType : BitVec 8
  minPriceIncrementAmountEx : BitVec 64
  userDefinedInstrument : Alpha 1
  tradingReferenceDate : BitVec 16
  blockLength : BitVec 16
  eventsGroup : Bounded 1 EventsGroup
...
def encode (message : MdInstrumentDefinitionFuture) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
    ...
    ++ encodeUIntLE 8 message.displayFactorEx
    ++ encodeUInt 1 message.mainFraction
    ++ encodeUInt 1 message.subFraction
    ++ encodeUInt 1 message.priceDisplayFormat
    ++ Alpha.encode message.unitOfMeasure
    ++ encodeUIntLE 8 message.unitOfMeasureQtyEx
    ++ encodeUIntLE 8 message.tradingReferencePriceEx
    ++ encodeUIntLE 1 message.settlPriceType
    ++ encodeUIntLE 4 message.openInterestQty
    ++ encodeUIntLE 4 message.clearedVolume
    ++ encodeUIntLE 8 message.highLimitPriceEx
    ...
    ++ encodeUInt 1 message.contractMultiplierUnit
    ++ encodeUInt 1 message.flowScheduleType
    ++ encodeUIntLE 8 message.minPriceIncrementAmountEx
    ++ Alpha.encode message.userDefinedInstrument
    ++ encodeUIntLE 2 message.tradingReferenceDate
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.eventsGroup.val.length)
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionFuture × List UInt8) := do
  ...
  let (displayFactorEx, bytes) ← decodeUIntLE 8 bytes
  let (mainFraction, bytes) ← decodeUInt 1 bytes
  let (subFraction, bytes) ← decodeUInt 1 bytes
  let (priceDisplayFormat, bytes) ← decodeUInt 1 bytes
  let (unitOfMeasure, bytes) ← Alpha.decode 30 bytes
  let (unitOfMeasureQtyEx, bytes) ← decodeUIntLE 8 bytes
  let (tradingReferencePriceEx, bytes) ← decodeUIntLE 8 bytes
  let (settlPriceType, bytes) ← decodeUIntLE 1 bytes
  let (openInterestQty, bytes) ← decodeUIntLE 4 bytes
  let (clearedVolume, bytes) ← decodeUIntLE 4 bytes
  let (highLimitPriceEx, bytes) ← decodeUIntLE 8 bytes
  ...
  let (contractMultiplierUnit, bytes) ← decodeUInt 1 bytes
  let (flowScheduleType, bytes) ← decodeUInt 1 bytes
  let (minPriceIncrementAmountEx, bytes) ← decodeUIntLE 8 bytes
  let (userDefinedInstrument, bytes) ← Alpha.decode 1 bytes
  let (tradingReferenceDate, bytes) ← decodeUIntLE 2 bytes
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
  let (numInGroup, bytes) ← decodeUInt 1 bytes
  ...
  if fits_eventsGroup : eventsGroup_.length < 256 then
    if fits_feedTypesGroup : feedTypesGroup_.length < 256 then
      if fits_instAttribGroup : instAttribGroup_.length < 256 then
        if fits_lotTypeRulesGroup : lotTypeRulesGroup_.length < 256 then
          pure ({ matchEventIndicator, totNumReportsOptional, securityUpdateAction, lastUpdateTime, mdSecurityTradingStatus, applId, marketSegmentId, underlyingProduct, securityExchange, securityGroup, asset, symbol, securityId, securityType, cfiCode, maturityMonthYear, currency, settlCurrency, matchAlgorithm, minTradeVol, maxTradeVol, minPriceIncrementEx, displayFactorEx, mainFraction, subFraction, priceDisplayFormat, unitOfMeasure, unitOfMeasureQtyEx, tradingReferencePriceEx, settlPriceType, openInterestQty, clearedVolume, highLimitPriceEx, lowLimitPriceEx, maxPriceVariationEx, decayQuantity, decayStartDate, originalContractSize, contractMultiplier, contractMultiplierUnit, flowScheduleType, minPriceIncrementAmountEx, userDefinedInstrument, tradingReferenceDate, blockLength, eventsGroup := ⟨eventsGroup_, fits_eventsGroup⟩, blockLength, feedTypesGroup := ⟨feedTypesGroup_, fits_feedTypesGroup⟩, blockLength, instAttribGroup := ⟨instAttribGroup_, fits_instAttribGroup⟩, blockLength, lotTypeRulesGroup := ⟨lotTypeRulesGroup_, fits_lotTypeRulesGroup⟩ }, bytes)
        else none
      else none
    else none
  else none
...
structure MdInstrumentDefinitionOption where
  ...
  maturityMonthYear : MaturityMonthYear
  currency : Alpha 3
  strikePriceEx : BitVec 64
  strikeCurrency : Alpha 3
  settlCurrency : Alpha 3
  minCabPriceEx : BitVec 64
  matchAlgorithm : Alpha 1
  minTradeVol : BitVec 32
  maxTradeVol : BitVec 32
...
def encode (message : MdInstrumentDefinitionOption) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
    ...
    ++ MaturityMonthYear.encode message.maturityMonthYear
    ++ Alpha.encode message.currency
    ++ encodeUIntLE 8 message.strikePriceEx
    ++ Alpha.encode message.strikeCurrency
    ++ Alpha.encode message.settlCurrency
    ++ encodeUIntLE 8 message.minCabPriceEx
    ++ Alpha.encode message.matchAlgorithm
    ++ encodeUIntLE 4 message.minTradeVol
    ++ encodeUIntLE 4 message.maxTradeVol
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionOption × List UInt8) := do
  ...
  let (maturityMonthYear, bytes) ← MaturityMonthYear.decode bytes
  let (currency, bytes) ← Alpha.decode 3 bytes
  let (strikePriceEx, bytes) ← decodeUIntLE 8 bytes
  let (strikeCurrency, bytes) ← Alpha.decode 3 bytes
  let (settlCurrency, bytes) ← Alpha.decode 3 bytes
  let (minCabPriceEx, bytes) ← decodeUIntLE 8 bytes
  let (matchAlgorithm, bytes) ← Alpha.decode 1 bytes
  let (minTradeVol, bytes) ← decodeUIntLE 4 bytes
  let (maxTradeVol, bytes) ← decodeUIntLE 4 bytes
    ...
    if fits_feedTypesGroup : feedTypesGroup_.length < 256 then
      if fits_instAttribGroup : instAttribGroup_.length < 256 then
        if fits_lotTypeRulesGroup : lotTypeRulesGroup_.length < 256 then
          if fits_optionUnderlyingsGroup : optionUnderlyingsGroup_.length < 256 then
            if fits_optionRelatedInstrumentsGroup : optionRelatedInstrumentsGroup_.length < 256 then
              pure ({ matchEventIndicator, totNumReportsOptional, securityUpdateAction, lastUpdateTime, mdSecurityTradingStatus, applId, marketSegmentId, underlyingProduct, securityExchange, securityGroup, asset, symbol, securityId, securityType, cfiCode, putOrCall, maturityMonthYear, currency, strikePriceEx, strikeCurrency, settlCurrency, minCabPriceEx, matchAlgorithm, minTradeVol, maxTradeVol, minPriceIncrementOptionalEx, minPriceIncrementAmountEx, displayFactorEx, tickRule, mainFraction, subFraction, priceDisplayFormat, unitOfMeasure, unitOfMeasureQtyEx, tradingReferencePriceEx, settlPriceType, clearedVolume, openInterestQty, lowLimitPriceEx, highLimitPriceEx, userDefinedInstrument, tradingReferenceDate, blockLength, eventsGroup := ⟨eventsGroup_, fits_eventsGroup⟩, blockLength, feedTypesGroup := ⟨feedTypesGroup_, fits_feedTypesGroup⟩, blockLength, instAttribGroup := ⟨instAttribGroup_, fits_instAttribGroup⟩, blockLength, lotTypeRulesGroup := ⟨lotTypeRulesGroup_, fits_lotTypeRulesGroup⟩, blockLength, optionUnderlyingsGroup := ⟨optionUnderlyingsGroup_, fits_optionUnderlyingsGroup⟩, blockLength, optionRelatedInstrumentsGroup := ⟨optionRelatedInstrumentsGroup_, fits_optionRelatedInstrumentsGroup⟩ }, bytes)
            else none
          else none
        else none
      else none
...
def encode (message : LegsGroup) : List UInt8 :=
  encodeUIntLE 4 message.legSecurityId
    ++ encodeUInt 1 message.legSide
    ++ encodeUInt 1 message.legRatioQty
    ++ encodeUIntLE 8 message.legPriceEx
    ++ encodeUIntLE 4 message.legOptionDelta
...
def decode (bytes : List UInt8) : Option (LegsGroup × List UInt8) := do
  ...
  let (legSide, bytes) ← decodeUInt 1 bytes
  let (legRatioQty, bytes) ← decodeUInt 1 bytes
  let (legPriceEx, bytes) ← decodeUIntLE 8 bytes
  let (legOptionDelta, bytes) ← decodeUIntLE 4 bytes
  pure ({ legSecurityId, legSide, legRatioQty, legPriceEx, legOptionDelta }, bytes)
...
def encode (message : MdInstrumentDefinitionSpread) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
    ...
    ++ encodeUIntLE 8 message.displayFactorEx
    ++ encodeUInt 1 message.priceDisplayFormat
    ++ encodeUIntLE 8 message.priceRatioEx
    ++ encodeUInt 1 message.tickRule
    ++ Alpha.encode message.unitOfMeasure
    ++ encodeUIntLE 8 message.tradingReferencePriceEx
    ++ encodeUIntLE 1 message.settlPriceType
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionSpread × List UInt8) := do
  ...
  let (displayFactorEx, bytes) ← decodeUIntLE 8 bytes
  let (priceDisplayFormat, bytes) ← decodeUInt 1 bytes
  let (priceRatioEx, bytes) ← decodeUIntLE 8 bytes
  let (tickRule, bytes) ← decodeUInt 1 bytes
  let (unitOfMeasure, bytes) ← Alpha.decode 30 bytes
  let (tradingReferencePriceEx, bytes) ← decodeUIntLE 8 bytes
  let (settlPriceType, bytes) ← decodeUIntLE 1 bytes
      ...
      if fits_instAttribGroup : instAttribGroup_.length < 256 then
        if fits_lotTypeRulesGroup : lotTypeRulesGroup_.length < 256 then
          if fits_legsGroup : legsGroup_.length < 256 then
            pure ({ matchEventIndicator, totNumReportsOptional, securityUpdateAction, lastUpdateTime, mdSecurityTradingStatus, applId, marketSegmentId, underlyingProductOptional, securityExchange, securityGroup, asset, symbol, securityId, securityType, cfiCode, maturityMonthYear, currency, securitySubType, userDefinedInstrument, matchAlgorithm, minTradeVol, maxTradeVol, minPriceIncrementOptionalEx, displayFactorEx, priceDisplayFormat, priceRatioEx, tickRule, unitOfMeasure, tradingReferencePriceEx, settlPriceType, openInterestQty, clearedVolume, highLimitPriceEx, lowLimitPriceEx, maxPriceVariationEx, mainFraction, subFraction, tradingReferenceDate, blockLength, eventsGroup := ⟨eventsGroup_, fits_eventsGroup⟩, blockLength, feedTypesGroup := ⟨feedTypesGroup_, fits_feedTypesGroup⟩, blockLength, instAttribGroup := ⟨instAttribGroup_, fits_instAttribGroup⟩, blockLength, lotTypeRulesGroup := ⟨lotTypeRulesGroup_, fits_lotTypeRulesGroup⟩, blockLength, legsGroup := ⟨legsGroup_, fits_legsGroup⟩ }, bytes)
          else none
        else none
      else none
    else none
...
def encode (message : MarketDataRequest) : List UInt8 :=
  encodeUIntLE 4 message.mdReqId
    ...
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.marketDataRequestSecurityGroup.val.length)
    ++ encodeMany MarketDataRequestSecurityGroup.encode message.marketDataRequestSecurityGroup.val
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.marketDataRequestRelatedSymbolGroup.val.length)
...
def decode (bytes : List UInt8) : Option (MarketDataRequest × List UInt8) := do
  ...
  if fits_marketDataRequestSecurityGroup : marketDataRequestSecurityGroup_.length < 256 then
    if fits_marketDataRequestRelatedSymbolGroup : marketDataRequestRelatedSymbolGroup_.length < 256 then
      pure ({ mdReqId, subscriptionReqType, blockLength, marketDataRequestSecurityGroup := ⟨marketDataRequestSecurityGroup_, fits_marketDataRequestSecurityGroup⟩, blockLength, marketDataRequestRelatedSymbolGroup := ⟨marketDataRequestRelatedSymbolGroup_, fits_marketDataRequestRelatedSymbolGroup⟩ }, bytes)
    else none
  else none

@[simp] theorem decode_encode (message : MarketDataRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  have fits_marketDataRequestSecurityGroup : message.marketDataRequestSecurityGroup.val.length < 256 := by simpa using message.marketDataRequestSecurityGroup.length_lt
  have fits_marketDataRequestRelatedSymbolGroup : message.marketDataRequestRelatedSymbolGroup.val.length < 256 := by simpa using message.marketDataRequestRelatedSymbolGroup.length_lt
  simp [decode, encode, List.append_assoc, Nat.mod_eq_of_lt fits_marketDataRequestSecurityGroup, fits_marketDataRequestSecurityGroup, decodeMany_encodeMany MarketDataRequestSecurityGroup.encode MarketDataRequestSecurityGroup.decode MarketDataRequestSecurityGroup.decode_encode, Nat.mod_eq_of_lt fits_marketDataRequestRelatedSymbolGroup, fits_marketDataRequestRelatedSymbolGroup, decodeMany_encodeMany MarketDataRequestRelatedSymbolGroup.encode MarketDataRequestRelatedSymbolGroup.decode MarketDataRequestRelatedSymbolGroup.decode_encode]
...
def encode (message : SecurityListRequest) : List UInt8 :=
  encodeUIntLE 4 message.mdReqId
    ++ encodeUInt 1 message.subscriptionReqType
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.securityListRequestSecurityGroup.val.length)
    ++ encodeMany SecurityListRequestSecurityGroup.encode message.securityListRequestSecurityGroup.val
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.securityListRequestRelatedSymbolGroup.val.length)
...
def decode (bytes : List UInt8) : Option (SecurityListRequest × List UInt8) := do
  ...
  if fits_securityListRequestSecurityGroup : securityListRequestSecurityGroup_.length < 256 then
    if fits_securityListRequestRelatedSymbolGroup : securityListRequestRelatedSymbolGroup_.length < 256 then
      pure ({ mdReqId, subscriptionReqType, blockLength, securityListRequestSecurityGroup := ⟨securityListRequestSecurityGroup_, fits_securityListRequestSecurityGroup⟩, blockLength, securityListRequestRelatedSymbolGroup := ⟨securityListRequestRelatedSymbolGroup_, fits_securityListRequestRelatedSymbolGroup⟩ }, bytes)
    else none
  else none

@[simp] theorem decode_encode (message : SecurityListRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  have fits_securityListRequestSecurityGroup : message.securityListRequestSecurityGroup.val.length < 256 := by simpa using message.securityListRequestSecurityGroup.length_lt
  have fits_securityListRequestRelatedSymbolGroup : message.securityListRequestRelatedSymbolGroup.val.length < 256 := by simpa using message.securityListRequestRelatedSymbolGroup.length_lt
  simp [decode, encode, List.append_assoc, Nat.mod_eq_of_lt fits_securityListRequestSecurityGroup, fits_securityListRequestSecurityGroup, decodeMany_encodeMany SecurityListRequestSecurityGroup.encode SecurityListRequestSecurityGroup.decode SecurityListRequestSecurityGroup.decode_encode, Nat.mod_eq_of_lt fits_securityListRequestRelatedSymbolGroup, fits_securityListRequestRelatedSymbolGroup, decodeMany_encodeMany SecurityListRequestRelatedSymbolGroup.encode SecurityListRequestRelatedSymbolGroup.decode SecurityListRequestRelatedSymbolGroup.decode_encode]
...
def encode (message : SecurityStatusRequest) : List UInt8 :=
  encodeUIntLE 4 message.mdReqId
    ++ encodeUInt 1 message.subscriptionReqType
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.securityStatusRequestSecurityGroup.val.length)
    ++ encodeMany SecurityStatusRequestSecurityGroup.encode message.securityStatusRequestSecurityGroup.val
    ++ encodeUIntLE 2 message.blockLength
    ++ encodeUInt 1 (BitVec.ofNat 8 message.securityStatusRequestRelatedSymbolGroup.val.length)
...
def decode (bytes : List UInt8) : Option (SecurityStatusRequest × List UInt8) := do
  ...
  if fits_securityStatusRequestSecurityGroup : securityStatusRequestSecurityGroup_.length < 256 then
    if fits_securityStatusRequestRelatedSymbolGroup : securityStatusRequestRelatedSymbolGroup_.length < 256 then
      pure ({ mdReqId, subscriptionReqType, blockLength, securityStatusRequestSecurityGroup := ⟨securityStatusRequestSecurityGroup_, fits_securityStatusRequestSecurityGroup⟩, blockLength, securityStatusRequestRelatedSymbolGroup := ⟨securityStatusRequestRelatedSymbolGroup_, fits_securityStatusRequestRelatedSymbolGroup⟩ }, bytes)
    else none
  else none

@[simp] theorem decode_encode (message : SecurityStatusRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  have fits_securityStatusRequestSecurityGroup : message.securityStatusRequestSecurityGroup.val.length < 256 := by simpa using message.securityStatusRequestSecurityGroup.length_lt
  have fits_securityStatusRequestRelatedSymbolGroup : message.securityStatusRequestRelatedSymbolGroup.val.length < 256 := by simpa using message.securityStatusRequestRelatedSymbolGroup.length_lt
  simp [decode, encode, List.append_assoc, Nat.mod_eq_of_lt fits_securityStatusRequestSecurityGroup, fits_securityStatusRequestSecurityGroup, decodeMany_encodeMany SecurityStatusRequestSecurityGroup.encode SecurityStatusRequestSecurityGroup.decode SecurityStatusRequestSecurityGroup.decode_encode, Nat.mod_eq_of_lt fits_securityStatusRequestRelatedSymbolGroup, fits_securityStatusRequestRelatedSymbolGroup, decodeMany_encodeMany SecurityStatusRequestRelatedSymbolGroup.encode SecurityStatusRequestRelatedSymbolGroup.decode SecurityStatusRequestRelatedSymbolGroup.decode_encode]
...
inductive Payload where
  ...
  | mdInstrumentDefinitionFutureLegacy (message : MdInstrumentDefinitionFutureLegacy) -- 27
  | mdInstrumentDefinitionSpreadLegacy (message : MdInstrumentDefinitionSpreadLegacy) -- 29
  | securityStatus (message : SecurityStatus) -- 30
  | mdIncrementalRefreshBookLegacy (message : MdIncrementalRefreshBookLegacy) -- 32
  | mdIncrementalRefreshDailyStatisticsLegacy (message : MdIncrementalRefreshDailyStatisticsLegacy) -- 33
  | mdIncrementalRefreshLimitsBandingLegacy (message : MdIncrementalRefreshLimitsBandingLegacy) -- 34
  | mdIncrementalRefreshSessionStatisticsLegacy (message : MdIncrementalRefreshSessionStatisticsLegacy) -- 35
  | mdIncrementalRefreshVolume (message : MdIncrementalRefreshVolume) -- 37
  | snapshotFullRefreshLegacy (message : SnapshotFullRefreshLegacy) -- 38
  | quoteRequest (message : QuoteRequest) -- 39
  | mdInstrumentDefinitionOptionLegacy (message : MdInstrumentDefinitionOptionLegacy) -- 41
  | mdIncrementalRefreshTradeSummaryLegacy (message : MdIncrementalRefreshTradeSummaryLegacy) -- 42
  | mdIncrementalRefreshOrderBookLegacy (message : MdIncrementalRefreshOrderBookLegacy) -- 43
  | snapshotFullRefreshOrderBookLegacy (message : SnapshotFullRefreshOrderBookLegacy) -- 44
...
def tag : Payload → BitVec 16
  ...
  | .adminLogin _ => 15
  | .adminLogout _ => 16
  | .mdInstrumentDefinitionFutureLegacy _ => 27
  | .mdInstrumentDefinitionSpreadLegacy _ => 29
  | .securityStatus _ => 30
  | .mdIncrementalRefreshBookLegacy _ => 32
  | .mdIncrementalRefreshDailyStatisticsLegacy _ => 33
  | .mdIncrementalRefreshLimitsBandingLegacy _ => 34
  | .mdIncrementalRefreshSessionStatisticsLegacy _ => 35
  | .mdIncrementalRefreshVolume _ => 37
  | .snapshotFullRefreshLegacy _ => 38
  | .quoteRequest _ => 39
  | .mdInstrumentDefinitionOptionLegacy _ => 41
  | .mdIncrementalRefreshTradeSummaryLegacy _ => 42
  | .mdIncrementalRefreshOrderBookLegacy _ => 43
  | .snapshotFullRefreshOrderBookLegacy _ => 44
...
def encode : Payload → List UInt8
  ...
  | .mdInstrumentDefinitionFutureLegacy message => MdInstrumentDefinitionFutureLegacy.encode message
  | .mdInstrumentDefinitionSpreadLegacy message => MdInstrumentDefinitionSpreadLegacy.encode message
  | .securityStatus message => SecurityStatus.encode message
  | .mdIncrementalRefreshBookLegacy message => MdIncrementalRefreshBookLegacy.encode message
  | .mdIncrementalRefreshDailyStatisticsLegacy message => MdIncrementalRefreshDailyStatisticsLegacy.encode message
  | .mdIncrementalRefreshLimitsBandingLegacy message => MdIncrementalRefreshLimitsBandingLegacy.encode message
  | .mdIncrementalRefreshSessionStatisticsLegacy message => MdIncrementalRefreshSessionStatisticsLegacy.encode message
  | .mdIncrementalRefreshVolume message => MdIncrementalRefreshVolume.encode message
  | .snapshotFullRefreshLegacy message => SnapshotFullRefreshLegacy.encode message
  | .quoteRequest message => QuoteRequest.encode message
  | .mdInstrumentDefinitionOptionLegacy message => MdInstrumentDefinitionOptionLegacy.encode message
  | .mdIncrementalRefreshTradeSummaryLegacy message => MdIncrementalRefreshTradeSummaryLegacy.encode message
  | .mdIncrementalRefreshOrderBookLegacy message => MdIncrementalRefreshOrderBookLegacy.encode message
  | .snapshotFullRefreshOrderBookLegacy message => SnapshotFullRefreshOrderBookLegacy.encode message
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (Payload × List UInt8) :=
  ...
  else if tag = 27 then (MdInstrumentDefinitionFutureLegacy.decode bytes).map fun (message, rest) => (.mdInstrumentDefinitionFutureLegacy message, rest)
  else if tag = 29 then (MdInstrumentDefinitionSpreadLegacy.decode bytes).map fun (message, rest) => (.mdInstrumentDefinitionSpreadLegacy message, rest)
  else if tag = 30 then (SecurityStatus.decode bytes).map fun (message, rest) => (.securityStatus message, rest)
  else if tag = 32 then (MdIncrementalRefreshBookLegacy.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshBookLegacy message, rest)
  else if tag = 33 then (MdIncrementalRefreshDailyStatisticsLegacy.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshDailyStatisticsLegacy message, rest)
  else if tag = 34 then (MdIncrementalRefreshLimitsBandingLegacy.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshLimitsBandingLegacy message, rest)
  else if tag = 35 then (MdIncrementalRefreshSessionStatisticsLegacy.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshSessionStatisticsLegacy message, rest)
  else if tag = 37 then (MdIncrementalRefreshVolume.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshVolume message, rest)
  else if tag = 38 then (SnapshotFullRefreshLegacy.decode bytes).map fun (message, rest) => (.snapshotFullRefreshLegacy message, rest)
  else if tag = 39 then (QuoteRequest.decode bytes).map fun (message, rest) => (.quoteRequest message, rest)
  else if tag = 41 then (MdInstrumentDefinitionOptionLegacy.decode bytes).map fun (message, rest) => (.mdInstrumentDefinitionOptionLegacy message, rest)
  else if tag = 42 then (MdIncrementalRefreshTradeSummaryLegacy.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshTradeSummaryLegacy message, rest)
  else if tag = 43 then (MdIncrementalRefreshOrderBookLegacy.decode bytes).map fun (message, rest) => (.mdIncrementalRefreshOrderBookLegacy message, rest)
  else if tag = 44 then (SnapshotFullRefreshOrderBookLegacy.decode bytes).map fun (message, rest) => (.snapshotFullRefreshOrderBookLegacy message, rest)
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 2 < 65536 := by
  ...
  | channelReset inner =>
    ...
    simp [encodeBody, h, Payload.encode, ChannelReset.encode, encodeMany_length_const ChannelResetGroup.encode 2 ChannelResetGroup.encode_length]
    omega
  | adminHeartbeat inner =>
    simp [encodeBody, h, Payload.encode, AdminHeartbeat.encode]
  | adminLogin inner =>
    simp [encodeBody, h, Payload.encode, AdminLogin.encode]
  | adminLogout inner =>
    simp [encodeBody, h, Payload.encode, AdminLogout.encode]
  | mdInstrumentDefinitionFutureLegacy inner =>
    have bound_mdInstrumentDefinitionFutureLegacy_eventsGroup := inner.eventsGroup.length_lt
    have bound_mdInstrumentDefinitionFutureLegacy_feedTypesGroup := inner.feedTypesGroup.length_lt
    have bound_mdInstrumentDefinitionFutureLegacy_instAttribGroup := inner.instAttribGroup.length_lt
    have bound_mdInstrumentDefinitionFutureLegacy_lotTypeRulesGroup := inner.lotTypeRulesGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionFutureLegacy.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length]
    omega
  | mdInstrumentDefinitionSpreadLegacy inner =>
    have bound_mdInstrumentDefinitionSpreadLegacy_eventsGroup := inner.eventsGroup.length_lt
    have bound_mdInstrumentDefinitionSpreadLegacy_feedTypesGroup := inner.feedTypesGroup.length_lt
    have bound_mdInstrumentDefinitionSpreadLegacy_instAttribGroup := inner.instAttribGroup.length_lt
    have bound_mdInstrumentDefinitionSpreadLegacy_lotTypeRulesGroup := inner.lotTypeRulesGroup.length_lt
    have bound_mdInstrumentDefinitionSpreadLegacy_legacyLegsGroup := inner.legacyLegsGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionSpreadLegacy.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length, encodeMany_length_const LegacyLegsGroup.encode 18 LegacyLegsGroup.encode_length]
    omega
  | securityStatus inner =>
    simp [encodeBody, h, Payload.encode, SecurityStatus.encode]
  | mdIncrementalRefreshBookLegacy inner =>
    have bound_mdIncrementalRefreshBookLegacy_incrementalRefreshBookGroup := inner.incrementalRefreshBookGroup.length_lt
    have bound_mdIncrementalRefreshBookLegacy_incrementalRefreshBookOrderIdGroup := inner.incrementalRefreshBookOrderIdGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshBookLegacy.encode, encodeMany_length_const IncrementalRefreshBookGroup.encode 32 IncrementalRefreshBookGroup.encode_length, encodeMany_length_const IncrementalRefreshBookOrderIdGroup.encode 24 IncrementalRefreshBookOrderIdGroup.encode_length]
    omega
  | mdIncrementalRefreshDailyStatisticsLegacy inner =>
    have bound_mdIncrementalRefreshDailyStatisticsLegacy_incrementalRefreshDailyStatisticsGroup := inner.incrementalRefreshDailyStatisticsGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshDailyStatisticsLegacy.encode, encodeMany_length_const IncrementalRefreshDailyStatisticsGroup.encode 32 IncrementalRefreshDailyStatisticsGroup.encode_length]
    omega
  | mdIncrementalRefreshLimitsBandingLegacy inner =>
    have bound_mdIncrementalRefreshLimitsBandingLegacy_incrementalRefreshLimitsBandingGroup := inner.incrementalRefreshLimitsBandingGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshLimitsBandingLegacy.encode, encodeMany_length_const IncrementalRefreshLimitsBandingGroup.encode 32 IncrementalRefreshLimitsBandingGroup.encode_length]
    omega
  | mdIncrementalRefreshSessionStatisticsLegacy inner =>
    have bound_mdIncrementalRefreshSessionStatisticsLegacy_incrementalRefreshSessionStatisticsGroup := inner.incrementalRefreshSessionStatisticsGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshSessionStatisticsLegacy.encode, encodeMany_length_const IncrementalRefreshSessionStatisticsGroup.encode 24 IncrementalRefreshSessionStatisticsGroup.encode_length]
    omega
  | mdIncrementalRefreshVolume inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshVolume.encode, encodeMany_length_const IncrementalRefreshVolumeGroup.encode 16 IncrementalRefreshVolumeGroup.encode_length]
    omega
  | snapshotFullRefreshLegacy inner =>
    have bound_snapshotFullRefreshLegacy_snapshotFullRefreshGroup := inner.snapshotFullRefreshGroup.length_lt
    simp [encodeBody, h, Payload.encode, SnapshotFullRefreshLegacy.encode, encodeMany_length_const SnapshotFullRefreshGroup.encode 22 SnapshotFullRefreshGroup.encode_length]
    omega
  | quoteRequest inner =>
    have bound_quoteRequest_relatedSymGroup := inner.relatedSymGroup.length_lt
    simp [encodeBody, h, Payload.encode, QuoteRequest.encode, encodeMany_length_const RelatedSymGroup.encode 32 RelatedSymGroup.encode_length]
    omega
  | mdInstrumentDefinitionOptionLegacy inner =>
    have bound_mdInstrumentDefinitionOptionLegacy_eventsGroup := inner.eventsGroup.length_lt
    have bound_mdInstrumentDefinitionOptionLegacy_feedTypesGroup := inner.feedTypesGroup.length_lt
    have bound_mdInstrumentDefinitionOptionLegacy_instAttribGroup := inner.instAttribGroup.length_lt
    have bound_mdInstrumentDefinitionOptionLegacy_lotTypeRulesGroup := inner.lotTypeRulesGroup.length_lt
    have bound_mdInstrumentDefinitionOptionLegacy_optionUnderlyingsGroup := inner.optionUnderlyingsGroup.length_lt
    have bound_mdInstrumentDefinitionOptionLegacy_optionRelatedInstrumentsGroup := inner.optionRelatedInstrumentsGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionOptionLegacy.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length, encodeMany_length_const OptionUnderlyingsGroup.encode 24 OptionUnderlyingsGroup.encode_length, encodeMany_length_const OptionRelatedInstrumentsGroup.encode 24 OptionRelatedInstrumentsGroup.encode_length]
    omega
  | mdIncrementalRefreshTradeSummaryLegacy inner =>
    have bound_mdIncrementalRefreshTradeSummaryLegacy_incrementalRefreshTradeSummaryGroup := inner.incrementalRefreshTradeSummaryGroup.length_lt
    have bound_mdIncrementalRefreshTradeSummaryLegacy_incrementalRefreshTradeSummaryOrderIdGroup := inner.incrementalRefreshTradeSummaryOrderIdGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshTradeSummaryLegacy.encode, encodeMany_length_const IncrementalRefreshTradeSummaryGroup.encode 32 IncrementalRefreshTradeSummaryGroup.encode_length, encodeMany_length_const IncrementalRefreshTradeSummaryOrderIdGroup.encode 16 IncrementalRefreshTradeSummaryOrderIdGroup.encode_length]
    omega
  | mdIncrementalRefreshOrderBookLegacy inner =>
    have bound_mdIncrementalRefreshOrderBookLegacy_incrementalRefreshOrderBookGroup := inner.incrementalRefreshOrderBookGroup.length_lt
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshOrderBookLegacy.encode, encodeMany_length_const IncrementalRefreshOrderBookGroup.encode 40 IncrementalRefreshOrderBookGroup.encode_length]
    omega
  | snapshotFullRefreshOrderBookLegacy inner =>
    have bound_snapshotFullRefreshOrderBookLegacy_snapshotFullRefreshOrderBookGroup := inner.snapshotFullRefreshOrderBookGroup.length_lt
    simp [encodeBody, h, Payload.encode, SnapshotFullRefreshOrderBookLegacy.encode, encodeMany_length_const SnapshotFullRefreshOrderBookGroup.encode 29 SnapshotFullRefreshOrderBookGroup.encode_length]
    omega
  | mdIncrementalRefreshBook inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshBook.encode, encodeMany_length_const IncrementalRefreshBookGroup.encode 32 IncrementalRefreshBookGroup.encode_length, encodeMany_length_const IncrementalRefreshBookOrderIdGroup.encode 24 IncrementalRefreshBookOrderIdGroup.encode_length]
    omega
  | mdIncrementalRefreshOrderBook inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshOrderBook.encode, encodeMany_length_const IncrementalRefreshOrderBookGroup.encode 40 IncrementalRefreshOrderBookGroup.encode_length]
    omega
  ...
  | mdIncrementalRefreshDailyStatistics inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshDailyStatistics.encode, encodeMany_length_const IncrementalRefreshDailyStatisticsGroup.encode 32 IncrementalRefreshDailyStatisticsGroup.encode_length]
    omega
  | mdIncrementalRefreshLimitsBanding inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshLimitsBanding.encode, encodeMany_length_const IncrementalRefreshLimitsBandingGroup.encode 32 IncrementalRefreshLimitsBandingGroup.encode_length]
    omega
  | mdIncrementalRefreshSessionStatistics inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdIncrementalRefreshSessionStatistics.encode, encodeMany_length_const IncrementalRefreshSessionStatisticsGroup.encode 24 IncrementalRefreshSessionStatisticsGroup.encode_length]
    omega
  | snapshotFullRefresh inner =>
    ...
    simp [encodeBody, h, Payload.encode, SnapshotFullRefresh.encode, encodeMany_length_const SnapshotFullRefreshGroup.encode 22 SnapshotFullRefreshGroup.encode_length]
    omega
  | snapshotFullRefreshOrderBook inner =>
    ...
    simp [encodeBody, h, Payload.encode, SnapshotFullRefreshOrderBook.encode, encodeMany_length_const SnapshotFullRefreshOrderBookGroup.encode 29 SnapshotFullRefreshOrderBookGroup.encode_length]
    omega
  ...
  | mdInstrumentDefinitionOption inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionOption.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length, encodeMany_length_const OptionUnderlyingsGroup.encode 24 OptionUnderlyingsGroup.encode_length, encodeMany_length_const OptionRelatedInstrumentsGroup.encode 24 OptionRelatedInstrumentsGroup.encode_length]
    omega
  | mdInstrumentDefinitionSpread inner =>
    ...
    simp [encodeBody, h, Payload.encode, MdInstrumentDefinitionSpread.encode, encodeMany_length_const EventsGroup.encode 9 EventsGroup.encode_length, encodeMany_length_const FeedTypesGroup.encode 4 FeedTypesGroup.encode_length, encodeMany_length_const InstAttribGroup.encode 4 InstAttribGroup.encode_length, encodeMany_length_const LotTypeRulesGroup.encode 5 LotTypeRulesGroup.encode_length, encodeMany_length_const LegsGroup.encode 18 LegsGroup.encode_length]
    omega
  | negotiate inner =>
    simp [encodeBody, h, Payload.encode, Negotiate.encode]
  | negotiationReject inner =>
    simp [encodeBody, h, Payload.encode, NegotiationReject.encode]
  | negotiationResponse inner =>
    simp [encodeBody, h, Payload.encode, NegotiationResponse.encode]
  | terminate inner =>
    simp [encodeBody, h, Payload.encode, Terminate.encode]
  | marketDataRequest inner =>
    have bound_marketDataRequest_marketDataRequestSecurityGroup := inner.marketDataRequestSecurityGroup.length_lt
    have bound_marketDataRequest_marketDataRequestRelatedSymbolGroup := inner.marketDataRequestRelatedSymbolGroup.length_lt
    simp [encodeBody, h, Payload.encode, MarketDataRequest.encode, encodeMany_length_const MarketDataRequestSecurityGroup.encode 6 MarketDataRequestSecurityGroup.encode_length, encodeMany_length_const MarketDataRequestRelatedSymbolGroup.encode 4 MarketDataRequestRelatedSymbolGroup.encode_length]
    omega
  | requestAck inner =>
    ...
    simp [encodeBody, h, Payload.encode, RequestAck.encode, encodeMany_length_const RequestAckSecurityGroup.encode 6 RequestAckSecurityGroup.encode_length, encodeMany_length_const RequestAckRelatedSymbolGroup.encode 4 RequestAckRelatedSymbolGroup.encode_length]
    omega
  | requestReject inner =>
    simp [encodeBody, h, Payload.encode, RequestReject.encode]
  | securityListRequest inner =>
    have bound_securityListRequest_securityListRequestSecurityGroup := inner.securityListRequestSecurityGroup.length_lt
    have bound_securityListRequest_securityListRequestRelatedSymbolGroup := inner.securityListRequestRelatedSymbolGroup.length_lt
    simp [encodeBody, h, Payload.encode, SecurityListRequest.encode, encodeMany_length_const SecurityListRequestSecurityGroup.encode 6 SecurityListRequestSecurityGroup.encode_length, encodeMany_length_const SecurityListRequestRelatedSymbolGroup.encode 4 SecurityListRequestRelatedSymbolGroup.encode_length]
    omega
  | securityStatusRequest inner =>
    have bound_securityStatusRequest_securityStatusRequestSecurityGroup := inner.securityStatusRequestSecurityGroup.length_lt
    have bound_securityStatusRequest_securityStatusRequestRelatedSymbolGroup := inner.securityStatusRequestRelatedSymbolGroup.length_lt
    simp [encodeBody, h, Payload.encode, SecurityStatusRequest.encode, encodeMany_length_const SecurityStatusRequestSecurityGroup.encode 6 SecurityStatusRequestSecurityGroup.encode_length, encodeMany_length_const SecurityStatusRequestRelatedSymbolGroup.encode 4 SecurityStatusRequestRelatedSymbolGroup.encode_length]
    omega
  | subscriberHeartbeat inner =>
    simp [encodeBody, h, Payload.encode, SubscriberHeartbeat.encode]
...
end Omi.CmeGlobexMdp3SbeV19Udp
