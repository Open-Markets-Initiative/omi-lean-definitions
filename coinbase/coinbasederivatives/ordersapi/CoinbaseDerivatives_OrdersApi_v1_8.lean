...
namespace Omi.CoinbaseCoinbasederivativesOrdersapiSbeV18
...
def encode (message : RejectMessage) : List UInt8 :=
  encodeUIntLE 4 message.refSequenceNumber
    ++ (encodeUIntLE 4 message.rejectReason
...
def decode (bytes : List UInt8) : Option (RejectMessage × List UInt8) := do
  ...
  let (rejectReason, bytes) ← decodeUIntLE 4 bytes
  let (details, bytes) ← Alpha.decode 64 bytes
  pure ({ refSequenceNumber, rejectReason, details }, bytes)
...
/-- Reset Options Fill Protection Message: 12 bytes -/
structure ResetOptionsFillProtectionMessage where
  correlationId : BitVec 64
  productId : BitVec 32
  deriving DecidableEq, Repr

namespace ResetOptionsFillProtectionMessage

def encode (message : ResetOptionsFillProtectionMessage) : List UInt8 :=
  encodeUIntLE 8 message.correlationId
    ++ (encodeUIntLE 4 message.productId)

def decode (bytes : List UInt8) : Option (ResetOptionsFillProtectionMessage × List UInt8) := do
  let (correlationId, bytes) ← decodeUIntLE 8 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  pure ({ correlationId, productId }, bytes)

@[simp] theorem encode_length (message : ResetOptionsFillProtectionMessage) : (encode message).length = 12 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]

theorem encode_length_pos (message : ResetOptionsFillProtectionMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ResetOptionsFillProtectionMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ResetOptionsFillProtectionMessage

/-- Reset Options Fill Protection Ack Message: 20 bytes -/
structure ResetOptionsFillProtectionAckMessage where
  timestamp : BitVec 64
  correlationId : BitVec 64
  productId : BitVec 32
  deriving DecidableEq, Repr

namespace ResetOptionsFillProtectionAckMessage

def encode (message : ResetOptionsFillProtectionAckMessage) : List UInt8 :=
  encodeUIntLE 8 message.timestamp
    ++ (encodeUIntLE 8 message.correlationId
    ++ (encodeUIntLE 4 message.productId))

def decode (bytes : List UInt8) : Option (ResetOptionsFillProtectionAckMessage × List UInt8) := do
  let (timestamp, bytes) ← decodeUIntLE 8 bytes
  let (correlationId, bytes) ← decodeUIntLE 8 bytes
  let (productId, bytes) ← decodeUIntLE 4 bytes
  pure ({ timestamp, correlationId, productId }, bytes)

@[simp] theorem encode_length (message : ResetOptionsFillProtectionAckMessage) : (encode message).length = 20 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]

theorem encode_length_pos (message : ResetOptionsFillProtectionAckMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ResetOptionsFillProtectionAckMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ResetOptionsFillProtectionAckMessage

/-- Reset Options Fill Protection Reject Message: 48 bytes -/
structure ResetOptionsFillProtectionRejectMessage where
  timestamp : BitVec 64
  correlationId : BitVec 64
  errorMessage : Alpha 32
  deriving DecidableEq, Repr

namespace ResetOptionsFillProtectionRejectMessage

def encode (message : ResetOptionsFillProtectionRejectMessage) : List UInt8 :=
  encodeUIntLE 8 message.timestamp
    ++ (encodeUIntLE 8 message.correlationId
    ++ (Alpha.encode message.errorMessage))

def decode (bytes : List UInt8) : Option (ResetOptionsFillProtectionRejectMessage × List UInt8) := do
  let (timestamp, bytes) ← decodeUIntLE 8 bytes
  let (correlationId, bytes) ← decodeUIntLE 8 bytes
  let (errorMessage, bytes) ← Alpha.decode 32 bytes
  pure ({ timestamp, correlationId, errorMessage }, bytes)

@[simp] theorem encode_length (message : ResetOptionsFillProtectionRejectMessage) : (encode message).length = 48 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length]

theorem encode_length_pos (message : ResetOptionsFillProtectionRejectMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ResetOptionsFillProtectionRejectMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
end ResetOptionsFillProtectionRejectMessage
...
inductive Payload where
  ...
  | rejectMessage (message : RejectMessage) -- 1100, 210
  ...
  | resetOptionsFillProtectionMessage (message : ResetOptionsFillProtectionMessage) -- 1101, 160
  | resetOptionsFillProtectionAckMessage (message : ResetOptionsFillProtectionAckMessage) -- 1101, 260
  | resetOptionsFillProtectionRejectMessage (message : ResetOptionsFillProtectionRejectMessage) -- 1101, 261
  deriving DecidableEq, Repr
...
def schemaId : Payload → BitVec 16
  ...
  | .resetOptionsFillProtectionMessage _ => 1101
  | .resetOptionsFillProtectionAckMessage _ => 1101
  | .resetOptionsFillProtectionRejectMessage _ => 1101
...
def templateId : Payload → BitVec 16
  ...
  | .resetOptionsFillProtectionMessage _ => 160
  | .resetOptionsFillProtectionAckMessage _ => 260
  | .resetOptionsFillProtectionRejectMessage _ => 261
...
def encode : Payload → List UInt8
  ...
  | .resetOptionsFillProtectionMessage message => ResetOptionsFillProtectionMessage.encode message
  | .resetOptionsFillProtectionAckMessage message => ResetOptionsFillProtectionAckMessage.encode message
  | .resetOptionsFillProtectionRejectMessage message => ResetOptionsFillProtectionRejectMessage.encode message
...
theorem encode_length_le (message : Payload) : (encode message).length ≤ 281 := by
  ...
  | eventResendRejectMessage inner =>
    ...
    omega
  | resetOptionsFillProtectionMessage inner =>
    simp only [encode, ResetOptionsFillProtectionMessage.encode_length]
    omega
  | resetOptionsFillProtectionAckMessage inner =>
    simp only [encode, ResetOptionsFillProtectionAckMessage.encode_length]
    omega
  | resetOptionsFillProtectionRejectMessage inner =>
    simp only [encode, ResetOptionsFillProtectionRejectMessage.encode_length]
    omega
...
def decode (schemaId : BitVec 16) (templateId : BitVec 16) (bytes : List UInt8) : Option (Payload × List UInt8) :=
  ...
  else if schemaId = 1100 ∧ templateId = 210 then (RejectMessage.decode bytes).map fun (message, rest) => (.rejectMessage message, rest)
  ...
  else if schemaId = 1101 ∧ templateId = 160 then (ResetOptionsFillProtectionMessage.decode bytes).map fun (message, rest) => (.resetOptionsFillProtectionMessage message, rest)
  else if schemaId = 1101 ∧ templateId = 260 then (ResetOptionsFillProtectionAckMessage.decode bytes).map fun (message, rest) => (.resetOptionsFillProtectionAckMessage message, rest)
  else if schemaId = 1101 ∧ templateId = 261 then (ResetOptionsFillProtectionRejectMessage.decode bytes).map fun (message, rest) => (.resetOptionsFillProtectionRejectMessage message, rest)
  else none
...
theorem encodeBody_length_lt (message : SbeMessage) : (encodeBody message).length + 4 < 256 ^ 2 := by
  ...
  | eventResendRejectMessage inner =>
    ...
    omega
  | resetOptionsFillProtectionMessage inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, encodeUIntLE_length, ResetOptionsFillProtectionMessage.encode_length]
    omega
  | resetOptionsFillProtectionAckMessage inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, encodeUIntLE_length, ResetOptionsFillProtectionAckMessage.encode_length]
    omega
  | resetOptionsFillProtectionRejectMessage inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, encodeUIntLE_length, ResetOptionsFillProtectionRejectMessage.encode_length]
    omega
...
end Omi.CoinbaseCoinbasederivativesOrdersapiSbeV18
