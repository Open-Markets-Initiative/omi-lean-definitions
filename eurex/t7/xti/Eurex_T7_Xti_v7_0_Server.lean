...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v7.0
...
namespace Omi.EurexT7XtiFbeV70Server
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : DeleteAllOrderResponse) : (encode message).length ≤ 2097202 := by
  have bound_notAffectedOrdersGrpComp := message.notAffectedOrdersGrpComp.length_lt
  have bound_affectedOrdGrpComp := message.affectedOrdGrpComp.length_lt
  unfold encode
...
/-- Delete Order Broadcast: 162 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteOrderBroadcast × List UInt8) := do
  ...
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, cumQty, cxlQty, orderIdSfx, marketSegmentId, partyIdEnteringTrader, partyIdSessionId, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, side, fixClOrdId, pad6 }, bytes)

@[simp] theorem encode_length (message : DeleteOrderBroadcast) : (encode message).length = 162 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (DeleteOrderNrResponse × List UInt8) := do
  ...
  let (execType, bytes) ← ExecType.decode bytes
  let (execRestatementReason, bytes) ← decodeUIntLE 2 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, cumQty, cxlQty, orderIdSfx, ordStatus, execType, execRestatementReason }, bytes)
...
def decode (bytes : List UInt8) : Option (DeleteOrderResponse × List UInt8) := do
  ...
  let (execType, bytes) ← ExecType.decode bytes
  let (execRestatementReason, bytes) ← decodeUIntLE 2 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, cumQty, cxlQty, orderIdSfx, ordStatus, execType, execRestatementReason }, bytes)
...
/-- Extended Deletion Report: 314 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderNrResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, stopPx, leavesQty, cumQty, cxlQty, displayQty, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (ModifyOrderResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, stopPx, leavesQty, cumQty, cxlQty, displayQty, trdRegTsTimePriority, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (OrderExecNotification × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, leavesQty, cumQty, cxlQty, displayQty, marketSegmentId, orderIdSfx, execRestatementReason, side, ordStatus, execType, matchType, triggered, crossedIndicator, fixClOrdId, pad3, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 8482 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 32 FillsGrpComp.encode_length]
  omega
...
theorem encode_length_le (message : OrderExecResponse) : (encode message).length ≤ 8338 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 32 FillsGrpComp.encode_length]
  omega
...
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 8306 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocGrpBcComp.encode 32 SideAllocGrpBcComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (TradeBroadcast × List UInt8) := do
  ...
  let (rootPartyContraSettlementLocation, bytes) ← Alpha.decode 3 bytes
  let (regulatoryTradeId, bytes) ← Alpha.decode 52 bytes
  pure ({ pad2, rbcHeaderComp, securityId, price, lastPx, lastQty, settlCurrAmt, settlCurrFxRate, transactTime, orderId, clOrdId, leavesQty, cumQty, sideGrossTradeAmt, accruedInteresAmt, couponRate, rootPartyIdClientId, executingTrader, rootPartyIdInvestmentDecisionMaker, tradeId, origTradeId, rootPartyIdExecutingUnit, rootPartyIdSessionId, rootPartyIdExecutingTrader, rootPartyIdSettlementUnit, rootPartyIdClearingUnit, rootPartyIdContraUnit, rootPartyIdContraSettlementUnit, orderIdSfx, marketSegmentId, sideTradeId, sideTradeReportId, tradeNumber, matchDate, settlDate, trdMatchId, numDaysInterest, lastMkt, tradeReportType, transferReason, matchType, matchSubType, side, sideLiquidityInd, deliveryType, tradingCapacity, lastCouponDeviationIndicator, refinancingEligibilityIndicator, orderAttributeLiquidityProvision, executingTraderQualifier, rootPartyIdInvestmentDecisionMakerQualifier, account, settlCurrency, currency, freeText1, freeText2, freeText4, orderCategory, ordType, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyClearingFirm, rootPartyExecutingFirmKvNumber, rootPartySettlementAccount, rootPartySettlementLocation, rootPartySettlementFirm, rootPartyContraFirm, rootPartyContraSettlementFirm, rootPartyContraFirmKvNumber, rootPartyContraSettlementAccount, rootPartyContraSettlementLocation, regulatoryTradeId }, bytes)

@[simp] theorem encode_length (message : TradeBroadcast) : (encode message).length = 474 := by
  unfold encode
...
end Omi.EurexT7XtiFbeV70Server
