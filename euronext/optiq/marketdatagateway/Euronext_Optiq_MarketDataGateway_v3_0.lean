...
/-!
# Euronext Market Data Gateway v3.0
...
namespace Omi.EuronextOptiqMarketdatagatewaySbeV30
...
def ExchangeCode.codes : List UInt8 :=
  [0x41, 0x42, 0x43, 0x44, 0x46, 0x47, 0x48, 0x4A, 0x4B, 0x4D, 0x50, 0x52, 0x53, 0x59, 0x5A]
...
def listed (byte : UInt8) : ExchangeCode :=
  ...
  else .amsterdamCurrencyDerivatives
...
def decode (bytes : List UInt8) : Option (MarketUpdatesGroup × List UInt8) := do
  ...
  pure ({ updateType, symbolIndex, numberOfOrders, price, quantity }, bytes)
...
def decode (bytes : List UInt8) : Option (PriceUpdatesGroup × List UInt8) := do
  ...
  pure ({ priceType, symbolIndex, price, quantity, imbalanceQty, imbalanceQtySide }, bytes)

@[simp] theorem encode_length (message : PriceUpdatesGroup) : (encode message).length = 30 := by
  unfold encode
  simp only [List.length_append, encodeUInt_length, encodeUIntLE_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : PriceUpdatesGroups) : (encode message).length ≤ 7652 := by
  have bound_priceUpdatesGroup := message.priceUpdatesGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUInt_length, encodeMany_length_const PriceUpdatesGroup.encode 30 PriceUpdatesGroup.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : PriceUpdateMessage) : (encode message).length ≤ 7670 := by
...
/-- Full Trade Information Message: 402 bytes -/
...
def decode (bytes : List UInt8) : Option (FullTradeInformationMessage × List UInt8) := do
  ...
  pure ({ mdSeqNum, rebroadcastIndicator, emm, eventTime, symbolIndexOptional, tradingDateTime, publicationDateTime, tradeType, mifidInstrumentIdTypeOptional, mifidInstrumentIdOptional, mifidExecutionId, mifidPriceOptional, mifidQuantity, mifidPriceNotation, mifidCurrency, mifidQtyInMsrmtUnitNotation, mifidQuantityMeasurementUnit, mifidNotionalAmount, notionalCurrency, mifidClearingFlag, mmtMarketMechanism, mmtTradingMode, mmtTransactionCategory, mmtNegotiationIndicator, mmtAgencyCrossTradeIndicator, mmtModificationIndicator, mmtBenchmarkIndicator, mmtSpecialDividendIndicator, mmtOffBookAutomatedIndicator, mmtContributiontoPrice, mmtAlgorithmicIndicator, mmtPublicationMode, mmtPostTradeDeferral, mmtDuplicativeIndicator, tradeQualifier, transactionType, effectiveDateIndicator, blockTradeCode, tradeReference, originalReportTimestamp, transparencyIndicator, currencyCoefficient, priceMultiplier, priceMultiplierDecimals, venue, startTimeVwap, endTimeVwap, mifidEmissionAllowanceType, marketOfReferenceMic, evaluatedPrice, messagePriceNotation, notUsedGroups }, bytes)

@[simp] theorem encode_length (message : FullTradeInformationMessage) : (encode message).length = 402 := by
  unfold encode
...
/-- Emm Pattern Rep Group: 15 bytes -/
...
def decode (bytes : List UInt8) : Option (EmmPatternRepGroup × List UInt8) := do
  ...
  pure ({ emm, patternId, tickSizeIndexId, marketModel, lotSizeOptional, instUnitExp }, bytes)

@[simp] theorem encode_length (message : EmmPatternRepGroup) : (encode message).length = 15 := by
  unfold encode
  simp only [List.length_append, encodeUInt_length, encodeUIntLE_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EmmPatternRepGroups) : (encode message).length ≤ 3827 := by
  have bound_emmPatternRepGroup := message.emmPatternRepGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUInt_length, encodeMany_length_const EmmPatternRepGroup.encode 15 EmmPatternRepGroup.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : StandingDataMessage) : (encode message).length ≤ 4236 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OutrightStandingDataMessage) : (encode message).length ≤ 364 := by
...
/-- Package Components Group: 8 bytes -/
...
def decode (bytes : List UInt8) : Option (PackageComponentsGroup × List UInt8) := do
  let (legSymbolIndex, bytes) ← decodeUIntLE 4 bytes
  let (legRatio, bytes) ← decodeUIntLE 4 bytes
  pure ({ legSymbolIndex, legRatio }, bytes)

@[simp] theorem encode_length (message : PackageComponentsGroup) : (encode message).length = 8 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : PackageComponentsGroups) : (encode message).length ≤ 2042 := by
  have bound_packageComponentsGroup := message.packageComponentsGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUInt_length, encodeMany_length_const PackageComponentsGroup.encode 8 PackageComponentsGroup.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : LisPackageStructureMessage) : (encode message).length ≤ 2117 := by
...
def decode (bytes : List UInt8) : Option (ApaQuotesMessage × List UInt8) := do
  ...
  let (mifidQuantity, bytes) ← Alpha.decode 20 bytes
  pure ({ mdSeqNumOptional, rebroadcastIndicator, mifidInstrumentIdType, mifidInstrumentId, mic, currency, leiCode, eventTime, quoteUpdateType, mifidPrice, mifidQuantity }, bytes)

@[simp] theorem encode_length (message : ApaQuotesMessage) : (encode message).length = 101 := by
  unfold encode
...
/-- Apa Full Trade Information Message: 333 bytes -/
...
@[simp] theorem encode_length (message : ApaFullTradeInformationMessage) : (encode message).length = 333 := by
  unfold encode
...
theorem encode_length_le (message : Payload) : (encode message).length ≤ 10220 := by
  cases message with
...
end Omi.EuronextOptiqMarketdatagatewaySbeV30
