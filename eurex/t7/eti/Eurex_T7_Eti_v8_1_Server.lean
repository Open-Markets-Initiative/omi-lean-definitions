...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ForcedUserLogoutNotification) : (encode message).length ≤ 4290838560 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
  simp only [List.length_append, Alpha.encode_length, NotifHeaderComp.encode_length, encodeUInt_length, encodeUIntLE_length, encodeMany_length_const Byte.encode 1 Byte.encode_length]
  omega
...
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 12706 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, PositionEffect.encode_length, encodeMany_length_const OrderBookItemGrpComp.encode 48 OrderBookItemGrpComp.encode_length]
  omega
...
theorem encode_length_le (message : SrqsNegotiationRequesterNotification) : (encode message).length ≤ 6426 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
...
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 12562 := by
  ...
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuotReqLegsGrpComp.encode 24 QuotReqLegsGrpComp.encode_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 4290869544 := by
...
theorem decode_encode (message : TesApproveBroadcast) : decode (encode message) = some message := by
  ...
  rw [decodeMany_bounded 2 Byte.encode Byte.decode Byte.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, message.varText.length_lt, message.alignmentPadding.length_le, ↓reduceDIte]
  rfl
...
theorem decode_encode (message : TesBroadcast) : decode (encode message) = some message := by
  ...
  rw [decodeMany_bounded 2 Byte.encode Byte.decode Byte.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.sideAllocGrpBcComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, message.varText.length_lt, message.alignmentPadding.length_le, ↓reduceDIte]
  rfl
...
theorem encode_length_le (message : TesTradeBroadcast) : (encode message).length ≤ 2458 := by
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TesUploadBroadcast) (rest : List UInt8) :
  ...
  rw [decodeMany_bounded 1 SrqsRelatedTradeIdGrpComp.encode SrqsRelatedTradeIdGrpComp.decode SrqsRelatedTradeIdGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.sideAllocExtGrpComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, message.srqsRelatedTradeIdGrpComp.length_lt, ↓reduceDIte]
  rfl
