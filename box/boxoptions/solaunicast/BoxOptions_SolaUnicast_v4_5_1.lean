...
/-!
# Box Options Market Sola Unicast v4.5.1
...
Note: Number Of Option Classes Requested counts Option Classes Requested in ascii digits: it is written from the list as its digits, and a list of more than 999 could not be written.
...
namespace Omi.BoxBoxoptionsSolaunicastHsvfV451

/-- Type Of Market Data: one byte code -/
def TypeOfMarketData.codes : List UInt8 :=
  [0x59, 0x4E, 0x54, 0x50, 0x49, 0x47]

inductive TypeOfMarketData where
  | marketDepthMessagesOnTheTop5BidsasksForTheTypeOfTradingInstrumentsChosen -- Market Depth Messages On The Top 5 Bidsasks For The Type Of Trading Instruments Chosen
  | best -- Best
  | trade -- Trade
  | priceImprovementAndExposedOrderMessages -- Price Improvement And Exposed Order Messages
  | marketDepthMessagesOnTheTop5AndTheCalculatedImpliedBestLimit -- Market Depth Messages On The Top 5 And The Calculated Implied Best Limit
  | quoteMessageOnTheFirstLevelAndTheCalculatedImpliedBestLimit -- Quote Message On The First Level And The Calculated Implied Best Limit
  | unlisted (byte : { byte : UInt8 // byte ∉ TypeOfMarketData.codes }) -- any other code, kept as it is
  deriving DecidableEq, Repr

namespace TypeOfMarketData

def toByte : TypeOfMarketData → UInt8
  | .marketDepthMessagesOnTheTop5BidsasksForTheTypeOfTradingInstrumentsChosen => 0x59
  | .best => 0x4E
  | .trade => 0x54
  | .priceImprovementAndExposedOrderMessages => 0x50
  | .marketDepthMessagesOnTheTop5AndTheCalculatedImpliedBestLimit => 0x49
  | .quoteMessageOnTheFirstLevelAndTheCalculatedImpliedBestLimit => 0x47
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : TypeOfMarketData :=
  if byte = 0x59 then .marketDepthMessagesOnTheTop5BidsasksForTheTypeOfTradingInstrumentsChosen
  else if byte = 0x4E then .best
  else if byte = 0x54 then .trade
  else if byte = 0x50 then .priceImprovementAndExposedOrderMessages
  else if byte = 0x49 then .marketDepthMessagesOnTheTop5AndTheCalculatedImpliedBestLimit
  else .quoteMessageOnTheFirstLevelAndTheCalculatedImpliedBestLimit

def ofByte (byte : UInt8) : TypeOfMarketData :=
  if known : byte ∈ codes then listed byte else .unlisted ⟨byte, known⟩

theorem ofByte_toByte (value : TypeOfMarketData) : ofByte value.toByte = value := by
  cases value with
  | marketDepthMessagesOnTheTop5BidsasksForTheTypeOfTradingInstrumentsChosen => decide
  | best => decide
  | trade => decide
  | priceImprovementAndExposedOrderMessages => decide
  | marketDepthMessagesOnTheTop5AndTheCalculatedImpliedBestLimit => decide
  | quoteMessageOnTheFirstLevelAndTheCalculatedImpliedBestLimit => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]

def encode (value : TypeOfMarketData) : List UInt8 :=
  [value.toByte]

def decode : List UInt8 → Option (TypeOfMarketData × List UInt8)
  | byte :: rest => some (ofByte byte, rest)
  | [] => none

@[simp] theorem encode_length (value : TypeOfMarketData) : (encode value).length = 1 :=
  rfl

@[simp] theorem decode_encode (value : TypeOfMarketData) (rest : List UInt8) :
    decode (encode value ++ rest) = some (value, rest) := by
  simp [decode, encode, ofByte_toByte]

end TypeOfMarketData

/-- Market Summaries: one byte code -/
def MarketSummaries.codes : List UInt8 :=
  [0x59, 0x4E]

inductive MarketSummaries where
  | yes -- Yes
  | no -- No
  | unlisted (byte : { byte : UInt8 // byte ∉ MarketSummaries.codes }) -- any other code, kept as it is
  deriving DecidableEq, Repr

namespace MarketSummaries

def toByte : MarketSummaries → UInt8
  | .yes => 0x59
  | .no => 0x4E
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : MarketSummaries :=
  if byte = 0x59 then .yes
  else .no

def ofByte (byte : UInt8) : MarketSummaries :=
  if known : byte ∈ codes then listed byte else .unlisted ⟨byte, known⟩

theorem ofByte_toByte (value : MarketSummaries) : ofByte value.toByte = value := by
  cases value with
  | yes => decide
  | no => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]

def encode (value : MarketSummaries) : List UInt8 :=
  [value.toByte]

def decode : List UInt8 → Option (MarketSummaries × List UInt8)
  | byte :: rest => some (ofByte byte, rest)
  | [] => none

@[simp] theorem encode_length (value : MarketSummaries) : (encode value).length = 1 :=
  rfl

@[simp] theorem decode_encode (value : MarketSummaries) (rest : List UInt8) :
    decode (encode value ++ rest) = some (value, rest) := by
  simp [decode, encode, ofByte_toByte]

end MarketSummaries

/-- Gap Control: one byte code -/
def GapControl.codes : List UInt8 :=
  [0x30, 0x31]

inductive GapControl where
  | willReceiveGapMessages -- Will Receive Gap Messages
  | willNotReceiveGapMessages -- Will Not Receive Gap Messages
  | unlisted (byte : { byte : UInt8 // byte ∉ GapControl.codes }) -- any other code, kept as it is
  deriving DecidableEq, Repr

namespace GapControl

def toByte : GapControl → UInt8
  | .willReceiveGapMessages => 0x30
  | .willNotReceiveGapMessages => 0x31
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : GapControl :=
  if byte = 0x30 then .willReceiveGapMessages
  else .willNotReceiveGapMessages

def ofByte (byte : UInt8) : GapControl :=
  if known : byte ∈ codes then listed byte else .unlisted ⟨byte, known⟩

theorem ofByte_toByte (value : GapControl) : ofByte value.toByte = value := by
  cases value with
  | willReceiveGapMessages => decide
  | willNotReceiveGapMessages => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]

def encode (value : GapControl) : List UInt8 :=
  [value.toByte]

def decode : List UInt8 → Option (GapControl × List UInt8)
  | byte :: rest => some (ofByte byte, rest)
  | [] => none

@[simp] theorem encode_length (value : GapControl) : (encode value).length = 1 :=
  rfl

@[simp] theorem decode_encode (value : GapControl) (rest : List UInt8) :
    decode (encode value ++ rest) = some (value, rest) := by
  simp [decode, encode, ofByte_toByte]
...
/-- Option Classes Requested: 6 bytes -/
structure OptionClassesRequested where
  rootSymbol : Alpha 6
  deriving DecidableEq, Repr

namespace OptionClassesRequested

def encode (message : OptionClassesRequested) : List UInt8 :=
  Alpha.encode message.rootSymbol

def decode (bytes : List UInt8) : Option (OptionClassesRequested × List UInt8) := do
  let (rootSymbol, bytes) ← Alpha.decode 6 bytes
  pure ({ rootSymbol }, bytes)

@[simp] theorem encode_length (message : OptionClassesRequested) : (encode message).length = 6 := by
  unfold encode
  simp only [Alpha.encode_length]

theorem encode_length_pos (message : OptionClassesRequested) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : OptionClassesRequested) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [Alpha.decode_encode, some_bind]
  rfl

end OptionClassesRequested

/-- Connection Message -/
structure ConnectionMessage where
  resetSequence : Alpha 10
  systemDefault : Alpha 1
  systemValue : Alpha 1
  typeOfMarketData : TypeOfMarketData
  complexOrder : Alpha 1
  marketSummaries : MarketSummaries
  gapControl : GapControl
  hsvfProtocol : Alpha 2
  optionClassesRequested : Digited 3 OptionClassesRequested
  deriving DecidableEq, Repr

namespace ConnectionMessage

def encode (message : ConnectionMessage) : List UInt8 :=
  Alpha.encode message.resetSequence
    ++ (Alpha.encode message.systemDefault
    ++ (Alpha.encode message.systemValue
    ++ (TypeOfMarketData.encode message.typeOfMarketData
    ++ (Alpha.encode message.complexOrder
    ++ (MarketSummaries.encode message.marketSummaries
    ++ (GapControl.encode message.gapControl
    ++ (Alpha.encode message.hsvfProtocol
    ++ (encodeDigits 3 message.optionClassesRequested.val.length
    ++ (encodeMany OptionClassesRequested.encode message.optionClassesRequested.val)))))))))

def decode (bytes : List UInt8) : Option (ConnectionMessage × List UInt8) := do
  let (resetSequence, bytes) ← Alpha.decode 10 bytes
  let (systemDefault, bytes) ← Alpha.decode 1 bytes
  let (systemValue, bytes) ← Alpha.decode 1 bytes
  let (typeOfMarketData, bytes) ← TypeOfMarketData.decode bytes
  let (complexOrder, bytes) ← Alpha.decode 1 bytes
  let (marketSummaries, bytes) ← MarketSummaries.decode bytes
  let (gapControl, bytes) ← GapControl.decode bytes
  let (hsvfProtocol, bytes) ← Alpha.decode 2 bytes
  let (numberOfOptionClassesRequested, bytes) ← decodeDigits 3 bytes
  let (optionClassesRequested_, bytes) ← decodeMany OptionClassesRequested.decode numberOfOptionClassesRequested bytes
  if fits_optionClassesRequested : optionClassesRequested_.length < 10 ^ 3 then
    pure ({ resetSequence, systemDefault, systemValue, typeOfMarketData, complexOrder, marketSummaries, gapControl, hsvfProtocol, optionClassesRequested := ⟨optionClassesRequested_, fits_optionClassesRequested⟩ }, bytes)
  else none

theorem encode_length_pos (message : ConnectionMessage) : (encode message).length > 0 := by
  unfold encode
  simp only [Alpha.encode_length, List.length_append, ← Nat.add_assoc]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ConnectionMessage) : (encode message).length ≤ 6015 := by
  have bound_optionClassesRequested := message.optionClassesRequested.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, TypeOfMarketData.encode_length, MarketSummaries.encode_length, GapControl.encode_length, encodeDigits_length, encodeMany_length_const OptionClassesRequested.encode 6 OptionClassesRequested.encode_length]
  omega

@[simp] theorem decode_encode (message : ConnectionMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, TypeOfMarketData.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, MarketSummaries.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, GapControl.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeDigits_encodeDigits _ _ message.optionClassesRequested.length_lt, some_bind]
  dsimp only
  rw [decodeMany_encodeMany OptionClassesRequested.encode OptionClassesRequested.decode OptionClassesRequested.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.optionClassesRequested.length_lt]
  rfl

end ConnectionMessage
...
/-- Gap Sequence Message: 9 bytes -/
structure GapSequenceMessage where
  sequenceNumbersSkipped : Alpha 9
  deriving DecidableEq, Repr

namespace GapSequenceMessage

def encode (message : GapSequenceMessage) : List UInt8 :=
  Alpha.encode message.sequenceNumbersSkipped

def decode (bytes : List UInt8) : Option (GapSequenceMessage × List UInt8) := do
  let (sequenceNumbersSkipped, bytes) ← Alpha.decode 9 bytes
  pure ({ sequenceNumbersSkipped }, bytes)

@[simp] theorem encode_length (message : GapSequenceMessage) : (encode message).length = 9 := by
  unfold encode
  simp only [Alpha.encode_length]

theorem encode_length_pos (message : GapSequenceMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : GapSequenceMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [Alpha.decode_encode, some_bind]
  rfl

end GapSequenceMessage
...
/-- Complex Order Group Status Message: 4 bytes -/
structure ComplexOrderGroupStatusMessage where
  exchangeId : Alpha 1
  groupOfTheComplexOrderInstrument : Alpha 2
  groupStatus : GroupStatus
  deriving DecidableEq, Repr

namespace ComplexOrderGroupStatusMessage

def encode (message : ComplexOrderGroupStatusMessage) : List UInt8 :=
  Alpha.encode message.exchangeId
    ++ (Alpha.encode message.groupOfTheComplexOrderInstrument
    ++ (GroupStatus.encode message.groupStatus))

def decode (bytes : List UInt8) : Option (ComplexOrderGroupStatusMessage × List UInt8) := do
  let (exchangeId, bytes) ← Alpha.decode 1 bytes
...
@[simp] theorem encode_length (message : ComplexOrderGroupStatusMessage) : (encode message).length = 4 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, GroupStatus.encode_length]

theorem encode_length_pos (message : ComplexOrderGroupStatusMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ComplexOrderGroupStatusMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
...
end ComplexOrderGroupStatusMessage
...
inductive MessageBody where
  | connectionMessage (message : ConnectionMessage) -- "RS" 0x5253
  ...
  | gapSequenceMessage (message : GapSequenceMessage) -- "W " 0x5720
  ...
  | complexOrderGroupStatusMessage (message : ComplexOrderGroupStatusMessage) -- "GS" 0x4753
...
def tag : MessageBody → BitVec 16
  ...
  | .complexOrderGroupStatusMessage _ => 18259
...
def encode : MessageBody → List UInt8
  | .connectionMessage message => ConnectionMessage.encode message
  ...
  | .gapSequenceMessage message => GapSequenceMessage.encode message
  ...
  | .complexOrderGroupStatusMessage message => ComplexOrderGroupStatusMessage.encode message
...
theorem encode_length_le (message : MessageBody) : (encode message).length ≤ 6015 := by
  cases message with
  | connectionMessage inner =>
    have bound_inner := ConnectionMessage.encode_length_le inner
    simp only [encode]
    omega
  ...
  | circuitAssuranceMessage inner =>
    ...
    omega
  | gapSequenceMessage inner =>
    simp only [encode, GapSequenceMessage.encode_length]
    omega
  ...
  | groupStatusMessage inner =>
    ...
    omega
  | complexOrderGroupStatusMessage inner =>
    simp only [encode, ComplexOrderGroupStatusMessage.encode_length]
    omega
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (MessageBody × List UInt8) :=
  if tag = 21075 then (ConnectionMessage.decode bytes).map fun (message, rest) => (.connectionMessage message, rest)
  ...
  else if tag = 22304 then (GapSequenceMessage.decode bytes).map fun (message, rest) => (.gapSequenceMessage message, rest)
  ...
  else if tag = 18259 then (ComplexOrderGroupStatusMessage.decode bytes).map fun (message, rest) => (.complexOrderGroupStatusMessage message, rest)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : Packet) : (encode message).length ≤ 6028 := by
  unfold encode
  ...
  | circuitAssuranceMessage inner =>
    ...
    omega
  | gapSequenceMessage inner =>
    simp only [MessageBody.encode, List.length_append, ← Nat.add_assoc, encodeUInt_length, Alpha.encode_length, GapSequenceMessage.encode_length]
    omega
  | systemTimestampMessage inner =>
  ...
  | groupStatusMessage inner =>
    ...
    omega
  | complexOrderGroupStatusMessage inner =>
    simp only [MessageBody.encode, List.length_append, ← Nat.add_assoc, encodeUInt_length, Alpha.encode_length, ComplexOrderGroupStatusMessage.encode_length]
    omega
  | bulletinsMessage inner =>
...
end Omi.BoxBoxoptionsSolaunicastHsvfV451
