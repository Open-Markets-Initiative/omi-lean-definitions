...
namespace Omi.CmeGlobexIlink3SbeV88Server
...
theorem encode_length_le (message : SpreadLegOrderEventsGroups) : (encode message).length ≤ 5868 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SpreadLegOrderEventsGroup.encode 23 SpreadLegOrderEventsGroup.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeSpreadLeg) : (encode message).length ≤ 9915 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
...
def decode (bytes : List UInt8) : Option (ExecutionReportStatus × List UInt8) := do
  ...
  pure ({ seqNum, uuid, text, execId, senderId, clordid, partyDetailsListReqId, orderId, priceOptional, stopPx, transactTime, sendingTimeEpoch, orderRequestId, ordStatusReqIdOptional, massStatusReqIdOptional, crossIdOptional, hostCrossId, location, securityId, orderQty, cumQty, leaves, minQty, displayQty, expireDate, ordStatus, ordTypeOptional, side, timeInForce, manualOrderIndicator, possRetransFlag, lastRptRequested, crossType, execInst, executionMode, liquidityFlag, managedOrder, shortSaleType, discretionPrice, reservationPrice, priorityIndicator, origOrderUser, cancelText }, bytes)
...
def decode (bytes : List UInt8) : Option (MassQuoteAck × List UInt8) := do
  ...
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, requestTime, sendingTimeEpoch, quoteReqIdOptional, location, quoteId, quoteRejectReason, delayDuration, quoteAckStatus, manualOrderIndicator, noProcessedEntries, mmProtectionReset, splitMsg, liquidityFlag, shortSaleType, totNoQuoteEntriesOptional, possRetransFlag, delayToTime, quoteEntryOpen, quoteAckEntriesGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MassQuoteAck) : (encode message).length ≤ 3159 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeAddendumOutright) : (encode message).length ≤ 15528 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
...
theorem encode_length_le (message : SpreadLegTradeEventsGroups) : (encode message).length ≤ 6888 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SpreadLegTradeEventsGroup.encode 27 SpreadLegTradeEventsGroup.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (ExecutionReportTradeAddendumSpreadLeg × List UInt8) := do
  ...
  pure ({ seqNum, uuid, execId, senderId, clordid, partyDetailsListReqId, lastPx, orderId, transactTime, sendingTimeEpoch, secExecId, origSecondaryExecutionId, location, securityId, lastQty, sideTradeId, origSideTradeId, tradeDate, ordStatusTrdCxl, execType, manualOrderIndicator, possRetransFlag, side, settlDate, calculatedCcyLastQty, grossTradeAmt, fillsGroups, spreadLegTradeEventsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeAddendumSpreadLeg) : (encode message).length ≤ 10912 := by
  have bound_fillsGroups := FillsGroups.encode_length_le message.fillsGroups
...
def decode (bytes : List UInt8) : Option (QuoteCancelAck × List UInt8) := do
  ...
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, requestTime, sendingTimeEpoch, cancelledSymbol, location, quoteId, quoteRejectReason, delayDuration, manualOrderIndicator, quoteCxlStatus, noProcessedEntries32, mmProtectionReset, unsolicitedCancelType, splitMsg, totNoQuoteEntriesOptional, liquidityFlag, possRetransFlag, delayToTime, origOrderUser, cancelText, quoteEntryOpen, quoteCancelAckEntriesGroups, quoteCancelAckSetsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteCancelAck) : (encode message).length ≤ 3689 := by
...
end Omi.CmeGlobexIlink3SbeV88Server
