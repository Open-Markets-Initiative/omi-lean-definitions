...
/-!
# Euronext Order Entry Gateway v5.19
...
namespace Omi.EuronextOptiqOrderentrygatewaySbeV519
...
/-- User Notification Message: 55 bytes -/
...
def decode (bytes : List UInt8) : Option (UserNotificationMessage × List UInt8) := do
  ...
  let (notUsedGroups, bytes) ← NotUsedGroups.decode bytes
  pure ({ msgSeqNum, firmId, executionWithinFirmShortCodeOptional, clientIdentificationShortcode, familyId, symbolIndexOptional, userStatus, logicalAccessIdOptional, orderSizeLimit, orderAmountLimit, notUsedGroups }, bytes)

@[simp] theorem encode_length (message : UserNotificationMessage) : (encode message).length = 55 := by
  unfold encode
...
end Omi.EuronextOptiqOrderentrygatewaySbeV519
