...
/-!
# Brasil, Bolsa, Balcão Binary Entry Point v8.0
...
namespace Omi.B3B3derivativesBinaryentrypointSbeV80
...
def decode (bytes : List UInt8) : Option (NewOrderSingleMessage × List UInt8) := do
  ...
  let (memo, bytes) ← Memo.decode bytes
  pure ({ inboundBusinessHeader, ordtagid, mmProtectionReset, clordid, account, senderLocation, enteringTrader, selfTradePreventionInstruction, securityId, side, ordtype, timeInForce, routingInstruction, orderQty, priceOptional, stopPx, minQty, maxFloor, executingTraderOptional, expireDate, custodianInfo, investorId, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderSingleMessage) : (encode message).length ≤ 639 := by
...
def decode (bytes : List UInt8) : Option (OrderCancelReplaceRequestMessage × List UInt8) := do
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ inboundBusinessHeader, ordtagid, mmProtectionReset, clordid, account, senderLocation, enteringTrader, selfTradePreventionInstruction, securityId, side, ordtype, timeInForceOptional, routingInstruction, orderQty, priceOptional, orderIdOptional, origclordid, stopPx, minQty, maxFloor, executingTraderOptional, accountType, expireDate, custodianInfo, investorId, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderCancelReplaceRequestMessage) : (encode message).length ≤ 656 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
theorem encode_length_le (message : ExecutionReportCancelMessage) : (encode message).length ≤ 668 := by
  ...
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, OrdStatus.encode_length, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length, Ordtype.encode_length, TimeInForce.encode_length]
  omega
...
end Omi.B3B3derivativesBinaryentrypointSbeV80
