...
theorem encode_length_le (message : AddComplexInstrument) : (encode message).length ≤ 8232 := by
  ...
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, encodeMany_length_const InstrmtLegGrpComp.encode 32 InstrmtLegGrpComp.encode_length]
  omega
