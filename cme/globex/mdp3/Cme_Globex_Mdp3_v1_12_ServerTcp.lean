...
@[simp] theorem decode_encode (message : MdInstrumentDefinitionRepo) (rest : List UInt8) :
  ...
  dsimp only
  rw [RepoRelatedInstrumentsGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 44153 := by
  cases message with
  | channelReset inner =>
...
def decode (bytes : List UInt8) : Option (ServerTcpFrame × List UInt8) := do
  ...
  pure ({ serverTechnicalHeader, serverTcpMessage }, bytes)
...
@[simp] theorem decode_encode (message : ServerTcpFrame) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, ServerTechnicalHeader.decode_encode, some_bind]
  dsimp only
...
end Omi.CmeGlobexMdp3SbeV112ServerTcp
