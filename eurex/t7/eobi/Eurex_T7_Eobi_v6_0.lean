...
theorem encode_length_le (message : InstrumentSummary) : (encode message).length ≤ 4112 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const MdInstrumentEntryGrpComp.encode 16 MdInstrumentEntryGrpComp.encode_length]
  omega
