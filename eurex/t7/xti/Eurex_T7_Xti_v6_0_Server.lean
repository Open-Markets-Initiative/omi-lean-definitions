...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v6.0
...
Note: Best Quote Execution Report is not framed: its length Body Len is not the integer that leads it.

Note: Best Quote Response is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7XtiFbeV60Server
...
/-- Best Quote Execution Report: 114 bytes -/
...
def encode (message : BestQuoteExecutionReport) : List UInt8 :=
  Alpha.encode message.pad2
    ...
    ++ encodeUIntLE 4 message.reservedSize
...
def decode (bytes : List UInt8) : Option (BestQuoteExecutionReport × List UInt8) := do
  ...
  let (reservedSize, bytes) ← decodeUIntLE 4 bytes
  ...
  pure ({ pad2, rbcHeaderMeComp, execId, quoteMsgId, securityId, quoteEventPx, marketSegmentId, quoteEventMatchId, quoteEventExecId, quoteEventQty, reservedSize, quoteEventType, quoteEventSide, pad2v2 }, bytes)

@[simp] theorem encode_length (message : BestQuoteExecutionReport) : (encode message).length = 114 := by
  unfold encode
...
theorem decode_encode_nil (message : BestQuoteExecutionReport) : decode (encode message) = some (message, []) := by
...
def decode (bytes : List UInt8) : Option (BestQuoteResponse × List UInt8) := do
  ...
  pure ({ pad2, nrResponseHeaderMeComp, quoteId, quoteResponseId, securityId, marketSegmentId, bidCxlSize, offerCxlSize, pad4 }, bytes)
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : BestQuoteResponse) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
def decode (bytes : List UInt8) : Option (DeleteAllOrderBroadcast × List UInt8) := do
  ...
  if fits_notAffectedOrdersGrpComp : notAffectedOrdersGrpComp_.length < 256 ^ 2 then
    if fits_affectedOrdGrpComp : affectedOrdGrpComp_.length < 256 ^ 2 then
      pure ({ pad2, rbcHeaderMeComp, massActionReportId, securityId, price, marketSegmentId, targetPartyIdSessionId, targetPartyIdExecutingTrader, partyIdEnteringTrader, partyIdEnteringFirm, massActionReason, execInst, side, notAffectedOrdersGrpComp := ⟨notAffectedOrdersGrpComp_, fits_notAffectedOrdersGrpComp⟩, affectedOrdGrpComp := ⟨affectedOrdGrpComp_, fits_affectedOrdGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : DeleteAllOrderBroadcast) : (encode message).length ≤ 2097226 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const NotAffectedOrdersGrpComp.encode 16 NotAffectedOrdersGrpComp.encode_length, encodeMany_length_const AffectedOrdGrpComp.encode 16 AffectedOrdGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : DeleteAllOrderBroadcast) (rest : List UInt8) :
  ...
  dsimp only
  simp only [message.notAffectedOrdersGrpComp.length_lt, message.affectedOrdGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (DeleteAllOrderResponse × List UInt8) := do
  ...
  if fits_notAffectedOrdersGrpComp : notAffectedOrdersGrpComp_.length < 256 ^ 2 then
    if fits_affectedOrdGrpComp : affectedOrdGrpComp_.length < 256 ^ 2 then
      pure ({ pad2, responseHeaderMeComp, massActionReportId, pad4, notAffectedOrdersGrpComp := ⟨notAffectedOrdersGrpComp_, fits_notAffectedOrdersGrpComp⟩, affectedOrdGrpComp := ⟨affectedOrdGrpComp_, fits_affectedOrdGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : DeleteAllOrderResponse) : (encode message).length ≤ 2097210 := by
  have bound_notAffectedOrdersGrpComp := message.notAffectedOrdersGrpComp.length_lt
  have bound_affectedOrdGrpComp := message.affectedOrdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeMany_length_const NotAffectedOrdersGrpComp.encode 16 NotAffectedOrdersGrpComp.encode_length, encodeMany_length_const AffectedOrdGrpComp.encode 16 AffectedOrdGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (DeleteOrderBroadcast × List UInt8) := do
  ...
  let (pad2v2, bytes) ← Alpha.decode 2 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, orderIdSfx, cumQty, cxlQty, marketSegmentId, partyIdEnteringTrader, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, side, fixClOrdId, pad2v2 }, bytes)
...
def decode (bytes : List UInt8) : Option (DeleteOrderNrResponse × List UInt8) := do
  ...
  let (execType, bytes) ← ExecType.decode bytes
  let (execRestatementReason, bytes) ← decodeUIntLE 2 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, orderIdSfx, cumQty, cxlQty, ordStatus, execType, execRestatementReason }, bytes)

@[simp] theorem encode_length (message : DeleteOrderNrResponse) : (encode message).length = 114 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, NrResponseHeaderMeComp.encode_length, encodeUIntLE_length, OrdStatus.encode_length, ExecType.encode_length]
...
def decode (bytes : List UInt8) : Option (DeleteOrderResponse × List UInt8) := do
  ...
  let (execType, bytes) ← ExecType.decode bytes
  let (execRestatementReason, bytes) ← decodeUIntLE 2 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, orderIdSfx, cumQty, cxlQty, ordStatus, execType, execRestatementReason }, bytes)

@[simp] theorem encode_length (message : DeleteOrderResponse) : (encode message).length = 130 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderMeComp.encode_length, encodeUIntLE_length, OrdStatus.encode_length, ExecType.encode_length]
...
/-- Extended Deletion Report: 282 bytes -/
...
def decode (bytes : List UInt8) : Option (ExtendedDeletionReport × List UInt8) := do
  ...
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, price, stopPx, volumeDiscoveryPrice, pegOffsetValueAbs, pegOffsetValuePct, marketSegmentId, orderIdSfx, leavesQty, cumQty, cxlQty, orderQty, displayQty, displayLowQty, displayHighQty, expireDate, matchInstCrossId, partyIdExecutingUnit, partyIdSessionId, partyIdExecutingTrader, partyIdEnteringTrader, execRestatementReason, ordStatus, execType, side, ordType, tradingCapacity, timeInForce, execInst, tradingSessionSubId, applSeqIndicator, exDestinationType, freeText1, freeText2, freeText4, fixClOrdId, triggered, pad3 }, bytes)

@[simp] theorem encode_length (message : ExtendedDeletionReport) : (encode message).length = 282 := by
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ForcedUserLogoutNotification) : (encode message).length ≤ 4291887120 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
/-- Modify Order Nr Response: 138 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderNrResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, stopPx, orderIdSfx, leavesQty, cumQty, cxlQty, displayQty, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, pad6 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderNrResponse) : (encode message).length = 138 := by
  unfold encode
...
/-- Modify Order Response: 162 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, stopPx, trdRegTsTimePriority, orderIdSfx, leavesQty, cumQty, cxlQty, displayQty, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, pad6 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderResponse) : (encode message).length = 162 := by
  unfold encode
...
/-- New Order Nr Response: 106 bytes -/
...
def decode (bytes : List UInt8) : Option (NewOrderNrResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, securityId, execId, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, pad6 }, bytes)

@[simp] theorem encode_length (message : NewOrderNrResponse) : (encode message).length = 106 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, NrResponseHeaderMeComp.encode_length, encodeUIntLE_length, OrdStatus.encode_length, ExecType.encode_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (NewOrderResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, orderIdSfx, ordStatus, execType, execRestatementReason, crossedIndicator, triggered, pad6 }, bytes)

@[simp] theorem encode_length (message : NewOrderResponse) : (encode message).length = 138 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderMeComp.encode_length, encodeUIntLE_length, OrdStatus.encode_length, ExecType.encode_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (OrderExecNotification × List UInt8) := do
  ...
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, marketSegmentId, orderIdSfx, leavesQty, cumQty, cxlQty, displayQty, execRestatementReason, side, ordStatus, execType, matchType, triggered, crossedIndicator, fixClOrdId, pad3, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecNotification) : (encode message).length ≤ 6274 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 24 FillsGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (OrderExecReportBroadcast × List UInt8) := do
  ...
  let (fillsGrpComp_, bytes) ← decodeMany FillsGrpComp.decode noFills.toNat bytes
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, price, stopPx, volumeDiscoveryPrice, pegOffsetValueAbs, pegOffsetValuePct, marketSegmentId, orderIdSfx, leavesQty, cumQty, cxlQty, orderQty, displayQty, displayLowQty, displayHighQty, expireDate, matchInstCrossId, partyIdExecutingUnit, partyIdSessionId, partyIdExecutingTrader, partyIdEnteringTrader, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, matchType, side, ordType, tradingCapacity, timeInForce, execInst, tradingSessionSubId, applSeqIndicator, exDestinationType, freeText1, freeText2, freeText4, fixClOrdId, triggered, crossedIndicator, pad7, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 6418 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 24 FillsGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (OrderExecResponse × List UInt8) := do
  ...
  let (fillsGrpComp_, bytes) ← decodeMany FillsGrpComp.decode noFills.toNat bytes
  if fits_fillsGrpComp : fillsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, marketSegmentId, orderIdSfx, leavesQty, cumQty, cxlQty, displayQty, execRestatementReason, side, ordStatus, execType, matchType, triggered, crossedIndicator, pad7, fillsGrpComp := ⟨fillsGrpComp_, fits_fillsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecResponse) : (encode message).length ≤ 6290 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 24 FillsGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteExecutionReport) : (encode message).length ≤ 10274 := by
  have bound_quoteEventGrpComp := message.quoteEventGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteEventGrpComp.encode 40 QuoteEventGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (RfqBroadcast × List UInt8) := do
  ...
  let (pad2v2, bytes) ← Alpha.decode 2 bytes
  pure ({ pad2, rbcHeaderMeComp, securityId, execId, marketSegmentId, orderQty, side, partyExecutingFirm, pad2v2 }, bytes)

@[simp] theorem encode_length (message : RfqBroadcast) : (encode message).length = 90 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : Reject) : (encode message).length ≤ 4291887160 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
def decode (bytes : List UInt8) : Option (ServiceAvailabilityBroadcast × List UInt8) := do
  ...
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ pad2, nrbcHeaderComp, matchingEngineTradeDate, tradeManagerTradeDate, applSeqTradeDate, partitionId, matchingEngineStatus, tradeManagerStatus, applSeqStatus, pad7 }, bytes)
...
def decode (bytes : List UInt8) : Option (TradeBroadcast × List UInt8) := do
  ...
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ pad2, rbcHeaderComp, securityId, price, lastPx, settlCurrAmt, settlCurrFxRate, transactTime, orderId, clOrdId, tradeId, origTradeId, rootPartyIdExecutingUnit, rootPartyIdSessionId, rootPartyIdExecutingTrader, rootPartyIdSettlementUnit, rootPartyIdClearingUnit, rootPartyIdContraUnit, rootPartyIdContraSettlementUnit, orderIdSfx, cumQty, leavesQty, marketSegmentId, lastQty, sideTradeId, sideTradeReportId, tradeNumber, matchDate, settlDate, trdMatchId, lastMkt, tradeReportType, transferReason, matchType, matchSubType, side, sideLiquidityInd, deliveryType, tradingCapacity, account, settlCurrency, currency, freeText1, freeText2, freeText4, orderCategory, ordType, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyClearingFirm, rootPartyExecutingFirmKvNumber, rootPartySettlementAccount, rootPartySettlementLocation, rootPartySettlementFirm, rootPartyContraFirm, rootPartyContraSettlementFirm, rootPartyContraFirmKvNumber, rootPartyContraSettlementAccount, rootPartyContraSettlementLocation, pad1 }, bytes)

@[simp] theorem encode_length (message : TradeBroadcast) : (encode message).length = 354 := by
  unfold encode
...
/-- Trailing Stop Update Notification: 146 bytes -/
...
def decode (bytes : List UInt8) : Option (TrailingStopUpdateNotification × List UInt8) := do
  ...
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, stopPx, orderIdSfx, marketSegmentId, orderQty, execRestatementReason, ordStatus, execType, side, fixClOrdId, pad3 }, bytes)

@[simp] theorem encode_length (message : TrailingStopUpdateNotification) : (encode message).length = 146 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, OrdStatus.encode_length, ExecType.encode_length, encodeUInt_length]
...
end Omi.EurexT7XtiFbeV60Server
