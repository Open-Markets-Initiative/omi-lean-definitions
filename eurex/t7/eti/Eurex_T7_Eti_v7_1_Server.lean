...
/-!
# Eurex Exchange Enhanced Trading Interface v7.1
...
namespace Omi.EurexT7EtiFbeV71Server
...
def decode (bytes : List UInt8) : Option (BasketApproveBroadcast × List UInt8) := do
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_basketSideAllocExtBcGrpComp : basketSideAllocExtBcGrpComp_.length < 256 ^ 1 then
      pure ({ pad2, rbcHeaderComp, basketTrdMatchId, transactTime, basketExecId, marketSegmentId, trdType, tradeReportType, basketTradeReportType, messageEventSource, partyIdEnteringFirm, partyEnteringTrader, basketTradeReportText, tradeReportId, pad2v2, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, basketSideAllocExtBcGrpComp := ⟨basketSideAllocExtBcGrpComp_, fits_basketSideAllocExtBcGrpComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : BasketApproveBroadcast) : (encode message).length ≤ 61314 := by
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_instrmtMatchSideGrpComp : instrmtMatchSideGrpComp_.length < 256 ^ 1 then
      if fits_basketSideAllocGrpComp : basketSideAllocGrpComp_.length < 256 ^ 1 then
        pure ({ pad2, rbcHeaderComp, basketTrdMatchId, transactTime, basketExecId, marketSegmentId, maturityMonthYear, trdType, tradeReportType, basketTradeReportType, messageEventSource, basketTradeReportText, tradeReportId, pad4, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, instrmtMatchSideGrpComp := ⟨instrmtMatchSideGrpComp_, fits_instrmtMatchSideGrpComp⟩, basketSideAllocGrpComp := ⟨basketSideAllocGrpComp_, fits_basketSideAllocGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
/-- Basket Delete Broadcast: 122 bytes -/
...
def decode (bytes : List UInt8) : Option (BasketDeleteBroadcast × List UInt8) := do
  ...
  let (tradeReportId, bytes) ← Alpha.decode 20 bytes
  let (basketSideTradeReportId, bytes) ← Alpha.decode 20 bytes
  pure ({ pad2, rbcHeaderComp, basketTrdMatchId, transactTime, marketSegmentId, basketExecId, trdType, deleteReason, messageEventSource, basketTradeReportText, tradeReportId, basketSideTradeReportId }, bytes)

@[simp] theorem encode_length (message : BasketDeleteBroadcast) : (encode message).length = 122 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (BasketExecutionBroadcast × List UInt8) := do
  ...
  if fits_basketExecGrpComp : basketExecGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, basketTrdMatchId, transactTime, basketExecId, marketSegmentId, trdType, tradeReportType, messageEventSource, basketSideTradeReportId, pad7, basketExecGrpComp := ⟨basketExecGrpComp_, fits_basketExecGrpComp⟩ }, bytes)
  else none
...
/-- Quote Entry Ack Grp Comp: 32 bytes -/
...
@[simp] theorem encode_length (message : QuoteEntryAckGrpComp) : (encode message).length = 32 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MassQuoteResponse) : (encode message).length ≤ 8234 := by
  have bound_quoteEntryAckGrpComp := message.quoteEntryAckGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, NrResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteEntryAckGrpComp.encode 32 QuoteEntryAckGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 12594 := by
  have bound_orderBookItemGrpComp := message.orderBookItemGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const OrderBookItemGrpComp.encode 48 OrderBookItemGrpComp.encode_length]
  omega
...
/-- Srqs Deal Notification: 290 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsDealNotification × List UInt8) := do
  ...
  let (firmNegotiationId, bytes) ← Alpha.decode 20 bytes
  let (freeText5, bytes) ← Alpha.decode 132 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, underlyingPriceStipValue, underlyingPx, lastPx, lastQty, negotiationId, tradeId, trdRptStatus, messageEventSource, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, targetPartyEnteringTrader, firmTradeId, firmNegotiationId, freeText5 }, bytes)

@[simp] theorem encode_length (message : SrqsDealNotification) : (encode message).length = 290 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length]
...
/-- Srqs Negotiation Notification: 298 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationNotification × List UInt8) := do
  ...
  let (freeText5, bytes) ← Alpha.decode 132 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, lastPx, leavesQty, lastQty, negotiationId, numberOfRespondents, quoteType, quoteSubType, quoteStatus, quoteInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, freeText5, pad6 }, bytes)

@[simp] theorem encode_length (message : SrqsNegotiationNotification) : (encode message).length = 298 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (TargetPartiesComp × List UInt8) := do
  ...
  let (targetPartyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ sideDisclosureInstruction, priceDisclosureInstruction, leavesQtyDisclosureInstruction, lastPxDisclosureInstruction, lastQtyDisclosureInstruction, freeText5DisclosureInstruction, quoteInstruction, targetPartyExecutingFirm, targetPartyExecutingTrader, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, trdRegTsExecutionTime, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, orderQty, lastPx, leavesQty, lastQty, negotiationId, numberOfRespondents, quoteType, quoteSubType, quoteStatus, numberOfRespDisclosureInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, freeText5, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsNegotiationRequesterNotification) : (encode message).length ≤ 6418 := by
...
/-- Srqs Negotiation Status Notification: 74 bytes -/
...
@[simp] theorem encode_length (message : SrqsNegotiationStatusNotification) : (encode message).length = 74 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationNotification × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, negotiationStartTime, securityId, bidPx, offerPx, leavesQty, lastQty, lastPx, quoteRefPrice, underlyingDeltaPercentage, negotiationId, marketSegmentId, securitySubType, numberOfRespondents, quoteType, quoteSubType, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, productComplex, negotiateUnderlying, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, freeText5, pad3, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationNotification) : (encode message).length ≤ 6442 := by
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ pad2, rbcHeaderComp, transactTime, securityId, bidPx, offerPx, orderQty, lastPx, lastQty, quoteRefPrice, underlyingDeltaPercentage, negotiationId, marketSegmentId, securitySubType, numberOfRespondents, quoteType, quoteSubType, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, negotiateUnderlying, productComplex, numberOfRespDisclosureInstruction, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, freeText5, pad4, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 12546 := by
  have bound_quotReqLegsGrpComp := message.quotReqLegsGrpComp.length_lt
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
...
/-- Srqs Quote Notification: 298 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsQuoteNotification × List UInt8) := do
  ...
  let (freeText5, bytes) ← Alpha.decode 132 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteId, secondaryQuoteId, bidPx, bidSize, offerPx, offerSize, underlyingPx, underlyingDeltaPercentage, negotiationId, quotingStatus, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, quoteReqId, freeText1, freeText5, pad6 }, bytes)

@[simp] theorem encode_length (message : SrqsQuoteNotification) : (encode message).length = 298 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- Trd Instrmnt Leg Grp Comp: 16 bytes -/
...
def decode (bytes : List UInt8) : Option (TrdInstrmntLegGrpComp × List UInt8) := do
  let (legSecurityId, bytes) ← decodeUIntLE 8 bytes
  let (legPrice, bytes) ← decodeUIntLE 8 bytes
  pure ({ legSecurityId, legPrice }, bytes)

@[simp] theorem encode_length (message : TrdInstrmntLegGrpComp) : (encode message).length = 16 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 28962 := by
...
@[simp] theorem decode_encode (message : TesApproveBroadcast) (rest : List UInt8) :
  ...
  dsimp only
  rw [dite_eq_left message.trdInstrmntLegGrpComp.length_lt, dite_eq_left message.instrumentEventGrpComp.length_lt, dite_eq_left message.instrumentAttributeGrpComp.length_lt, dite_eq_left message.underlyingStipGrpComp.length_lt, dite_eq_left message.srqsRelatedTradeIdGrpComp.length_lt]
  rfl
    ...
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
              pure ({ pad2, rbcHeaderComp, securityId, lastPx, transactTime, underlyingPx, transBkdTime, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad1, trdType, tradeReportType, productComplex, tradePublishIndicator, partyIdSettlementLocation, hedgeType, messageEventSource, tradeReportText, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad3, sideAllocGrpBcComp := ⟨sideAllocGrpBcComp_, fits_sideAllocGrpBcComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
            else none
          else none
        else none
      else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 36994 := by
  ...
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocGrpBcComp.encode 32 SideAllocGrpBcComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (TesTradeBroadcast × List UInt8) := do
  ...
  let (basketSideTradeReportId, bytes) ← Alpha.decode 20 bytes
  let (pad2v1, bytes) ← Alpha.decode 2 bytes
  let (srqsRelatedTradeIdGrpComp_, bytes) ← decodeMany SrqsRelatedTradeIdGrpComp.decode noSrqsRelatedTradeIDs.toNat bytes
  if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, securityId, lastPx, lastQty, clearingTradePrice, clearingTradeQty, transactTime, relatedSecurityId, rootPartyIdClientId, executingTrader, rootPartyIdInvestmentDecisionMaker, basketTrdMatchId, packageId, marketSegmentId, tradeId, tradeDate, sideTradeId, rootPartyIdSessionId, origTradeId, rootPartyIdExecutingUnit, rootPartyIdExecutingTrader, rootPartyIdClearingUnit, strategyLinkId, relatedSymbol, totNumTradeReports, negotiationId, pad1, trdType, productComplex, relatedProductComplex, side, tradingCapacity, tradeReportType, transferReason, tradePublishIndicator, multiLegReportingType, positionEffect, multilegPriceModel, orderAttributeLiquidityProvision, orderAttributeRiskReduction, executingTraderQualifier, rootPartyIdInvestmentDecisionMakerQualifier, account, rootPartyIdPositionAccount, custOrderHandlingInst, freeText1, freeText2, freeText3, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyClearingFirm, rootPartyClearingOrganization, rootPartyIdBeneficiary, rootPartyIdTakeUpTradingFirm, rootPartyIdOrderOriginationFirm, rootPartyIdExecutionVenue, regulatoryTradeId, basketSideTradeReportId, pad2v1, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesTradeBroadcast) : (encode message).length ≤ 2426 := by
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
...
theorem encode_length_le (message : TesUploadBroadcast) : (encode message).length ≤ 73698 := by
  ...
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocExtGrpComp.encode 176 SideAllocExtGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
end Omi.EurexT7EtiFbeV71Server
