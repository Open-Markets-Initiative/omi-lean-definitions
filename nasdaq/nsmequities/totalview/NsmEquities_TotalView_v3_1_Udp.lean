...
namespace Omi.NasdaqNsmequitiesTotalviewItchV31Udp
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 0 < 256 ^ 2 := by
  ...
  | secondsMessage inner =>
    ...
    omega
  | millisecondsMessage inner =>
    simp only [Payload.encode, List.length_append, encodeUInt_length, MillisecondsMessage.encode_length]
    omega
  | systemEventMessage inner =>
...
end Omi.NasdaqNsmequitiesTotalviewItchV31Udp
