...
/-!
# Eurex Exchange Enhanced Trading Interface v8.1
...
namespace Omi.EurexT7EtiFbeV81Server
...
def decode (bytes : List UInt8) : Option (ClipResponse × List UInt8) := do
  ...
  if fits_crossRequestAckSideGrpComp : crossRequestAckSideGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, nrResponseHeaderMeComp, execId, securityId, marketSegmentId, pad3, crossRequestId, pad4, crossRequestAckSideGrpComp := ⟨crossRequestAckSideGrpComp_, fits_crossRequestAckSideGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 12706 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, PositionEffect.encode_length, encodeMany_length_const OrderBookItemGrpComp.encode 48 OrderBookItemGrpComp.encode_length]
  omega
...
@[simp] theorem encode_length (message : SrqsDealNotification) : (encode message).length = 418 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, PositionEffect.encode_length]
...
/-- Srqs Negotiation Notification: 306 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationNotification × List UInt8) := do
  ...
  let (freeText5, bytes) ← Alpha.decode 132 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, lastPx, leavesQty, lastQty, effectiveTime, negotiationId, numberOfRespondents, quoteType, quoteSubType, quoteStatus, quoteInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, freeText5, pad6 }, bytes)

@[simp] theorem encode_length (message : SrqsNegotiationNotification) : (encode message).length = 306 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, trdRegTsExecutionTime, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, orderQty, lastPx, leavesQty, lastQty, effectiveTime, negotiationId, numberOfRespondents, quoteType, quoteSubType, quoteStatus, numberOfRespDisclosureInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, freeText5, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsNegotiationRequesterNotification) : (encode message).length ≤ 6426 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationNotification) : (encode message).length ≤ 6458 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 12562 := by
  have bound_quotReqLegsGrpComp := message.quotReqLegsGrpComp.length_lt
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuotReqLegsGrpComp.encode 24 QuotReqLegsGrpComp.encode_length, encodeMany_length_const TargetPartiesComp.encode 24 TargetPartiesComp.encode_length]
  omega
      ...
      if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
        if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
          if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
            if fits_varText : varText_.length < 256 ^ 2 then
              if fits_alignmentPadding : alignmentPadding_.length ≤ 7 then
                pure { pad2, rbcHeaderComp, securityId, lastPx, allocQty, transactTime, underlyingPx, transBkdTime, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, packageId, tesExecId, allocId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad3, tesEnrichmentRuleId, autoApprovalRuleId, trdType, side, tradePublishIndicator, productComplex, tradeReportType, tradingCapacity, partyIdSettlementLocation, tradeAllocStatus, hedgeType, messageEventSource, tradeReportId, partyExecutingFirm, partyExecutingTrader, partyIdEnteringFirm, partyEnteringTrader, positionEffect, rootPartyExecutingFirm, rootPartyExecutingTrader, freeText1, freeText2, freeText3, partyIdTakeUpTradingFirm, account, partyIdPositionAccount, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdLocationId, custOrderHandlingInst, complianceText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩, varText := ⟨varText_, fits_varText⟩, alignmentPadding := ⟨alignmentPadding_, fits_alignmentPadding⟩ }
              else none
            else none
          else none
        else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 96552 := by
  ...
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, PositionEffect.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 24 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length, encodeMany_length_const Byte.encode 1 Byte.encode_length]
  omega
        ...
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
              if fits_varText : varText_.length < 256 ^ 2 then
                if fits_alignmentPadding : alignmentPadding_.length ≤ 7 then
                  pure { pad2, rbcHeaderComp, securityId, lastPx, transactTime, underlyingPx, transBkdTime, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad3, autoApprovalRuleId, trdType, tradeReportType, productComplex, tradePublishIndicator, partyIdSettlementLocation, hedgeType, messageEventSource, tradeReportText, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad3v1, sideAllocGrpBcComp := ⟨sideAllocGrpBcComp_, fits_sideAllocGrpBcComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩, varText := ⟨varText_, fits_varText⟩, alignmentPadding := ⟨alignmentPadding_, fits_alignmentPadding⟩ }
                else none
              else none
            else none
          else none
...
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 104584 := by
  ...
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocGrpBcComp.encode 32 SideAllocGrpBcComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 24 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length, encodeMany_length_const Byte.encode 1 Byte.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesTradeBroadcast) : (encode message).length ≤ 2458 := by
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
    ...
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            if fits_srqsRelatedTradeIdGrpComp : srqsRelatedTradeIdGrpComp_.length < 256 ^ 1 then
              pure ({ pad2, rbcHeaderComp, securityId, lastPx, transBkdTime, transactTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, pad1, trdType, productComplex, tradeReportType, tradePublishIndicator, tradePlatform, hedgeType, partyIdSettlementLocation, messageEventSource, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad6, sideAllocExtGrpComp := ⟨sideAllocExtGrpComp_, fits_sideAllocExtGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩, srqsRelatedTradeIdGrpComp := ⟨srqsRelatedTradeIdGrpComp_, fits_srqsRelatedTradeIdGrpComp⟩ }, bytes)
            else none
          else none
        else none
      else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesUploadBroadcast) : (encode message).length ≤ 75738 := by
  ...
  have bound_srqsRelatedTradeIdGrpComp := message.srqsRelatedTradeIdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocExtGrpComp.encode 176 SideAllocExtGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 24 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length, encodeMany_length_const SrqsRelatedTradeIdGrpComp.encode 8 SrqsRelatedTradeIdGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TesUploadBroadcast) (rest : List UInt8) :
  ...
  dsimp only
  rw [List.append_assoc, decodeMany_bounded 1 UnderlyingStipGrpComp.encode UnderlyingStipGrpComp.decode UnderlyingStipGrpComp.decode_encode, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SrqsRelatedTradeIdGrpComp.encode SrqsRelatedTradeIdGrpComp.decode SrqsRelatedTradeIdGrpComp.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.sideAllocExtGrpComp.length_lt, dite_eq_left message.trdInstrmntLegGrpComp.length_lt, dite_eq_left message.instrumentEventGrpComp.length_lt, dite_eq_left message.instrumentAttributeGrpComp.length_lt, dite_eq_left message.underlyingStipGrpComp.length_lt, dite_eq_left message.srqsRelatedTradeIdGrpComp.length_lt]
  rfl
...
end Omi.EurexT7EtiFbeV81Server
