...
/-!
# Eurex Exchange Enhanced Trading Interface v6.1
...
namespace Omi.EurexT7EtiFbeV61Server
...
def decode (bytes : List UInt8) : Option (QuoteActivationNotification × List UInt8) := do
  ...
  let (notAffectedSecuritiesGrpComp_, bytes) ← decodeMany NotAffectedSecuritiesGrpComp.decode noNotAffectedSecurities.toNat bytes
  if fits_notAffectedSecuritiesGrpComp : notAffectedSecuritiesGrpComp_.length < 256 ^ 2 then
    pure ({ pad2, rbcHeaderMeComp, massActionReportId, marketSegmentId, partyIdEnteringTrader, partyIdEnteringFirm, productComplex, massActionType, massActionSubType, massActionReason, pad1, notAffectedSecuritiesGrpComp := ⟨notAffectedSecuritiesGrpComp_, fits_notAffectedSecuritiesGrpComp⟩ }, bytes)
  else none
...
def encode (message : OrderBookItemGrpComp) : List UInt8 :=
  encodeUIntLE 8 message.securityId
    ...
    ++ encodeUIntLE 4 message.bestBidSize
    ++ encodeUIntLE 4 message.bestOfferSize
...
def decode (bytes : List UInt8) : Option (OrderBookItemGrpComp × List UInt8) := do
  ...
  let (bestBidSize, bytes) ← decodeUIntLE 4 bytes
  let (bestOfferSize, bytes) ← decodeUIntLE 4 bytes
  let (mdBookType, bytes) ← decodeUInt 1 bytes
  let (mdSubBookType, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ securityId, bestBidPx, bestOfferPx, bestBidSize, bestOfferSize, mdBookType, mdSubBookType, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (SrqsCreateDealNotification × List UInt8) := do
  ...
  if fits_orderBookItemGrpComp : orderBookItemGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, lastPx, quoteId, securityId, expireTime, underlyingPx, underlyingDeltaPercentage, underlyingEffectiveDeltaPercentage, underlyingQty, negotiationId, tradeId, lastQty, trdRptStatus, messageEventSource, side, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmTradeId, firmNegotiationId, freeText1, orderBookItemGrpComp := ⟨orderBookItemGrpComp_, fits_orderBookItemGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 10402 := by
...
/-- Srqs Negotiation Notification: 154 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationNotification × List UInt8) := do
  ...
  let (firmNegotiationId, bytes) ← Alpha.decode 20 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, lastPx, negotiationId, leavesQty, lastQty, numberOfRespondents, quoteType, quoteStatus, quoteInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsNegotiationNotification) : (encode message).length = 154 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, trdRegTsExecutionTime, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, lastPx, negotiationId, orderQty, leavesQty, lastQty, numberOfRespondents, quoteType, quoteStatus, numberOfRespDisclosureInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, pad1, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsNegotiationRequesterNotification) : (encode message).length ≤ 6274 := by
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationNotification × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, negotiationStartTime, securityId, bidPx, offerPx, lastPx, quoteRefPrice, underlyingDeltaPercentage, negotiationId, marketSegmentId, leavesQty, lastQty, securitySubType, numberOfRespondents, quoteType, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, productComplex, negotiateUnderlying, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationNotification) : (encode message).length ≤ 6298 := by
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ pad2, rbcHeaderComp, transactTime, securityId, bidPx, offerPx, lastPx, quoteRefPrice, underlyingDeltaPercentage, negotiationId, marketSegmentId, orderQty, lastQty, securitySubType, numberOfRespondents, quoteType, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, negotiateUnderlying, productComplex, numberOfRespDisclosureInstruction, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, pad1, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 12402 := by
  have bound_quotReqLegsGrpComp := message.quotReqLegsGrpComp.length_lt
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
...
/-- Srqs Quote Notification: 154 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsQuoteNotification × List UInt8) := do
  ...
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteId, secondaryQuoteId, bidPx, offerPx, underlyingPx, underlyingDeltaPercentage, negotiationId, bidSize, offerSize, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, quoteReqId, freeText1, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsQuoteNotification) : (encode message).length = 154 := by
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 26914 := by
  ...
  if fits_sideAllocGrpBcComp : sideAllocGrpBcComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ pad2, rbcHeaderComp, securityId, lastPx, transactTime, underlyingPx, transBkdTime, relatedClosePrice, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, relatedTradeQuantity, underlyingQty, negotiationId, srqsRelatedTradeId, trdType, tradeReportType, productComplex, tradePublishIndicator, partyIdSettlementLocation, hedgeType, messageEventSource, tradeReportText, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad5, sideAllocGrpBcComp := ⟨sideAllocGrpBcComp_, fits_sideAllocGrpBcComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 32914 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesUploadBroadcast) : (encode message).length ≤ 63490 := by
...
end Omi.EurexT7EtiFbeV61Server
