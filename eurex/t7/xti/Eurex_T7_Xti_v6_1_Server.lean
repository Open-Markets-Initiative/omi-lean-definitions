...
namespace Omi.EurexT7XtiFbeV61Server
...
def decode (bytes : List UInt8) : Option (DeleteOrderBroadcast × List UInt8) := do
  ...
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, orderIdSfx, cumQty, cxlQty, marketSegmentId, partyIdEnteringTrader, partyIdSessionId, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, side, fixClOrdId, pad6 }, bytes)
...
end Omi.EurexT7XtiFbeV61Server
