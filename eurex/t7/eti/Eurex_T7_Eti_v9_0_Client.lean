...
Note: Tes Compression Run Status Request is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7EtiFbeV90Client
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ApproveBasketTradeRequest) : (encode message).length ≤ 47026 := by
...
/-- Approve Tes Trade Request: 234 bytes -/
...
/-- Delete Tes Trade Request: 66 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteTesTradeRequest × List UInt8) := do
  ...
  let (tradeReportId, bytes) ← Alpha.decode 20 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, compressionId, packageId, marketSegmentId, tesExecId, relatedMarketSegmentId, trdType, tradeReportType, tradeReportId, pad1 }, bytes)

@[simp] theorem encode_length (message : DeleteTesTradeRequest) : (encode message).length = 66 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
      ...
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ networkMsgId, pad2, requestHeaderComp, securityId, lastPx, transBkdTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, compressionId, marketSegmentId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, trdType, productComplex, tradeReportType, tradePublishIndicator, partyIdSettlementLocation, hedgeType, tradeReportText, tradeReportId, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad5, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
theorem encode_length_le (message : ModifyOrderComplexRequest) : (encode message).length ≤ 2298 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, CustOrderHandlingInst.encode_length, encodeMany_length_const LegOrdGrpComp.encode 8 LegOrdGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (ModifyTesTradeRequest × List UInt8) := do
  ...
  if fits_sideAllocGrpComp : sideAllocGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, lastPx, transBkdTime, compressionId, marketSegmentId, packageId, tesExecId, relatedMarketSegmentId, trdType, tradeReportType, tradePublishIndicator, tradeReportText, tradeReportId, pad2v2, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩ }, bytes)
    else none
  else none
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequest × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, securityId, bidPx, offerPx, orderQty, quoteRefPrice, underlyingDeltaPercentage, validUntilTime, marketSegmentId, securitySubType, quoteType, quoteSubType, numberOfRespDisclosureInstruction, side, quoteRefPriceSource, tradeUnderlying, negotiateUnderlying, productComplex, respondentType, showLastDealOnClosure, partyExecutingFirm, partyExecutingTrader, freeText5, quoteReqId, partyOrderOriginationTrader, pad5, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequest) : (encode message).length ≤ 14674 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuotReqLegsGrpComp.encode 24 QuotReqLegsGrpComp.encode_length, encodeMany_length_const TargetPartiesComp.encode 32 TargetPartiesComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsUpdateNegotiationRequest) : (encode message).length ≤ 8514 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TargetPartiesComp.encode 32 TargetPartiesComp.encode_length]
  omega
...
/-- Tes Compression Run Status Request: 34 bytes -/
structure TesCompressionRunStatusRequest where
  networkMsgId : Alpha 8
  pad2 : Alpha 2
  requestHeaderComp : RequestHeaderComp
  compressionId : BitVec 64
...
namespace TesCompressionRunStatusRequest

def encode (message : TesCompressionRunStatusRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ++ Alpha.encode message.pad2
    ++ RequestHeaderComp.encode message.requestHeaderComp
    ++ encodeUIntLE 8 message.compressionId
...
def decode (bytes : List UInt8) : Option (TesCompressionRunStatusRequest × List UInt8) := do
  let (networkMsgId, bytes) ← Alpha.decode 8 bytes
  let (pad2, bytes) ← Alpha.decode 2 bytes
  let (requestHeaderComp, bytes) ← RequestHeaderComp.decode bytes
  let (compressionId, bytes) ← decodeUIntLE 8 bytes
  ...
  let (tradePublishIndicator, bytes) ← decodeUInt 1 bytes
  let (pad2v2, bytes) ← Alpha.decode 2 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, compressionId, marketSegmentId, compressionAction, tradePublishIndicator, pad2v2 }, bytes)

@[simp] theorem encode_length (message : TesCompressionRunStatusRequest) : (encode message).length = 34 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : TesCompressionRunStatusRequest) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : TesCompressionRunStatusRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [Alpha.decode_encode, Option.bind_some]
...
end TesCompressionRunStatusRequest
  ...
  if fits_sideAllocExtGrpComp : sideAllocExtGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ networkMsgId, pad2, requestHeaderComp, securityId, lastPx, transBkdTime, underlyingPx, relatedClosePrice, relatedTradeQuantity, underlyingQty, compressionId, marketSegmentId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, trdType, productComplex, tradeReportType, tradePublishIndicator, skipValidations, trdRptStatus, tradePlatform, hedgeType, partyIdSettlementLocation, valueCheckTypeMinLotSize, tradeReportId, tradeReportText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad1, sideAllocExtGrpComp := ⟨sideAllocExtGrpComp_, fits_sideAllocExtGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
inductive ClientPayload where
  ...
  | tesCompressionRunStatusRequest (message : TesCompressionRunStatusRequest) -- 10616
...
def tag : ClientPayload → BitVec 16
  ...
  | .tesCompressionRunStatusRequest _ => 10616
...
def encode : ClientPayload → List UInt8
  ...
  | .tesCompressionRunStatusRequest message => TesCompressionRunStatusRequest.encode message
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (ClientPayload × List UInt8) :=
  ...
  else if tag = 10616 then (TesCompressionRunStatusRequest.decode bytes).map fun (message, rest) => (.tesCompressionRunStatusRequest message, rest)
...
theorem encodeBody_length_lt (message : ClientMessage) : (encodeBody message).length + 4 < 256 ^ 4 := by
  ...
  | subscribeRequest inner =>
    ...
    omega
  | tesCompressionRunStatusRequest inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUIntLE_length, TesCompressionRunStatusRequest.encode_length]
    omega
...
end Omi.EurexT7EtiFbeV90Client
