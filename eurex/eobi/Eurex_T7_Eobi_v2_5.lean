...
/-!
# Eurex Exchange Enhanced Order Book Interface v2.5
...
namespace Omi.EurexT7EobiFbeV25
...
def decode (bytes : List UInt8) : Option (ExecutionSummary × List UInt8) := do
  ...
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ securityId, aggressorTimestamp, requestTime, execId, lastQty, aggressorSide, tradeCondition, pad2, lastPx, restingHiddenQty, pad4 }, bytes)
