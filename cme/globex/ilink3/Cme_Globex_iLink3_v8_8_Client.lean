...
namespace Omi.CmeGlobexIlink3SbeV88Client
...
def decode (bytes : List UInt8) : Option (MassQuote × List UInt8) := do
  ...
  pure ({ partyDetailsListReqId, sendingTimeEpoch, manualOrderIndicator, seqNum, senderId, quoteReqIdOptional, location, quoteId, totNoQuoteEntries, mmProtectionReset, liquidityFlag, shortSaleType, reserved, future30, quoteEntryOpen, quoteEntriesGroups }, bytes)
...
theorem encode_length_le (message : BrokenDatesRequestGroups) : (encode message).length ≤ 1023 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const BrokenDatesRequestGroup.encode 4 BrokenDatesRequestGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : BrokenDatesRequestGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 BrokenDatesRequestGroup.encode BrokenDatesRequestGroup.decode BrokenDatesRequestGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.brokenDatesRequestGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : SecurityDefinitionRequest) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, RequestLegsGroups.decode_encode, some_bind]
  dsimp only
  rw [BrokenDatesRequestGroups.decode_encode, some_bind]
  rfl
...
end Omi.CmeGlobexIlink3SbeV88Client
