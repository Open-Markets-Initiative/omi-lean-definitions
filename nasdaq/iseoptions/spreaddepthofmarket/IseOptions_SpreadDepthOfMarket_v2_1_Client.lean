...
namespace Omi.NasdaqIseoptionsSpreaddepthofmarketItchV21Client
...
theorem encodeBody_length_lt (message : ClientSoupBinTcpPacket) : (encodeBody message).length + 0 < 256 ^ 2 := by
  ...
  | unsequencedDataPacket inner =>
    ...
    omega
  | clientHeartbeatPacket inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUInt_length, ClientHeartbeatPacket.encode_length]
    omega
  | logoutRequestPacket inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUInt_length, LogoutRequestPacket.encode_length]
    omega
...
end Omi.NasdaqIseoptionsSpreaddepthofmarketItchV21Client
