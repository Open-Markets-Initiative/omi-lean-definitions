...
Note: Xetra En Light Approve Deal Request is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7XtiFbeV130Client
...
def decode (bytes : List UInt8) : Option (InquirePreTradeRiskLimitsRequest × List UInt8) := do
  ...
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, partyExecutingUnit, riskLimitGroup, pad4 }, bytes)
...
/-- Modify Order Request: 258 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, displayQty, displayLowQty, displayHighQty, clOrdId, origClOrdId, stopPx, pegOffsetValueAbs, pegOffsetValuePct, partyIdClientId, executingTrader, matchInstCrossId, expireDate, targetPartyIdSessionId, tradingSessionSubId, stopPxIndicator, ownershipIndicator, partyExecutingFirm, partyExecutingTrader, applSeqIndicator, ordType, priceValidityCheckType, valueCheckTypeValue, valueCheckTypeQuantity, orderOrigination, freeText1, freeText2, freeText4, fixClOrdId, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, pad2v3, partyIdInvestmentDecisionMaker, orderAttributeLiquidityProvision, tradingCapacity, pad2v1, marketSegmentId, securityId, minQty, orderQty, price, side, execInst, timeInForce, midPointType, checkSumCorrection, pad2v2 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderRequest) : (encode message).length = 258 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleRequest × List UInt8) := do
  ...
  let (freeText4, bytes) ← Alpha.decode 16 bytes
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, securityId, price, orderQty, displayQty, displayLowQty, displayHighQty, minQty, stopPx, pegOffsetValueAbs, pegOffsetValuePct, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, expireDate, marketSegmentId, matchInstCrossId, targetPartyIdSessionId, applSeqIndicator, side, ordType, priceValidityCheckType, valueCheckTypeValue, valueCheckTypeQuantity, orderAttributeLiquidityProvision, timeInForce, execInst, tradingSessionSubId, stopPxIndicator, tradingCapacity, midPointType, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, ownershipIndicator, partyExecutingFirm, partyExecutingTrader, freeText1, freeText2, freeText4, fixClOrdId }, bytes)
...
/-- Modify Order Single Short Request: 114 bytes -/
...
def decode (bytes : List UInt8) : Option (ModifyOrderSingleShortRequest × List UInt8) := do
  ...
  let (executingTraderQualifier, bytes) ← decodeUInt 1 bytes
  let (pad5, bytes) ← Alpha.decode 5 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, securityId, price, orderQty, minQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, matchInstCrossId, enrichmentRuleId, side, priceValidityCheckType, valueCheckTypeValue, valueCheckTypeQuantity, orderAttributeLiquidityProvision, timeInForce, applSeqIndicator, execInst, tradingCapacity, midPointType, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, pad5 }, bytes)

@[simp] theorem encode_length (message : ModifyOrderSingleShortRequest) : (encode message).length = 114 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (NewOrderSingleRequest × List UInt8) := do
  ...
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, price, orderQty, displayQty, displayLowQty, displayHighQty, minQty, stopPx, pegOffsetValueAbs, pegOffsetValuePct, clOrdId, securityId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, quoteId, expireDate, marketSegmentId, targetPartyIdSessionId, matchInstCrossId, applSeqIndicator, side, ordType, priceValidityCheckType, valueCheckTypeValue, valueCheckTypeQuantity, orderAttributeLiquidityProvision, timeInForce, execInst, tradingSessionSubId, tradeAtCloseOptIn, tradingCapacity, midPointType, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, partyExecutingFirm, partyExecutingTrader, freeText1, freeText2, freeText4, fixClOrdId, pad1 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderSingleShortRequest × List UInt8) := do
  ...
  let (executingTraderQualifier, bytes) ← decodeUInt 1 bytes
  let (pad5, bytes) ← Alpha.decode 5 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, price, orderQty, minQty, clOrdId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, matchInstCrossId, enrichmentRuleId, side, applSeqIndicator, priceValidityCheckType, valueCheckTypeValue, valueCheckTypeQuantity, orderAttributeLiquidityProvision, timeInForce, execInst, tradingCapacity, midPointType, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, pad5 }, bytes)
...
def decode (bytes : List UInt8) : Option (PreTradeRiskLimitsDefinitionRequest × List UInt8) := do
  ...
  if fits_riskLimitAmountGrpComp : riskLimitAmountGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, partyDetailStatus, riskLimitGroup, partyDetailExecutingUnit, pad2v2, riskLimitAmountGrpComp := ⟨riskLimitAmountGrpComp_, fits_riskLimitAmountGrpComp⟩ }, bytes)
  else none
...
/-- Xetra En Light Approve Deal Request: 42 bytes -/
structure XetraEnLightApproveDealRequest where
  networkMsgId : Alpha 8
  pad2 : Alpha 2
  requestHeaderComp : RequestHeaderComp
  marketSegmentId : BitVec 32
...
namespace XetraEnLightApproveDealRequest

def encode (message : XetraEnLightApproveDealRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ++ Alpha.encode message.pad2
    ++ RequestHeaderComp.encode message.requestHeaderComp
    ++ encodeUIntLE 4 message.marketSegmentId
...
def decode (bytes : List UInt8) : Option (XetraEnLightApproveDealRequest × List UInt8) := do
  let (networkMsgId, bytes) ← Alpha.decode 8 bytes
  let (pad2, bytes) ← Alpha.decode 2 bytes
  let (requestHeaderComp, bytes) ← RequestHeaderComp.decode bytes
  let (marketSegmentId, bytes) ← decodeUIntLE 4 bytes
  ...
  let (partyExecutingFirm, bytes) ← Alpha.decode 5 bytes
  let (partyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, marketSegmentId, negotiationId, tradeId, tradeReportType, partyExecutingFirm, partyExecutingTrader }, bytes)

@[simp] theorem encode_length (message : XetraEnLightApproveDealRequest) : (encode message).length = 42 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : XetraEnLightApproveDealRequest) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : XetraEnLightApproveDealRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [Alpha.decode_encode]
...
end XetraEnLightApproveDealRequest
...
inductive ClientPayload where
  ...
  | xetraEnLightApproveDealRequest (message : XetraEnLightApproveDealRequest) -- 10819
...
def tag : ClientPayload → BitVec 16
  ...
  | .xetraEnLightApproveDealRequest _ => 10819
...
def encode : ClientPayload → List UInt8
  ...
  | .xetraEnLightApproveDealRequest message => XetraEnLightApproveDealRequest.encode message
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (ClientPayload × List UInt8) :=
  ...
  else if tag = 10819 then (XetraEnLightApproveDealRequest.decode bytes).map fun (message, rest) => (.xetraEnLightApproveDealRequest message, rest)
...
theorem encodeBody_length_lt (message : ClientMessage) : (encodeBody message).length + 4 < 256 ^ 4 := by
  ...
  | userLogoutRequest inner =>
    ...
    omega
  | xetraEnLightApproveDealRequest inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUIntLE_length, XetraEnLightApproveDealRequest.encode_length]
    omega
...
end Omi.EurexT7XtiFbeV130Client
