...
/-!
# Eurex Exchange Enhanced Trading Interface v7.0
...
namespace Omi.EurexT7EtiFbeV70Server
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 12466 := by
...
/-- Srqs Deal Notification: 162 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsDealNotification × List UInt8) := do
  ...
  let (firmNegotiationId, bytes) ← Alpha.decode 20 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, underlyingPriceStipValue, underlyingPx, lastPx, negotiationId, tradeId, lastQty, trdRptStatus, messageEventSource, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, targetPartyEnteringTrader, firmTradeId, firmNegotiationId, pad4 }, bytes)

@[simp] theorem encode_length (message : SrqsDealNotification) : (encode message).length = 162 := by
  unfold encode
...
/-- Srqs Negotiation Notification: 162 bytes -/
...
def decode (bytes : List UInt8) : Option (TargetPartiesComp × List UInt8) := do
  ...
  let (targetPartyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ sideDisclosureInstruction, priceDisclosureInstruction, leavesQtyDisclosureInstruction, lastPxDisclosureInstruction, lastQtyDisclosureInstruction, quoteInstruction, targetPartyExecutingFirm, targetPartyExecutingTrader, pad7 }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsNegotiationRequesterNotification) : (encode message).length ≤ 6290 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationNotification) : (encode message).length ≤ 6314 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 12410 := by
  have bound_quotReqLegsGrpComp := message.quotReqLegsGrpComp.length_lt
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
...
/-- Srqs Quote Notification: 162 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsQuoteNotification × List UInt8) := do
  ...
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteId, secondaryQuoteId, bidPx, bidSize, offerPx, offerSize, underlyingPx, underlyingDeltaPercentage, negotiationId, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, quoteReqId, freeText1, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsQuoteNotification) : (encode message).length = 162 := by
  unfold encode
    ...
    if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
      if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
        if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
          pure ({ pad2, rbcHeaderComp, securityId, lastPx, allocQty, transactTime, underlyingPx, transBkdTime, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, packageId, tesExecId, allocId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, srqsRelatedTradeId, trdType, side, tradePublishIndicator, productComplex, tradeReportType, tradingCapacity, partyIdSettlementLocation, tradeAllocStatus, hedgeType, messageEventSource, tradeReportId, partyExecutingFirm, partyExecutingTrader, partyIdEnteringFirm, partyEnteringTrader, positionEffect, rootPartyExecutingFirm, rootPartyExecutingTrader, freeText1, freeText2, freeText3, partyIdTakeUpTradingFirm, account, partyIdPositionAccount, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdLocationId, custOrderHandlingInst, complianceText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad2v1, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
        else none
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 26922 := by
...
@[simp] theorem decode_encode (message : TesApproveBroadcast) (rest : List UInt8) :
  ...
  dsimp only
  simp only [message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, ↓reduceDIte]
  rfl
  ...
  if fits_sideAllocGrpBcComp : sideAllocGrpBcComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ pad2, rbcHeaderComp, securityId, lastPx, transactTime, underlyingPx, transBkdTime, relatedClosePrice, relatedTradeQuantity, underlyingQty, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, negotiationId, srqsRelatedTradeId, trdType, tradeReportType, productComplex, tradePublishIndicator, partyIdSettlementLocation, hedgeType, messageEventSource, tradeReportText, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad1, sideAllocGrpBcComp := ⟨sideAllocGrpBcComp_, fits_sideAllocGrpBcComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 34954 := by
...
@[simp] theorem decode_encode (message : TesBroadcast) (rest : List UInt8) :
  ...
  rw [decodeMany_bounded 1 UnderlyingStipGrpComp.encode UnderlyingStipGrpComp.decode UnderlyingStipGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.sideAllocGrpBcComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def encode (message : TesTradeBroadcast) : List UInt8 :=
  Alpha.encode message.pad2
    ...
    ++ encodeUIntLE 4 message.basketTrdMatchId
    ++ encodeUIntLE 2 message.trdType
    ++ encodeUInt 1 message.productComplex
...
def decode (bytes : List UInt8) : Option (TesTradeBroadcast × List UInt8) := do
  ...
  let (basketTrdMatchId, bytes) ← decodeUIntLE 4 bytes
  let (trdType, bytes) ← decodeUIntLE 2 bytes
  let (productComplex, bytes) ← decodeUInt 1 bytes
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesUploadBroadcast) : (encode message).length ≤ 71658 := by
...
end Omi.EurexT7EtiFbeV70Server
