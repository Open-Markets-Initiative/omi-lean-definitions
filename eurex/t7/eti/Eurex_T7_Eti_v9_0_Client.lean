...
Note: Tes Compression Run Status Request is not framed: its length Body Len is not an integer it reads.
...
theorem encode_length_le (message : ClientPayload) : (encode message).length ≤ 73674 := by
  ...
  | tesCompressionRunStatusRequest inner =>
    simp only [encode, TesCompressionRunStatusRequest.encode_length]
    omega
