...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v10.1
...
namespace Omi.EurexT7XtiFbeV101Server
...
def decode (bytes : List UInt8) : Option (XetraEnLightNegotiationNotification × List UInt8) := do
  ...
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, bidPx, offerPx, leavesQty, negotiationId, numberOfRespondents, settlDate, side, quoteCondition, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, freeText5, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (XetraEnLightNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_xetraEnLightTargetPartiesComp : xetraEnLightTargetPartiesComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, trdRegTsExecutionTime, bidPx, offerPx, orderQty, lastPx, leavesQty, lastQty, autoExecLimitPrice, expireTime, autoExecExpiryTime, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, negotiationId, numberOfRespondents, settlDate, autoExecReferencePriceOffset, autoExecMinNoOfQuotes, numberOfRespDisclosureInstruction, side, tradingCapacity, valueCheckTypeValue, valueCheckTypeQuantity, autoExecType, sideDisclosureInstruction, orderAttributeLiquidityProvision, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, orderOrigination, quoteCondition, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, freeText1, freeText2, freeText4, freeText5, closureReason, pad2v2, xetraEnLightTargetPartiesComp := ⟨xetraEnLightTargetPartiesComp_, fits_xetraEnLightTargetPartiesComp⟩ }, bytes)
  else none
...
def decode (bytes : List UInt8) : Option (XetraEnLightQuoteNotification × List UInt8) := do
  ...
  let (freeText4, bytes) ← Alpha.decode 16 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteId, secondaryQuoteId, bidPx, bidSize, offerPx, offerSize, expireTime, negotiationId, tradingCapacity, quotingStatus, quoteEventReason, quoteCancelReason, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, quoteReqId, freeText1, freeText2, freeText4, pad3 }, bytes)
...
theorem encode_length_le (message : XetraEnLightQuoteRequesterNotification) : (encode message).length ≤ 20474 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SrqsQuoteEntryGrpComp.encode 80 SrqsQuoteEntryGrpComp.encode_length]
  omega
...
end Omi.EurexT7XtiFbeV101Server
