...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v14.1
...
namespace Omi.EurexT7XtiFbeV141Server
...
theorem encode_length_le (message : BroadcastErrorNotification) : (encode message).length ≤ 4291887116 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ForcedLogoutNotification) : (encode message).length ≤ 4291887106 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ForcedUserLogoutNotification) : (encode message).length ≤ 4291887111 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
  simp only [List.length_append, Alpha.encode_length, NotifHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const Byte.encode 1 Byte.encode_length]
...
theorem encode_length_le (message : InquireEnrichmentRuleIdListResponse) : (encode message).length ≤ 3145730 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderComp.encode_length, encodeUIntLE_length, encodeMany_length_const EnrichmentRulesGrpComp.encode 48 EnrichmentRulesGrpComp.encode_length]
  omega
...
theorem encode_length_le (message : InquireSessionListResponse) : (encode message).length ≤ 524314 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderComp.encode_length, encodeUIntLE_length, encodeMany_length_const SessionsGrpComp.encode 8 SessionsGrpComp.encode_length]
  omega
...
theorem encode_length_le (message : InquireUserResponse) : (encode message).length ≤ 1048610 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderComp.encode_length, encodeUIntLE_length, encodeMany_length_const PartyDetailsGrpComp.encode 16 PartyDetailsGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : LegalNotificationBroadcast) : (encode message).length ≤ 4291887139 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : LogonResponse) : (encode message).length ≤ 4291887190 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewsBroadcast) : (encode message).length ≤ 4291887394 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
def decode (bytes : List UInt8) : Option (OtcUploadBroadcast × List UInt8) := do
  ...
  if fits_sideAllocOtcGrpComp : sideAllocOtcGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, otcExecId, pad4, securityId, lastPx, lastQty, settlCurrFxRate, transactionCostAmt, transBkdTime, transactTime, reversalInitiationTime, marketSegmentId, packageId, settlDate, trdType, tradeReportType, trdRptStatus, tradePublishIndicator, transactionCostCode, tradePlatform, settlCurrency, tradeReportId, regulatoryTradeId, pad1, sideAllocOtcGrpComp := ⟨sideAllocOtcGrpComp_, fits_sideAllocOtcGrpComp⟩ }, bytes)
  else none
...
def decode (bytes : List UInt8) : Option (RiskLimitsRptGrpComp × List UInt8) := do
  ...
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ riskLimitAmount, riskLimitOpenAmount, riskLimitNetPositionAmount, activationDate, riskLimitType, riskLimitRequestingPartyRole, riskLimitViolationIndicator, riskLimitGroup, pad6 }, bytes)
...
def decode (bytes : List UInt8) : Option (PreTradeRiskLimitResponse × List UInt8) := do
  ...
  if fits_riskLimitsRptGrpComp : riskLimitsRptGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, nrResponseHeaderMeComp, riskLimitReportId, marketSegmentId, partyDetailStatus, partyDetailExecutingUnit, pad5, riskLimitsRptGrpComp := ⟨riskLimitsRptGrpComp_, fits_riskLimitsRptGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : Reject) : (encode message).length ≤ 4291887151 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 4291887306 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
end Omi.EurexT7XtiFbeV141Server
