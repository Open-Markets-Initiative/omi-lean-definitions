...
@[simp] theorem decode_encode (message : IncrementalRefreshBtecGroups) (rest : List UInt8) :
  ...
  rw [decodeUInt_encodeUInt]
  simp only [Option.bind_some]
  rw [decodeMany_bounded 1 IncrementalRefreshBtecGroup.encode IncrementalRefreshBtecGroup.decode IncrementalRefreshBtecGroup.decode_encode]
  simp only [Option.bind_some]
