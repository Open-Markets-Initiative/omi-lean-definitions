...
/-!
# Aquis Exchange Real Time Market Data Feed v4.1
...
namespace Omi.AquisAquisequitiesRealtimeAmdV41
...
def decode (bytes : List UInt8) : Option (Trade × List UInt8) := do
  ...
  let (mdFlags, bytes) ← decodeUIntLE 1 bytes
  pure ({ securityId, tradeType, quantity, price, orderRef, tradeRef, timestamp, mic, binaryMmt, mdFlags }, bytes)

@[simp] theorem encode_length (message : Trade) : (encode message).length = 40 := by
  unfold encode
...
end Omi.AquisAquisequitiesRealtimeAmdV41
