...
@[simp] theorem decode_encode (message : ServerTcpFrame) (rest : List UInt8) :
  ...
  dsimp only
  rw [ServerTcpMessage.decode_encode, Option.bind_some]
  rfl
...
theorem decode_encode (message : ServerTcpPacket) : decode (encode message) = some message := by
  unfold decode encode
  simp only [Option.bind_eq_bind]
  rw [decodeAll_encodeMany ServerTcpFrame.encode ServerTcpFrame.decode ServerTcpFrame.decode_encode ServerTcpFrame.encode_length_pos message.serverTcpFrame _ (encodeMany_length_ge ServerTcpFrame.encode ServerTcpFrame.encode_length_pos message.serverTcpFrame), Option.bind_some]
  rfl
