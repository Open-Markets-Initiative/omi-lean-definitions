...
namespace Omi.CmeGlobexIlink3SbeV85Client
...
def listed (byte : UInt8) : OrdType :=
  ...
  else if byte = 0x34 then .stopLimitt
...
def decode (bytes : List UInt8) : Option (MassQuote × List UInt8) := do
  ...
  let (liquidityFlag, bytes) ← decodeUInt 1 bytes
  let (shortSaleType, bytes) ← decodeUInt 1 bytes
  let (reserved, bytes) ← Alpha.decode 30 bytes
  let (quoteEntriesGroups, bytes) ← QuoteEntriesGroups.decode bytes
  pure ({ partyDetailsListReqId, sendingTimeEpoch, manualOrderIndicator, seqNum, senderId, quoteReqIdOptional, location, quoteId, totNoQuoteEntries, mmProtectionReset, liquidityFlag, shortSaleType, reserved, quoteEntriesGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MassQuote) : (encode message).length ≤ 9785 := by
...
end Omi.CmeGlobexIlink3SbeV85Client
