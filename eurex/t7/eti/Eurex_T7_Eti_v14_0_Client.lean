...
namespace Omi.EurexT7EtiFbeV140Client
...
def decode (bytes : List UInt8) : Option (AddComplexInstrumentRequest × List UInt8) := do
  ...
  if fits_instrmtLegGrpComp : instrmtLegGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, relatedPx, marketSegmentId, securitySubType, quantityScalingFactor, productComplex, multilegModel, complianceText, pad7, instrmtLegGrpComp := ⟨instrmtLegGrpComp_, fits_instrmtLegGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : AddComplexInstrumentRequest) : (encode message).length ≤ 8226 := by
...
def decode (bytes : List UInt8) : Option (MmParameterDefinitionRequest × List UInt8) := do
  ...
  pure ({ networkMsgId, pad2, requestHeaderComp, exposureDuration, cumQty, delta, vega, mmRiskLimitActionType, pad3, marketSegmentId, targetPartyIdSessionId, pctCount }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyOrderComplexRequest) : (encode message).length ≤ 2322 := by
...
def decode (bytes : List UInt8) : Option (ModifyOrderComplexShortRequest × List UInt8) := do
  ...
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, securityId, price, orderQty, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, matchInstCrossId, enrichmentRuleId, applSeqIndicator, selfMatchPreventionInstruction, crossMatchInstruction, productComplex, side, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, execInst, timeInForce, tradingCapacity, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, complianceText, pad4 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderSingleRequest × List UInt8) := do
  ...
  let (fixClOrdId, bytes) ← Alpha.decode 20 bytes
  let (partyEndClientIdentification, bytes) ← Alpha.decode 20 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, price, orderQty, stopPx, clOrdId, securityId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, expireDate, marketSegmentId, matchInstCrossId, selfMatchPreventionInstruction, crossMatchInstruction, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, applSeqIndicator, productComplex, side, ordType, priceValidityCheckType, valueCheckTypeValue, orderAttributeLiquidityProvision, orderAttributeRiskReduction, timeInForce, execInst, tradingSessionSubId, tradingCapacity, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, account, partyIdPositionAccount, positionEffect, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, fixClOrdId, partyEndClientIdentification }, bytes)
...
def decode (bytes : List UInt8) : Option (PreTradeRiskLimitsDefinitionRequest × List UInt8) := do
  ...
  if fits_riskLimitQtyGrpComp : riskLimitQtyGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, nettingCoefficient, marketSegmentId, riskLimitPlatform, partyDetailStatus, riskLimitGroup, partyDetailExecutingUnit, pad1, riskLimitQtyGrpComp := ⟨riskLimitQtyGrpComp_, fits_riskLimitQtyGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : SrqsHitQuoteRequest) : (encode message).length ≤ 6506 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, CustOrderHandlingInst.encode_length, encodeMany_length_const SrqsHitQuoteGrpComp.encode 24 SrqsHitQuoteGrpComp.encode_length]
  omega
...
end Omi.EurexT7EtiFbeV140Client
