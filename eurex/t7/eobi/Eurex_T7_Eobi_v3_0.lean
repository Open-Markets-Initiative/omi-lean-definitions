...
/-!
# Eurex Exchange Enhanced Order Book Interface v3.0
...
namespace Omi.EurexT7EobiFbeV30
...
@[simp] theorem encode_length (message : AuctionBbo) : (encode message).length = 32 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]
...
/-- Auction Clearing Price: 24 bytes -/
...
def decode (bytes : List UInt8) : Option (AuctionClearingPrice × List UInt8) := do
  ...
  pure ({ transactTime, securityId, lastPx }, bytes)

@[simp] theorem encode_length (message : AuctionClearingPrice) : (encode message).length = 24 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]
...
def decode (bytes : List UInt8) : Option (ExecutionSummary × List UInt8) := do
  ...
  pure ({ securityId, aggressorTimestamp, requestTime, execId, lastQty, aggressorSide, tradeCondition, pad2, lastPx, restingHiddenQty, restingCxlQty }, bytes)
...
def decode (bytes : List UInt8) : Option (FullOrderExecution × List UInt8) := do
  ...
  let (lastQty, bytes) ← decodeUIntLE 4 bytes
  let (lastPx, bytes) ← decodeUIntLE 8 bytes
  pure ({ side, pad7, price, trdRegTsTimePriority, securityId, trdMatchId, lastQty, lastPx }, bytes)
...
def decode (bytes : List UInt8) : Option (InstrumentSummary × List UInt8) := do
  ...
  if fits_mdInstrumentEntryGrpComp : mdInstrumentEntryGrpComp_.length < 256 ^ 1 then
    pure ({ securityId, lastUpdateTime, trdRegTsExecutionTime, totNoOrders, securityStatus, securityTradingStatus, fastMarketIndicator, pad2, mdInstrumentEntryGrpComp := ⟨mdInstrumentEntryGrpComp_, fits_mdInstrumentEntryGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : InstrumentSummary) : (encode message).length ≤ 4112 := by
  ...
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length, encodeMany_length_const MdInstrumentEntryGrpComp.encode 16 MdInstrumentEntryGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (OrderDetailsComp × List UInt8) := do
  ...
  let (pad3, bytes) ← Alpha.decode 3 bytes
  let (price, bytes) ← decodeUIntLE 8 bytes
  pure ({ trdRegTsTimePriority, displayQty, side, pad3, price }, bytes)
...
@[simp] theorem encode_length (message : ProductSummary) : (encode message).length = 8 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (TradeReport × List UInt8) := do
  ...
  pure ({ securityId, transactTime, trdMatchId, lastQty, lastPx, matchType, matchSubType, pad6 }, bytes)
