...
/-!
# Brasil, Bolsa, Balcão Binary Unified Market Data Feed v2.1
...
namespace Omi.B3B3derivativesBinaryumdfSbeV21
...
def decode (bytes : List UInt8) : Option (OrderMbO50Message × List UInt8) := do
  ...
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, matchEventIndicator, mdUpdateAction, mdEntryType, offset11Padding1, mdCorporateOffsetPriceOptional, mdEntrySizeQuantity, mdEntryPositionNo, enteringFirm, mdInsertTimestamp, secondaryOrderId, rptSeq, transactTime }, bytes)
...
def decode (bytes : List UInt8) : Option (DeleteOrderMbO51Message × List UInt8) := do
  ...
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  let (mdCorporateOffsetPriceOptional, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, matchEventIndicator, offset9Padding1, mdEntryType, offset11Padding1, mdEntryPositionNo, mdEntrySizeQuantity, secondaryOrderId, transactTime, rptSeq, mdCorporateOffsetPriceOptional }, bytes)
...
def decode (bytes : List UInt8) : Option (MassDeleteOrdersMbO52Message × List UInt8) := do
  ...
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (rptSeq, bytes) ← decodeUIntLE 4 bytes
  pure ({ securityId, matchEventIndicator, mdUpdateAction, mdEntryType, offset11Padding1, mdEntryPositionNo, transactTime, rptSeq }, bytes)
...
def decode (bytes : List UInt8) : Option (SnapshotFullRefreshOrdersMbO71MessageNoMDEntriesGroup × List UInt8) := do
  ...
  let (mdEntryType, bytes) ← MdEntryType.decode bytes
  let (matchEventIndicator, bytes) ← decodeUIntLE 1 bytes
  pure ({ mdCorporateOffsetPriceOptional, mdEntrySizeQuantity, mdEntryPositionNo, enteringFirm, mdInsertTimestamp, secondaryOrderId, mdEntryType, matchEventIndicator }, bytes)
...
end Omi.B3B3derivativesBinaryumdfSbeV21
