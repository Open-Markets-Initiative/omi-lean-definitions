...
namespace Omi.B3B3derivativesBinaryentrypointSbeV84
...
/-- Ord Type Cross Ord Type optional: one byte code -/
def OrdTypeCrossOrdTypeOptional.codes : List UInt8 :=
  [0x31, 0x32]

inductive OrdTypeCrossOrdTypeOptional where
  | market -- Market
  | limit -- Limit
  | unlisted (byte : { byte : UInt8 // byte ∉ OrdTypeCrossOrdTypeOptional.codes }) -- any other code, kept as it is
  deriving DecidableEq, Repr

namespace OrdTypeCrossOrdTypeOptional

def toByte : OrdTypeCrossOrdTypeOptional → UInt8
  | .market => 0x31
  | .limit => 0x32
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : OrdTypeCrossOrdTypeOptional :=
  if byte = 0x31 then .market
  else .limit

def ofByte (byte : UInt8) : OrdTypeCrossOrdTypeOptional :=
  if known : byte ∈ codes then listed byte else .unlisted ⟨byte, known⟩

theorem ofByte_toByte (value : OrdTypeCrossOrdTypeOptional) : ofByte value.toByte = value := by
  cases value with
  | market => decide
  | limit => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]

def encode (value : OrdTypeCrossOrdTypeOptional) : List UInt8 :=
  [value.toByte]

def decode : List UInt8 → Option (OrdTypeCrossOrdTypeOptional × List UInt8)
  | byte :: rest => some (ofByte byte, rest)
  | [] => none

@[simp] theorem encode_length (value : OrdTypeCrossOrdTypeOptional) : (encode value).length = 1 :=
  rfl

@[simp] theorem decode_encode (value : OrdTypeCrossOrdTypeOptional) (rest : List UInt8) :
    decode (encode value ++ rest) = some (value, rest) := by
  simp [decode, encode, ofByte_toByte]

end OrdTypeCrossOrdTypeOptional
...
def OrderCategory.codes : List UInt8 :=
  [0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x37]
...
def listed (byte : UInt8) : OrderCategory :=
  ...
  else if byte = 0x48 then .resultOfTradeAtAverage
  else .impliedOrder
...
/-- Semantic Version: 4 bytes -/
structure SemanticVersion where
  majorNumber : BitVec 8
  minorNumber : BitVec 8
  patchNumber : BitVec 8
  buildNumber : BitVec 8
  deriving DecidableEq, Repr

namespace SemanticVersion

def encode (message : SemanticVersion) : List UInt8 :=
  encodeUInt 1 message.majorNumber
    ++ (encodeUInt 1 message.minorNumber
    ++ (encodeUInt 1 message.patchNumber
    ++ (encodeUInt 1 message.buildNumber)))

def decode (bytes : List UInt8) : Option (SemanticVersion × List UInt8) := do
  let (majorNumber, bytes) ← decodeUInt 1 bytes
  let (minorNumber, bytes) ← decodeUInt 1 bytes
  let (patchNumber, bytes) ← decodeUInt 1 bytes
  let (buildNumber, bytes) ← decodeUInt 1 bytes
  pure ({ majorNumber, minorNumber, patchNumber, buildNumber }, bytes)

@[simp] theorem encode_length (message : SemanticVersion) : (encode message).length = 4 := by
  unfold encode
  simp only [List.length_append, encodeUInt_length]

theorem encode_length_pos (message : SemanticVersion) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : SemanticVersion) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
...
/-- Negotiate Response Message: 28 bytes -/
...
def encode (message : NegotiateResponseMessage) : List UInt8 :=
  encodeUIntLE 4 message.sessionId
    ...
    ++ (encodeUIntLE 4 message.enteringFirm
    ++ (SemanticVersion.encode message.semanticVersion))))
...
def decode (bytes : List UInt8) : Option (NegotiateResponseMessage × List UInt8) := do
  ...
  let (enteringFirm, bytes) ← decodeUIntLE 4 bytes
  let (semanticVersion, bytes) ← SemanticVersion.decode bytes
  pure ({ sessionId, sessionVerId, requestTimestamp, enteringFirm, semanticVersion }, bytes)

@[simp] theorem encode_length (message : NegotiateResponseMessage) : (encode message).length = 28 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, SemanticVersion.encode_length]
...
@[simp] theorem decode_encode (message : NegotiateResponseMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [SemanticVersion.decode_encode, some_bind]
  rfl
...
/-- Establish Ack Message: 40 bytes -/
...
def decode (bytes : List UInt8) : Option (EstablishAckMessage × List UInt8) := do
  ...
  let (semanticVersion, bytes) ← SemanticVersion.decode bytes
  pure ({ sessionId, sessionVerId, requestTimestamp, keepAliveInterval, nextSeqNo, lastIncomingSeqNo, semanticVersion }, bytes)

@[simp] theorem encode_length (message : EstablishAckMessage) : (encode message).length = 40 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, SemanticVersion.encode_length]
...
def encode (message : InvestorId) : List UInt8 :=
  encodeUIntLE 2 message.prefix_
    ++ (Alpha.encode message.offset2Padding2
    ++ (encodeUIntLE 4 message.document))

def decode (bytes : List UInt8) : Option (InvestorId × List UInt8) := do
  let (prefix_, bytes) ← decodeUIntLE 2 bytes
  let (offset2Padding2, bytes) ← Alpha.decode 2 bytes
  let (document, bytes) ← decodeUIntLE 4 bytes
  pure ({ prefix_, offset2Padding2, document }, bytes)

@[simp] theorem encode_length (message : InvestorId) : (encode message).length = 8 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (SimpleNewOrderMessage × List UInt8) := do
  ...
  pure ({ inboundBusinessHeader, ordtagid, mmProtectionReset, clordid, account, senderLocation, enteringTrader, selfTradePreventionInstruction, securityId, side, simpleOrdtype, timeInForceSimple, routingInstruction, orderQty, priceOptional, investorId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SimpleNewOrderMessage) : (encode message).length ≤ 340 := by
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, InboundBusinessHeader.encode_length, encodeUInt_length, encodeUIntLE_length, Alpha.encode_length, Side.encode_length, SimpleOrdtype.encode_length, TimeInForceSimple.encode_length, InvestorId.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (SimpleModifyOrderMessage × List UInt8) := do
  ...
  let (investorId, bytes) ← InvestorId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ inboundBusinessHeader, ordtagid, mmProtectionReset, clordid, account, senderLocation, enteringTrader, selfTradePreventionInstruction, securityId, side, simpleOrdtype, timeInForceSimple, routingInstruction, orderQty, priceOptional, orderIdOptional, origclordid, investorId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SimpleModifyOrderMessage) : (encode message).length ≤ 356 := by
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, InboundBusinessHeader.encode_length, encodeUInt_length, encodeUIntLE_length, Alpha.encode_length, Side.encode_length, SimpleOrdtype.encode_length, TimeInForceSimple.encode_length, InvestorId.encode_length]
  omega
...
def encode (message : NewOrderSingleMessage) : List UInt8 :=
  InboundBusinessHeader.encode message.inboundBusinessHeader
    ...
    ++ (encodeUIntLE 4 message.tradingSubAccount
...
def decode (bytes : List UInt8) : Option (NewOrderSingleMessage × List UInt8) := do
  ...
  let (tradingSubAccount, bytes) ← decodeUIntLE 4 bytes
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ inboundBusinessHeader, ordtagid, mmProtectionReset, clordid, account, senderLocation, enteringTrader, selfTradePreventionInstruction, securityId, side, ordtype, timeInForce, routingInstruction, orderQty, priceOptional, stopPx, minQty, maxFloor, executingTraderOptional, expireDate, custodianInfo, investorId, strategyId, tradingSubAccount, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderSingleMessage) : (encode message).length ≤ 647 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, InboundBusinessHeader.encode_length, encodeUInt_length, encodeUIntLE_length, Alpha.encode_length, Side.encode_length, Ordtype.encode_length, TimeInForce.encode_length, CustodianInfo.encode_length, InvestorId.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (OrderCancelReplaceRequestMessage × List UInt8) := do
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ inboundBusinessHeader, ordtagid, mmProtectionReset, clordid, account, senderLocation, enteringTrader, selfTradePreventionInstruction, securityId, side, ordtype, timeInForceOptional, routingInstruction, orderQty, priceOptional, orderIdOptional, origclordid, stopPx, minQty, maxFloor, executingTraderOptional, accountType, expireDate, custodianInfo, investorId, strategyId, tradingSubAccount, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderCancelReplaceRequestMessage) : (encode message).length ≤ 664 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, InboundBusinessHeader.encode_length, encodeUInt_length, encodeUIntLE_length, Alpha.encode_length, Side.encode_length, Ordtype.encode_length, TimeInForceOptional.encode_length, CustodianInfo.encode_length, InvestorId.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (CrossSidesGroup × List UInt8) := do
  ...
  let (clordid, bytes) ← decodeUIntLE 8 bytes
  let (tradingSubAccount, bytes) ← decodeUIntLE 4 bytes
  pure ({ side, offset1Padding1, account, enteringFirmOptional, clordid, tradingSubAccount }, bytes)

@[simp] theorem encode_length (message : CrossSidesGroup) : (encode message).length = 22 := by
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : CrossSidesGroups) : (encode message).length ≤ 5613 := by
  have bound_crossSidesGroup := message.crossSidesGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const CrossSidesGroup.encode 22 CrossSidesGroup.encode_length]
  omega
...
structure NewOrderCrossMessage where
  inboundBusinessHeader : InboundBusinessHeader
  ordTypeCrossOrdTypeOptional : OrdTypeCrossOrdTypeOptional
...
def encode (message : NewOrderCrossMessage) : List UInt8 :=
  InboundBusinessHeader.encode message.inboundBusinessHeader
    ++ (OrdTypeCrossOrdTypeOptional.encode message.ordTypeCrossOrdTypeOptional
    ...
    ++ (encodeUIntLE 8 message.maxSweepQty
...
def decode (bytes : List UInt8) : Option (NewOrderCrossMessage × List UInt8) := do
  let (inboundBusinessHeader, bytes) ← InboundBusinessHeader.decode bytes
  let (ordTypeCrossOrdTypeOptional, bytes) ← OrdTypeCrossOrdTypeOptional.decode bytes
  ...
  let (maxSweepQty, bytes) ← decodeUIntLE 8 bytes
  let (crossSidesGroups, bytes) ← CrossSidesGroups.decode bytes
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ inboundBusinessHeader, ordTypeCrossOrdTypeOptional, offset19Padding1, crossid, senderLocation, enteringTrader, executingTraderOptional, securityId, orderQty, priceOptional, crossedIndicator, crossType, crossPrioritization, maxSweepQty, crossSidesGroups, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderCrossMessage) : (encode message).length ≤ 6209 := by
  have bound_crossSidesGroups := CrossSidesGroups.encode_length_le message.crossSidesGroups
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, InboundBusinessHeader.encode_length, OrdTypeCrossOrdTypeOptional.encode_length, Alpha.encode_length, encodeUIntLE_length, encodeUInt_length]
  omega
...
@[simp] theorem decode_encode (message : NewOrderCrossMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, InboundBusinessHeader.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, OrdTypeCrossOrdTypeOptional.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
...
def encode (message : OutboundBusinessHeader) : List UInt8 :=
  encodeUIntLE 4 message.sessionId
    ++ (encodeUIntLE 4 message.msgSeqNum
    ++ (encodeUIntLE 8 message.sendingTime
    ++ (encodeUInt 1 message.eventIndicator
...
def decode (bytes : List UInt8) : Option (OutboundBusinessHeader × List UInt8) := do
  ...
  let (msgSeqNum, bytes) ← decodeUIntLE 4 bytes
  let (sendingTime, bytes) ← decodeUIntLE 8 bytes
  let (eventIndicator, bytes) ← decodeUInt 1 bytes
  let (marketSegmentIdOptional, bytes) ← decodeUInt 1 bytes
  pure ({ sessionId, msgSeqNum, sendingTime, eventIndicator, marketSegmentIdOptional }, bytes)
...
structure ExecutionReportNewMessage where
  ...
  receivedTime : BitVec 64
  offset152Padding3 : Alpha 3
  ordtagid : BitVec 8
  investorId : InvestorId
  crossType : BitVec 8
  crossPrioritization : BitVec 8
  mmProtectionResetOptional : BitVec 8
  offset167Padding1 : Alpha 1
  strategyId : BitVec 32
  tradingSubAccount : BitVec 32
  deskId : DeskId
  memo : Memo
...
def encode (message : ExecutionReportNewMessage) : List UInt8 :=
  OutboundBusinessHeader.encode message.outboundBusinessHeader
    ...
    ++ (encodeUIntLE 8 message.receivedTime
    ++ (Alpha.encode message.offset152Padding3
    ++ (encodeUInt 1 message.ordtagid
    ++ (InvestorId.encode message.investorId
    ++ (encodeUInt 1 message.crossType
    ++ (encodeUInt 1 message.crossPrioritization
    ++ (encodeUInt 1 message.mmProtectionResetOptional
    ++ (Alpha.encode message.offset167Padding1
    ++ (encodeUIntLE 4 message.strategyId
    ++ (encodeUIntLE 4 message.tradingSubAccount
    ++ (DeskId.encode message.deskId
    ++ (Memo.encode message.memo)))))))))))))))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (ExecutionReportNewMessage × List UInt8) := do
  ...
  let (receivedTime, bytes) ← decodeUIntLE 8 bytes
  let (offset152Padding3, bytes) ← Alpha.decode 3 bytes
  let (ordtagid, bytes) ← decodeUInt 1 bytes
  let (investorId, bytes) ← InvestorId.decode bytes
  let (crossType, bytes) ← decodeUInt 1 bytes
  let (crossPrioritization, bytes) ← decodeUInt 1 bytes
  let (mmProtectionResetOptional, bytes) ← decodeUInt 1 bytes
  let (offset167Padding1, bytes) ← Alpha.decode 1 bytes
  let (strategyId, bytes) ← decodeUIntLE 4 bytes
  let (tradingSubAccount, bytes) ← decodeUIntLE 4 bytes
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ outboundBusinessHeader, side, ordStatus, clordid, secondaryOrderId, securityId, orderId, account, execId, transactTime, marketSegmentReceivedTime, protectionPrice, tradeDate, workingIndicator, multiLegReportingType, ordtype, timeInForce, expireDate, orderQty, priceOptional, stopPx, minQty, maxFloor, crossidOptional, receivedTime, offset152Padding3, ordtagid, investorId, crossType, crossPrioritization, mmProtectionResetOptional, offset167Padding1, strategyId, tradingSubAccount, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportNewMessage) : (encode message).length ≤ 688 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, OrdStatus.encode_length, encodeUIntLE_length, encodeUInt_length, MultiLegReportingType.encode_length, Ordtype.encode_length, TimeInForce.encode_length, Alpha.encode_length, InvestorId.encode_length]
  omega
...
def encode (message : ExecutionReportModifyMessage) : List UInt8 :=
  OutboundBusinessHeader.encode message.outboundBusinessHeader
    ...
    ++ (encodeUIntLE 8 message.maxFloor
    ++ (encodeUIntLE 8 message.receivedTime
    ++ (Alpha.encode message.offset168Padding3
    ++ (encodeUInt 1 message.ordtagid
    ++ (InvestorId.encode message.investorId
    ++ (encodeUInt 1 message.mmProtectionResetOptional
...
def decode (bytes : List UInt8) : Option (ExecutionReportModifyMessage × List UInt8) := do
  ...
  let (maxFloor, bytes) ← decodeUIntLE 8 bytes
  let (receivedTime, bytes) ← decodeUIntLE 8 bytes
  let (offset168Padding3, bytes) ← Alpha.decode 3 bytes
  let (ordtagid, bytes) ← decodeUInt 1 bytes
  let (investorId, bytes) ← InvestorId.decode bytes
  let (mmProtectionResetOptional, bytes) ← decodeUInt 1 bytes
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ outboundBusinessHeader, side, ordStatus, clordid, secondaryOrderId, securityId, leavesQty, account, execId, transactTime, cumQty, marketSegmentReceivedTime, orderId, origclordid, protectionPrice, tradeDate, workingIndicator, multiLegReportingType, ordtype, timeInForce, expireDate, orderQty, priceOptional, stopPx, minQty, maxFloor, receivedTime, offset168Padding3, ordtagid, investorId, mmProtectionResetOptional, execRestatementReason, strategyId, tradingSubAccount, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportModifyMessage) : (encode message).length ≤ 702 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, OrdStatus.encode_length, encodeUIntLE_length, encodeUInt_length, MultiLegReportingType.encode_length, Ordtype.encode_length, TimeInForce.encode_length, Alpha.encode_length, InvestorId.encode_length]
...
def encode (message : ExecutionReportCancelMessage) : List UInt8 :=
  OutboundBusinessHeader.encode message.outboundBusinessHeader
    ...
    ++ (encodeUInt 1 message.workingIndicator
    ++ (encodeUInt 1 message.execRestatementReason
    ++ (Alpha.encode message.offset100Padding4
    ...
    ++ (encodeUIntLE 8 message.maxFloor
    ++ (encodeUIntLE 8 message.receivedTime
    ++ (Alpha.encode message.offset164Padding3
    ++ (encodeUInt 1 message.ordtagid
    ++ (InvestorId.encode message.investorId
    ++ (encodeUIntLE 4 message.strategyId
...
def decode (bytes : List UInt8) : Option (ExecutionReportCancelMessage × List UInt8) := do
  ...
  let (workingIndicator, bytes) ← decodeUInt 1 bytes
  let (execRestatementReason, bytes) ← decodeUInt 1 bytes
  let (offset100Padding4, bytes) ← Alpha.decode 4 bytes
  ...
  let (maxFloor, bytes) ← decodeUIntLE 8 bytes
  let (receivedTime, bytes) ← decodeUIntLE 8 bytes
  let (offset164Padding3, bytes) ← Alpha.decode 3 bytes
  let (ordtagid, bytes) ← decodeUInt 1 bytes
  let (investorId, bytes) ← InvestorId.decode bytes
  let (strategyId, bytes) ← decodeUIntLE 4 bytes
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ outboundBusinessHeader, side, ordStatus, clordid, secondaryOrderId, securityId, cumQty, account, execId, transactTime, marketSegmentReceivedTime, orderId, origclordid, tradeDate, workingIndicator, execRestatementReason, offset100Padding4, massActionReportIdOptional, ordtype, timeInForce, expireDate, orderQty, priceOptional, stopPx, minQty, maxFloor, receivedTime, offset164Padding3, ordtagid, investorId, strategyId, actionRequestedFromSessionId, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportCancelMessage) : (encode message).length ≤ 696 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, OrdStatus.encode_length, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length, Ordtype.encode_length, TimeInForce.encode_length, InvestorId.encode_length]
  omega
...
/-- Implied Event Id: 6 bytes -/
structure ImpliedEventId where
  eventId : BitVec 32
  noRelatedTrades : BitVec 16
  deriving DecidableEq, Repr

namespace ImpliedEventId

def encode (message : ImpliedEventId) : List UInt8 :=
  encodeUIntLE 4 message.eventId
    ++ (encodeUIntLE 2 message.noRelatedTrades)

def decode (bytes : List UInt8) : Option (ImpliedEventId × List UInt8) := do
  let (eventId, bytes) ← decodeUIntLE 4 bytes
  let (noRelatedTrades, bytes) ← decodeUIntLE 2 bytes
  pure ({ eventId, noRelatedTrades }, bytes)

@[simp] theorem encode_length (message : ImpliedEventId) : (encode message).length = 6 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]

theorem encode_length_pos (message : ImpliedEventId) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : ImpliedEventId) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
...
structure ExecutionReportTradeMessage where
  ...
  crossType : BitVec 8
  crossPrioritization : BitVec 8
  offset159Padding1 : Alpha 1
  strategyId : BitVec 32
  impliedEventId : ImpliedEventId
  tradingSubAccount : BitVec 32
  deskId : DeskId
  memo : Memo
  deriving DecidableEq, Repr
...
def encode (message : ExecutionReportTradeMessage) : List UInt8 :=
  OutboundBusinessHeader.encode message.outboundBusinessHeader
    ...
    ++ (encodeUInt 1 message.crossType
    ++ (encodeUInt 1 message.crossPrioritization
    ++ (Alpha.encode message.offset159Padding1
    ++ (encodeUIntLE 4 message.strategyId
    ++ (ImpliedEventId.encode message.impliedEventId
    ++ (encodeUIntLE 4 message.tradingSubAccount
    ++ (DeskId.encode message.deskId
    ++ (Memo.encode message.memo))))))))))))))))))))))))))))))))))))))
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeMessage) : (encode message).length ≤ 686 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, OrdStatus.encode_length, encodeUIntLE_length, encodeUInt_length, ExecType.encode_length, OrderCategory.encode_length, MultiLegReportingType.encode_length, Alpha.encode_length, ImpliedEventId.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ExecutionReportTradeMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, ImpliedEventId.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, DeskId.decode_encode, some_bind]
...
def encode (message : ExecutionReportRejectMessage) : List UInt8 :=
  OutboundBusinessHeader.encode message.outboundBusinessHeader
    ...
    ++ (encodeUIntLE 2 message.crossedIndicator
    ++ (encodeUIntLE 8 message.receivedTime
    ++ (Alpha.encode message.offset146Padding3
    ++ (encodeUInt 1 message.ordtagid
    ++ (InvestorId.encode message.investorId
    ++ (encodeUIntLE 4 message.strategyId
    ++ (encodeUIntLE 4 message.tradingSubAccount
...
def decode (bytes : List UInt8) : Option (ExecutionReportRejectMessage × List UInt8) := do
  ...
  let (crossedIndicator, bytes) ← decodeUIntLE 2 bytes
  let (receivedTime, bytes) ← decodeUIntLE 8 bytes
  let (offset146Padding3, bytes) ← Alpha.decode 3 bytes
  let (ordtagid, bytes) ← decodeUInt 1 bytes
  let (investorId, bytes) ← InvestorId.decode bytes
  let (strategyId, bytes) ← decodeUIntLE 4 bytes
  let (tradingSubAccount, bytes) ← decodeUIntLE 4 bytes
  ...
  pure ({ outboundBusinessHeader, side, cxlRejResponseTo, clordid, secondaryOrderIdOptional, securityId, ordRejReason, transactTime, execId, orderIdOptional, origclordid, account, ordtype, timeInForce, expireDate, orderQtyOptional, priceOptional, stopPx, minQty, maxFloor, crossidOptional, crossedIndicator, receivedTime, offset146Padding3, ordtagid, investorId, strategyId, tradingSubAccount, deskId, memo, text }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportRejectMessage) : (encode message).length ≤ 934 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  have bound_text := Text.encode_length_le message.text
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, Side.encode_length, encodeUInt_length, encodeUIntLE_length, Ordtype.encode_length, TimeInForce.encode_length, Alpha.encode_length, InvestorId.encode_length]
  omega
...
structure ExecutionReportForwardMessage where
  ...
  secondaryExecId : BitVec 64
  execRefId : BitVec 64
  fixedRatePercentage8Optional : BitVec 64
  orderQty : BitVec 64
  tradingSessionId : BitVec 8
  tradingSessionSubId : BitVec 8
  securityTradingStatus : BitVec 8
...
def encode (message : ExecutionReportForwardMessage) : List UInt8 :=
  OutboundBusinessHeader.encode message.outboundBusinessHeader
    ...
    ++ (encodeUIntLE 8 message.secondaryExecId
    ++ (encodeUIntLE 8 message.execRefId
    ++ (encodeUIntLE 8 message.fixedRatePercentage8Optional
    ++ (encodeUIntLE 8 message.orderQty
    ++ (encodeUInt 1 message.tradingSessionId
    ++ (encodeUInt 1 message.tradingSessionSubId
    ++ (encodeUInt 1 message.securityTradingStatus
...
def decode (bytes : List UInt8) : Option (ExecutionReportForwardMessage × List UInt8) := do
  ...
  let (secondaryExecId, bytes) ← decodeUIntLE 8 bytes
  let (execRefId, bytes) ← decodeUIntLE 8 bytes
  let (fixedRatePercentage8Optional, bytes) ← decodeUIntLE 8 bytes
  let (orderQty, bytes) ← decodeUIntLE 8 bytes
  let (tradingSessionId, bytes) ← decodeUInt 1 bytes
  let (tradingSessionSubId, bytes) ← decodeUInt 1 bytes
  let (securityTradingStatus, bytes) ← decodeUInt 1 bytes
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ outboundBusinessHeader, side, ordStatus, clordidOptional, secondaryOrderId, securityId, account, lastQty, lastPx, execId, transactTime, leavesQty, cumQty, tradeId, contraBroker, orderId, aggressorIndicator, settlTypeOptional, tradeDate, daysToSettlementOptional, offset118Padding2, secondaryExecId, execRefId, fixedRatePercentage8Optional, orderQty, tradingSessionId, tradingSessionSubId, securityTradingStatus, tradingSubAccount, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportForwardMessage) : (encode message).length ≤ 671 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
def decode (bytes : List UInt8) : Option (SecurityDefinitionResponseMessage × List UInt8) := do
  ...
  let (senderLocation, bytes) ← Alpha.decode 10 bytes
  let (enteringTrader, bytes) ← Alpha.decode 5 bytes
  pure ({ outboundBusinessHeader, offset18Padding2, securityReqId, securityIdOptional, securityResponseType, securityStrategyType, symbol, securityResponseId, senderLocation, enteringTrader }, bytes)
...
def decode (bytes : List UInt8) : Option (BidirectionalBusinessHeader × List UInt8) := do
  ...
  let (marketSegmentIdOptional, bytes) ← decodeUInt 1 bytes
  let (padding, bytes) ← Alpha.decode 1 bytes
  pure ({ sessionId, msgSeqNum, sendingTime, eventIndicator, marketSegmentIdOptional, padding }, bytes)
...
def decode (bytes : List UInt8) : Option (SidesGroup × List UInt8) := do
  ...
  let (account, bytes) ← decodeUIntLE 4 bytes
  let (tradingSubAccount, bytes) ← decodeUIntLE 4 bytes
  pure ({ side, account, tradingSubAccount }, bytes)

@[simp] theorem encode_length (message : SidesGroup) : (encode message).length = 9 := by
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SidesGroups) : (encode message).length ≤ 2298 := by
  have bound_sidesGroup := message.sidesGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SidesGroup.encode 9 SidesGroup.encode_length]
  omega
...
structure QuoteRequestMessage where
  ...
  fixedRatePercentage8 : BitVec 64
...
def encode (message : QuoteRequestMessage) : List UInt8 :=
  BidirectionalBusinessHeader.encode message.bidirectionalBusinessHeader
    ...
    ++ (encodeUIntLE 8 message.fixedRatePercentage8
...
def decode (bytes : List UInt8) : Option (QuoteRequestMessage × List UInt8) := do
  ...
  let (fixedRatePercentage8, bytes) ← decodeUIntLE 8 bytes
  let (daysToSettlement, bytes) ← decodeUIntLE 2 bytes
  let (sidesGroups, bytes) ← SidesGroups.decode bytes
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ bidirectionalBusinessHeader, securityId, quoteReqId, quoteIdOptional, tradeIdOptional, contraBroker, transactTime, price8, settlType, executeUnderlyingTrade, orderQty, senderLocation, enteringTrader, executingTrader, fixedRatePercentage8, daysToSettlement, sidesGroups, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteRequestMessage) : (encode message).length ≤ 2917 := by
...
def encode (message : QuoteStatusReportMessage) : List UInt8 :=
  BidirectionalBusinessHeader.encode message.bidirectionalBusinessHeader
    ...
    ++ (SettlTypeOptional.encode message.settlTypeOptional
    ++ (encodeUIntLE 8 message.price8Optional
    ++ (encodeUIntLE 8 message.orderQty
    ++ (Alpha.encode message.senderLocation
    ++ (Alpha.encode message.enteringTrader
    ++ (Alpha.encode message.executingTrader
...
def decode (bytes : List UInt8) : Option (QuoteStatusReportMessage × List UInt8) := do
  ...
  let (settlTypeOptional, bytes) ← SettlTypeOptional.decode bytes
  let (price8Optional, bytes) ← decodeUIntLE 8 bytes
  let (orderQty, bytes) ← decodeUIntLE 8 bytes
  let (senderLocation, bytes) ← Alpha.decode 10 bytes
  let (enteringTrader, bytes) ← Alpha.decode 5 bytes
  let (executingTrader, bytes) ← Alpha.decode 5 bytes
  ...
  let (memo, bytes) ← Memo.decode bytes
  let (text, bytes) ← Text.decode bytes
  pure ({ bidirectionalBusinessHeader, quoteRejectReason, securityId, quoteReqId, quoteId, tradeIdOptional, contraBroker, transactTime, quoteStatus, quoteStatusResponseTo, account, sideOptional, settlTypeOptional, price8Optional, orderQty, senderLocation, enteringTrader, executingTrader, fixedRatePercentage8Optional, executeUnderlyingTrade, daysToSettlementOptional, tradingSubAccount, deskId, memo, text }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteStatusReportMessage) : (encode message).length ≤ 890 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  have bound_text := Text.encode_length_le message.text
  unfold encode
...
def decode (bytes : List UInt8) : Option (QuoteMessage × List UInt8) := do
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ bidirectionalBusinessHeader, securityId, quoteReqId, quoteId, transactTime, price8Optional, orderQty, side, settlType, account, senderLocation, enteringTrader, executingTrader, fixedRatePercentage8, executeUnderlyingTrade, daysToSettlement, tradingSubAccount, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteMessage) : (encode message).length ≤ 620 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
def decode (bytes : List UInt8) : Option (QuoteCancelMessage × List UInt8) := do
  ...
  let (deskId, bytes) ← DeskId.decode bytes
  let (memo, bytes) ← Memo.decode bytes
  pure ({ bidirectionalBusinessHeader, securityId, quoteReqIdOptional, quoteIdOptional, account, senderLocation, enteringTrader, executingTrader, deskId, memo }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteCancelMessage) : (encode message).length ≤ 579 := by
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  unfold encode
...
def decode (bytes : List UInt8) : Option (QuoteRequestRejectMessage × List UInt8) := do
  ...
  let (memo, bytes) ← Memo.decode bytes
  let (text, bytes) ← Text.decode bytes
  pure ({ bidirectionalBusinessHeader, quoteRequestRejectReason, securityId, quoteReqId, quoteIdOptional, tradeIdOptional, contraBroker, transactTime, enteringTrader, settlTypeOptional, price8Optional, orderQtyOptional, senderLocation, executingTrader, fixedRatePercentage8Optional, daysToSettlementOptional, sidesGroups, deskId, memo, text }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuoteRequestRejectMessage) : (encode message).length ≤ 3176 := by
  have bound_sidesGroups := SidesGroups.encode_length_le message.sidesGroups
  have bound_deskId := DeskId.encode_length_le message.deskId
  have bound_memo := Memo.encode_length_le message.memo
  have bound_text := Text.encode_length_le message.text
...
def decode (bytes : List UInt8) : Option (OrderMassActionRequestMessage × List UInt8) := do
  ...
  let (securityIdOptional, bytes) ← decodeUIntLE 8 bytes
  let (investorId, bytes) ← InvestorId.decode bytes
  pure ({ inboundBusinessHeader, massActionType, massActionScope, clordid, massCancelRestatementReason, ordtagid, sideOptional, offset31Padding1, asset, securityIdOptional, investorId }, bytes)

@[simp] theorem encode_length (message : OrderMassActionRequestMessage) : (encode message).length = 54 := by
  unfold encode
  simp only [List.length_append, InboundBusinessHeader.encode_length, encodeUInt_length, encodeUIntLE_length, SideOptional.encode_length, Alpha.encode_length, InvestorId.encode_length]
...
@[simp] theorem decode_encode (message : OrderMassActionRequestMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [InvestorId.decode_encode, some_bind]
  rfl
...
def decode (bytes : List UInt8) : Option (OrderMassActionReportMessage × List UInt8) := do
  ...
  let (investorId, bytes) ← InvestorId.decode bytes
  let (text, bytes) ← Text.decode bytes
  pure ({ outboundBusinessHeader, massActionType, massActionScope, clordid, massActionReportId, transactTime, massActionResponse, massActionRejectReason, massCancelRestatementReason, ordtagid, sideOptional, offset49Padding1, asset, securityIdOptional, investorId, text }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderMassActionReportMessage) : (encode message).length ≤ 328 := by
  have bound_text := Text.encode_length_le message.text
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, OutboundBusinessHeader.encode_length, encodeUInt_length, encodeUIntLE_length, MassActionResponse.encode_length, SideOptional.encode_length, Alpha.encode_length, InvestorId.encode_length]
  omega
...
theorem encode_length_le (message : Payload) : (encode message).length ≤ 7694 := by
  cases message with
  ...
  | negotiateResponseMessage inner =>
    simp only [encode, NegotiateResponseMessage.encode_length]
    omega
  | negotiateRejectMessage inner =>
    simp only [encode, NegotiateRejectMessage.encode_length]
    omega
  ...
  | establishAckMessage inner =>
    simp only [encode, EstablishAckMessage.encode_length]
    omega
  | establishRejectMessage inner =>
    simp only [encode, EstablishRejectMessage.encode_length]
    omega
  | terminateMessage inner =>
    simp only [encode, TerminateMessage.encode_length]
    omega
  | notAppliedMessage inner =>
    simp only [encode, NotAppliedMessage.encode_length]
    omega
  ...
  | retransmissionMessage inner =>
    simp only [encode, RetransmissionMessage.encode_length]
    omega
  ...
  | securityDefinitionResponseMessage inner =>
    simp only [encode, SecurityDefinitionResponseMessage.encode_length]
    omega
  ...
  | positionMaintenanceCancelRequestMessage inner =>
    simp only [encode, PositionMaintenanceCancelRequestMessage.encode_length]
    omega
  ...
  | allocationReportMessage inner =>
    simp only [encode, AllocationReportMessage.encode_length]
    omega
  | orderMassActionRequestMessage inner =>
    simp only [encode, OrderMassActionRequestMessage.encode_length]
    omega
...
end Omi.B3B3derivativesBinaryentrypointSbeV84
