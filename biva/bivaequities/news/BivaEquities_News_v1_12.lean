...
/-!
# Bolsa Institucional de Valores News v1.12
...
Note: Title is read up to the first NullCharacter and carries the proof it holds none itself, which is what lets it be written back with the one that closes it.

Note: Reference is read up to the first NullCharacter and carries the proof it holds none itself, which is what lets it be written back with the one that closes it.

Note: News Text is read up to the first NullCharacter and carries the proof it holds none itself, which is what lets it be written back with the one that closes it.
...
namespace Omi.BivaBivaequitiesNewsItchV112
...
structure NewsMessage where
  ...
  participantId : BitVec 32
  title : Terminated 0
  reference : Terminated 0
  newsText : Terminated 0
  deriving DecidableEq, Repr
...
def encode (message : NewsMessage) : List UInt8 :=
  encodeUInt 4 message.nanoseconds
    ...
    ++ (encodeUInt 4 message.participantId
    ++ (encodeTerminated 0 message.title
    ++ (encodeTerminated 0 message.reference
    ++ (encodeTerminated 0 message.newsText))))))
...
def decode (bytes : List UInt8) : Option (NewsMessage × List UInt8) := do
  ...
  let (participantId, bytes) ← decodeUInt 4 bytes
  let (title, bytes) ← decodeTerminated 0 bytes
  let (reference, bytes) ← decodeTerminated 0 bytes
  let (newsText, bytes) ← decodeTerminated 0 bytes
  pure ({ nanoseconds, orderbook, newsId, participantId, title, reference, newsText }, bytes)
...
@[simp] theorem decode_encode (message : NewsMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, decodeTerminated_encodeTerminated 0, some_bind]
  dsimp only
  rw [List.append_assoc, decodeTerminated_encodeTerminated 0, some_bind]
  dsimp only
  rw [decodeTerminated_encodeTerminated 0, some_bind]
  rfl
...
end Omi.BivaBivaequitiesNewsItchV112
