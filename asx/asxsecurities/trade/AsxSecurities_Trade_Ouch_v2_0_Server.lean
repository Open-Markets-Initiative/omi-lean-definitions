...
namespace Omi.AsxAsxsecuritiesTradeOuchV20Server
...
def decode (bytes : List UInt8) : Option (OrderAcceptedMessage × List UInt8) := do
  ...
  pure ({ timestampNanoseconds, orderToken, orderBookId, side, orderId, quantity, price, timeInForce, openClose, clientAccount, orderState, customerInfo, exchangeInfo, clearingParticipant, crossingKey, capacityOfParticipant, directedWholesale, executionVenue, intermediaryId, orderOrigin, filler, ouchOrderType, shortSellQuantity, minimumAcceptableQuantity }, bytes)

@[simp] theorem encode_length (message : OrderAcceptedMessage) : (encode message).length = 173 := by
  unfold encode
  simp only [List.length_append, encodeUInt_length, Alpha.encode_length, Side.encode_length, CapacityOfParticipant.encode_length, DirectedWholesale.encode_length, OuchOrderType.encode_length]
...
def decode (bytes : List UInt8) : Option (OrderReplacedMessage × List UInt8) := do
  ...
  let (shortSellQuantity, bytes) ← decodeUInt 8 bytes
  let (minimumAcceptableQuantity, bytes) ← decodeUInt 8 bytes
  pure ({ timestampNanoseconds, replacementOrderToken, previousOrderToken, orderBookId, side, orderId, quantity, price, timeInForce, openClose, clientAccount, orderState, customerInfo, exchangeInfo, clearingParticipant, crossingKey, capacityOfParticipant, directedWholesale, executionVenue, intermediaryId, orderOrigin, filler, ouchOrderType, shortSellQuantity, minimumAcceptableQuantity }, bytes)
...
end Omi.AsxAsxsecuritiesTradeOuchV20Server
