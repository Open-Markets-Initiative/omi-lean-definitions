...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v13.0
...
Note: Xetra En Light Approve Deal Notification is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7XtiFbeV130Server
...
def decode (bytes : List UInt8) : Option LogonResponse := do
  ...
  if fits_publicKey : publicKey_.length < 256 ^ 2 then
    if fits_alignmentPadding : alignmentPadding_.length ≤ 4291821559 then
      pure { pad2, responseHeaderComp, throttleTimeInterval, throttleNoMsgs, throttleDisconnectLimit, heartBtInt, sessionInstanceId, latestPublicKeySeqNo, marketId, tradSesMode, defaultCstmApplVerId, defaultCstmApplVerSubId, publicKey := ⟨publicKey_, fits_publicKey⟩, alignmentPadding := ⟨alignmentPadding_, fits_alignmentPadding⟩ }
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : LogonResponse) : (encode message).length ≤ 4291887188 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OtcUploadBroadcast) : (encode message).length ≤ 14474 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 14634 := by
  have bound_fillsGrpComp := message.fillsGrpComp.length_lt
  have bound_orderEventGrpComp := message.orderEventGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, OrdStatus.encode_length, ExecType.encode_length, encodeMany_length_const FillsGrpComp.encode 32 FillsGrpComp.encode_length, encodeMany_length_const OrderEventGrpComp.encode 24 OrderEventGrpComp.encode_length]
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : RfqSpecialistBroadcast) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : SpecialistRfqReplyNotification) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : SpecialistRfqReplyResponse) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
/-- Xetra En Light Approve Deal Notification: 122 bytes -/
structure XetraEnLightApproveDealNotification where
  pad2 : Alpha 2
  rbcHeaderComp : RbcHeaderComp
  transactTime : BitVec 64
  securityId : BitVec 64
  ...
  tradeId : BitVec 32
  trdRptStatus : BitVec 8
  requestingSide : BitVec 8
  targetSide : BitVec 8
  messageEventSource : MessageEventSource
  partyExecutingFirm : Alpha 5
  partyExecutingTrader : Alpha 6
  requestingPartyExecutingFirm : Alpha 5
  requestingPartyExecutingTrader : Alpha 6
  targetPartyExecutingFirm : Alpha 5
  targetPartyExecutingTrader : Alpha 6
  pad7 : Alpha 7
  deriving DecidableEq, Repr

namespace XetraEnLightApproveDealNotification

def encode (message : XetraEnLightApproveDealNotification) : List UInt8 :=
  Alpha.encode message.pad2
    ++ RbcHeaderComp.encode message.rbcHeaderComp
    ++ encodeUIntLE 8 message.transactTime
    ++ encodeUIntLE 8 message.securityId
    ...
    ++ encodeUIntLE 4 message.tradeId
    ++ encodeUInt 1 message.trdRptStatus
    ++ encodeUInt 1 message.requestingSide
    ++ encodeUInt 1 message.targetSide
    ++ MessageEventSource.encode message.messageEventSource
    ++ Alpha.encode message.partyExecutingFirm
    ++ Alpha.encode message.partyExecutingTrader
    ++ Alpha.encode message.requestingPartyExecutingFirm
    ++ Alpha.encode message.requestingPartyExecutingTrader
    ++ Alpha.encode message.targetPartyExecutingFirm
    ++ Alpha.encode message.targetPartyExecutingTrader
    ++ Alpha.encode message.pad7

def decode (bytes : List UInt8) : Option (XetraEnLightApproveDealNotification × List UInt8) := do
  let (pad2, bytes) ← Alpha.decode 2 bytes
  let (rbcHeaderComp, bytes) ← RbcHeaderComp.decode bytes
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  let (securityId, bytes) ← decodeUIntLE 8 bytes
  ...
  let (tradeId, bytes) ← decodeUIntLE 4 bytes
  let (trdRptStatus, bytes) ← decodeUInt 1 bytes
  let (requestingSide, bytes) ← decodeUInt 1 bytes
  let (targetSide, bytes) ← decodeUInt 1 bytes
  let (messageEventSource, bytes) ← MessageEventSource.decode bytes
  let (partyExecutingFirm, bytes) ← Alpha.decode 5 bytes
  let (partyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (requestingPartyExecutingFirm, bytes) ← Alpha.decode 5 bytes
  let (requestingPartyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (targetPartyExecutingFirm, bytes) ← Alpha.decode 5 bytes
  let (targetPartyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, securityId, lastPx, lastQty, marketSegmentId, negotiationId, tradeId, trdRptStatus, requestingSide, targetSide, messageEventSource, partyExecutingFirm, partyExecutingTrader, requestingPartyExecutingFirm, requestingPartyExecutingTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, pad7 }, bytes)

@[simp] theorem encode_length (message : XetraEnLightApproveDealNotification) : (encode message).length = 122 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length]

theorem encode_length_pos (message : XetraEnLightApproveDealNotification) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : XetraEnLightApproveDealNotification) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [Alpha.decode_encode, Option.bind_some]
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : XetraEnLightApproveDealNotification) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing

end XetraEnLightApproveDealNotification
...
inductive ServerPayload where
  ...
  | xetraEnLightApproveDealNotification (message : XetraEnLightApproveDealNotification) -- 10818
...
def tag : ServerPayload → BitVec 16
  ...
  | .xetraEnLightApproveDealNotification _ => 10818
...
def encode : ServerPayload → List UInt8
  ...
  | .xetraEnLightApproveDealNotification message => XetraEnLightApproveDealNotification.encode message
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option ServerPayload :=
  ...
  else if tag = 10818 then (XetraEnLightApproveDealNotification.decode bytes).bind fun (message, rest) => if rest.isEmpty then some (.xetraEnLightApproveDealNotification message) else none
...
theorem decode_encode (message : ServerPayload) :
  ...
  | xetraEnLightApproveDealNotification message => simp [decode, encode, tag, XetraEnLightApproveDealNotification.decode_encode_nil]
...
theorem encodeBody_length_lt (message : ServerMessage) : (encodeBody message).length + 4 < 256 ^ 4 := by
  ...
  | userLogoutResponse inner =>
    ...
    omega
  | xetraEnLightApproveDealNotification inner =>
    simp only [ServerPayload.encode, List.length_append, encodeUIntLE_length, XetraEnLightApproveDealNotification.encode_length]
    omega
...
end Omi.EurexT7XtiFbeV130Server
