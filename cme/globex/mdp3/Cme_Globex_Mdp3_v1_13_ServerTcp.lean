...
theorem encode_length_le (message : MdInstrumentDefinitionFixedIncome) : (encode message).length ≤ 5960 := by
  ...
  have bound_lotTypeRulesGroups := LotTypeRulesGroups.encode_length_le message.lotTypeRulesGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, SecurityUpdateAction.encode_length, encodeUInt_length, Alpha.encode_length]
  omega
...
theorem encode_length_le (message : RepoUnderlyingsGroups) : (encode message).length ≤ 30093 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const RepoUnderlyingsGroup.encode 118 RepoUnderlyingsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : RepoUnderlyingsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 RepoUnderlyingsGroup.encode RepoUnderlyingsGroup.decode RepoUnderlyingsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.repoUnderlyingsGroup.length_lt]
  rfl
...
theorem encode_length_le (message : RepoRelatedInstrumentsGroups) : (encode message).length ≤ 8163 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const RepoRelatedInstrumentsGroup.encode 32 RepoRelatedInstrumentsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : RepoRelatedInstrumentsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 RepoRelatedInstrumentsGroup.encode RepoRelatedInstrumentsGroup.decode RepoRelatedInstrumentsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.repoRelatedInstrumentsGroup.length_lt]
  rfl
...
/-- Broken Dates Groups -/
structure BrokenDatesGroups where
  blockLength : BitVec 16
  brokenDatesGroup : Bounded 1 BrokenDatesGroup
  deriving DecidableEq, Repr

namespace BrokenDatesGroups

def encode (message : BrokenDatesGroups) : List UInt8 :=
  encodeUIntLE 2 message.blockLength
    ++ (encodeUInt 1 (BitVec.ofNat (8 * 1) message.brokenDatesGroup.val.length)
    ++ (encodeMany BrokenDatesGroup.encode message.brokenDatesGroup.val))

def decode (bytes : List UInt8) : Option (BrokenDatesGroups × List UInt8) := do
  let (blockLength, bytes) ← decodeUIntLE 2 bytes
  let (numInGroup, bytes) ← decodeUInt 1 bytes
  let (brokenDatesGroup_, bytes) ← decodeMany BrokenDatesGroup.decode numInGroup.toNat bytes
  if fits_brokenDatesGroup : brokenDatesGroup_.length < 256 ^ 1 then
    pure ({ blockLength, brokenDatesGroup := ⟨brokenDatesGroup_, fits_brokenDatesGroup⟩ }, bytes)
  else none

theorem encode_length_pos (message : BrokenDatesGroups) : (encode message).length > 0 := by
  unfold encode
  simp only [encodeUIntLE_length, List.length_append, ← Nat.add_assoc]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : BrokenDatesGroups) : (encode message).length ≤ 4083 := by
  have bound_brokenDatesGroup := message.brokenDatesGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const BrokenDatesGroup.encode 16 BrokenDatesGroup.encode_length]
  omega

@[simp] theorem decode_encode (message : BrokenDatesGroups) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 BrokenDatesGroup.encode BrokenDatesGroup.decode BrokenDatesGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.brokenDatesGroup.length_lt]
  rfl

end BrokenDatesGroups
...
structure MdInstrumentDefinitionRepo where
  ...
  brokenDatesGroups : BrokenDatesGroups
  deriving DecidableEq, Repr
...
def encode (message : MdInstrumentDefinitionRepo) : List UInt8 :=
  encodeUIntLE 1 message.matchEventIndicator
    ...
    ++ (BrokenDatesGroups.encode message.brokenDatesGroups))))))))))))))))))))))))))))))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionRepo × List UInt8) := do
  ...
  let (brokenDatesGroups, bytes) ← BrokenDatesGroups.decode bytes
  pure ({ matchEventIndicator, totNumReportsOptional, securityUpdateAction, lastUpdateTime, mdSecurityTradingStatus, applId, marketSegmentId, underlyingProduct, securityExchange, securityGroup, asset, symbol, securityId, securityType, cfiCode, currency, settlCurrency, matchAlgorithm, minTradeVol, maxTradeVol, minPriceIncrementEx, displayFactor, unitOfMeasure, unitOfMeasureQty, tradingReferencePrice, tradingReferenceDate, highLimitPrice, lowLimitPrice, maxPriceVariation, financialInstrumentFullName, partyRoleClearingOrg, startDate, endDate, terminationType, repoSubType, moneyOrPar, maxNoOfSubstitutions, priceQuoteMethod, userDefinedInstrument, riskSet, marketSet, instrumentGuid, termCode, brokenDateTermType, eventsGroups, feedTypesGroups, instAttribGroups, lotTypeRulesGroups, repoUnderlyingsGroups, repoRelatedInstrumentsGroups, brokenDatesGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MdInstrumentDefinitionRepo) : (encode message).length ≤ 48237 := by
  have bound_eventsGroups := EventsGroups.encode_length_le message.eventsGroups
  have bound_feedTypesGroups := FeedTypesGroups.encode_length_le message.feedTypesGroups
  have bound_instAttribGroups := InstAttribGroups.encode_length_le message.instAttribGroups
  have bound_lotTypeRulesGroups := LotTypeRulesGroups.encode_length_le message.lotTypeRulesGroups
  ...
  have bound_brokenDatesGroups := BrokenDatesGroups.encode_length_le message.brokenDatesGroups
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, SecurityUpdateAction.encode_length, encodeUInt_length, Alpha.encode_length]
  omega
...
@[simp] theorem decode_encode (message : MdInstrumentDefinitionRepo) (rest : List UInt8) :
  ...
  rw [List.append_assoc, LotTypeRulesGroups.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, RepoUnderlyingsGroups.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, RepoRelatedInstrumentsGroups.decode_encode, some_bind]
  dsimp only
  rw [BrokenDatesGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : SnapshotRefreshTopOrdersGroups) : (encode message).length ≤ 7398 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SnapshotRefreshTopOrdersGroup.encode 29 SnapshotRefreshTopOrdersGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SnapshotRefreshTopOrdersGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SnapshotRefreshTopOrdersGroup.encode SnapshotRefreshTopOrdersGroup.decode SnapshotRefreshTopOrdersGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.snapshotRefreshTopOrdersGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : SnapshotRefreshTopOrders) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [SnapshotRefreshTopOrdersGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : SecurityStatusWorkupGroups) : (encode message).length ≤ 2553 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SecurityStatusWorkupGroup.encode 10 SecurityStatusWorkupGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SecurityStatusWorkupGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SecurityStatusWorkupGroup.encode SecurityStatusWorkupGroup.decode SecurityStatusWorkupGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.securityStatusWorkupGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : SecurityStatusWorkup) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [SecurityStatusWorkupGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : SnapshotFullRefreshTcpGroups) : (encode message).length ≤ 6633 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SnapshotFullRefreshTcpGroup.encode 26 SnapshotFullRefreshTcpGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshTcpGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SnapshotFullRefreshTcpGroup.encode SnapshotFullRefreshTcpGroup.decode SnapshotFullRefreshTcpGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.snapshotFullRefreshTcpGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshTcp) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [SnapshotFullRefreshTcpGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : CollateralMarketValueGroups) : (encode message).length ≤ 10203 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const CollateralMarketValueGroup.encode 40 CollateralMarketValueGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : CollateralMarketValueGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 CollateralMarketValueGroup.encode CollateralMarketValueGroup.decode CollateralMarketValueGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.collateralMarketValueGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : CollateralMarketValue) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [CollateralMarketValueGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : FxLotTypeRulesGroups) : (encode message).length ≤ 2298 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const FxLotTypeRulesGroup.encode 9 FxLotTypeRulesGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : FxLotTypeRulesGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 FxLotTypeRulesGroup.encode FxLotTypeRulesGroup.decode FxLotTypeRulesGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.fxLotTypeRulesGroup.length_lt]
  rfl
...
theorem encode_length_le (message : TradingSessionsGroups) : (encode message).length ≤ 4593 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TradingSessionsGroup.encode 18 TradingSessionsGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : TradingSessionsGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 TradingSessionsGroup.encode TradingSessionsGroup.decode TradingSessionsGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.tradingSessionsGroup.length_lt]
  rfl
...
def decode (bytes : List UInt8) : Option (MdInstrumentDefinitionFx × List UInt8) := do
  ...
  pure ({ matchEventIndicator, totNumReportsOptional, securityUpdateAction, lastUpdateTime, mdSecurityTradingStatus, applId, marketSegmentId, underlyingProduct, securityExchange, securityGroup, asset, symbol, securityId, securityType, cfiCode, currency, settlCurrency, priceQuoteCurrency, matchAlgorithm, minTradeVol, maxTradeVol, minPriceIncrementEx, displayFactor, pricePrecision, unitOfMeasure, unitOfMeasureQty, highLimitPrice, lowLimitPrice, maxPriceVariation, userDefinedInstrument, financialInstrumentFullName, fxCurrencySymbol, settlType, interveningDays, fxBenchmarkRateFix, rateSource, fixRateLocalTime, fixRateLocalTimeZone, minQuoteLife, maxPriceDiscretionOffset, instrumentGuid, maturityMonthYear, settlementLocale, altMinPriceIncrement, altMinQuoteLife, altPriceIncrementConstraint, maxBidAskConstraint, eventsGroups, feedTypesGroups, instAttribGroups, fxLotTypeRulesGroups, tradingSessionsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MdInstrumentDefinitionFx) : (encode message).length ≤ 11572 := by
  have bound_eventsGroups := EventsGroups.encode_length_le message.eventsGroups
  have bound_feedTypesGroups := FeedTypesGroups.encode_length_le message.feedTypesGroups
  have bound_instAttribGroups := InstAttribGroups.encode_length_le message.instAttribGroups
...
@[simp] theorem decode_encode (message : MdInstrumentDefinitionFx) (rest : List UInt8) :
  ...
  rw [List.append_assoc, InstAttribGroups.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, FxLotTypeRulesGroups.decode_encode, some_bind]
  dsimp only
  rw [TradingSessionsGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : IncrementalRefreshBookLongGroups) : (encode message).length ≤ 8163 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const IncrementalRefreshBookLongGroup.encode 32 IncrementalRefreshBookLongGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : IncrementalRefreshBookLongGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 IncrementalRefreshBookLongGroup.encode IncrementalRefreshBookLongGroup.decode IncrementalRefreshBookLongGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.incrementalRefreshBookLongGroup.length_lt]
  rfl
...
theorem encode_length_le (message : IncrementalRefreshBookLongOrderIdGroups) : (encode message).length ≤ 6128 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, encodeMany_length_const IncrementalRefreshBookLongOrderIdGroup.encode 24 IncrementalRefreshBookLongOrderIdGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : IncrementalRefreshBookLongOrderIdGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 IncrementalRefreshBookLongOrderIdGroup.encode IncrementalRefreshBookLongOrderIdGroup.decode IncrementalRefreshBookLongOrderIdGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.incrementalRefreshBookLongOrderIdGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshBookLongQty) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, IncrementalRefreshBookLongGroups.decode_encode, some_bind]
  dsimp only
  rw [IncrementalRefreshBookLongOrderIdGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : IncrementalRefreshTradeSummaryLongGroups) : (encode message).length ≤ 10203 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const IncrementalRefreshTradeSummaryLongGroup.encode 40 IncrementalRefreshTradeSummaryLongGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : IncrementalRefreshTradeSummaryLongGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 IncrementalRefreshTradeSummaryLongGroup.encode IncrementalRefreshTradeSummaryLongGroup.decode IncrementalRefreshTradeSummaryLongGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.incrementalRefreshTradeSummaryLongGroup.length_lt]
  rfl
...
theorem encode_length_le (message : IncrementalRefreshTradeSummaryLongOrderIdGroups) : (encode message).length ≤ 4088 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length, encodeMany_length_const IncrementalRefreshTradeSummaryLongOrderIdGroup.encode 16 IncrementalRefreshTradeSummaryLongOrderIdGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : IncrementalRefreshTradeSummaryLongOrderIdGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 IncrementalRefreshTradeSummaryLongOrderIdGroup.encode IncrementalRefreshTradeSummaryLongOrderIdGroup.decode IncrementalRefreshTradeSummaryLongOrderIdGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.incrementalRefreshTradeSummaryLongOrderIdGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshTradeSummaryLongQty) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, IncrementalRefreshTradeSummaryLongGroups.decode_encode, some_bind]
  dsimp only
  rw [IncrementalRefreshTradeSummaryLongOrderIdGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : IncrementalRefreshVolumeLongGroups) : (encode message).length ≤ 6123 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const IncrementalRefreshVolumeLongGroup.encode 24 IncrementalRefreshVolumeLongGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : IncrementalRefreshVolumeLongGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 IncrementalRefreshVolumeLongGroup.encode IncrementalRefreshVolumeLongGroup.decode IncrementalRefreshVolumeLongGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.incrementalRefreshVolumeLongGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshVolumeLongQty) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [IncrementalRefreshVolumeLongGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : IncrementalRefreshSessionStatisticsLongGroups) : (encode message).length ≤ 8163 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const IncrementalRefreshSessionStatisticsLongGroup.encode 32 IncrementalRefreshSessionStatisticsLongGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : IncrementalRefreshSessionStatisticsLongGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 IncrementalRefreshSessionStatisticsLongGroup.encode IncrementalRefreshSessionStatisticsLongGroup.decode IncrementalRefreshSessionStatisticsLongGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.incrementalRefreshSessionStatisticsLongGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : MdIncrementalRefreshSessionStatisticsLongQty) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [IncrementalRefreshSessionStatisticsLongGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : SnapshotfullRefreshTcpLongGroups) : (encode message).length ≤ 5868 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SnapshotfullRefreshTcpLongGroup.encode 23 SnapshotfullRefreshTcpLongGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SnapshotfullRefreshTcpLongGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SnapshotfullRefreshTcpLongGroup.encode SnapshotfullRefreshTcpLongGroup.decode SnapshotfullRefreshTcpLongGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.snapshotfullRefreshTcpLongGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshTcpLongQty) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [SnapshotfullRefreshTcpLongGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : SnapshotFullRefreshLongGroups) : (encode message).length ≤ 5868 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SnapshotFullRefreshLongGroup.encode 23 SnapshotFullRefreshLongGroup.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshLongGroups) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [decodeMany_bounded 1 SnapshotFullRefreshLongGroup.encode SnapshotFullRefreshLongGroup.decode SnapshotFullRefreshLongGroup.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.snapshotFullRefreshLongGroup.length_lt]
  rfl
...
@[simp] theorem decode_encode (message : SnapshotFullRefreshLongQty) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUIntLE_encodeUIntLE, some_bind]
  dsimp only
  rw [SnapshotFullRefreshLongGroups.decode_encode, some_bind]
  rfl
...
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 48237 := by
  cases message with
  | channelReset inner =>
