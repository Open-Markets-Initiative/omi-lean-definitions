...
/-!
# Eurex Exchange Enhanced Trading Interface v11.1
...
namespace Omi.EurexT7EtiFbeV111Server
...
@[simp] theorem decode_encode (message : LogonResponse) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeMany_bounded 2 Byte.encode Byte.decode Byte.decode_encode, some_bind]
  dsimp only
  rw [Alpha.decode_encode, some_bind]
  dsimp only
  rw [dite_eq_left message.publicKey.length_lt]
  rfl
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 108664 := by
...
end Omi.EurexT7EtiFbeV111Server
