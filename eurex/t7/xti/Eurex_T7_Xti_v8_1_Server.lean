...
/-!
# Eurex Exchange Cash Enhanced Trading Interface v8.1
...
namespace Omi.EurexT7XtiFbeV81Server
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 4291887304 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
def decode (bytes : List UInt8) : Option TesBroadcast := do
  ...
  if fits_sideAllocGrpBcComp : sideAllocGrpBcComp_.length < 256 ^ 1 then
    if fits_varText : varText_.length < 256 ^ 2 then
      if fits_alignmentPadding : alignmentPadding_.length ≤ 4291821559 then
        pure { pad2, rbcHeaderComp, securityId, lastPx, transactTime, transBkdTime, settlCurrFxRate, marketSegmentId, packageId, tesExecId, settlDate, autoApprovalRuleId, trdType, tradeReportType, messageEventSource, tradeReportText, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, pad2v2, sideAllocGrpBcComp := ⟨sideAllocGrpBcComp_, fits_sideAllocGrpBcComp⟩, varText := ⟨varText_, fits_varText⟩, alignmentPadding := ⟨alignmentPadding_, fits_alignmentPadding⟩ }
      else none
    else none
  else none
...
end Omi.EurexT7XtiFbeV81Server
