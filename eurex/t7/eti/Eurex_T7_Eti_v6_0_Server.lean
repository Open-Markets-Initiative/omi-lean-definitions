...
/-!
# Eurex Exchange Enhanced Trading Interface v6.0
...
Note: Gateway Response is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7EtiFbeV60Server
...
theorem encode_length_le (message : DeleteAllOrderBroadcast) : (encode message).length ≤ 1048666 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const NotAffectedOrdersGrpComp.encode 16 NotAffectedOrdersGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : DeleteAllOrderBroadcast) (rest : List UInt8) :
  ...
  dsimp only
  simp only [message.notAffectedOrdersGrpComp.length_lt, ↓reduceDIte]
  rfl
...
theorem encode_length_le (message : DeleteAllOrderResponse) : (encode message).length ≤ 1048650 := by
  have bound_notAffectedOrdersGrpComp := message.notAffectedOrdersGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, ResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeMany_length_const NotAffectedOrdersGrpComp.encode 16 NotAffectedOrdersGrpComp.encode_length]
  omega
...
/-- Delete Order Broadcast: 146 bytes -/
...
def decode (bytes : List UInt8) : Option (DeleteOrderBroadcast × List UInt8) := do
  ...
  pure ({ pad2, rbcHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, cumQty, cxlQty, marketSegmentId, partyIdEnteringTrader, execRestatementReason, partyIdEnteringFirm, ordStatus, execType, productComplex, side, fixClOrdId, pad5 }, bytes)

@[simp] theorem encode_length (message : DeleteOrderBroadcast) : (encode message).length = 146 := by
  unfold encode
...
structure GatewayResponse where
  ...
  gatewayStatus : BitVec 8
  secondaryGatewayStatus : BitVec 8
...
def encode (message : GatewayResponse) : List UInt8 :=
  Alpha.encode message.pad2
    ...
    ++ encodeUInt 1 message.gatewayStatus
    ++ encodeUInt 1 message.secondaryGatewayStatus
...
def decode (bytes : List UInt8) : Option (GatewayResponse × List UInt8) := do
  ...
  let (gatewayStatus, bytes) ← decodeUInt 1 bytes
  let (secondaryGatewayStatus, bytes) ← decodeUInt 1 bytes
  let (sessionMode, bytes) ← decodeUInt 1 bytes
  let (tradSesMode, bytes) ← decodeUInt 1 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ pad2, responseHeaderComp, gatewayId, gatewaySubId, secondaryGatewayId, secondaryGatewaySubId, gatewayStatus, secondaryGatewayStatus, sessionMode, tradSesMode, pad4 }, bytes)
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : GatewayResponse) : decode (encode message) = some (message, []) := by
  have trailing := decode_encode message []
  rw [List.append_nil] at trailing
  exact trailing
...
theorem encode_length_le (message : InquireMmParameterResponse) : (encode message).length ≤ 8234 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, NrResponseHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const MmParameterGrpComp.encode 32 MmParameterGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (ModifyOrderNrResponse × List UInt8) := do
  ...
  let (pad5, bytes) ← Alpha.decode 5 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, leavesQty, cumQty, cxlQty, ordStatus, execType, execRestatementReason, crossedIndicator, productComplex, triggered, pad5 }, bytes)
...
def decode (bytes : List UInt8) : Option (ModifyOrderResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad5, bytes) ← Alpha.decode 5 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, origClOrdId, securityId, execId, trdRegTsTimePriority, leavesQty, cumQty, cxlQty, ordStatus, execType, execRestatementReason, crossedIndicator, productComplex, triggered, pad5 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderNrResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ pad2, nrResponseHeaderMeComp, orderId, clOrdId, securityId, execId, ordStatus, execType, execRestatementReason, crossedIndicator, productComplex, triggered, pad1 }, bytes)
...
def decode (bytes : List UInt8) : Option (NewOrderResponse × List UInt8) := do
  ...
  let (triggered, bytes) ← decodeUInt 1 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ pad2, responseHeaderMeComp, orderId, clOrdId, securityId, execId, trdRegTsEntryTime, trdRegTsTimePriority, ordStatus, execType, execRestatementReason, crossedIndicator, productComplex, triggered, pad1 }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecNotification) : (encode message).length ≤ 2103386 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : OrderExecReportBroadcast) : (encode message).length ≤ 2105610 := by
...
theorem encode_length_le (message : QuoteExecutionReport) : (encode message).length ≤ 2107394 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderMeComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuoteEventGrpComp.encode 40 QuoteEventGrpComp.encode_length, encodeMany_length_const QuoteLegExecGrpComp.encode 32 QuoteLegExecGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : Reject) : (encode message).length ≤ 4290838600 := by
  have bound_varText := message.varText.length_lt
  have bound_alignmentPadding := message.alignmentPadding.length_le
  unfold encode
...
/-- Srqs Trd Instrmnt Leg Grp Comp: 32 bytes -/
structure SrqsTrdInstrmntLegGrpComp where
  legSecurityId : BitVec 64
  legBestBidPx : BitVec 64
  legBestOfferPx : BitVec 64
  legBestBidSize : BitVec 32
  legBestOfferSize : BitVec 32
  deriving DecidableEq, Repr

namespace SrqsTrdInstrmntLegGrpComp

def encode (message : SrqsTrdInstrmntLegGrpComp) : List UInt8 :=
  encodeUIntLE 8 message.legSecurityId
    ++ encodeUIntLE 8 message.legBestBidPx
    ++ encodeUIntLE 8 message.legBestOfferPx
    ++ encodeUIntLE 4 message.legBestBidSize
    ++ encodeUIntLE 4 message.legBestOfferSize

def decode (bytes : List UInt8) : Option (SrqsTrdInstrmntLegGrpComp × List UInt8) := do
  let (legSecurityId, bytes) ← decodeUIntLE 8 bytes
  let (legBestBidPx, bytes) ← decodeUIntLE 8 bytes
  let (legBestOfferPx, bytes) ← decodeUIntLE 8 bytes
  let (legBestBidSize, bytes) ← decodeUIntLE 4 bytes
  let (legBestOfferSize, bytes) ← decodeUIntLE 4 bytes
  pure ({ legSecurityId, legBestBidPx, legBestOfferPx, legBestBidSize, legBestOfferSize }, bytes)

@[simp] theorem encode_length (message : SrqsTrdInstrmntLegGrpComp) : (encode message).length = 32 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]

theorem encode_length_pos (message : SrqsTrdInstrmntLegGrpComp) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : SrqsTrdInstrmntLegGrpComp) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [decodeUIntLE_encodeUIntLE, Option.bind_some]
...
structure SrqsCreateDealNotification where
  ...
  freeText1 : Alpha 12
  srqsTrdInstrmntLegGrpComp : Bounded 1 SrqsTrdInstrmntLegGrpComp
  deriving DecidableEq, Repr
...
def encode (message : SrqsCreateDealNotification) : List UInt8 :=
  Alpha.encode message.pad2
    ...
    ++ encodeUInt 1 message.side
    ++ encodeUInt 1 (BitVec.ofNat (8 * 1) message.srqsTrdInstrmntLegGrpComp.val.length)
    ...
    ++ Alpha.encode message.freeText1
    ++ encodeMany SrqsTrdInstrmntLegGrpComp.encode message.srqsTrdInstrmntLegGrpComp.val
...
def decode (bytes : List UInt8) : Option (SrqsCreateDealNotification × List UInt8) := do
  ...
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (srqsTrdInstrmntLegGrpComp_, bytes) ← decodeMany SrqsTrdInstrmntLegGrpComp.decode noLegs.toNat bytes
  if fits_srqsTrdInstrmntLegGrpComp : srqsTrdInstrmntLegGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, lastPx, quoteId, securityId, expireTime, negotiationId, tradeId, lastQty, trdRptStatus, messageEventSource, side, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmTradeId, firmNegotiationId, freeText1, srqsTrdInstrmntLegGrpComp := ⟨srqsTrdInstrmntLegGrpComp_, fits_srqsTrdInstrmntLegGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsCreateDealNotification) : (encode message).length ≤ 8330 := by
  have bound_srqsTrdInstrmntLegGrpComp := message.srqsTrdInstrmntLegGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SrqsTrdInstrmntLegGrpComp.encode 32 SrqsTrdInstrmntLegGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SrqsCreateDealNotification) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 SrqsTrdInstrmntLegGrpComp.encode SrqsTrdInstrmntLegGrpComp.decode SrqsTrdInstrmntLegGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.srqsTrdInstrmntLegGrpComp.length_lt, ↓reduceDIte]
  rfl
...
/-- Srqs Deal Notification: 130 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsDealNotification × List UInt8) := do
  ...
  let (firmNegotiationId, bytes) ← Alpha.decode 20 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, negotiationId, tradeId, trdRptStatus, messageEventSource, rootPartyExecutingFirm, rootPartyExecutingTrader, rootPartyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, targetPartyEnteringTrader, firmTradeId, firmNegotiationId, pad4 }, bytes)

@[simp] theorem encode_length (message : SrqsDealNotification) : (encode message).length = 130 := by
  unfold encode
...
/-- Srqs Negotiation Notification: 130 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationNotification × List UInt8) := do
  ...
  let (firmNegotiationId, bytes) ← Alpha.decode 20 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteRefPrice, bidPx, offerPx, negotiationId, leavesQty, quoteType, quoteStatus, quoteInstruction, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsNegotiationNotification) : (encode message).length = 130 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (TargetPartiesComp × List UInt8) := do
  ...
  let (targetPartyExecutingTrader, bytes) ← Alpha.decode 6 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ sideDisclosureInstruction, priceDisclosureInstruction, leavesQtyDisclosureInstruction, quoteInstruction, targetPartyExecutingFirm, targetPartyExecutingTrader, pad1 }, bytes)

@[simp] theorem encode_length (message : TargetPartiesComp) : (encode message).length = 16 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (SrqsNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, quoteRefPrice, bidPx, offerPx, negotiationId, orderQty, leavesQty, quoteType, quoteStatus, side, quoteRefPriceSource, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, pad2v2, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsNegotiationRequesterNotification) : (encode message).length ≤ 4202 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const TargetPartiesComp.encode 16 TargetPartiesComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationNotification × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    pure ({ pad2, rbcHeaderComp, transactTime, securityId, bidPx, offerPx, quoteRefPrice, negotiationId, marketSegmentId, leavesQty, quoteType, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, targetPartyExecutingFirm, targetPartyExecutingTrader, firmNegotiationId, pad6, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationNotification) : (encode message).length ≤ 6266 := by
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequesterNotification × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ pad2, rbcHeaderComp, transactTime, securityId, bidPx, offerPx, quoteRefPrice, negotiationId, marketSegmentId, orderQty, quoteType, quoteStatus, side, quoteRefPriceSource, tradeUnderlying, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, firmNegotiationId, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequesterNotification) : (encode message).length ≤ 10330 := by
  have bound_quotReqLegsGrpComp := message.quotReqLegsGrpComp.length_lt
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const QuotReqLegsGrpComp.encode 24 QuotReqLegsGrpComp.encode_length, encodeMany_length_const TargetPartiesComp.encode 16 TargetPartiesComp.encode_length]
  omega
...
/-- Srqs Quote Notification: 138 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsQuoteNotification × List UInt8) := do
  ...
  let (freeText1, bytes) ← Alpha.decode 12 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ pad2, rbcHeaderComp, transactTime, quoteId, secondaryQuoteId, bidPx, offerPx, negotiationId, bidSize, offerSize, partyExecutingFirm, partyExecutingTrader, partyEnteringTrader, quoteReqId, freeText1, pad3 }, bytes)

@[simp] theorem encode_length (message : SrqsQuoteNotification) : (encode message).length = 138 := by
  unfold encode
    ...
    if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
      if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
        if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
          pure ({ pad2, rbcHeaderComp, securityId, lastPx, transactTime, underlyingPx, transBkdTime, relatedClosePrice, marketSegmentId, packageId, tesExecId, allocQty, allocId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, relatedTradeQuantity, underlyingQty, trdType, side, tradePublishIndicator, productComplex, tradeReportType, tradingCapacity, partyIdSettlementLocation, tradeAllocStatus, hedgeType, messageEventSource, tradeReportId, partyExecutingFirm, partyExecutingTrader, partyIdEnteringFirm, partyEnteringTrader, positionEffect, rootPartyExecutingFirm, rootPartyExecutingTrader, freeText1, freeText2, freeText3, partyIdTakeUpTradingFirm, account, partyIdPositionAccount, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdLocationId, custOrderHandlingInst, complianceText, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad2v2, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
        else none
      else none
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 26906 := by
  ...
  if fits_sideAllocGrpBcComp : sideAllocGrpBcComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ pad2, rbcHeaderComp, securityId, lastPx, transactTime, underlyingPx, transBkdTime, relatedClosePrice, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, relatedTradeQuantity, underlyingQty, trdType, tradeReportType, productComplex, tradePublishIndicator, partyIdSettlementLocation, hedgeType, messageEventSource, tradeReportText, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad5, sideAllocGrpBcComp := ⟨sideAllocGrpBcComp_, fits_sideAllocGrpBcComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 32906 := by
  ...
  have bound_underlyingStipGrpComp := message.underlyingStipGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocGrpBcComp.encode 24 SideAllocGrpBcComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length]
  omega
...
/-- Side Alloc Ext Grp Comp: 144 bytes -/
...
def decode (bytes : List UInt8) : Option (SideAllocExtGrpComp × List UInt8) := do
  ...
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ individualAllocId, allocQty, partyExecutingFirm, partyExecutingTrader, side, tradeAllocStatus, tradingCapacity, positionEffect, orderAttributeLiquidityProvision, account, partyIdPositionAccount, partyIdTakeUpTradingFirm, freeText1, freeText2, freeText3, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdLocationId, custOrderHandlingInst, complianceText, pad6 }, bytes)

@[simp] theorem encode_length (message : SideAllocExtGrpComp) : (encode message).length = 144 := by
  unfold encode
  ...
  if fits_sideAllocExtGrpComp : sideAllocExtGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      if fits_instrumentEventGrpComp : instrumentEventGrpComp_.length < 256 ^ 1 then
        if fits_instrumentAttributeGrpComp : instrumentAttributeGrpComp_.length < 256 ^ 1 then
          if fits_underlyingStipGrpComp : underlyingStipGrpComp_.length < 256 ^ 1 then
            pure ({ pad2, rbcHeaderComp, securityId, lastPx, transBkdTime, transactTime, underlyingPx, relatedClosePrice, marketSegmentId, packageId, tesExecId, underlyingSettlementDate, underlyingMaturityDate, relatedTradeId, relatedMarketSegmentId, relatedTradeQuantity, underlyingQty, trdType, productComplex, tradeReportType, tradePublishIndicator, hedgeType, partyIdSettlementLocation, messageEventSource, tradeReportId, rootPartyExecutingFirm, rootPartyExecutingTrader, underlyingSecurityId, underlyingSecurityDesc, underlyingCurrency, underlyingIssuer, pad1, sideAllocExtGrpComp := ⟨sideAllocExtGrpComp_, fits_sideAllocExtGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩, instrumentEventGrpComp := ⟨instrumentEventGrpComp_, fits_instrumentEventGrpComp⟩, instrumentAttributeGrpComp := ⟨instrumentAttributeGrpComp_, fits_instrumentAttributeGrpComp⟩, underlyingStipGrpComp := ⟨underlyingStipGrpComp_, fits_underlyingStipGrpComp⟩ }, bytes)
          else none
        else none
      else none
    else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesUploadBroadcast) : (encode message).length ≤ 63482 := by
  ...
  have bound_underlyingStipGrpComp := message.underlyingStipGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RbcHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, MessageEventSource.encode_length, encodeMany_length_const SideAllocExtGrpComp.encode 144 SideAllocExtGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 16 TrdInstrmntLegGrpComp.encode_length, encodeMany_length_const InstrumentEventGrpComp.encode 8 InstrumentEventGrpComp.encode_length, encodeMany_length_const InstrumentAttributeGrpComp.encode 40 InstrumentAttributeGrpComp.encode_length, encodeMany_length_const UnderlyingStipGrpComp.encode 40 UnderlyingStipGrpComp.encode_length]
  omega
...
end Omi.EurexT7EtiFbeV60Server
