...
namespace Omi.EurexT7EtiFbeV131Client
...
def decode (bytes : List UInt8) : Option (DeleteOrderComplexRequest × List UInt8) := do
  ...
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, securityId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, targetPartyIdSessionId, orderOrigination, partyIdInvestmentDecisionMakerQualifier, executingTraderQualifier, complianceText, pad1 }, bytes)
...
@[simp] theorem encode_length (message : DeleteOrderRequest) : (encode message).length = 98 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
def decode (bytes : List UInt8) : Option (ModifyOrderRequest × List UInt8) := do
  ...
  let (legOrdGrpComp_, bytes) ← decodeMany LegOrdGrpComp.decode noLegOnbooks.toNat bytes
  if fits_legOrdGrpComp : legOrdGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, orderId, clOrdId, origClOrdId, stopPx, partyIdClientId, executingTrader, matchInstCrossId, expireDate, targetPartyIdSessionId, tradingSessionSubId, ownershipIndicator, applSeqIndicator, ordType, priceValidityCheckType, valueCheckTypeValue, orderOrigination, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, account, partyIdPositionAccount, positionEffect, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, partyEndClientIdentification, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, pad4, partyIdInvestmentDecisionMaker, orderAttributeLiquidityProvision, tradingCapacity, productComplex, selfMatchPreventionInstruction, pad3, marketSegmentId, pad4v1, securityId, orderQty, price, side, execInst, timeInForce, pad1, checkSumCorrection, pad2v3, legOrdGrpComp := ⟨legOrdGrpComp_, fits_legOrdGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyOrderRequest) : (encode message).length ≤ 2322 := by
  have bound_legOrdGrpComp := message.legOrdGrpComp.length_lt
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderRequest) : (encode message).length ≤ 2298 := by
  have bound_legOrdGrpComp := message.legOrdGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, CustOrderHandlingInst.encode_length, encodeMany_length_const LegOrdGrpComp.encode 8 LegOrdGrpComp.encode_length]
  omega
...
/-- New Order Single Request: 250 bytes -/
...
end Omi.EurexT7EtiFbeV131Client
