...
namespace Omi.CmeGlobexIlink3SbeV86Server
...
def listed (byte : UInt8) : OrdTypeOptional :=
  if byte = 0x30 then .null
  else if byte = 0x31 then .marketWithProtection
...
/-- Execution Report New: 217 bytes -/
...
/-- Execution Report Reject: 475 bytes -/
...
@[simp] theorem encode_length (message : ExecutionReportReject) : (encode message).length = 475 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length, ExecutionMode.encode_length]
...
/-- Execution Report Elimination: 210 bytes -/
...
@[simp] theorem encode_length (message : ExecutionReportElimination) : (encode message).length = 210 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length, ExecutionMode.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ExecutionReportTradeOutright) : (encode message).length ≤ 14562 := by
...
/-- Execution Report Modify: 225 bytes -/
...
@[simp] theorem encode_length (message : ExecutionReportModify) : (encode message).length = 225 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, OrdTypeOptional.encode_length, encodeUInt_length, ExecutionMode.encode_length]
...
/-- Execution Report Status: 488 bytes -/
...
@[simp] theorem encode_length (message : ExecutionReportStatus) : (encode message).length = 488 := by
  unfold encode
...
/-- Execution Report Cancel: 222 bytes -/
...
def decode (bytes : List UInt8) : Option (MassQuoteAck × List UInt8) := do
  ...
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, requestTime, sendingTimeEpoch, quoteReqIdOptional, location, quoteId, quoteRejectReason, delayDuration, quoteAckStatus, manualOrderIndicator, noProcessedEntries, mmProtectionReset, splitMsg, liquidityFlag, shortSaleType, totNoQuoteEntriesOptional, possRetransFlag, delayToTime, quoteAckEntriesGroups }, bytes)
...
@[simp] theorem decode_encode (message : SecurityDefinitionResponse) (rest : List UInt8) :
  ...
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [ResponseLegsGroups.decode_encode, some_bind]
  rfl
...
def decode (bytes : List UInt8) : Option (QuoteCancelAck × List UInt8) := do
  ...
  pure ({ seqNum, uuid, text, senderId, partyDetailsListReqId, requestTime, sendingTimeEpoch, cancelledSymbol, location, quoteId, quoteRejectReason, delayDuration, manualOrderIndicator, quoteCxlStatus, noProcessedEntries32, mmProtectionReset, unsolicitedCancelType, splitMsg, totNoQuoteEntriesOptional, liquidityFlag, possRetransFlag, delayToTime, quoteCancelAckEntriesGroups, quoteCancelAckSetsGroups }, bytes)
...
/-- Execution Report Pending Cancel: 194 bytes -/
...
/-- Execution Report Pending Replace: 195 bytes -/
...
end Omi.CmeGlobexIlink3SbeV86Server
