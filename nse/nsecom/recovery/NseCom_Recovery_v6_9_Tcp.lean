...
/-!
# National Stock Exchange of India Ltd Mtbt Tick Data Recovery v6.9
...
namespace Omi.NseNsecomRecoveryBinaryV69Tcp
...
/-- Tick Data Recovery Response Message: 1 bytes -/
structure TickDataRecoveryResponseMessage where
  requestStatus : RequestStatus
  deriving DecidableEq, Repr

namespace TickDataRecoveryResponseMessage

def encode (message : TickDataRecoveryResponseMessage) : List UInt8 :=
  RequestStatus.encode message.requestStatus

def decode (bytes : List UInt8) : Option (TickDataRecoveryResponseMessage × List UInt8) := do
  let (requestStatus, bytes) ← RequestStatus.decode bytes
  pure ({ requestStatus }, bytes)

@[simp] theorem encode_length (message : TickDataRecoveryResponseMessage) : (encode message).length = 1 := by
  unfold encode
  simp only [RequestStatus.encode_length]

theorem encode_length_pos (message : TickDataRecoveryResponseMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : TickDataRecoveryResponseMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [RequestStatus.decode_encode, some_bind]
  rfl

end TickDataRecoveryResponseMessage
...
inductive Payload where
  ...
  | tickDataRecoveryResponseMessage (message : TickDataRecoveryResponseMessage) -- "Y" 0x59
  deriving DecidableEq, Repr
...
def tag : Payload → BitVec 8
  ...
  | .tickDataRecoveryResponseMessage _ => 89
...
def decode (tag : BitVec 8) (bytes : List UInt8) : Option (Payload × List UInt8) :=
  ...
  else if tag = 89 then (TickDataRecoveryResponseMessage.decode bytes).map fun (message, rest) => (.tickDataRecoveryResponseMessage message, rest)
  else none
...
theorem encode_length_le (message : Packet) : (encode message).length ≤ 45 := by
  ...
  | heartbeatMessage inner =>
    ...
    omega
  | tickDataRecoveryResponseMessage inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, StreamHeader.encode_length, encodeUInt_length, TickDataRecoveryResponseMessage.encode_length]
    omega
...
end Omi.NseNsecomRecoveryBinaryV69Tcp
