...
/-!
# National Association of Securities Dealers Automated Quotations (Nasdaq) Best Bid And Offer v2.1
...
namespace Omi.NasdaqPsxequitiesBboItchV21
...
/-- Bbo Security Class: one byte code -/
def BboSecurityClass.codes : List UInt8 :=
  [0x51, 0x4E, 0x41, 0x50, 0x5A, 0x56]

inductive BboSecurityClass where
  | nasdaqListedIssue -- Nasdaq Listed Issue
  | nyse -- Nyse
  | nyseAmerican -- Nyse American
  | nyseArca -- Nyse Arca
  | bats -- Bats
  | investorsExchangeLlc -- Investors Exchange Llc
  | unlisted (byte : { byte : UInt8 // byte ∉ BboSecurityClass.codes }) -- any other code, kept as it is
  deriving DecidableEq, Repr

namespace BboSecurityClass

def toByte : BboSecurityClass → UInt8
  | .nasdaqListedIssue => 0x51
  | .nyse => 0x4E
  | .nyseAmerican => 0x41
  | .nyseArca => 0x50
...
/-- The constructor of a listed code -/
def listed (byte : UInt8) : BboSecurityClass :=
  if byte = 0x51 then .nasdaqListedIssue
  else if byte = 0x4E then .nyse
  else if byte = 0x41 then .nyseAmerican
  else if byte = 0x50 then .nyseArca
...
def ofByte (byte : UInt8) : BboSecurityClass :=
  if known : byte ∈ codes then listed byte else .unlisted ⟨byte, known⟩

theorem ofByte_toByte (value : BboSecurityClass) : ofByte value.toByte = value := by
  cases value with
  | nasdaqListedIssue => decide
  | nyse => decide
  | nyseAmerican => decide
...
def encode (value : BboSecurityClass) : List UInt8 :=
  [value.toByte]

def decode : List UInt8 → Option (BboSecurityClass × List UInt8)
  | byte :: rest => some (ofByte byte, rest)
  | [] => none

@[simp] theorem encode_length (value : BboSecurityClass) : (encode value).length = 1 :=
  rfl

@[simp] theorem decode_encode (value : BboSecurityClass) (rest : List UInt8) :
    decode (encode value ++ rest) = some (value, rest) := by
  simp [decode, encode, ofByte_toByte]

end BboSecurityClass
...
structure QuotationMessage where
  ...
  timestamp : BitVec 48
  stock : Alpha 8
  bboSecurityClass : BboSecurityClass
...
def encode (message : QuotationMessage) : List UInt8 :=
  encodeUInt 2 message.trackingNumber
    ++ (encodeUInt 6 message.timestamp
    ++ (Alpha.encode message.stock
    ++ (BboSecurityClass.encode message.bboSecurityClass
...
def decode (bytes : List UInt8) : Option (QuotationMessage × List UInt8) := do
  ...
  let (timestamp, bytes) ← decodeUInt 6 bytes
  let (stock, bytes) ← Alpha.decode 8 bytes
  let (bboSecurityClass, bytes) ← BboSecurityClass.decode bytes
  ...
  pure ({ trackingNumber, timestamp, stock, bboSecurityClass, bestBidPrice, bestBidSize, bestOfferPrice, bestOfferSize }, bytes)

@[simp] theorem encode_length (message : QuotationMessage) : (encode message).length = 33 := by
  unfold encode
  simp only [List.length_append, encodeUInt_length, Alpha.encode_length, BboSecurityClass.encode_length]
...
@[simp] theorem decode_encode (message : QuotationMessage) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, BboSecurityClass.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
  dsimp only
  rw [List.append_assoc, decodeUInt_encodeUInt, some_bind]
...
end Omi.NasdaqPsxequitiesBboItchV21
