...
def MdEntryType.codes : List UInt8 :=
  [0x30, 0x31, 0x32, 0x34, 0x36, 0x37, 0x38, 0x39, 0x42, 0x43, 0x45, 0x46, 0x4A, 0x4E, 0x4F, 0x57, 0x65, 0x67]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : RepoUnderlyingsGroups) : (encode message).length ≤ 27543 := by
  have bound_repoUnderlyingsGroup := message.repoUnderlyingsGroup.length_lt
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const RepoUnderlyingsGroup.encode 108 RepoUnderlyingsGroup.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : MdInstrumentDefinitionRepo) : (encode message).length ≤ 41583 := by
  have bound_eventsGroups := EventsGroups.encode_length_le message.eventsGroups
  have bound_feedTypesGroups := FeedTypesGroups.encode_length_le message.feedTypesGroups
  have bound_instAttribGroups := InstAttribGroups.encode_length_le message.instAttribGroups
  have bound_lotTypeRulesGroups := LotTypeRulesGroups.encode_length_le message.lotTypeRulesGroups
