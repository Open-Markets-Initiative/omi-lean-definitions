...
Note: Xetra En Light Approve Deal Request is not framed: its length Body Len is not an integer it reads.
...
theorem encode_length_le (message : ClientPayload) : (encode message).length ≤ 14434 := by
  ...
  | xetraEnLightApproveDealRequest inner =>
    simp only [encode, XetraEnLightApproveDealRequest.encode_length]
    omega
