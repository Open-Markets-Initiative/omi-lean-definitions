...
/-!
# Box Options Market Sola Multicast v1.9
...
namespace Omi.BoxBoxoptionsSolamulticastHsvfV19
...
def encode (message : OptionTradeMessage) : List UInt8 :=
  Alpha.encode message.exchangeId
    ...
    ++ (Alpha.encode message.opraTradeMarker
    ++ (Alpha.encode message.filler5
    ++ (Alpha.encode message.timestamp
    ++ (Alpha.encode message.filler8
...
def decode (bytes : List UInt8) : Option (OptionTradeMessage × List UInt8) := do
  ...
  let (opraTradeMarker, bytes) ← Alpha.decode 1 bytes
  let (filler5, bytes) ← Alpha.decode 5 bytes
  let (timestamp, bytes) ← Alpha.decode 6 bytes
  let (filler8, bytes) ← Alpha.decode 8 bytes
  let (priceIndicatorMarker, bytes) ← PriceIndicatorMarker.decode bytes
  pure ({ exchangeId, instrumentDescription, volume, tradePrice, tradePriceFractionIndicator, netChangeSign, netChange, netChangeFractionIndicator, opraTradeMarker, filler5, timestamp, filler8, priceIndicatorMarker }, bytes)
...
/-- Strategy Trade Message: 68 bytes -/
...
def decode (bytes : List UInt8) : Option (StrategyTradeMessage × List UInt8) := do
  ...
  let (timestamp, bytes) ← Alpha.decode 6 bytes
  let (priceIndicatorMarker, bytes) ← PriceIndicatorMarker.decode bytes
  pure ({ exchangeId, complexOrderInstrumentSymbol, volume, tradePriceSign, tradePrice, tradePriceFractionIndicator, netChangeSign, netChange, netChangeFractionIndicator, opraTradeMarker, filler5, timestamp, priceIndicatorMarker }, bytes)

@[simp] theorem encode_length (message : StrategyTradeMessage) : (encode message).length = 68 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (OptionTradeCancellationMessage × List UInt8) := do
  ...
  let (filler8, bytes) ← Alpha.decode 8 bytes
  let (priceIndicatorMarker, bytes) ← PriceIndicatorMarker.decode bytes
  pure ({ exchangeId, instrumentDescription, volume, tradePrice, tradePriceFractionIndicator, opraTradeMarker, filler5, timestamp, filler8, priceIndicatorMarker }, bytes)
...
def decode (bytes : List UInt8) : Option (ComplexOrderTradeCancellationMessage × List UInt8) := do
  ...
  let (timestamp, bytes) ← Alpha.decode 6 bytes
  let (priceIndicatorMarker, bytes) ← PriceIndicatorMarker.decode bytes
  pure ({ exchangeId, complexOrderInstrumentSymbol, volume, tradePriceSign, tradePrice, tradePriceFractionIndicator, opraTradeMarker, filler5, timestamp, priceIndicatorMarker }, bytes)
...
def decode (bytes : List UInt8) : Option (OptionSummaryMessage × List UInt8) := do
  ...
  let (filler7, bytes) ← Alpha.decode 7 bytes
...
end Omi.BoxBoxoptionsSolamulticastHsvfV19
