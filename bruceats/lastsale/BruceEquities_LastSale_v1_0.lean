...
namespace Omi.BruceatsBruceequitiesLastsaleItchV10
...
def decode (bytes : List UInt8) : Option (TradeReportMessage × List UInt8) := do
  ...
  pure ({ stockLocate, timestamp, stock, matchId, price, size }, bytes)

@[simp] theorem encode_length (message : TradeReportMessage) : (encode message).length = 38 := by
  unfold encode
...
@[simp] theorem encode_length (message : TradeCancelMessage) : (encode message).length = 38 := by
  unfold encode
  simp only [List.length_append, encodeUInt_length, Alpha.encode_length]
...
end Omi.BruceatsBruceequitiesLastsaleItchV10
