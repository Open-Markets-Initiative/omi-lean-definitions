...
namespace Omi.CoinbaseCoinbasederivativesMarketdataapiSbeV19
...
/-- Outright Instrument Definition Message: 182 bytes -/
structure OutrightInstrumentDefinitionMessage where
  ...
  fundingIntervalMinutes : BitVec 32
  fairValueLimit : BitVec 32
  deriving DecidableEq, Repr
...
def encode (message : OutrightInstrumentDefinitionMessage) : List UInt8 :=
  InstrHeader.encode message.instrHeader
    ...
    ++ (encodeUIntLE 4 message.fundingIntervalMinutes
    ++ (encodeUIntLE 4 message.fairValueLimit)))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (OutrightInstrumentDefinitionMessage × List UInt8) := do
  ...
  let (fundingIntervalMinutes, bytes) ← decodeUIntLE 4 bytes
  let (fairValueLimit, bytes) ← decodeUIntLE 4 bytes
  pure ({ instrHeader, symbol, productCode, description, tickSize, cfiCode, currency, firstTradingSessionDate, lastTradingSessionDate, oldContractSize, priorSettlementPriceOptional, settlementPrice, limitDownPrice, limitUpPrice, productId, productGroup, tradingStatus, definitionFlags, contractSize, logicalExpiry, fundingIntervalMinutes, fairValueLimit }, bytes)

@[simp] theorem encode_length (message : OutrightInstrumentDefinitionMessage) : (encode message).length = 182 := by
  unfold encode
...
/-- Funding Rate Message: 71 bytes -/
structure FundingRateMessage where
  instrHeader : InstrHeader
  fundingRate : BitVec 64
  futuresMarkPrice : BitVec 64
  spotMarkPrice : BitVec 64
  fairValue : BitVec 64
  finalFundingRateTimestamp : BitVec 64
  correlationId : BitVec 64
  flags : BitVec 8
  deriving DecidableEq, Repr

namespace FundingRateMessage

def encode (message : FundingRateMessage) : List UInt8 :=
  InstrHeader.encode message.instrHeader
    ++ (encodeUIntLE 8 message.fundingRate
    ++ (encodeUIntLE 8 message.futuresMarkPrice
    ++ (encodeUIntLE 8 message.spotMarkPrice
    ++ (encodeUIntLE 8 message.fairValue
    ++ (encodeUIntLE 8 message.finalFundingRateTimestamp
    ++ (encodeUIntLE 8 message.correlationId
    ++ (encodeUIntLE 1 message.flags)))))))

def decode (bytes : List UInt8) : Option (FundingRateMessage × List UInt8) := do
  let (instrHeader, bytes) ← InstrHeader.decode bytes
  let (fundingRate, bytes) ← decodeUIntLE 8 bytes
  let (futuresMarkPrice, bytes) ← decodeUIntLE 8 bytes
  let (spotMarkPrice, bytes) ← decodeUIntLE 8 bytes
  let (fairValue, bytes) ← decodeUIntLE 8 bytes
  let (finalFundingRateTimestamp, bytes) ← decodeUIntLE 8 bytes
  let (correlationId, bytes) ← decodeUIntLE 8 bytes
  let (flags, bytes) ← decodeUIntLE 1 bytes
  pure ({ instrHeader, fundingRate, futuresMarkPrice, spotMarkPrice, fairValue, finalFundingRateTimestamp, correlationId, flags }, bytes)

@[simp] theorem encode_length (message : FundingRateMessage) : (encode message).length = 71 := by
  unfold encode
  simp only [List.length_append, InstrHeader.encode_length, encodeUIntLE_length]

theorem encode_length_pos (message : FundingRateMessage) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : FundingRateMessage) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  rw [List.append_assoc, InstrHeader.decode_encode, some_bind]
  dsimp only
...
end FundingRateMessage

/-- Start Of Outright Instrument Snapshot Message: 138 bytes -/
...
def decode (bytes : List UInt8) : Option (StartOfOutrightInstrumentSnapshotMessage × List UInt8) := do
  ...
  let (fundingIntervalMinutes, bytes) ← decodeUIntLE 4 bytes
  let (fairValueLimit, bytes) ← decodeUIntLE 4 bytes
  pure ({ snapshotSeqNum, lastInstrSeqNum, symbol, productCode, description, tickSize, cfiCode, currency, productId, oldContractSize, orderCount, firstTradingSessionDate, lastTradingSessionDate, tradingSessionDate, productGroup, tradingStatus, contractSize, logicalExpiry, fundingIntervalMinutes, fairValueLimit }, bytes)

@[simp] theorem encode_length (message : StartOfOutrightInstrumentSnapshotMessage) : (encode message).length = 138 := by
  unfold encode
...
/-- End Of Snapshot Message: 216 bytes -/
structure EndOfSnapshotMessage where
  ...
  priorSettlementPriceOptional : BitVec 64
  definitionFlags : BitVec 16
  finalFundingRate : BitVec 64
  finalFuturesMarkPrice : BitVec 64
  finalFundingRateTimestamp : BitVec 64
  futuresMarkPriceOptional : BitVec 64
  predictedFundingRate : BitVec 64
  spotMarkPriceOptional : BitVec 64
  fairValueOptional : BitVec 64
  deriving DecidableEq, Repr
...
def encode (message : EndOfSnapshotMessage) : List UInt8 :=
  encodeUIntLE 2 message.snapshotSeqNum
    ...
    ++ (encodeUIntLE 8 message.priorSettlementPriceOptional
    ++ (encodeUIntLE 2 message.definitionFlags
    ++ (encodeUIntLE 8 message.finalFundingRate
    ++ (encodeUIntLE 8 message.finalFuturesMarkPrice
    ++ (encodeUIntLE 8 message.finalFundingRateTimestamp
    ++ (encodeUIntLE 8 message.futuresMarkPriceOptional
    ++ (encodeUIntLE 8 message.predictedFundingRate
    ++ (encodeUIntLE 8 message.spotMarkPriceOptional
    ++ (encodeUIntLE 8 message.fairValueOptional)))))))))))))))))))))))))))))))
...
def decode (bytes : List UInt8) : Option (EndOfSnapshotMessage × List UInt8) := do
  ...
  let (priorSettlementPriceOptional, bytes) ← decodeUIntLE 8 bytes
  let (definitionFlags, bytes) ← decodeUIntLE 2 bytes
  let (finalFundingRate, bytes) ← decodeUIntLE 8 bytes
  let (finalFuturesMarkPrice, bytes) ← decodeUIntLE 8 bytes
  let (finalFundingRateTimestamp, bytes) ← decodeUIntLE 8 bytes
  let (futuresMarkPriceOptional, bytes) ← decodeUIntLE 8 bytes
  let (predictedFundingRate, bytes) ← decodeUIntLE 8 bytes
  let (spotMarkPriceOptional, bytes) ← decodeUIntLE 8 bytes
  let (fairValueOptional, bytes) ← decodeUIntLE 8 bytes
  pure ({ snapshotSeqNum, tradeVolume, indicativeOpenPrice, dayOpenPrice, closePrice, lowPrice, highPrice, vwapPriceOptional, settlementPrice, lastTradePrice, lastTradeTime, bestBidImpliedPrice, bestAskImpliedPrice, nextBidImpliedPrice, nextAskImpliedPrice, limitDownPrice, limitUpPrice, lastTradeQty, openInterest, bestBidImpliedQty, bestAskImpliedQty, nextBidImpliedQty, nextAskImpliedQty, priorSettlementPriceOptional, definitionFlags, finalFundingRate, finalFuturesMarkPrice, finalFundingRateTimestamp, futuresMarkPriceOptional, predictedFundingRate, spotMarkPriceOptional, fairValueOptional }, bytes)

@[simp] theorem encode_length (message : EndOfSnapshotMessage) : (encode message).length = 216 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length]
...
inductive Payload where
  ...
  | fundingRateMessage (message : FundingRateMessage) -- 43
...
def encode : Payload → List UInt8
  ...
  | .fundingRateMessage message => FundingRateMessage.encode message
...
theorem encode_length_le (message : Payload) : (encode message).length ≤ 216 := by
  cases message with
  ...
  | openInterestMessage inner =>
    ...
    omega
  | fundingRateMessage inner =>
    simp only [encode, FundingRateMessage.encode_length]
    omega
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (Payload × List UInt8) :=
  ...
  else if tag = 43 then (FundingRateMessage.decode bytes).map fun (message, rest) => (.fundingRateMessage message, rest)
...
def decodeBody (bytes : List UInt8) : Option SbeMessage := do
  ...
  let padding_ := bytes
  if fits_padding : padding_.length ≤ 65309 then
...
theorem encodeBody_length_lt (message : SbeMessage) : (encodeBody message).length + 2 < 256 ^ 2 := by
  ...
  | openInterestMessage inner =>
    ...
    omega
  | fundingRateMessage inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, encodeUIntLE_length, FundingRateMessage.encode_length]
    omega
  | startOfOutrightInstrumentSnapshotMessage inner =>
...
end Omi.CoinbaseCoinbasederivativesMarketdataapiSbeV19
