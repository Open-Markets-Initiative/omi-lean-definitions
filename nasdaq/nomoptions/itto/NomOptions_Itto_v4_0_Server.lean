...
namespace Omi.NasdaqNomoptionsIttoItchV40Server
...
/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 57 := by
  cases message with
  | debugPacket inner =>
...
end Omi.NasdaqNomoptionsIttoItchV40Server
