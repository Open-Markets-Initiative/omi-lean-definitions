...
/-!
# Euronext Order Entry Gateway v4.2
...
namespace Omi.EuronextOptiqOrderentrygatewaySbeV42
...
def decode (bytes : List UInt8) : Option (NewOrderMessage × List UInt8) := do
  ...
  pure ({ clMsgSeqNum, firmId, sendingTime, clientOrderId, symbolIndex, emm, side, orderType, timeInForce, orderPxOptional, orderQty, executionWithinFirmShortCode, tradingCapacity, accountType, lpRoleOptional, executionInstruction, darkExecutionInstruction, mifidIndicators, stpid, nonExecutingClientId, freeTextGroups, mifidShortCodesGroups, orderOptionalFieldsGroups, orderClearingFieldsGroups, notUsedGroups, otherNotUsedGroups, additionalInfosGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : NewOrderMessage) : (encode message).length ≤ 33230 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : AckMessage) : (encode message).length ≤ 2422 := by
...
/-- Fill Strategy Field Group: 25 bytes -/
...
def decode (bytes : List UInt8) : Option (FillStrategyFieldGroup × List UInt8) := do
  ...
  pure ({ legLastPx, legLastQty, legInstrumentId, legSideOptional, executionIdOptional }, bytes)

@[simp] theorem encode_length (message : FillStrategyFieldGroup) : (encode message).length = 25 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : FillStrategyFieldGroups) : (encode message).length ≤ 6377 := by
  have bound_fillStrategyFieldGroup := message.fillStrategyFieldGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUInt_length, encodeMany_length_const FillStrategyFieldGroup.encode 25 FillStrategyFieldGroup.encode_length]
  omega
...
/-- Fill Derivatives Field Group: 9 bytes -/
...
def decode (bytes : List UInt8) : Option (FillDerivativesFieldGroup × List UInt8) := do
  ...
  pure ({ evaluatedPrice, messagePriceNotation }, bytes)

@[simp] theorem encode_length (message : FillDerivativesFieldGroup) : (encode message).length = 9 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : FillDerivativesFieldGroups) : (encode message).length ≤ 2297 := by
  have bound_fillDerivativesFieldGroup := message.fillDerivativesFieldGroup.length_lt
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, encodeUInt_length, encodeMany_length_const FillDerivativesFieldGroup.encode 9 FillDerivativesFieldGroup.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : FillMessage) : (encode message).length ≤ 19235 := by
...
def decode (bytes : List UInt8) : Option (QuotesMessage × List UInt8) := do
  ...
  pure ({ clMsgSeqNum, firmId, sendingTime, clientOrderId, executionWithinFirmShortCode, tradingCapacity, accountType, lpRole, mifidIndicators, rfeAnswer, executionInstructionOptional, mifidShortCodesGroups, clearingDatasetGroups, quotesRepGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : QuotesMessage) : (encode message).length ≤ 25544 := by
  have bound_mifidShortCodesGroups := MifidShortCodesGroups.encode_length_le message.mifidShortCodesGroups
...
/-- Quote Request Message: 71 bytes -/
...
def decode (bytes : List UInt8) : Option (QuoteRequestMessage × List UInt8) := do
  ...
  let (darkExecutionInstruction, bytes) ← decodeUIntLE 1 bytes
  let (minOrderQty, bytes) ← decodeUIntLE 8 bytes
  pure ({ clMsgSeqNum, firmId, sendingTime, executionWithinFirmShortCode, clientIdentificationShortcode, clientOrderId, orderQty, symbolIndex, emm, sideOptional, firmIdPublication, endClient, darkExecutionInstruction, minOrderQty }, bytes)

@[simp] theorem encode_length (message : QuoteRequestMessage) : (encode message).length = 71 := by
  unfold encode
...
/-- Trade Bust Notification Message: 81 bytes -/
...
def decode (bytes : List UInt8) : Option (TradeBustNotificationMessage × List UInt8) := do
  ...
  pure ({ msgSeqNum, firmId, bookIn, bookOutTimeOptional, oegInFromMeOptional, oegOutToMemberOptional, symbolIndex, emm, executionId, lastTradedPx, lastShares, lisTransactionId, parentExecId, parentSymbolIndex }, bytes)

@[simp] theorem encode_length (message : TradeBustNotificationMessage) : (encode message).length = 81 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length]
...
/-- Rfq Notification Message: 95 bytes -/
...
def decode (bytes : List UInt8) : Option (RfqNotificationMessage × List UInt8) := do
  ...
  let (darkExecutionInstruction, bytes) ← decodeUIntLE 1 bytes
  let (minOrderQty, bytes) ← decodeUIntLE 8 bytes
  pure ({ msgSeqNum, firmId, bookIn, bookOutTimeOptional, oegInFromMeOptional, oegOutToMemberOptional, quoteReqId, orderQty, counterpartFirmId, symbolIndex, emm, rfqUpdateType, sideOptional, endClient, darkExecutionInstruction, minOrderQty }, bytes)

@[simp] theorem encode_length (message : RfqNotificationMessage) : (encode message).length = 95 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length]
...
/-- User Notification Message: 47 bytes -/
...
def decode (bytes : List UInt8) : Option (UserNotificationMessage × List UInt8) := do
  ...
  let (notUsedGroups, bytes) ← NotUsedGroups.decode bytes
  pure ({ msgSeqNum, firmId, executionWithinFirmShortCodeOptional, clientIdentificationShortcode, familyId, symbolIndexOptional, userStatus, logicalAccessIdOptional, orderSizeLimit, notUsedGroups }, bytes)

@[simp] theorem encode_length (message : UserNotificationMessage) : (encode message).length = 47 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (CrossOrderMessage × List UInt8) := do
  ...
  pure ({ clMsgSeqNum, firmId, sendingTime, clientOrderId, symbolIndex, emm, side, orderType, orderPx, orderQty, executionWithinFirmShortCode, mifidIndicators, nonExecutingClientId, orderActorType, messagePriceNotation, freeTextGroups, mifidShortCodesGroups, crossClearingFieldsGroups, crossStrategyFieldsGroups }, bytes)
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : CrossOrderMessage) : (encode message).length ≤ 23783 := by
  have bound_freeTextGroups := FreeTextGroups.encode_length_le message.freeTextGroups
  have bound_mifidShortCodesGroups := MifidShortCodesGroups.encode_length_le message.mifidShortCodesGroups
...
/-- Declaration Notice Message: 206 bytes -/
...
theorem encode_length_le (message : Payload) : (encode message).length ≤ 33471 := by
  cases message with
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : Message) : (encode message).length ≤ 33481 := by
  unfold encode
...
end Omi.EuronextOptiqOrderentrygatewaySbeV42
