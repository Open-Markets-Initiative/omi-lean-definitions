...
/-!
# National Association of Securities Dealers Automated Quotations (Nasdaq) Net Order Imbalance View v3.0.2013
...
namespace Omi.NasdaqNsmequitiesNoiviewItchV302013
...
def MarketCategory.codes : List UInt8 :=
  [0x51, 0x47, 0x53, 0x4E, 0x41, 0x50, 0x5A, 0x20]
...
inductive RoundLotsOnly where
  | nasdaqOmxSystemOnlyAcceptsRoundLotsOrdersForThisSecurity -- Nasdaq Omx System Only Accepts Round Lots Orders For This Security
  | nasdaqOmxSystemDoesNotHaveAnyOrderSizeRestrictionsForThisSecurity -- Nasdaq Omx System Does Not Have Any Order Size Restrictions For This Security
...
def toByte : RoundLotsOnly → UInt8
  | .nasdaqOmxSystemOnlyAcceptsRoundLotsOrdersForThisSecurity => 0x59
  | .nasdaqOmxSystemDoesNotHaveAnyOrderSizeRestrictionsForThisSecurity => 0x4E
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : RoundLotsOnly :=
  if byte = 0x59 then .nasdaqOmxSystemOnlyAcceptsRoundLotsOrdersForThisSecurity
  else .nasdaqOmxSystemDoesNotHaveAnyOrderSizeRestrictionsForThisSecurity
...
theorem ofByte_toByte (value : RoundLotsOnly) : ofByte value.toByte = value := by
  cases value with
  | nasdaqOmxSystemOnlyAcceptsRoundLotsOrdersForThisSecurity => decide
  | nasdaqOmxSystemDoesNotHaveAnyOrderSizeRestrictionsForThisSecurity => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
inductive IpoFlag where
  | nasdaqListedInstrumentIsSetUpAsANewIpoSecurity -- Nasdaq Listed Instrument Is Set Up As A New Ipo Security
  | nasdaqListedInstrumentIsNotSetUpAsANewIpoSecurity -- Nasdaq Listed Instrument Is Not Set Up As A New Ipo Security
  | notAvailable -- Not Available
...
def toByte : IpoFlag → UInt8
  | .nasdaqListedInstrumentIsSetUpAsANewIpoSecurity => 0x59
  | .nasdaqListedInstrumentIsNotSetUpAsANewIpoSecurity => 0x4E
  | .notAvailable => 0x20
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : IpoFlag :=
  if byte = 0x59 then .nasdaqListedInstrumentIsSetUpAsANewIpoSecurity
  else if byte = 0x4E then .nasdaqListedInstrumentIsNotSetUpAsANewIpoSecurity
  else .notAvailable
...
theorem ofByte_toByte (value : IpoFlag) : ofByte value.toByte = value := by
  cases value with
  | nasdaqListedInstrumentIsSetUpAsANewIpoSecurity => decide
  | nasdaqListedInstrumentIsNotSetUpAsANewIpoSecurity => decide
  | notAvailable => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
inductive EtpFlag where
  | instrumentIsAnEtp -- Instrument Is An Etp
  | instrumentIsNotAnEtp -- Instrument Is Not An Etp
  | notAvailable -- Not Available
...
def toByte : EtpFlag → UInt8
  | .instrumentIsAnEtp => 0x59
  | .instrumentIsNotAnEtp => 0x4E
  | .notAvailable => 0x20
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : EtpFlag :=
  if byte = 0x59 then .instrumentIsAnEtp
  else if byte = 0x4E then .instrumentIsNotAnEtp
  else .notAvailable
...
theorem ofByte_toByte (value : EtpFlag) : ofByte value.toByte = value := by
  cases value with
  | instrumentIsAnEtp => decide
  | instrumentIsNotAnEtp => decide
  | notAvailable => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
inductive InverseIndicator where
  | etpIsAnInverseEtp -- Etp Is An Inverse Etp
  | etpIsNotAnInverseEtp -- Etp Is Not An Inverse Etp
...
def toByte : InverseIndicator → UInt8
  | .etpIsAnInverseEtp => 0x59
  | .etpIsNotAnInverseEtp => 0x4E
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : InverseIndicator :=
  if byte = 0x59 then .etpIsAnInverseEtp
  else .etpIsNotAnInverseEtp
...
theorem ofByte_toByte (value : InverseIndicator) : ofByte value.toByte = value := by
  cases value with
  | etpIsAnInverseEtp => decide
  | etpIsNotAnInverseEtp => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
def encode (message : StockDirectoryMessage) : List UInt8 :=
  Alpha.encode message.stockAlphanumeric8
    ...
    ++ (Alpha.encode message.etpLeverageFactor
...
def decode (bytes : List UInt8) : Option (StockDirectoryMessage × List UInt8) := do
  ...
  let (etpLeverageFactor, bytes) ← Alpha.decode 5 bytes
  let (inverseIndicator, bytes) ← InverseIndicator.decode bytes
  pure ({ stockAlphanumeric8, marketCategory, financialStatusIndicator, roundLotSize, roundLotsOnly, issueClassification, issueSubType, authenticity, shortSaleThresholdIndicator, ipoFlag, luldReferencePriceTier, etpFlag, etpLeverageFactor, inverseIndicator }, bytes)

@[simp] theorem encode_length (message : StockDirectoryMessage) : (encode message).length = 31 := by
  unfold encode
...
def encode (message : StockTradingActionMessage) : List UInt8 :=
  Alpha.encode message.stockAlphanumeric8
    ...
    ++ (Alpha.encode message.trackingId)))
...
def decode (bytes : List UInt8) : Option (StockTradingActionMessage × List UInt8) := do
  ...
  let (trackingId, bytes) ← Alpha.decode 13 bytes
  pure ({ stockAlphanumeric8, currentTradingState, reason, trackingId }, bytes)

@[simp] theorem encode_length (message : StockTradingActionMessage) : (encode message).length = 26 := by
  unfold encode
...
def decode (bytes : List UInt8) : Option (CrossTradeMessage × List UInt8) := do
  ...
  let (crossType, bytes) ← CrossType.decode bytes
  let (trackingId, bytes) ← Alpha.decode 13 bytes
  pure ({ shares, stockAlpha8, crossPrice, matchNumber, crossType, trackingId }, bytes)

@[simp] theorem encode_length (message : CrossTradeMessage) : (encode message).length = 53 := by
  unfold encode
...
structure IpoQuotingPeriodUpdateMessage where
  stockAlphanumeric8 : Alpha 8
  ipoQuotationReleaseTime : Alpha 6
...
def encode (message : IpoQuotingPeriodUpdateMessage) : List UInt8 :=
  Alpha.encode message.stockAlphanumeric8
    ++ (Alpha.encode message.ipoQuotationReleaseTime
    ++ (IpoQuotationReleaseQualifier.encode message.ipoQuotationReleaseQualifier
    ++ (Alpha.encode message.ipoPrice)))

def decode (bytes : List UInt8) : Option (IpoQuotingPeriodUpdateMessage × List UInt8) := do
  let (stockAlphanumeric8, bytes) ← Alpha.decode 8 bytes
  let (ipoQuotationReleaseTime, bytes) ← Alpha.decode 6 bytes
  let (ipoQuotationReleaseQualifier, bytes) ← IpoQuotationReleaseQualifier.decode bytes
  let (ipoPrice, bytes) ← Alpha.decode 10 bytes
  pure ({ stockAlphanumeric8, ipoQuotationReleaseTime, ipoQuotationReleaseQualifier, ipoPrice }, bytes)

@[simp] theorem encode_length (message : IpoQuotingPeriodUpdateMessage) : (encode message).length = 25 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, IpoQuotationReleaseQualifier.encode_length]
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 0 < 256 ^ 2 := by
  ...
  | crossTradeMessage inner =>
    ...
    omega
  | ipoQuotingPeriodUpdateMessage inner =>
    simp only [Payload.encode, List.length_append, ← Nat.add_assoc, Alpha.encode_length, encodeUInt_length, IpoQuotingPeriodUpdateMessage.encode_length]
    omega
...
end Omi.NasdaqNsmequitiesNoiviewItchV302013
