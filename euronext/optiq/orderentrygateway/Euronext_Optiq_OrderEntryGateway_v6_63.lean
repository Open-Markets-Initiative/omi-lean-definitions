...
/-!
# Euronext Order Entry Gateway v6.63
...
namespace Omi.EuronextOptiqOrderentrygatewaySbeV663
...
/-- User Notification Message: 58 bytes -/
...
def decode (bytes : List UInt8) : Option (UserNotificationMessage × List UInt8) := do
  ...
  let (emmOptional, bytes) ← decodeUInt 1 bytes
  let (notUsedGroups, bytes) ← NotUsedGroups.decode bytes
  pure ({ msgSeqNum, firmId, executionWithinFirmShortCodeOptional, clientIdentificationShortcode, familyId, symbolIndexOptional, userStatus, logicalAccessIdOptional, orderSizeLimit, orderAmountLimit, exposureSide, marketCondition, emmOptional, notUsedGroups }, bytes)

@[simp] theorem encode_length (message : UserNotificationMessage) : (encode message).length = 58 := by
  unfold encode
...
end Omi.EuronextOptiqOrderentrygatewaySbeV663
