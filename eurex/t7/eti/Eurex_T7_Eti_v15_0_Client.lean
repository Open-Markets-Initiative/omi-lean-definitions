...
@[simp] theorem decode_encode (message : BasketRollRequest) (rest : List UInt8) :
  ...
  rw [List.append_assoc, Alpha.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, OldBasketDataGrpComp.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, NewBasketDataGrpComp.decode_encode, some_bind]
  dsimp only
  rw [List.append_assoc, decodeMany_bounded 1 BasketRootPartyGrpComp.encode BasketRootPartyGrpComp.decode BasketRootPartyGrpComp.decode_encode, some_bind]
  dsimp only
...
theorem encode_length_le (message : SrqsHitQuoteRequest) : (encode message).length ≤ 8546 := by
  ...
  unfold encode
  simp only [List.length_append, ← Nat.add_assoc, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, CustOrderHandlingInst.encode_length, encodeMany_length_const SrqsHitQuoteGrpComp.encode 32 SrqsHitQuoteGrpComp.encode_length]
  omega
