...
Note: Xetra En Light Approve Deal Notification is not framed: its length Body Len is not an integer it reads.
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : LogonResponse) : (encode message).length ≤ 65636 := by
...
/-- Decoded as the whole of a frame: nothing follows -/
theorem decode_encode_nil (message : XetraEnLightApproveDealNotification) : decode (encode message) = some (message, []) :=
  List.append_nil (encode message) ▸ decode_encode message []
...
theorem encode_length_le (message : ServerPayload) : (encode message).length ≤ 3145730 := by
  ...
  | xetraEnLightApproveDealNotification inner =>
    simp only [encode, XetraEnLightApproveDealNotification.encode_length]
    omega
