...
Note: Tes Inquire Boundary Prices Response is not framed: its length Body Len is not the integer that leads it.
...
Note: Update Reference Prices Response is not framed: its length Body Len is not the integer that leads it.
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesApproveBroadcast) : (encode message).length ≤ 4282468744 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : TesBroadcast) : (encode message).length ≤ 4282482888 := by
