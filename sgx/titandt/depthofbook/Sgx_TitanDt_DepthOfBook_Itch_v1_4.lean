...
namespace Omi.SgxTitandtDepthofbookItchV14
...
inductive OccurredAtCross where
  ...
  | tradeOccurredAtContinuousMarket -- Trade Occurred At Continuous Market
...
def toByte : OccurredAtCross → UInt8
  ...
  | .tradeOccurredAtContinuousMarket => 0x4E
  | .unlisted byte => byte.val

/-- The constructor of a listed code -/
def listed (byte : UInt8) : OccurredAtCross :=
  if byte = 0x59 then .tradeOccurredAtTheCross
  else .tradeOccurredAtContinuousMarket
...
theorem ofByte_toByte (value : OccurredAtCross) : ofByte value.toByte = value := by
  ...
  | tradeOccurredAtContinuousMarket => decide
  | unlisted byte => simp [ofByte, toByte, byte.property]
...
def encode (message : OrderExecutedMessage) : List UInt8 :=
  encodeUInt 4 message.nanoseconds
    ...
    ++ (Alpha.encode message.reserved7A
    ++ (Alpha.encode message.reserved7B))))))))
...
def decode (bytes : List UInt8) : Option (OrderExecutedMessage × List UInt8) := do
  ...
  let (reserved7A, bytes) ← Alpha.decode 7 bytes
  let (reserved7B, bytes) ← Alpha.decode 7 bytes
  pure ({ nanoseconds, orderId, orderBookId, side, executedQuantity, matchId, comboGroupId, reserved7A, reserved7B }, bytes)
...
def decode (bytes : List UInt8) : Option (OrderExecutedWithPriceMessage × List UInt8) := do
  ...
  pure ({ nanoseconds, orderId, orderBookId, side, executedQuantity, matchId, comboGroupId, reserved7A, reserved7B, tradePrice, occurredAtCross, printable := printable_ }, bytes)
...
def decode (bytes : List UInt8) : Option (TradeMessage × List UInt8) := do
  ...
  let (printable_, bytes) ← Printable.decode bytes
  let (occurredAtCross, bytes) ← OccurredAtCross.decode bytes
  pure ({ nanoseconds, matchId, comboGroupId, side, quantity, orderBookId, tradePrice, reserved7A, reserved7B, printable := printable_, occurredAtCross }, bytes)
...
end Omi.SgxTitandtDepthofbookItchV14
