...
/-!
# National Association of Securities Dealers Automated Quotations (Nasdaq) TotalView Itch v3.1.f
...
namespace Omi.NasdaqNsmequitiesTotalviewItchV31FServer
...
def decode (bytes : List UInt8) : Option (AddOrderMessage × List UInt8) := do
  ...
  pure ({ orderReferenceNumber, side, sharesNumeric6, stockAlpha6, price, display }, bytes)
...
/-- Order Replace Message: 41 bytes -/
...
def decode (bytes : List UInt8) : Option (OrderReplaceMessage × List UInt8) := do
  ...
  let (price, bytes) ← Alpha.decode 10 bytes
  let (display, bytes) ← Display.decode bytes
  pure ({ originalOrderReferenceNumber, newOrderReferenceNumber, sharesNumeric6, price, display }, bytes)

@[simp] theorem encode_length (message : OrderReplaceMessage) : (encode message).length = 41 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, Display.encode_length]
...
theorem encode_length_le (message : SequencedDataPacket) : (encode message).length ≤ 58 := by
  ...
  | orderReplaceMessage inner =>
    ...
    omega
  | orderDisplayMessage inner =>
    simp only [SequencedMessage.encode, List.length_append, encodeUInt_length, OrderDisplayMessage.encode_length]
    omega
  | tradeMessage inner =>
...
end Omi.NasdaqNsmequitiesTotalviewItchV31FServer
