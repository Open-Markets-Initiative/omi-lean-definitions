...
Note: Add Scaled Simple Instrument Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Approve Reverse Tes Trade Request is not framed: its length Body Len is not the integer that leads it.

Note: Approve Tes Trade Request is not framed: its length Body Len is not the integer that leads it.

Note: Basket Roll Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Delete Order Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Inquire Margin Based Risk Limit Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Mass Order is not framed: its length Body Len is not the integer that leads it.
...
Note: Srqs Inquire Smart Respondent Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Open Negotiation Request is not framed: its length Body Len is not the integer that leads it.

Note: Srqs Quote Snapshot Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Tes Inquire Boundary Prices Request is not framed: its length Body Len is not the integer that leads it.
...
Note: Update Reference Price Request is not framed: its length Body Len is not the integer that leads it.

Note: Update Remaining Risk Allowance Base Request is not framed: its length Body Len is not the integer that leads it.
...
namespace Omi.EurexT7EtiFbeV150Client
...
structure AddComplexInstrumentRequest where
  ...
  enableCustomCommodityStrip : BitVec 8
  complianceText : Alpha 20
  displayNameAlias : Alpha 190
  periodAlias : Alpha 45
  pad3 : Alpha 3
...
def encode (message : AddComplexInstrumentRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ...
    ++ encodeUInt 1 message.enableCustomCommodityStrip
    ++ Alpha.encode message.complianceText
    ++ Alpha.encode message.displayNameAlias
    ++ Alpha.encode message.periodAlias
...
def decode (bytes : List UInt8) : Option (AddComplexInstrumentRequest × List UInt8) := do
  ...
  let (enableCustomCommodityStrip, bytes) ← decodeUInt 1 bytes
  let (complianceText, bytes) ← Alpha.decode 20 bytes
  let (displayNameAlias, bytes) ← Alpha.decode 190 bytes
  let (periodAlias, bytes) ← Alpha.decode 45 bytes
  ...
  if fits_instrmtLegGrpComp : instrmtLegGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, relatedPx, marketSegmentId, securitySubType, quantityScalingFactor, productComplex, multilegModel, enableCustomCommodityStrip, complianceText, displayNameAlias, periodAlias, pad3, instrmtLegGrpComp := ⟨instrmtLegGrpComp_, fits_instrmtLegGrpComp⟩ }, bytes)
  else none
...
theorem encode_length_le (message : AddComplexInstrumentRequest) : (encode message).length ≤ 8458 := by
...
/-- Add Scaled Simple Instrument Request: 274 bytes -/
...
def decode (bytes : List UInt8) : Option (AddScaledSimpleInstrumentRequest × List UInt8) := do
  ...
  let (periodAlias, bytes) ← Alpha.decode 45 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, relatedSecurityId, marketSegmentId, quantityScalingFactor, multilegModel, displayNameAlias, periodAlias, pad6 }, bytes)

@[simp] theorem encode_length (message : AddScaledSimpleInstrumentRequest) : (encode message).length = 274 := by
  unfold encode
...
theorem encode_length_le (message : ApproveBasketTradeRequest) : (encode message).length ≤ 12058546 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const BasketSideAllocExtGrpComp.encode 184 BasketSideAllocExtGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ApproveBasketTradeRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 2 BasketSideAllocExtGrpComp.encode BasketSideAllocExtGrpComp.decode BasketSideAllocExtGrpComp.decode_encode, Option.bind_some]
  dsimp only
...
/-- Old Basket Data Grp Comp: 64 bytes -/
structure OldBasketDataGrpComp where
  basketTrdMatchId : BitVec 64
  basketExecId : BitVec 32
  tradeReportId : Alpha 20
  maturityMonthYear : BitVec 32
...
namespace OldBasketDataGrpComp

def encode (message : OldBasketDataGrpComp) : List UInt8 :=
  encodeUIntLE 8 message.basketTrdMatchId
    ++ encodeUIntLE 4 message.basketExecId
    ++ Alpha.encode message.tradeReportId
    ++ encodeUIntLE 4 message.maturityMonthYear
...
def decode (bytes : List UInt8) : Option (OldBasketDataGrpComp × List UInt8) := do
  let (basketTrdMatchId, bytes) ← decodeUIntLE 8 bytes
  let (basketExecId, bytes) ← decodeUIntLE 4 bytes
  let (tradeReportId, bytes) ← Alpha.decode 20 bytes
  let (maturityMonthYear, bytes) ← decodeUIntLE 4 bytes
  ...
  let (basketTradeReportText, bytes) ← Alpha.decode 20 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ basketTrdMatchId, basketExecId, tradeReportId, maturityMonthYear, basketTradeReportType, basketTradeReportText, pad7 }, bytes)

@[simp] theorem encode_length (message : OldBasketDataGrpComp) : (encode message).length = 64 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, Alpha.encode_length, encodeUInt_length]

theorem encode_length_pos (message : OldBasketDataGrpComp) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : OldBasketDataGrpComp) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [decodeUIntLE_encodeUIntLE, Option.bind_some]
...
end OldBasketDataGrpComp

/-- New Basket Data Grp Comp: 48 bytes -/
structure NewBasketDataGrpComp where
  tradeReportId : Alpha 20
  maturityMonthYear : BitVec 32
  basketTradeReportType : BitVec 8
  basketTradeReportText : Alpha 20
...
namespace NewBasketDataGrpComp

def encode (message : NewBasketDataGrpComp) : List UInt8 :=
  Alpha.encode message.tradeReportId
    ++ encodeUIntLE 4 message.maturityMonthYear
    ++ encodeUInt 1 message.basketTradeReportType
    ++ Alpha.encode message.basketTradeReportText
...
def decode (bytes : List UInt8) : Option (NewBasketDataGrpComp × List UInt8) := do
  let (tradeReportId, bytes) ← Alpha.decode 20 bytes
  let (maturityMonthYear, bytes) ← decodeUIntLE 4 bytes
  let (basketTradeReportType, bytes) ← decodeUInt 1 bytes
  let (basketTradeReportText, bytes) ← Alpha.decode 20 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ tradeReportId, maturityMonthYear, basketTradeReportType, basketTradeReportText, pad3 }, bytes)

@[simp] theorem encode_length (message : NewBasketDataGrpComp) : (encode message).length = 48 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : NewBasketDataGrpComp) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : NewBasketDataGrpComp) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [Alpha.decode_encode, Option.bind_some]
...
/-- Basket Roll Request -/
structure BasketRollRequest where
  networkMsgId : Alpha 8
  pad2 : Alpha 2
  requestHeaderComp : RequestHeaderComp
  marketSegmentId : BitVec 32
  ...
  tradeReportId : Alpha 20
  pad2v2 : Alpha 2
  oldBasketDataGrpComp : OldBasketDataGrpComp
  newBasketDataGrpComp : NewBasketDataGrpComp
  basketRootPartyGrpComp : Bounded 1 BasketRootPartyGrpComp
...
namespace BasketRollRequest

def encode (message : BasketRollRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ++ Alpha.encode message.pad2
    ++ RequestHeaderComp.encode message.requestHeaderComp
    ++ encodeUIntLE 4 message.marketSegmentId
    ...
    ++ OldBasketDataGrpComp.encode message.oldBasketDataGrpComp
    ++ NewBasketDataGrpComp.encode message.newBasketDataGrpComp
    ++ encodeMany BasketRootPartyGrpComp.encode message.basketRootPartyGrpComp.val
...
def decode (bytes : List UInt8) : Option (BasketRollRequest × List UInt8) := do
  let (networkMsgId, bytes) ← Alpha.decode 8 bytes
  let (pad2, bytes) ← Alpha.decode 2 bytes
  let (requestHeaderComp, bytes) ← RequestHeaderComp.decode bytes
  let (marketSegmentId, bytes) ← decodeUIntLE 4 bytes
  ...
  let (oldBasketDataGrpComp, bytes) ← OldBasketDataGrpComp.decode bytes
  let (newBasketDataGrpComp, bytes) ← NewBasketDataGrpComp.decode bytes
  let (basketRootPartyGrpComp_, bytes) ← decodeMany BasketRootPartyGrpComp.decode noBasketRootPartyGrps.toNat bytes
...
theorem encode_length_pos (message : BasketRollRequest) : (encode message).length > 0 := by
  unfold encode
  simp only [Alpha.encode_length, List.length_append]
  omega

/-- The most bytes an encoding can take -/
theorem encode_length_le (message : BasketRollRequest) : (encode message).length ≤ 4245410 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
...
@[simp] theorem decode_encode (message : BasketRollRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [OldBasketDataGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [NewBasketDataGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 BasketRootPartyGrpComp.encode BasketRootPartyGrpComp.decode BasketRootPartyGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 OldBasketInstrmtMatchSideGrpComp.encode OldBasketInstrmtMatchSideGrpComp.decode OldBasketInstrmtMatchSideGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 NewBasketInstrmtMatchSideGrpComp.encode NewBasketInstrmtMatchSideGrpComp.decode NewBasketInstrmtMatchSideGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 2 OldBasketSideAllocGrpComp.encode OldBasketSideAllocGrpComp.decode OldBasketSideAllocGrpComp.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 2 NewBasketSideAllocGrpComp.encode NewBasketSideAllocGrpComp.decode NewBasketSideAllocGrpComp.decode_encode, Option.bind_some]
  dsimp only
...
def decode (bytes : List UInt8) : Option (DeleteClipRequest × List UInt8) := do
  ...
  let (partyIdInvestmentDecisionMakerQualifier, bytes) ← decodeUInt 1 bytes
  let (pad6, bytes) ← Alpha.decode 6 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, orderId, securityId, partyIdInvestmentDecisionMaker, executingTrader, marketSegmentId, crossRequestId, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, pad6 }, bytes)

@[simp] theorem encode_length (message : DeleteClipRequest) : (encode message).length = 74 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]
...
/-- Cross Request Side Grp Comp: 192 bytes -/
...
def decode (bytes : List UInt8) : Option (CrossRequestSideGrpComp × List UInt8) := do
  ...
  let (partyEndClientIdentification, bytes) ← Alpha.decode 20 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ clOrdId, partyIdClientId, partyIdInvestmentDecisionMaker, executingTrader, maximumPrice, matchInstCrossId, inputSource, side, selfMatchPreventionInstruction, crossMatchInstruction, tradingCapacity, executingTraderQualifier, orderAttributeLiquidityProvision, partyIdInvestmentDecisionMakerQualifier, orderAttributeRiskReduction, orderOrigination, positionEffect, custOrderHandlingInst, account, partyIdPositionAccount, freeText1, freeText2, freeText3, partyIdOrderOriginationFirm, partyIdBeneficiary, partyIdTakeUpTradingFirm, sideComplianceText, partyIdLocationId, partyEndClientIdentification, pad3 }, bytes)

@[simp] theorem encode_length (message : CrossRequestSideGrpComp) : (encode message).length = 192 := by
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterClipRequest) : (encode message).length ≤ 51082 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const CrossRequestSideGrpComp.encode 192 CrossRequestSideGrpComp.encode_length, encodeMany_length_const SideCrossLegGrpComp.encode 8 SideCrossLegGrpComp.encode_length]
  omega
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterTesTradeRequest) : (encode message).length ≤ 36946 := by
...
@[simp] theorem decode_encode (message : EnterTesTradeRequest) (rest : List UInt8) :
  ...
  dsimp only
  simp only [message.sideAllocGrpComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, message.instrumentEventGrpComp.length_lt, message.instrumentAttributeGrpComp.length_lt, message.underlyingStipGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (MmParameterDefinitionRequest × List UInt8) := do
  ...
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, exposureDuration, cumQty, delta, vega, marketSegmentId, targetPartyIdSessionId, pctCount, mmRiskLimitActionType, pad3 }, bytes)
...
theorem encode_length_le (message : MassOrder) : (encode message).length ≤ 10402 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, CustOrderHandlingInst.encode_length, encodeMany_length_const OrderEntryGrpComp.encode 40 OrderEntryGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : MassOrder) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 OrderEntryGrpComp.encode OrderEntryGrpComp.decode OrderEntryGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.orderEntryGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (ModifyBasketTradeRequest × List UInt8) := do
  ...
  if fits_basketRootPartyGrpComp : basketRootPartyGrpComp_.length < 256 ^ 1 then
    if fits_instrmtMatchSideGrpComp : instrmtMatchSideGrpComp_.length < 256 ^ 1 then
      if fits_basketSideAllocGrpComp : basketSideAllocGrpComp_.length < 256 ^ 2 then
        pure ({ networkMsgId, pad2, requestHeaderComp, basketTrdMatchId, basketExecId, marketSegmentId, maturityMonthYear, basketProfileId, trdType, tradeReportType, optionalEarlyTerminationIndicator, basketTradeReportText, tradeReportId, basketRootPartyGrpComp := ⟨basketRootPartyGrpComp_, fits_basketRootPartyGrpComp⟩, instrmtMatchSideGrpComp := ⟨instrmtMatchSideGrpComp_, fits_instrmtMatchSideGrpComp⟩, basketSideAllocGrpComp := ⟨basketSideAllocGrpComp_, fits_basketSideAllocGrpComp⟩ }, bytes)
      else none
    else none
  else none
...
theorem encode_length_le (message : ModifyOrderRequest) : (encode message).length ≤ 2338 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, CustOrderHandlingInst.encode_length, encodeMany_length_const LegOrdGrpComp.encode 8 LegOrdGrpComp.encode_length]
  omega
...
def decode (bytes : List UInt8) : Option (ModifyOrderShortRequest × List UInt8) := do
  ...
  let (checkSumCorrection, bytes) ← decodeUIntLE 2 bytes
  let (pad2v2, bytes) ← Alpha.decode 2 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, origClOrdId, partyIdClientId, executingTrader, matchInstCrossId, applSeqIndicator, priceValidityCheckType, valueCheckTypeValue, tradingCapacity, orderOrigination, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, complianceText, pad1, partyIdInvestmentDecisionMaker, enrichmentRuleId, orderAttributeLiquidityProvision, productComplex, selfMatchPreventionInstruction, crossMatchInstruction, pad2v1, marketSegmentId, pad4v1, securityId, orderQty, price, side, execInst, timeInForce, pad1v1, checkSumCorrection, pad2v2 }, bytes)
...
def decode (bytes : List UInt8) : Option (ModifyTesTradeRequest × List UInt8) := do
  ...
  if fits_sideAllocGrpComp : sideAllocGrpComp_.length < 256 ^ 1 then
    if fits_trdInstrmntLegGrpComp : trdInstrmntLegGrpComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, lastPx, transBkdTime, relatedClosePrice, marketSegmentId, packageId, tesExecId, relatedMarketSegmentId, trdType, tradeReportType, tradePublishIndicator, swapClearer, tradeReportText, tradeReportId, pad1, sideAllocGrpComp := ⟨sideAllocGrpComp_, fits_sideAllocGrpComp⟩, trdInstrmntLegGrpComp := ⟨trdInstrmntLegGrpComp_, fits_trdInstrmntLegGrpComp⟩ }, bytes)
    else none
  else none
...
theorem encode_length_le (message : ModifyTesTradeRequest) : (encode message).length ≤ 14386 := by
  ...
  have bound_trdInstrmntLegGrpComp := message.trdInstrmntLegGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, encodeMany_length_const SideAllocGrpComp.encode 32 SideAllocGrpComp.encode_length, encodeMany_length_const TrdInstrmntLegGrpComp.encode 24 TrdInstrmntLegGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : ModifyTesTradeRequest) (rest : List UInt8) :
  ...
  rw [decodeMany_bounded 1 TrdInstrmntLegGrpComp.encode TrdInstrmntLegGrpComp.decode TrdInstrmntLegGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.sideAllocGrpComp.length_lt, message.trdInstrmntLegGrpComp.length_lt, ↓reduceDIte]
  rfl
...
def decode (bytes : List UInt8) : Option (NewOrderRequest × List UInt8) := do
  ...
  let (legOrdGrpComp_, bytes) ← decodeMany LegOrdGrpComp.decode noLegOnbooks.toNat bytes
  if fits_legOrdGrpComp : legOrdGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, clOrdId, stopPx, partyIdClientId, executingTrader, matchInstCrossId, expireDate, tradingSessionSubId, selfMatchPreventionInstruction, crossMatchInstruction, applSeqIndicator, ordType, priceValidityCheckType, valueCheckTypeValue, orderOrigination, partyIdTakeUpTradingFirm, partyIdOrderOriginationFirm, partyIdBeneficiary, account, partyIdPositionAccount, positionEffect, partyIdLocationId, custOrderHandlingInst, complianceText, freeText1, freeText2, freeText3, fixClOrdId, partyEndClientIdentification, executingTraderQualifier, partyIdInvestmentDecisionMakerQualifier, orderAttributeRiskReduction, pad2v2, partyIdInvestmentDecisionMaker, orderAttributeLiquidityProvision, tradingCapacity, productComplex, marketSegmentId, securityId, orderQty, price, side, execInst, timeInForce, pad1, checkSumCorrection, pad2v3, legOrdGrpComp := ⟨legOrdGrpComp_, fits_legOrdGrpComp⟩ }, bytes)
  else none
...
def decode (bytes : List UInt8) : Option (PreTradeRiskLimitsDefinitionRequest × List UInt8) := do
  ...
  if fits_riskLimitQtyGrpComp : riskLimitQtyGrpComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, nettingCoefficient, marketSegmentId, riskLimitPlatform, partyDetailStatus, riskLimitGroupStatus, riskLimitGroup, partyDetailExecutingUnit, riskLimitQtyGrpComp := ⟨riskLimitQtyGrpComp_, fits_riskLimitQtyGrpComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : PreTradeRiskLimitsDefinitionRequest) : (encode message).length ≤ 4122 := by
...
/-- Srqs Hit Quote Grp Comp: 32 bytes -/
...
def decode (bytes : List UInt8) : Option (SrqsHitQuoteGrpComp × List UInt8) := do
  ...
  let (side, bytes) ← decodeUInt 1 bytes
  let (pad7, bytes) ← Alpha.decode 7 bytes
  pure ({ orderQty, price, quoteId, side, pad7 }, bytes)

@[simp] theorem encode_length (message : SrqsHitQuoteGrpComp) : (encode message).length = 32 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length]
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsHitQuoteRequest) : (encode message).length ≤ 8546 := by
  have bound_srqsHitQuoteGrpComp := message.srqsHitQuoteGrpComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, PositionEffect.encode_length, CustOrderHandlingInst.encode_length, encodeMany_length_const SrqsHitQuoteGrpComp.encode 32 SrqsHitQuoteGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : SrqsHitQuoteRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 1 SrqsHitQuoteGrpComp.encode SrqsHitQuoteGrpComp.decode SrqsHitQuoteGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.srqsHitQuoteGrpComp.length_lt, ↓reduceDIte]
  rfl
...
theorem encode_length_le (message : SrqsUpdateNegotiationRequest) : (encode message).length ≤ 8642 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length, QuoteCondition.encode_length, encodeMany_length_const TargetPartiesComp.encode 32 TargetPartiesComp.encode_length]
  omega
...
/-- Tes Inquire Boundary Prices Request: 34 bytes -/
structure TesInquireBoundaryPricesRequest where
  networkMsgId : Alpha 8
  pad2 : Alpha 2
  requestHeaderComp : RequestHeaderComp
  securityId : BitVec 64
...
namespace TesInquireBoundaryPricesRequest

def encode (message : TesInquireBoundaryPricesRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ++ Alpha.encode message.pad2
    ++ RequestHeaderComp.encode message.requestHeaderComp
    ++ encodeUIntLE 8 message.securityId
...
def decode (bytes : List UInt8) : Option (TesInquireBoundaryPricesRequest × List UInt8) := do
  let (networkMsgId, bytes) ← Alpha.decode 8 bytes
  let (pad2, bytes) ← Alpha.decode 2 bytes
  let (requestHeaderComp, bytes) ← RequestHeaderComp.decode bytes
  let (securityId, bytes) ← decodeUIntLE 8 bytes
  ...
  let (productComplex, bytes) ← decodeUInt 1 bytes
  let (pad1, bytes) ← Alpha.decode 1 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, marketSegmentId, trdType, productComplex, pad1 }, bytes)

@[simp] theorem encode_length (message : TesInquireBoundaryPricesRequest) : (encode message).length = 34 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeUInt_length]

theorem encode_length_pos (message : TesInquireBoundaryPricesRequest) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : TesInquireBoundaryPricesRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [Alpha.decode_encode, Option.bind_some]
...
end TesInquireBoundaryPricesRequest
...
/-- Update Reference Price Request: 42 bytes -/
structure UpdateReferencePriceRequest where
  networkMsgId : Alpha 8
  pad2 : Alpha 2
  requestHeaderComp : RequestHeaderComp
  securityId : BitVec 64
...
namespace UpdateReferencePriceRequest

def encode (message : UpdateReferencePriceRequest) : List UInt8 :=
  Alpha.encode message.networkMsgId
    ++ Alpha.encode message.pad2
    ++ RequestHeaderComp.encode message.requestHeaderComp
    ++ encodeUIntLE 8 message.securityId
...
def decode (bytes : List UInt8) : Option (UpdateReferencePriceRequest × List UInt8) := do
  let (networkMsgId, bytes) ← Alpha.decode 8 bytes
  let (pad2, bytes) ← Alpha.decode 2 bytes
  let (requestHeaderComp, bytes) ← RequestHeaderComp.decode bytes
  let (securityId, bytes) ← decodeUIntLE 8 bytes
  ...
  let (marketSegmentId, bytes) ← decodeUIntLE 4 bytes
  let (pad4, bytes) ← Alpha.decode 4 bytes
  pure ({ networkMsgId, pad2, requestHeaderComp, securityId, tradingReferencePrice, marketSegmentId, pad4 }, bytes)

@[simp] theorem encode_length (message : UpdateReferencePriceRequest) : (encode message).length = 42 := by
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length]

theorem encode_length_pos (message : UpdateReferencePriceRequest) : (encode message).length > 0 := by
  rw [encode_length]
  decide

@[simp] theorem decode_encode (message : UpdateReferencePriceRequest) (rest : List UInt8) :
    decode (encode message ++ rest) = some (message, rest) := by
  unfold decode encode
  simp only [List.append_assoc, Option.bind_eq_bind]
  rw [Alpha.decode_encode, Option.bind_some]
...
end UpdateReferencePriceRequest
...
theorem encode_length_le (message : UpdateRemainingRiskAllowanceBaseRequest) : (encode message).length ≤ 2097146 := by
  ...
  unfold encode
  simp only [List.length_append, Alpha.encode_length, RequestHeaderComp.encode_length, encodeUIntLE_length, encodeMany_length_const RraUpdateBasePartyGrpComp.encode 32 RraUpdateBasePartyGrpComp.encode_length]
  omega
...
@[simp] theorem decode_encode (message : UpdateRemainingRiskAllowanceBaseRequest) (rest : List UInt8) :
  ...
  rw [Alpha.decode_encode, Option.bind_some]
  dsimp only
  rw [decodeMany_bounded 2 RraUpdateBasePartyGrpComp.encode RraUpdateBasePartyGrpComp.decode RraUpdateBasePartyGrpComp.decode_encode, Option.bind_some]
  dsimp only
  simp only [message.rraUpdateBasePartyGrpComp.length_lt, ↓reduceDIte]
  rfl
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : UploadTesTradeRequest) : (encode message).length ≤ 79786 := by
...
inductive ClientPayload where
  ...
  | basketRollRequest (message : BasketRollRequest) -- 10633
  ...
  | tesInquireBoundaryPricesRequest (message : TesInquireBoundaryPricesRequest) -- 10329
  | tradingActionRequest (message : TradingActionRequest) -- 10904
  | unsubscribeRequest (message : UnsubscribeRequest) -- 10006
  | updateReferencePriceRequest (message : UpdateReferencePriceRequest) -- 10331
...
def tag : ClientPayload → BitVec 16
  ...
  | .tesInquireBoundaryPricesRequest _ => 10329
  | .tradingActionRequest _ => 10904
  | .unsubscribeRequest _ => 10006
  | .updateReferencePriceRequest _ => 10331
...
def encode : ClientPayload → List UInt8
  ...
  | .basketRollRequest message => BasketRollRequest.encode message
  ...
  | .tesInquireBoundaryPricesRequest message => TesInquireBoundaryPricesRequest.encode message
  ...
  | .updateReferencePriceRequest message => UpdateReferencePriceRequest.encode message
...
def decode (tag : BitVec 16) (bytes : List UInt8) : Option (ClientPayload × List UInt8) :=
  ...
  else if tag = 10633 then (BasketRollRequest.decode bytes).map fun (message, rest) => (.basketRollRequest message, rest)
  ...
  else if tag = 10329 then (TesInquireBoundaryPricesRequest.decode bytes).map fun (message, rest) => (.tesInquireBoundaryPricesRequest message, rest)
  ...
  else if tag = 10331 then (UpdateReferencePriceRequest.decode bytes).map fun (message, rest) => (.updateReferencePriceRequest message, rest)
...
theorem encodeBody_length_lt (message : ClientMessage) : (encodeBody message).length + 4 < 256 ^ 4 := by
  ...
  | tesInquireBoundaryPricesRequest inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUIntLE_length, TesInquireBoundaryPricesRequest.encode_length]
    omega
  ...
  | updateReferencePriceRequest inner =>
    simp only [ClientPayload.encode, List.length_append, encodeUIntLE_length, UpdateReferencePriceRequest.encode_length]
    omega
...
end Omi.EurexT7EtiFbeV150Client
