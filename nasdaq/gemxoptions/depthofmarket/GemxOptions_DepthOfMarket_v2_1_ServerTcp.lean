...
namespace Omi.NasdaqGemxoptionsDepthofmarketItchV21ServerTcp
...
/-- The most bytes any message's encoding can take -/
theorem encode_length_le (message : ServerTcpPayload) : (encode message).length ≤ 63 := by
  cases message with
  | debugPacket inner =>
...
end Omi.NasdaqGemxoptionsDepthofmarketItchV21ServerTcp
