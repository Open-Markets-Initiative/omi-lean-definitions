...
namespace Omi.EurexT7EtiFbeV91Client
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : AmendBasketTradeRequest) : (encode message).length ≤ 2125770 := by
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : EnterBasketTradeRequest) : (encode message).length ≤ 2125762 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
  have bound_instrmtMatchSideGrpComp := message.instrmtMatchSideGrpComp.length_lt
  have bound_basketSideAllocGrpComp := message.basketSideAllocGrpComp.length_lt
  unfold encode
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : ModifyBasketTradeRequest) : (encode message).length ≤ 2125770 := by
  have bound_basketRootPartyGrpComp := message.basketRootPartyGrpComp.length_lt
  have bound_instrmtMatchSideGrpComp := message.instrmtMatchSideGrpComp.length_lt
  have bound_basketSideAllocGrpComp := message.basketSideAllocGrpComp.length_lt
  unfold encode
...
def decode (bytes : List UInt8) : Option (SrqsOpenNegotiationRequest × List UInt8) := do
  ...
  if fits_quotReqLegsGrpComp : quotReqLegsGrpComp_.length < 256 ^ 1 then
    if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
      pure ({ networkMsgId, pad2, requestHeaderComp, securityId, bidPx, offerPx, orderQty, quoteRefPrice, underlyingDeltaPercentage, validUntilTime, marketSegmentId, securitySubType, quoteType, quoteSubType, numberOfRespDisclosureInstruction, side, productComplex, respondentType, showLastDealOnClosure, partyExecutingFirm, partyExecutingTrader, freeText5, quoteReqId, partyOrderOriginationTrader, quotReqLegsGrpComp := ⟨quotReqLegsGrpComp_, fits_quotReqLegsGrpComp⟩, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
    else none
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsOpenNegotiationRequest) : (encode message).length ≤ 14666 := by
...
def decode (bytes : List UInt8) : Option (SrqsUpdateNegotiationRequest × List UInt8) := do
  ...
  let (targetPartiesComp_, bytes) ← decodeMany TargetPartiesComp.decode noTargetPartyIDs.toNat bytes
  if fits_targetPartiesComp : targetPartiesComp_.length < 256 ^ 1 then
    pure ({ networkMsgId, pad2, requestHeaderComp, quoteRefPrice, underlyingDeltaPercentage, bidPx, offerPx, orderQty, marketSegmentId, negotiationId, numberOfRespDisclosureInstruction, side, quoteCancelType, showLastDealOnClosure, partyExecutingFirm, partyExecutingTrader, freeText5, partyOrderOriginationTrader, targetPartiesComp := ⟨targetPartiesComp_, fits_targetPartiesComp⟩ }, bytes)
  else none
...
/-- The most bytes an encoding can take -/
theorem encode_length_le (message : SrqsUpdateNegotiationRequest) : (encode message).length ≤ 8506 := by
  have bound_targetPartiesComp := message.targetPartiesComp.length_lt
  unfold encode
...
end Omi.EurexT7EtiFbeV91Client
