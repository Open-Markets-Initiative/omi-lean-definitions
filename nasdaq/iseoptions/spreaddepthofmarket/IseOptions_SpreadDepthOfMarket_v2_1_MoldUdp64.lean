...
/-!
# National Association of Securities Dealers Automated Quotations (Nasdaq) Phlx Options Spread Depth v2.1
...
namespace Omi.NasdaqIseoptionsSpreaddepthofmarketItchV21MoldUdp64
...
theorem encodeBody_length_lt (message : Message) : (encodeBody message).length + 0 < 256 ^ 2 := by
  ...
  | singleSideUpdateMessage inner =>
    ...
    omega
  | complexStrategyTradeMessage inner =>
    simp only [UdpPayload.encode, List.length_append, encodeUInt_length, ComplexStrategyTradeMessage.encode_length]
    omega
  | complexStrategyAuctionMessage inner =>
    have bound_inner := ComplexStrategyAuctionMessage.encode_length_le inner
    simp only [UdpPayload.encode, List.length_append, encodeUInt_length]
...
end Omi.NasdaqIseoptionsSpreaddepthofmarketItchV21MoldUdp64
