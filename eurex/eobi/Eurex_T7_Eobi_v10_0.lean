...
/-!
# Eurex Exchange Enhanced Order Book Interface v10.0
...
namespace Omi.EurexT7EobiFbeV100
...
/-- Instrument State Change: 40 bytes -/
...
def decode (bytes : List UInt8) : Option (InstrumentStateChange × List UInt8) := do
  ...
  let (transactTime, bytes) ← decodeUIntLE 8 bytes
  pure ({ securityId, securityStatus, securityTradingStatus, marketCondition, fastMarketIndicator, securityTradingEvent, soldOutIndicator, pad2, highPx, lowPx, transactTime }, bytes)

@[simp] theorem encode_length (message : InstrumentStateChange) : (encode message).length = 40 := by
  unfold encode
  simp only [List.length_append, encodeUIntLE_length, encodeUInt_length, Alpha.encode_length]
...
def decode (bytes : List UInt8) : Option (SecMassStatGrpComp × List UInt8) := do
  ...
  let (soldOutIndicator, bytes) ← decodeUInt 1 bytes
  let (pad3, bytes) ← Alpha.decode 3 bytes
  pure ({ securityId, highPx, lowPx, securityStatus, securityTradingStatus, marketCondition, securityTradingEvent, soldOutIndicator, pad3 }, bytes)
...
theorem encode_length_le (message : MassInstrumentStateChange) : (encode message).length ≤ 8184 := by
  ...
  unfold encode
  simp only [List.length_append, encodeUInt_length, Alpha.encode_length, encodeUIntLE_length, encodeMany_length_const SecMassStatGrpComp.encode 32 SecMassStatGrpComp.encode_length]
  omega
